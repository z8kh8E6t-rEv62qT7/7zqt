...
namespace z7::app {
    ...
    using ArchiveRequestPayload = std::variant<AddRequest,
                                               ExtractRequest,
                                               TestRequest,
                                               BenchmarkRequest,
                                               SplitRequest,
    ...
    using OperationPayload = std::variant<std::monostate,
                                          AddResult,
                                          ExtractResult,
                                          TestResult,
                                          BenchmarkResult,
    ...
    public:
        ...
        virtual std::optional<OverwriteDecision> request_overwrite(OverwritePrompt const& prompt) {
            (void)prompt;
            return std::nullopt;
        }

        virtual std::optional<PasswordReply> request_password(PasswordPrompt const& prompt) {
            (void)prompt;
            return std::nullopt;
        }

        virtual std::optional<ChoiceReply> request_choice(ChoicePrompt const& prompt) {
            (void)prompt;
            return std::nullopt;
        }

        virtual std::optional<MemoryLimitReply> request_memory_limit(MemoryLimitPrompt const& prompt) {
            (void)prompt;
            return std::nullopt;
        }
        ...
        virtual void on_log(ArchiveLog const& log) { (void)log; }

        virtual void on_progress(ProgressSnapshot const& progress) { (void)progress; }
        ...
        virtual void on_finished(OperationOutcome const& outcome) { (void)outcome; }
    };
    ...
    class ArchiveEngine {
    public:
        ArchiveSession start(ArchiveRequest const& request, std::shared_ptr<IArchiveDelegate> delegate = {});
    ...
    std::optional<TResult> outcome_payload_as(OperationOutcome const& outcome) {
        if (auto const* value = std::get_if<TResult>(&outcome.payload)) {
            return *value;
        }
        return std::nullopt;
    }
