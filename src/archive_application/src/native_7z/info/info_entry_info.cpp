...
namespace z7::app {
    ...
    namespace {
        ...
        void compute_subtree_stats(IInArchive* archive,
            ...
            for (UInt32 i = 0; i < num_items; ++i) {
                std::string const item_path = normalize_archive_item_path(archive_get_prop_text(archive, i, kpidPath));
                if (item_path.empty()) {
                    continue;
                }
                ...
                bool const is_child =
                    prefix.empty()
                    || (item_path.size() >= prefix.size() && item_path.compare(0, prefix.size(), prefix) == 0);
                if (!is_child) {
                    continue;
                }
                bool is_dir = false;
        ...
        GetEntryInfoResult
        resolve_entry_info_from_arc(IInArchive* archive, UInt32 num_items, std::string const& raw_entry_path) {
            std::string const norm = normalize_archive_item_path(raw_entry_path);
            ...
            std::string const prefix = norm + '/';
            for (UInt32 i = 0; i < num_items; ++i) {
                std::string const item_path = normalize_archive_item_path(archive_get_prop_text(archive, i, kpidPath));
                if (item_path.size() > prefix.size() && item_path.compare(0, prefix.size(), prefix) == 0) {
                    result.exists = true;
                    result.is_directory = true;
    ...
    GetEntryInfoResult NativeArchiveBackend::get_entry_info(GetEntryInfoRequest const& request,
