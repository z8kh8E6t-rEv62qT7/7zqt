...
namespace z7::app {
    ...
    int collect_archive_properties_from_open_state(ArchivePropertiesRequest const& request,
        ...
        const HRESULT context_res = info_properties_detail::collect_property_context(
            arc, request.directory, request.entries, selected_items, folder_context, cancel_requested);
        if (context_res != S_OK) {
            return context_res;
        }
        if (const HRESULT canceled = abort_if_canceled(cancel_requested); canceled != S_OK) {
        ...
        if (selected_items.size() == 1) {
            info_properties_detail::append_selected_item_properties(
                arc, selected_items.front(), request.flat_view, out_lines, cancel_requested);
            if (const HRESULT canceled = abort_if_canceled(cancel_requested); canceled != S_OK) {
                return canceled;
            }
        ...
        info_properties_detail::append_archive_link_properties(codecs, archive_link, out_lines, cancel_requested);
        if (const HRESULT canceled = abort_if_canceled(cancel_requested); canceled != S_OK) {
            return canceled;
        }
        return S_OK;
    ...
    int collect_archive_properties_via_original_api(ArchivePropertiesRequest const& request,
        ...
        return run_with_open_archive_read_hresult(
            ...
            [&](OpenArchiveReadState& open_state, UInt32) -> int {
                ...
                return collect_archive_properties_from_open_state(
                    request, *arc, codecs, open_state.archive_link, cancel_requested, out_lines);
            });
    }
