...
namespace z7::app::info_properties_detail {
    ...
    std::string format_grouped_uint64(uint64_t const value) {
        ...
        size_t const first_group = digits.size() % 3 == 0 ? 3 : digits.size() % 3;
    ...
    std::string property_name_utf8(const PROPID prop_id, wchar_t const* property_name) {
    ...
    std::string raw_property_data_to_text(const PROPID prop_id, void const* data, UInt32 const data_size) {
        ...
        if (prop_id == kpidNtSecure) {
            AString sec;
            ConvertNtSecureToString(static_cast<Byte const*>(data), data_size, sec);
        ...
        if (data_size <= 8 && (prop_id == kpidCRC || prop_id == kpidChecksum)) {
            ConvertDataToHex_Upper(temp, static_cast<Byte const*>(data), data_size);
        } else {
            ConvertDataToHex_Lower(temp, static_cast<Byte const*>(data), data_size);
        }
        return temp;
    }
