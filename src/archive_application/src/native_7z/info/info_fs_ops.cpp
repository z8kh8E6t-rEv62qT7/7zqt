...
namespace z7::app {
    namespace {
        ...
        bool rename_request_targets_archive(RenameRequest const& request) {
            return (request.session_token.has_value() && request.session_token->is_valid())
                || !request.archive_path.empty()
                || !request.entry_path.empty();
        }

        std::string rename_entry_parent(std::string const& entry_path) {
            size_t const slash = entry_path.rfind('/');
        ...
        std::string rename_entry_target_path(std::string const& entry_path, std::string const& new_name) {
            std::string const parent = rename_entry_parent(entry_path);
            if (parent.empty()) {
                return new_name;
            }
        ...
        bool archive_path_is_child_of(std::string const& path, std::string const& parent) {
        ...
        bool
        archive_rename_item_matches(std::string const& item_path, std::string const& old_path, bool old_path_is_dir) {
            return item_path == old_path || (old_path_is_dir && archive_path_is_child_of(item_path, old_path));
        }

        std::string renamed_archive_item_path(std::string const& item_path,
        ...
        bool archive_rename_conflicts_with_unselected_item(std::string const& item_path,
                                                           std::string const& new_path,
                                                           bool new_path_is_dir) {
            return item_path == new_path || (new_path_is_dir && archive_path_is_child_of(item_path, new_path));
        }

        fs::path make_rename_temp_path(fs::path const& archive_path, std::error_code& ec) {
            ec.clear();
            ...
            for (uint64_t i = 0; i < 64; ++i) {
                fs::path candidate =
                    archive_path.parent_path() / fs::path(stem + ".z7_rename_tmp_" + std::to_string(base + i));
        ...
        void fill_overwrite_prompt_file_info(fs::path const& path, bool existing, OverwritePrompt& prompt) {
            std::error_code ec;
        ...
        OverwritePrompt filesystem_overwrite_prompt(fs::path const& source, fs::path const& destination) {
        ...
        ArchiveError filesystem_io_error(std::error_code const& ec, std::string const& fallback) {
            return make_archive_error(ArchiveErrorDomain::kIo, ec ? ec.message() : fallback, 2);
        }
        ...
        FilesystemTransferResult make_transfer_status(FilesystemTransferStatus status) {
            FilesystemTransferResult result;
            result.status = status;
            return result;
        }

        FilesystemTransferResult make_transfer_error(ArchiveError const& error) {
            FilesystemTransferResult result;
        ...
        FilesystemTransferResult copy_path_with_overwrite_mode(fs::path const& source,
            ...
            if (!fs::exists(source, ec)) {
                return make_transfer_error(filesystem_io_error(ec, "Source path does not exist"));
            }

            fs::path final_destination = destination;
            fs::path renamed_existing_backup;
            bool const destination_exists = fs::exists(destination, ec);
            if (ec) {
                return make_transfer_error(filesystem_io_error(ec, "Cannot query destination path"));
            }
            ...
            if (destination_exists) {
                ...
                switch (mode) {
                    ...
                    case OverwriteMode::kRenameExisting:
                        {
                            std::error_code unique_ec;
                            fs::path const renamed_existing = make_unique_destination_path(destination, unique_ec);
                            if (unique_ec) {
                                return make_transfer_error(
                                    filesystem_io_error(unique_ec, "Cannot allocate renamed destination path"));
                            }
                            if (renamed_existing.empty()) {
                                return make_transfer_error(make_archive_error(
                                    ArchiveErrorDomain::kIo, "Cannot allocate renamed destination path", 2));
                            }
                    ...
                    case OverwriteMode::kAsk:
                        {
                            ...
                            OverwriteDecision const decision =
                                hooks.ask_overwrite(filesystem_overwrite_prompt(source, destination));
                            switch (decision) {
                                ...
                                case OverwriteDecision::kAutoRename:
                                    {
                                        ...
                                        final_destination = make_unique_destination_path(destination, unique_ec);
                                        if (unique_ec) {
                                            return make_transfer_error(filesystem_io_error(
                                                unique_ec, "Cannot allocate renamed destination path"));
                                        }
                                        break;
                                    }
                ...
                if (final_destination.empty()) {
                    return make_transfer_error(
                        make_archive_error(ArchiveErrorDomain::kIo, "Cannot allocate destination path", 2));
                }

                if (overwrite_destination && !remove_path_any(final_destination, ec)) {
                    return make_transfer_error(filesystem_io_error(ec, "Cannot remove existing destination"));
                }
            }
            ...
            if (!copy_path_any(source, final_destination, false, ec)) {
                ...
                return make_transfer_error(filesystem_io_error(ec, "Failed to copy path"));
            }
            return make_transfer_status(FilesystemTransferStatus::kTransferred);
        }

        template <typename TResult, typename OnTransferred>
        TResult run_filesystem_transfer_batch(std::vector<std::string> const& source_paths,
                                              ArchiveBackendHooks const& hooks,
                                              std::atomic<bool>& cancel_requested,
                                              std::string const& destination_dir,
                                              std::string const& destination_path,
                                              OverwriteMode overwrite_mode,
                                              bool move_mode,
                                              std::string success_summary,
                                              OnTransferred&& on_transferred) {
            uint64_t const total = static_cast<uint64_t>(source_paths.size());
            uint64_t processed = 0;
            uint64_t transferred = 0;
            FilesystemOverwriteState overwrite_state;

            for (std::string const& source : source_paths) {
                ...
                fs::path const dst =
                    destination_path.empty() ? fs::path(destination_dir) / src.filename() : fs::path(destination_path);
                FilesystemTransferResult const transfer =
                    copy_path_with_overwrite_mode(src, dst, overwrite_mode, hooks, overwrite_state);
    ...
    OpenArchiveSessionResult NativeArchiveBackend::open_archive_from_path(OpenArchiveFromPathRequest const& request,
                                                                          ArchiveBackendHooks const& hooks) {
    ...
    OpenArchiveResult NativeArchiveBackend::open_archive(OpenArchiveRequest const& request,
                                                         ArchiveBackendHooks const& hooks) {
    ...
    ListResult NativeArchiveBackend::list(ListRequest const& request, ArchiveBackendHooks const& hooks) {
        size_t const batch_size =
            request.streaming_mode
                ? (request.batch_size_hint.value_or(256) > 0 ? request.batch_size_hint.value_or(256) : 256)
                : 0;
        auto const& batch_cb = hooks.on_list_batch;
    ...
    CopyResult NativeArchiveBackend::copy(CopyRequest const& request, ArchiveBackendHooks const& hooks) {
        return run_filesystem_transfer_batch<CopyResult>(
            ...
            false,
            "Copy completed",
            [](CopyResult& result, uint64_t completed) { result.copied_count = static_cast<size_t>(completed); });
    }

    MoveResult NativeArchiveBackend::move(MoveRequest const& request, ArchiveBackendHooks const& hooks) {
        return run_filesystem_transfer_batch<MoveResult>(
            ...
            true,
            "Move completed",
            [](MoveResult& result, uint64_t completed) { result.moved_count = static_cast<size_t>(completed); });
    }

    RenameResult NativeArchiveBackend::rename(RenameRequest const& request, ArchiveBackendHooks const& hooks) {
        if (rename_request_targets_archive(request)) {
            ...
            std::string const old_path = normalize_archive_virtual_directory(request.entry_path);
            std::string const new_path = rename_entry_target_path(old_path, request.new_name);
            fs::path const archive_path(request.archive_path);
                ...
                [&](OpenArchiveReadState& open_state, UInt32 num_items) -> RenameResult {
                    ...
                    if (query_out_res != S_OK || !out_archive) {
                        return from_base_result<RenameResult>(
                            make_operation_failure<OperationResult>(map_archive_rename_hresult(query_out_res)));
                    }
                    ...
                    for (UInt32 i = 0; i < num_items; ++i) {
                        ...
                        if (item_path.empty()
                            || !archive_rename_item_matches(item_path, old_path, request.entry_is_dir)) {
                            continue;
                        }

                        matched_any = true;
                        renamed_items[static_cast<size_t>(i)] = true;
                        std::string item_new_path = renamed_archive_item_path(item_path, old_path, new_path);
                    ...
                    if (!matched_any) {
                        return make_operation_failure<RenameResult>(
                            ArchiveErrorDomain::kInvalidArguments, "Rename request entry was not found in archive", 7);
                    }
                    ...
                    for (UInt32 i = 0; i < num_items; ++i) {
                        ...
                        if (renamed_items[index]) {
                            ...
                            if (anti_res != S_OK) {
                                return from_base_result<RenameResult>(
                                    make_operation_failure<OperationResult>(map_archive_rename_hresult(anti_res)));
                            }
                            pair.NewNameIndex = static_cast<int>(new_names.Add(utf8_to_ustring(new_item_paths[index])));
                    ...
                    fs::path const temp_path = make_rename_temp_path(archive_path, temp_path_ec);
                    if (temp_path.empty()) {
                        std::string message = "Failed to allocate temporary archive output path";
                        if (temp_path_ec) {
                            message += ": " + temp_path_ec.message();
                        }
                        return make_operation_failure<RenameResult>(ArchiveErrorDomain::kIo, std::move(message), 2);
                    }
                    ...
                    if (!out_stream_spec->Create_ALWAYS(us2fs(utf8_to_ustring(temp_path.string())))) {
                        return make_operation_failure<RenameResult>(
                            ArchiveErrorDomain::kIo, "Failed to create temporary archive output", 2);
                    }
                    ...
                    if (arc->ArcStreamOffset == 0) {
                        ...
                        if (seek_res != S_OK) {
                            ...
                            return from_base_result<RenameResult>(
                                make_operation_failure<OperationResult>(map_archive_rename_hresult(seek_res)));
                        }
                        ...
                        if (copy_res != S_OK) {
                            ...
                            fs::remove(temp_path, cleanup_ec);
                            return from_base_result<RenameResult>(
                                make_operation_failure<OperationResult>(map_archive_rename_hresult(copy_res)));
                        }
                    ...
                    if (cancel_requested_.load()) {
                        ...
                        fs::remove(temp_path, cleanup_ec);
                        return make_operation_failure<RenameResult>(
                            ArchiveErrorDomain::kCanceled, "Operation canceled", 255);
                    }
                    ...
                    if (update_res != S_OK) {
                        ...
                        fs::remove(temp_path, cleanup_ec);
                        return from_base_result<RenameResult>(
                            make_operation_failure<OperationResult>(map_archive_rename_hresult(update_res)));
                    }
                    ...
                    AtomicReplaceResult const replace_result =
                        replace_file_atomically(temp_path, archive_path, ".z7-rename-backup-");
                    if (!replace_result.success) {
                        return from_base_result<RenameResult>(make_operation_failure<OperationResult>(
        ...
        fs::path const src(request.source_path);
        fs::path const dst = src.parent_path() / request.new_name;
        std::error_code ec;
    ...
    CreateResult NativeArchiveBackend::create(CreateRequest const& request, ArchiveBackendHooks const& hooks) {
        std::string const created_path = (fs::path(request.parent_dir) / request.name).string();
        std::error_code ec;
        return run_filesystem_single_step<CreateResult>(
            ...
            [&]() -> std::optional<ArchiveError> {
                if (request.kind == CreateNodeKind::kDirectory) {
                    ...
                    if (!ec) {
                        return make_archive_error(ArchiveErrorDomain::kIo, "Target already exists: " + created_path, 2);
                    }
                    return make_archive_error(ArchiveErrorDomain::kIo, ec.message(), 2);
                }
