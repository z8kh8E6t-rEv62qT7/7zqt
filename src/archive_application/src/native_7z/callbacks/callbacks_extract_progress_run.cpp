...
namespace z7::app {
    ...
    STDMETHODIMP NativeExtractCallback::SetCompleted(UInt64 const* complete_value) throw() {
    ...
    STDMETHODIMP NativeExtractCallback::SetRatioInfo(UInt64 const* in_size, UInt64 const* out_size) throw() {
        {
            std::lock_guard<std::mutex> lock(mutex_);
            if (in_size != nullptr) {
    ...
    STDMETHODIMP NativeExtractCallback::PrepareOperation(Int32 ask_extract_mode) throw() {
        if (ask_extract_mode == NArchive::NExtract::NAskMode::kExtract) {
            ...
            if (!path.empty()) {
                emit_log_event(hooks_, OperationStage::kRunning, OutputChannel::kNone, "Extracting " + path);
            }
        }
        emit_progress_snapshot();
        return check_canceled();
