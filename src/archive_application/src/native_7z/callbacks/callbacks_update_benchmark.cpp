...
namespace z7::app {
    ...
    NativeBenchStructuredCallback::NativeBenchStructuredCallback(ArchiveBackendHooks const& hooks,
    ...
    void NativeBenchStructuredCallback::update_encode_locked(CBenchInfo const& info, bool final) {
    ...
    void NativeBenchStructuredCallback::update_decode_locked(CBenchInfo const& info, bool final) {
    ...
    BenchmarkTypedMetrics NativeBenchStructuredCallback::make_metrics_locked(bool final) const {
        BenchTotals const& enc = final ? final_encode_ : current_encode_;
        BenchTotals const& dec = final ? final_decode_ : current_decode_;
        ...
        if (enc.available) {
            ...
            metrics.compress_cpu_usage_percent = Benchmark_GetUsage_Percents(enc.values.Usage);
        ...
        if (dec.available) {
            ...
            metrics.decompress_cpu_usage_percent = Benchmark_GetUsage_Percents(dec.values.Usage);
    ...
    void NativeBenchStructuredCallback::emit_snapshot_locked(BenchmarkSnapshotKind kind,
                                                             BenchmarkTypedMetrics const& metrics) {
        BenchmarkTypedSnapshot snapshot;
        snapshot.kind = kind;
        snapshot.metrics = metrics;

        emit_progress_event(
            hooks_, OperationStage::kRunning, -1, false, 0, 0, 0, 0, 0, {}, {}, std::nullopt, snapshot, summary_);
    }
    ...
    void NativeBenchStructuredCallback::emit_total_locked() {
        ...
        uint64_t const avg_usage = total.Usage / total.NumIterations2;
        uint64_t const avg_rpu = total.RPU / total.NumIterations2;
        uint64_t const avg_rating = total.Rating / total.NumIterations2;
    ...
    HRESULT NativeBenchStructuredCallback::SetEncodeResult(CBenchInfo const& info, bool final) {
        if (check_break() != S_OK) {
            return E_ABORT;
        }
        ...
        BenchmarkTypedMetrics const metrics = make_metrics_locked(final);
        emit_snapshot_locked(final ? BenchmarkSnapshotKind::kAveragePass : BenchmarkSnapshotKind::kDictionaryPass,
                             metrics);
        if (final) {
            summary_.has_average_metrics = true;
            summary_.average_metrics = metrics;
            uint64_t const completed = std::min(final_encode_updates_, final_decode_updates_);
    ...
    HRESULT NativeBenchStructuredCallback::SetDecodeResult(CBenchInfo const& info, bool final) {
        if (check_break() != S_OK) {
            return E_ABORT;
        }
    ...
    NativeBenchFreqCallback::NativeBenchFreqCallback(ArchiveBackendHooks const& hooks,
                                                     std::atomic<bool>* cancel_requested,
                                                     std::function<bool()> wait_while_paused) :
        CallbackBase(cancel_requested, std::move(wait_while_paused)), hooks_(hooks) {}

    HRESULT NativeBenchFreqCallback::check_break() const {
        return should_abort() ? E_ABORT : S_OK;
    }

    void NativeBenchFreqCallback::emit_frequency_line(std::string const& line) const {
        if (line.empty()) {
            return;
        }
        BenchmarkTypedSnapshot snapshot;
    ...
    NativeBenchmarkPrintCallback::NativeBenchmarkPrintCallback(ArchiveBackendHooks const& hooks,
                                                               std::atomic<bool>* cancel_requested,
                                                               std::function<bool()> wait_while_paused,
                                                               BenchmarkTypedParser* parser) :
        CallbackBase(cancel_requested, std::move(wait_while_paused)), hooks_(hooks), parser_(parser) {}

    void NativeBenchmarkPrintCallback::Print(char const* s) {
        if (s == nullptr) {
            return;
        }
        ...
        for (std::string const& line : ready_lines) {
            emit_line(line);
        }
    }
    ...
    void NativeBenchmarkPrintCallback::emit_line(std::string const& line) {
        if (line.empty()) {
            return;
        }
