...
namespace z7::app {
    ...
    bool NativeExtractCallback::close_pending_entry_stream_locked(PendingEntry& pending_entry,
        ...
        if (close_error_message != nullptr) {
            *close_error_message = "Failed to finalize extracted output: " + pending_entry.output_path.generic_string();
        }
        return false;
    }
    ...
    NativeExtractCallback::apply_item_attributes(fs::path const& output_path,
...
#else
        if (NWindows::NFile::NDir::MyCreateHardLink(filesystem_path_to_fstring(output_path),
                                                    filesystem_path_to_fstring(target_path))) {
            return std::nullopt;
        }
        return "Cannot create hard link: " + output_path.generic_string() + " -> " + target_path.generic_string();
#endif
    }
    ...
    std::optional<std::string> NativeExtractCallback::materialize_link(OutputTarget const& output_target,
                                                                       LinkCreationPlan const& plan,
                                                                       bool allow_defer) {
        if (plan.type == ExtractItemLinkInfo::Type::kSymLink) {
            std::optional<std::string> warning = create_symbolic_link(output_target.output_path, plan.symlink_target);
        ...
        if (plan.type == ExtractItemLinkInfo::Type::kHardLink) {
            ...
            std::optional<std::string> warning = create_hard_link(output_target.output_path, plan.hardlink_target_path);
            if (warning.has_value()) {
                return warning;
            }
            std::lock_guard<std::mutex> lock(mutex_);
    ...
    void NativeExtractCallback::finish_deferred_links() {
        ...
        for (DeferredHardLink const& deferred : deferred_links) {
            ...
            if (std::optional<std::string> const warning = materialize_link(deferred.output_target, plan, false);
                warning.has_value()) {
    ...
    void NativeExtractCallback::apply_zone_identifier_to_output(fs::path const& output_path, bool is_directory) const {
        ...
        std::string const zone_data = read_zone_identifier_stream(fs::path(archive_path_));
        ...
        std::string const quarantine_data = read_quarantine_xattr(fs::path(archive_path_));
        ...
        std::optional<std::string> attribute_warning;
        {
            ...
            if (op_res == NArchive::NExtract::NOperationResult::kOK) {
                ...
                if (!failure_message.empty()) {
                    if (pending_entry_.has_value() && pending_entry_->budget_bytes_reserved) {
                        release_reserved_budget_bytes(pending_entry_->declared_size);
                    }
                    io_error_ = true;
