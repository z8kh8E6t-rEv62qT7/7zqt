...
namespace z7::app {
    ...
    NativeUpdateOperationCallback::NativeUpdateOperationCallback(ArchiveBackendHooks const& hooks,
    ...
    NativeUpdateOperationCallback::ProgressSnapshot NativeUpdateOperationCallback::snapshot_progress() const {
        std::lock_guard<std::mutex> lock(mutex_);
    ...
    void NativeUpdateOperationCallback::note_error(std::string const& message) {
        {
            std::lock_guard<std::mutex> lock(mutex_);
            ++error_count_;
        }
