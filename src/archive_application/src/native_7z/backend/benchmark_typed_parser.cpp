...
namespace z7::app {
    namespace {
        ...
        bool parse_uint64(std::string const& text, uint64_t* out) {
        ...
        bool parse_metrics_match(std::smatch const& match, bool has_dictionary, BenchmarkTypedMetrics* out) {
            if (out == nullptr) {
                return false;
            }
            ...
            int const expected_count = has_dictionary ? kWithDictCount : kWithoutDictCount;
    ...
    std::optional<BenchmarkTypedSnapshot> BenchmarkTypedParser::consume_line(std::string const& line) {
        static std::regex const kDictLine(
            R"(^(\d+):\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+\|\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s*$)");
        static std::regex const kAvrLine(
            R"(^Avr:\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+\|\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s*$)");
        static std::regex const kTotLine(R"(^Tot:\s+(\d+)\s+(\d+)\s+(\d+)(?:\s+\d+\s+\d+)?\s*$)");
        static std::regex const kRamSizeLine(R"(^RAM size:\s+([0-9]+)\s*MB.*$)", std::regex_constants::icase);
        static std::regex const kRamUsageLine(R"(^RAM usage:\s+([0-9]+)\s*MB.*$)", std::regex_constants::icase);
        static std::regex const kFrequencyLine(R"(^\d+T\s+(?:CPU\s+)?(?:Freq|Frequency)\s+\(MHz\):.*$)");

        std::string const trimmed = z7::common::trim_ascii_space_copy(line);
        if (trimmed.empty()) {
            return std::nullopt;
        }
    ...
    BenchmarkTypedSummary const& BenchmarkTypedParser::summary() const {
        return summary_;
    }
    ...
    std::string BenchmarkTypedParser::summary_line() const {
        if (summary_.has_total_rating) {
            return "Tot: "
                 + std::to_string(summary_.total_cpu_usage_percent)
                 + " "
        ...
        if (summary_.has_average_metrics) {
            BenchmarkTypedMetrics const& m = summary_.average_metrics;
            return "Avr: "
