...
namespace z7::app {
    ...
    typename OperationRunner<TRequest, TResult>::Options make_runner_options(TRequest const& request) {
        using Runner = OperationRunner<TRequest, TResult>;
        typename OperationRunner<TRequest, TResult>::Options options;
        options.validator = [](TRequest const& req) {
        ...
        if constexpr (std::is_same_v<TRequest, DeleteRequest>) {
            ...
            options.execution_envelope_when = [](TRequest const& req) {
            ...
            options.require_codecs_when = [](TRequest const& req) {
                return (req.session_token.has_value() && req.session_token->is_valid())
                    || !req.archive_path.empty()
                    || !req.entry_path.empty();
            };
            options.execution_envelope_when = [](TRequest const& req) {
                return ((req.session_token.has_value() && req.session_token->is_valid())
                        || !req.archive_path.empty()
        ...
        if constexpr (std::is_same_v<TRequest, AddRequest> || std::is_same_v<TRequest, ArchiveCommentRequest>) {
