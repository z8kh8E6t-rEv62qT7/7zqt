...
namespace z7::app {
    namespace {
        ...
        void sort_entries(std::vector<CompressFormatCatalogEntry>& entries) {
            std::sort(entries.begin(),
                      entries.end(),
                      [](CompressFormatCatalogEntry const& lhs, CompressFormatCatalogEntry const& rhs) {
                          std::string const lhs_display = z7::common::to_lower_ascii_copy(lhs.display_name);
                          std::string const rhs_display = z7::common::to_lower_ascii_copy(rhs.display_name);
    ...
    std::vector<CompressFormatCatalogEntry> list_update_archive_formats() {
        ...
        for (unsigned i = 0; i < codecs.Formats.Size(); ++i) {
            CArcInfoEx const& info = codecs.Formats[i];
            ...
            std::string const type_id = normalize_type_id(display_name);
            if (type_id.empty()) {
                continue;
            }
