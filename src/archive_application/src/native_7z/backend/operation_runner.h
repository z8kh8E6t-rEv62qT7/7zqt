...
namespace z7::app {
    ...
    public:
        using Handler = TResult (NativeArchiveBackend::*)(TRequest const&, ArchiveBackendHooks const&);
        using Validator = std::function<std::optional<OperationResult>(TRequest const&)>;
        using Preflight = std::function<std::optional<OperationResult>(TRequest const&)>;
        using CodecGate = std::function<bool(TRequest const&)>;
        enum class ExecutionEnvelope {
            kDirect,
            kCancelable,
            kPauseable
        };
        using EnvelopeSelector = std::function<ExecutionEnvelope(TRequest const&)>;
        ...
        OperationRunner(NativeArchiveBackend& backend, ArchiveBackendHooks const& callbacks, Handler handler) :
            backend_(backend), callbacks_(callbacks), handler_(handler) {}

        TResult run(TRequest const& request, Options const& options = {}) const {
            ...
            bool const require_codecs =
                options.require_codecs || (options.require_codecs_when && options.require_codecs_when(request));
            CCodecs preloaded_codecs;
            CCodecs* bound_codecs = nullptr;
            if (require_codecs) {
                if (std::optional<OperationResult> preflight_error = preflight_load_codecs(&preloaded_codecs);
                    preflight_error.has_value()) {
                    TResult result = from_base_result<TResult>(std::move(*preflight_error));
                    lifecycle.finish();
                    normalize_result(&result);
            ...
            try {
                ...
                switch (envelope) {
                    ...
                    case ExecutionEnvelope::kPauseable:
                        {
                            std::atomic<bool>* active_flag = resolve_pauseable_flag(options.pauseable_flag);
                ...
                lifecycle.finish();
                return from_base_result<TResult>(make_exception_failure(options.operation_name, "unknown exception"));
            }
        }
    ...
    private:
        ...
        public:
            LifecycleEnvelope(ArchiveBackendHooks const& base_callbacks, bool enabled) :
                ...
                callbacks_.on_event = [this](OperationEvent const& event) {
                    this->on_event(event);
                };
            }

            ArchiveBackendHooks const& callbacks() const { return enabled_ ? callbacks_ : base_callbacks_; }
        ...
        private:
            void on_event(OperationEvent const& event) {
                if (event.kind == OperationEventKind::kLifecycle) {
                    if (event.stage == OperationStage::kFinalizing || event.stage == OperationStage::kCompleted) {
                        emit_prepare_and_running_if_missing();
                    }
            ...
            ArchiveBackendHooks const& base_callbacks_;
            bool enabled_ = false;
        ...
        public:
            ScopedCodecBinding(NativeArchiveBackend& backend, CCodecs* codecs) :
                backend_(backend), previous_(backend_.bound_codecs_) {
                backend_.bound_codecs_ = codecs;
            }

            ~ScopedCodecBinding() { backend_.bound_codecs_ = previous_; }
        ...
        static OperationResult make_exception_failure(std::string_view operation_name, std::string_view message) {
        ...
        static void normalize_result(TResult* result) {
            ...
            if (!result->ok && result->error.domain == ArchiveErrorDomain::kNone) {
                result->error = make_archive_error(ArchiveErrorDomain::kUnknown, "Unknown operation failure", 2);
            }
        ...
        NativeArchiveBackend& backend_;
        ArchiveBackendHooks const& callbacks_;
        Handler handler_;
    };
