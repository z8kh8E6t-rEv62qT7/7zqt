...
namespace z7::app {
    namespace {
        ...
        NUpdateArchive::CActionSet action_set_for_mode(std::string const& update_mode) {
            std::string const mode = z7::common::to_lower_ascii_copy(update_mode);
            if (mode == "update") {
        ...
        bool parse_raw_update_switch(std::string const& raw_switch,
                                     NUpdateArchive::CActionSet const& default_action_set,
                                     ParsedRawUpdateSwitch* out) {
            if (out == nullptr) {
                return false;
            }

            std::string const raw = z7::common::trim_ascii_space_copy(raw_switch);
            if (raw.empty()) {
                return false;
            }
            ...
            while (index < raw.size()) {
                char const state_id = static_cast<char>(std::tolower(static_cast<unsigned char>(raw[index])));
                size_t const state_pos = kUpdatePairStateIds.find(state_id);
        ...
        bool apply_update_action_set(AddRequest const& request, CUpdateOptions& options, std::string* error_summary) {
            ...
            for (std::string const& raw_switch : raw_switches) {
                ...
                std::string const user_archive_path = parsed.post_string.substr(1);
...
#ifndef _WIN32
        void mark_sfx_archive_executable(AddRequest const& request, AddResult* result) {
            if (result == nullptr || !result->ok || !request.create_sfx || request.archive_path.empty()) {
                return;
            }

            std::error_code ec;
            std::filesystem::permissions(std::filesystem::path(request.archive_path),
            ...
            *result = make_operation_failure<AddResult>(
                ArchiveErrorDomain::kIo, "Failed to mark SFX archive executable: " + ec.message(), 2);
        }
#endif
    ...
    std::string session_format_token(ArchiveOpenSession const& session) {
        ...
        return z7::common::canonical_archive_type_token_copy(ustring_to_utf8(UString(format_name)));
    }

    AddResult NativeArchiveBackend::add(AddRequest const& request, ArchiveBackendHooks const& hooks) {
        ...
        AddResult add_result = run_update_operation_with_mode<AddResult>(
            ...
            [&](CCodecs& codecs, CObjectVector<COpenType>& types, NWildcard::CCensor& censor, CUpdateOptions& options)
                ...
                if (!ParseOpenTypes(codecs, utf8_to_ustring(prepared_request.format), types) || types.IsEmpty()) {
                    return static_cast<OperationResult>(make_operation_failure<AddResult>(
                        ArchiveErrorDomain::kUnsupportedFormat, "Unsupported archive format", 2));
                }

                for (std::string const& input : prepared_request.input_paths) {
                    censor.AddPreItem_NoWildcard(utf8_to_ustring(input));
                }

                options.InputItemSourcePaths.Clear();
                options.InputItemArchivePaths.Clear();
                for (AddInputItem const& item : prepared_request.input_items) {
                ...
                if (!apply_update_action_set(prepared_request, options, &update_switch_error)) {
                    return static_cast<OperationResult>(make_operation_failure<AddResult>(
                        ArchiveErrorDomain::kInvalidArguments, std::move(update_switch_error), 7));
                }
