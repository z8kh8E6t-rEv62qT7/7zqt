    ...
    namespace {
        ...
        primary_output_from_remaps(ExtractRequest const& request,
            ...
            for (ExtractPathRemap const& remap : request.path_remaps) {
                ...
                std::string const remap_destination = fs::absolute(fs::path(remap.destination_path)).generic_string();
        ...
        void cleanup_extract_overwrite_backups_best_effort(std::vector<ExtractRollbackEntry> const& entries) {
        ...
        RollbackAttemptResult rollback_extract_entries(std::vector<ExtractRollbackEntry> const& entries) {
            ...
            for (auto it = entries.rbegin(); it != entries.rend(); ++it) {
                ...
                if (entry.had_original) {
                    if (entry.backup_path.empty()) {
                        ...
                        if (result.first_error.empty()) {
                            result.first_error = "Cannot restore overwritten output without a backup: "
                    ...
                    if (ec) {
                        result.ok = false;
                        if (result.first_error.empty()) {
                            result.first_error = "Failed to restore overwritten output: " + ec.message();
                        }
                    }
                    continue;
                }
                ...
                if (ec && result.first_error.empty()) {
                    result.ok = false;
                    result.first_error = "Failed to remove rollback output: " + ec.message();
                }
            }
            ...
            if (status.budget_triggered) {
                ...
                switch (status.budget_policy) {
                    case BudgetExceededAction::kFailAndRollback:
                        {
                            RollbackAttemptResult const rollback = rollback_extract_entries(status.rollback_entries);
                            result.ok = false;
                            if (rollback.ok) {
    ...
    ExtractResult NativeArchiveBackend::extract(ExtractRequest const& request, ArchiveBackendHooks const& hooks) {
        if (!request.archive_paths.empty()) {
            ...
            for (std::string const& archive : request.archive_paths) {
                ...
                std::vector<ExtractMaterializedEntry> accumulated = std::move(merged.materialized_entries);
        ...
        if (request.session_token.has_value() && request.session_token->is_valid()) {
            ...
            if (session_request.archive_path.empty()) {
                session_request.archive_path = ArchiveOpenSessionNativeAccess::source_archive_path(*session);
            }
            ...
            ExtractResult result =
                run_extract_on_arc(arc, num_items, session_request, session_hooks, cancel_requested_, [this]() {
        ...
        if (extract_open_requires_callback(request, hooks)) {
            ...
            ArchiveBackendHooks const open_hooks = hooks_with_extract_open_password(request, hooks);
            ...
            OperationResult const close_result = close_native_archive_session(
                ArchiveSessionRegistry::instance(), opened.token, hooks, &cancel_requested_, [this]() {
                    return this->wait_while_paused();
                });
        ...
        return run_open_archive_read_pipeline<ExtractResult>(
            ...
            [&](OpenArchiveReadState const& open_state, UInt32 num_items) -> ExtractResult {
                return run_extract_on_arc(open_state.arc, num_items, request, hooks, cancel_requested_, [this]() {
                    return this->wait_while_paused();
                });
            });
    }
