...
namespace z7::app {
    namespace {
        ...
        bool is_valid_level_value(std::string const& value) {
            if (value.empty()) {
                return false;
            }
            uint64_t parsed = 0;
        ...
        bool parse_add_path_mode(std::string const& path_mode_raw, NWildcard::ECensorPathMode* out_mode) {
            if (out_mode == nullptr) {
                return false;
            }
            std::string const path_mode =
                z7::common::to_lower_ascii_copy(z7::common::trim_ascii_space_copy(path_mode_raw));
        ...
        bool parse_positive_thread_count(std::string const& raw, std::string* normalized) {
            if (normalized == nullptr) {
                return false;
            }
            std::string const value = z7::common::trim_ascii_space_copy(raw);
        ...
        bool parse_size_like_method_value(std::string const& raw, bool allow_on_off, std::string* normalized) {
            if (normalized == nullptr) {
                return false;
            }
            std::string const value = z7::common::trim_ascii_space_copy(raw);
            ...
            std::string const lower = z7::common::to_lower_ascii_copy(value);
        ...
        std::string normalize_extra_property_token(std::string const& raw) {
        ...
        bool append_extra_method_properties(std::vector<std::string> const& raw_tokens,
                                            CObjectVector<CProperty>& properties,
                                            std::string* error_summary) {
            for (std::string const& raw_token : raw_tokens) {
                std::string const trimmed = z7::common::trim_ascii_space_copy(raw_token);
                std::string const token = normalize_extra_property_token(raw_token);
                if (token.empty()) {
                    if (!trimmed.empty()) {
                ...
                std::string const name = separator == std::string::npos ? token : token.substr(0, separator);
                if (name.empty()) {
                    if (error_summary != nullptr) {
                        *error_summary = "Invalid compression property token: " + raw_token;
                    }
                ...
                std::string const value = separator == std::string::npos ? std::string() : token.substr(separator + 1);
                add_method_property(properties, name, value);
            }
            return true;
        }

        bool add_format_supports_encrypted_headers(std::string const& format) {
    ...
    bool apply_add_runtime_options(AddRequest const& request, CUpdateOptions& options, std::string* error_summary) {
        ...
        if (options.SfxMode) {
            ...
            std::error_code ec;
            std::filesystem::path const module_path(request.sfx_module_path);
            if (!std::filesystem::is_regular_file(module_path, ec)) {
                if (error_summary != nullptr) {
                    *error_summary = "SFX module file does not exist: " + request.sfx_module_path;
                }
                return false;
            }
        ...
        if (!request.volume_size.empty()) {
            ...
            if (!parse_volume_sizes_spec(request.volume_size, parsed_volumes) || parsed_volumes.empty()) {
                if (error_summary != nullptr) {
        ...
        if (!request.compression_level.empty()) {
            std::string const level = z7::common::trim_ascii_space_copy(request.compression_level);
        ...
        if (!request.method_value.empty()) {
            add_method_property(properties, is_7z ? "0" : "m", z7::common::trim_ascii_space_copy(request.method_value));
        }
        ...
        if (!request.password.empty()) {
            ...
            if (request.encrypt_headers_defined) {
                if (is_7z) {
                    ...
                    if (error_summary != nullptr) {
                        *error_summary = "Encrypted file names are supported only for 7z archives";
                    }
                    return false;
                }
            }
    ...
    CompressionResourcesEstimate estimate_compression_resources_for_request(AddRequest const& request) {
        ...
        out.resolved_threads = std::max<uint32_t>(1, parse_thread_count_or_default(request.thread_count, cpu_threads));
        out.resolved_dictionary_bytes = parse_size_to_bytes_or_default(request.dictionary_size, 32ULL << 20);

        std::string const method =
            z7::common::to_lower_ascii_copy(z7::common::trim_ascii_space_copy(request.method_value));
        ...
        out.compression_usage_bytes =
            out.resolved_dictionary_bytes * static_cast<uint64_t>(out.resolved_threads) * compress_multiplier;
        out.decompression_usage_bytes = out.resolved_dictionary_bytes / std::max<uint64_t>(1, decompress_divider);

        FOR_VECTOR(i, options.MethodMode.Properties) {
            CProperty const& prop = options.MethodMode.Properties[i];
            std::string const name =
                z7::common::to_lower_ascii_copy(z7::common::trim_ascii_space_copy(ustring_to_utf8(prop.Name)));
            if (name != "memuse") {
                continue;
            }
            std::string const value =
                z7::common::to_lower_ascii_copy(z7::common::trim_ascii_space_copy(ustring_to_utf8(prop.Value)));
            if (value.empty()) {
                continue;
            }
            constexpr uint64_t kInvalidSentinel = std::numeric_limits<uint64_t>::max();
            uint64_t const parsed = parse_size_to_bytes_or_default(value, kInvalidSentinel);
