...
namespace z7::app {
    ...
    TResult make_operation_failure(ArchiveError error,
    ...
    template <typename TResult>
    TResult make_operation_failure(ArchiveErrorDomain domain,
                                   std::string message,
                                   int native_exit_code,
                                   NativeTerminationReason termination_reason = NativeTerminationReason::kCompleted) {
        return make_operation_failure<TResult>(make_archive_error(domain, std::move(message), native_exit_code),
                                               termination_reason);
    }
    ...
    TResult make_operation_failure_from_hresult(int hr) {
        ...
        NativeTerminationReason const termination_reason = error.domain == ArchiveErrorDomain::kCanceled
    ...
    template <typename TResult>
    TResult make_operation_partial_success(std::string message = "Operation completed with warnings") {
        return make_operation_failure<TResult>(ArchiveErrorDomain::kPartialSuccess, std::move(message), 1);
    }

    template <typename TResult>
    TResult make_operation_canceled() {
        return make_operation_failure<TResult>(
            ArchiveErrorDomain::kCanceled, "Operation canceled", 255, NativeTerminationReason::kCanceled);
    }
    ...
    template <typename TResult>
    TResult finalize_update_operation_result(ArchiveBackendHooks const& callbacks,
    ...
    TResult finalize_hresult_operation_result(std::atomic<bool> const& cancel_requested,
                                              int hresult,
    ...
    struct HasExtractIoErrorMethods<CallbackT,
                                    std::void_t<decltype(std::declval<CallbackT*>()->has_io_error()),
                                                decltype(std::declval<CallbackT*>()->io_error_message())>>
        : std::true_type {};
    ...
    template <typename CallbackT>
    struct HasExtractDiagnosticMethod<CallbackT,
    ...
    template <typename CallbackT>
    struct HasTakeMaterializedEntries<CallbackT,
    ...
    template <typename CallbackT>
    struct HasTakeRollbackEntries<CallbackT, std::void_t<decltype(std::declval<CallbackT*>()->take_rollback_entries())>>
        : std::true_type {};
    ...
    TResult finalize_extract_operation_result(ArchiveBackendHooks const& hooks,
                                              std::atomic<bool> const& cancel_requested,
    ...
    TResult run_filesystem_path_batch(Paths const& paths,
                                      ArchiveBackendHooks const& hooks,
                                      std::atomic<bool>& cancel_requested,
                                      ActionFn&& action,
                                      std::string success_summary,
                                      OnSuccess&& on_success) {
        uint64_t const total = static_cast<uint64_t>(paths.size());
    ...
    TResult run_filesystem_single_step(ArchiveBackendHooks const& hooks,
                                       std::atomic<bool>& cancel_requested,
        ...
        emit_progress_event(hooks, OperationStage::kRunning, -1, false, 0, 0, 1, 0, 0, progress_path, {});
        ...
        TResult result = make_operation_success<TResult>(std::move(success_summary));
        on_success(result);
        emit_progress_event(hooks, OperationStage::kRunning, 100, false, 0, 0, 1, 1, 0, progress_path, {});
        return result;
    }

    std::string normalize_archive_virtual_directory(std::string directory);
    std::vector<std::string> split_archive_virtual_directory(std::string const& directory);
    bool archive_virtual_path_is_safe_for_materialization(std::string const& normalized_path);

    bool archive_get_prop_uint64(IInArchive* archive, UInt32 index, PROPID prop_id, uint64_t& value);
    bool archive_get_prop_uint32(IInArchive* archive, UInt32 index, PROPID prop_id, uint32_t& value);
    bool archive_get_prop_bool(IInArchive* archive, UInt32 index, PROPID prop_id, bool& value);
    bool archive_get_prop_time_msecs_utc(IInArchive* archive, UInt32 index, PROPID prop_id, int64_t& value);
    std::string archive_get_prop_text(IInArchive* archive, UInt32 index, PROPID prop_id);

    void fill_archive_list_entry_details(IInArchive* archive, UInt32 arc_index, ArchiveListEntry& entry);
