...
namespace z7::app {
    namespace {
        ...
        std::string proxy_dir_name(CProxyDir const& dir) {
            return z7::common::trim_ascii_space_copy(ustring_to_utf8(UString(dir.Name)));
        }

        std::string proxy_file_name(CProxyFile const& file) {
            return z7::common::trim_ascii_space_copy(ustring_to_utf8(UString(file.Name)));
        }
        ...
        find_subdir_through_current_dirs(CProxyArc const& proxy, unsigned dir_index, std::string const& name) {
            ...
            for (unsigned i = 0; i < dir.SubDirs.Size(); ++i) {
                ...
                if (std::optional<unsigned> found = find_subdir_through_current_dirs(proxy, child_dir_index, name);
                    found.has_value()) {
                    return found;
                }
            }
        ...
        std::optional<unsigned>
        find_proxy2_subdir_through_current_dirs(CProxyArc2 const& proxy, unsigned dir_index, std::string const& name) {
            UString const name_u = utf8_to_ustring(name);
            ...
            if (direct >= 0) {
                ...
                if (static_cast<unsigned>(direct) < dir.Items.Size()) {
                    UInt32 const arc_index = dir.Items[static_cast<unsigned>(direct)];
            ...
            for (unsigned i = 0; i < dir.Items.Size(); ++i) {
                ...
                if (std::optional<unsigned> found =
                        find_proxy2_subdir_through_current_dirs(proxy, static_cast<unsigned>(file.DirIndex), name);
                    found.has_value()) {
                    return found;
                }
            }
    ...
    int list_archive_entries_from_arc(CArc const* arc,
        ...
        if (use_proxy2) {
            ...
            for (std::string const& part : path_parts) {
                ...
                std::optional<unsigned> const next_dir =
                    find_proxy2_subdir_through_current_dirs(proxy, target_dir_index, part);
            ...
            auto emit_proxy2_dir = [&](unsigned dir_index, std::string const& relative_base, auto&& self) -> HRESULT {
                CProxyDir2 const& dir = proxy.Dirs[dir_index];
                for (unsigned i = 0; i < dir.Items.Size(); ++i) {
                    if (const HRESULT canceled = abort_if_canceled(cancel_requested); canceled != S_OK) {
                        return canceled;
                    ...
                    bool const can_recurse =
                        file.IsDir() && file.DirIndex >= 0 && static_cast<unsigned>(file.DirIndex) < proxy.Dirs.Size();
                    unsigned child_dir_index = 0;
                    if (can_recurse) {
                        child_dir_index = static_cast<unsigned>(file.DirIndex);
                        if (is_current_dir_segment(name)) {
                            if (const HRESULT hr = self(child_dir_index, relative_base, self); hr != S_OK) {
                                return hr;
                            }
                            continue;
                        }
                    ...
                    if (recursive_dirs && can_recurse) {
                        if (const HRESULT hr = self(child_dir_index, child_relative_path, self); hr != S_OK) {
                            return hr;
                        }
                    }
                }
            ...
            if (const HRESULT hr = emit_proxy2_dir(target_dir_index, std::string(), emit_proxy2_dir); hr != S_OK) {
                return hr;
            }
        ...
        for (std::string const& part : path_parts) {
            ...
            std::optional<unsigned> const next_dir = find_subdir_through_current_dirs(proxy, target_dir_index, part);
            if (!next_dir.has_value()) {
                return S_OK;
            }
            target_dir_index = *next_dir;
        ...
        auto emit_proxy_dir = [&](unsigned dir_index, std::string const& relative_base, auto&& self) -> HRESULT {
            CProxyDir const& dir = proxy.Dirs[dir_index];
            ...
            for (unsigned i = 0; i < dir.SubFiles.Size(); ++i) {
                ...
                entry.is_dir = false;
                entry.size =
                    (arc->GetItem_Size(arc_index, item_size, size_defined) == S_OK && size_defined) ? item_size : 0;
                if (include_detailed_props) {
                    fill_archive_list_entry_details(arc->Archive, arc_index, entry);
                }
                if (const HRESULT hr = deliver(std::move(entry)); hr != S_OK) {
        ...
        if (const HRESULT hr = emit_proxy_dir(target_dir_index, std::string(), emit_proxy_dir); hr != S_OK) {
            return hr;
        }
        return flush_pending_batch();
    }
