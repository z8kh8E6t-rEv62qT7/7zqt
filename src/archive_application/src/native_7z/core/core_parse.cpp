...
namespace z7::app {
    ...
    bool is_auto_value(std::string const& value) {
        std::string const normalized = z7::common::to_lower_ascii_copy(z7::common::trim_ascii_space_copy(value));
    ...
    uint64_t parse_size_to_bytes_or_default(std::string const& raw, uint64_t default_bytes) {
        std::string value = z7::common::to_lower_ascii_copy(z7::common::trim_ascii_space_copy(raw));
        if (value.empty()) {
            return default_bytes;
        }

        uint64_t mult = 1;
        char const suffix = value.back();
    ...
    uint32_t parse_thread_count_or_default(std::string const& raw, uint32_t default_threads) {
        ...
        uint64_t parsed = 0;
        if (!parse_uint64_decimal(z7::common::trim_ascii_space_copy(raw), parsed) || parsed == 0) {
            return default_threads;
        }
    ...
    uint64_t get_number_of_volumes(uint64_t size, std::vector<uint64_t> const& vol_sizes) {
        ...
        for (size_t i = 0; i < vol_sizes.size(); ++i) {
            uint64_t const vol_size = vol_sizes[i];
            if (vol_size >= size) {
        ...
        uint64_t const vol_size = vol_sizes.back();
        if (vol_size == 0) {
    ...
    std::string VolumeSequenceState::next_name() {
        for (int i = static_cast<int>(changed_part.size()) - 1; i >= 0; --i) {
            char const c = changed_part[static_cast<size_t>(i)];
            if (c != '9') {
    ...
    bool VolumeSequenceState::parse_name(std::string const& name) {
