...
namespace z7::app {
    namespace {
        ...
        constexpr UInt32 kLargePagesEnabledFlags = Z7_LARGE_PAGES_FLAG_USE_HUGEPAGE
...
#endif
        bool const was_enabled = g_large_pages_runtime_enabled;
        apply_large_pages_runtime_mode(true, true);
        void* allocation = BigAlloc(kMacLargePageSize);
        bool const ok = allocation != nullptr;
