...
namespace z7::app {
    ...
    std::string normalize_archive_virtual_directory(std::string directory) {
        ...
        while (start <= directory.size()) {
            size_t const slash = directory.find('/', start);
            std::string const token =
                directory.substr(start, slash == std::string::npos ? std::string::npos : slash - start);
    ...
    bool archive_virtual_path_is_safe_for_materialization(std::string const& normalized_path) {
        for (std::string const& part : split_archive_virtual_directory(normalized_path)) {
            if (part == "..") {
                return false;
            }
        }
    ...
    bool archive_get_prop_time_msecs_utc(IInArchive* archive, UInt32 index, PROPID prop_id, int64_t& value) {
        NWindows::NCOM::CPropVariant prop;
        if (archive->GetProperty(index, prop_id, &prop) != S_OK || prop.vt != VT_FILETIME) {
            return false;
        }

        UInt32 quantums = 0;
        Int64 const seconds = NWindows::NTime::FileTime_To_UnixTime64_and_Quantums(prop.filetime, quantums);
    ...
    std::string archive_get_prop_text(IInArchive* archive, UInt32 index, PROPID prop_id) {
        NWindows::NCOM::CPropVariant prop;
        if (archive->GetProperty(index, prop_id, &prop) != S_OK || prop.vt == VT_EMPTY) {
            return {};
        }
