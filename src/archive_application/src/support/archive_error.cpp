...
namespace z7::app {
    ...
    ArchiveError make_archive_error(ArchiveErrorDomain domain, std::string message, int native_exit_code) {
        ArchiveError error;
        error.domain = domain;
    ...
    ArchiveError
    map_native_exit_code(int native_exit_code, NativeTerminationReason termination_reason, std::string const&) {
        if (native_exit_code == 0) {
            return make_archive_error(ArchiveErrorDomain::kNone, {}, native_exit_code);
        }

        if (termination_reason == NativeTerminationReason::kCanceled || native_exit_code == 255) {
            return make_archive_error(ArchiveErrorDomain::kCanceled, "Operation canceled", native_exit_code);
        }

        if (native_exit_code == 1) {
            return make_archive_error(
                ArchiveErrorDomain::kPartialSuccess, "Operation completed with warnings", native_exit_code);
        }

        if (native_exit_code == 7) {
            return make_archive_error(
                ArchiveErrorDomain::kInvalidArguments, "Invalid request arguments", native_exit_code);
        }

        if (native_exit_code == 8 || termination_reason == NativeTerminationReason::kAborted) {
            return make_archive_error(
                ArchiveErrorDomain::kBackendUnavailable, "Requested backend is unavailable", native_exit_code);
        }

        return make_archive_error(ArchiveErrorDomain::kUnknown, "Unknown archive backend error", native_exit_code);
    }
