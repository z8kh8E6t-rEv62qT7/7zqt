...
namespace z7::app {
    ...
    ArchiveSessionEventSink::ArchiveSessionEventSink(std::shared_ptr<IArchiveDelegate> delegate,
    ...
    void ArchiveSessionEventSink::on_event(OperationEvent const& event) {
    ...
    void ArchiveSessionEventSink::merge_diagnostics(OperationOutcome& outcome) const {
        std::lock_guard<std::mutex> lock(mutex_);
        for (std::string const& diagnostic : collected_diagnostics_) {
            if (diagnostic.empty()) {
                continue;
            }
            if (std::find(outcome.diagnostics.begin(), outcome.diagnostics.end(), diagnostic)
    ...
    void ArchiveSessionEventSink::on_lifecycle_event(OperationEvent const& event) {
        if (delegate_) {
    ...
    void ArchiveSessionEventSink::on_log_event(OperationEvent const& event) {
    ...
    void ArchiveSessionEventSink::on_progress_event(OperationEvent const& event) {
