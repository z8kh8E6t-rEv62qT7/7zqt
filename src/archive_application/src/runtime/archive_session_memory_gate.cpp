...
namespace z7::app::archive_session_helpers {
    ...
    std::optional<OperationResult> maybe_block_benchmark_for_memory_limit(ArchiveRequest const& request,
        ...
        auto const& benchmark = std::get<BenchmarkRequest>(request.payload);
        BenchmarkMemoryEstimate const estimate = estimate_benchmark_memory_native(benchmark);
        ...
        if (delegate != nullptr) {
            if (std::optional<MemoryLimitReply> const maybe_reply = delegate->request_memory_limit(prompt);
        ...
        bool const allow_once = reply.action == MemoryLimitAction::kAllowOnce;
        bool const allow_with_updated_limit = reply.action == MemoryLimitAction::kUpdateLimitAndContinue
        ...
        result.error = make_archive_error(
            ArchiveErrorDomain::kCanceled, "Operation skipped due to memory limit policy.", result.native_exit_code);
        result.summary = describe_archive_error(result.error);
        return result;
    }
