...
namespace z7::app {
    ...
    ArchiveSessionCore::ArchiveSessionCore(ArchiveRequest const& request,
                                           std::shared_ptr<IArchiveDelegate> delegate,
                                           std::unique_ptr<INativeArchiveBackend> backend) :
        request_(request),
        delegate_(std::move(delegate)),
        backend_(std::move(backend)),
        event_sink_(delegate_, request_, [this](ArchiveSessionState state) { set_state(state); }),
    ...
    void ArchiveSessionCore::run_worker() {
        ...
        ArchiveBackendHooks callbacks = interaction_broker_.make_hooks(
            request_, [this](OperationEvent const& event) { event_sink_.on_event(event); });

        NativeInvokeResult invoke_result;
        if (backend_ != nullptr) {
            if (std::optional<OperationResult> const blocked =
                ...
                if (std::optional<OperationResult> const missing_interaction =
        ...
        OperationOutcome outcome = make_outcome(invoke_result.base, std::move(invoke_result.payload));
