...
namespace z7::task_ipc_runtime::task_ipc_internal {
    namespace {
        ...
        bool create_or_attach_shared_memory(QSharedMemory* memory,
                                            bool allow_create,
                                            int size,
                                            QString const& create_failure,
                                            QString const& attach_failure,
                                            bool* out_created,
            ...
            if (allow_create) {
                ...
                if (memory->error() != QSharedMemory::AlreadyExists) {
                    if (error_message != nullptr) {
                        *error_message = error_string_from_shared_memory(memory, create_failure);
                    }
                    return false;
                }
            }
    ...
    bool initialize_bootstrap_if_needed(QSharedMemory* memory, bool force_init, QString* error_message) {
        ...
        if (raw == nullptr) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Task IPC bootstrap shared memory is null.");
            }
            return false;
        }
        if (memory->size() < static_cast<int>(sizeof(TaskIpcBootstrapRaw))) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Task IPC bootstrap shared memory size is invalid "
                                                "(actual=%1 expected=%2).")
                                     .arg(memory->size())
        ...
        if (force_init) {
            ...
            for (int i = 0; i < kTaskIpcSlotCount; ++i) {
                ...
                raw->slot_records[i].state = static_cast<quint32>(TaskIpcSlotState::kEmpty);
        ...
        if (raw->magic != kTaskIpcMagic
            ...
            || raw->slot_count != static_cast<quint16>(kTaskIpcSlotCount)) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Task IPC bootstrap shared memory header is invalid.");
            }
            return false;
        }
        return true;
    ...
    bool initialize_request_pool_if_needed(QSharedMemory* memory, bool force_init, QString* error_message) {
        ...
        if (force_init) {
            std::memset(raw, 0, static_cast<size_t>(kTaskIpcRequestPoolSharedMemorySize));
        ...
        if (raw->magic != kTaskIpcRequestPoolMagic
            ...
            || raw->slot_count != static_cast<quint16>(kTaskIpcSlotCount)) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Task IPC request-pool shared memory header is invalid.");
            }
            return false;
        }
        return true;
    ...
    bool open_bootstrap_memory(bool allow_create, std::shared_ptr<QSharedMemory>* out_memory, QString* error_message) {
        ...
        if (out_memory == nullptr) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Missing task IPC bootstrap output storage.");
            }
            return false;
        }

        std::shared_ptr<QSharedMemory> const leased_memory = current_bootstrap_memory_lease();
        if (leased_memory != nullptr
            && leased_memory->isAttached()
        ...
        for (int attempt = 0; attempt < 2; ++attempt) {
            ...
            bool created = false;
            if (!create_or_attach_shared_memory(memory.get(),
                                                allow_create,
                                                static_cast<int>(sizeof(TaskIpcBootstrapRaw)),
                                                QStringLiteral("Failed to create task IPC bootstrap shared memory."),
                                                QStringLiteral("Failed to attach task IPC bootstrap shared memory."),
                                                &created,
                                                error_message)) {
                return false;
            }
        ...
        if (error_message != nullptr) {
            *error_message = QStringLiteral("Failed to initialize task IPC bootstrap shared memory.");
        }
        return false;
#endif
    }
    ...
    open_request_pool_memory(bool allow_create, std::shared_ptr<QSharedMemory>* out_memory, QString* error_message) {
        ...
        if (out_memory == nullptr) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Missing task IPC request-pool output storage.");
            }
            return false;
        }

        std::shared_ptr<QSharedMemory> const leased_memory = current_request_pool_memory_lease();
        if (leased_memory != nullptr
            && leased_memory->isAttached()
        ...
        for (int attempt = 0; attempt < 2; ++attempt) {
            ...
            if (!create_or_attach_shared_memory(memory.get(),
                                                allow_create,
                                                kTaskIpcRequestPoolSharedMemorySize,
                                                QStringLiteral("Failed to create task IPC request-pool shared memory."),
                                                QStringLiteral("Failed to attach task IPC request-pool shared memory."),
                                                &created,
                                                error_message)) {
                return false;
            }
        ...
        if (error_message != nullptr) {
            *error_message = QStringLiteral("Failed to initialize task IPC request-pool shared memory.");
        }
        return false;
#endif
    }
...
namespace z7::task_ipc_runtime {
    ...
    QString ensure_task_ipc_owner_instance_id() {
        ...
        QVariant const existing = app->property(task_ipc_internal::kTaskIpcOwnerIdProperty);
        QString const value = existing.toString().trimmed();
        if (!value.isEmpty()) {
            return value;
        }
        QString const generated = QUuid::createUuid().toString(QUuid::WithoutBraces);
    ...
    bool ensure_task_ipc_bootstrap_ready(QString* error_message) {
        ...
        return task_ipc_internal::open_request_pool_memory(true, &request_pool_memory, error_message);
#endif
    }
    ...
    bool set_task_ipc_event_notifier(QString const& owner_instance_id,
    ...
    bool clear_task_ipc_event_notifier(QString const& owner_instance_id, QString* error_message) {
        QString normalized_owner_instance_id;
        if (!task_ipc_internal::validate_task_ipc_owner_instance_id(
                owner_instance_id, &normalized_owner_instance_id, error_message)) {
            return false;
    ...
    bool set_task_ipc_cancel_notifier(TaskIpcClaimedTask const& task,
                                      TaskIpcCancelNotifier notifier,
                                      QString* error_message) {
        return task_ipc_internal::start_task_ipc_cancel_notification_thread(task, std::move(notifier), error_message);
    }
