...
namespace z7::task_ipc_runtime {
    ...
    namespace {
        ...
        bool start_task_ipc_worker_process(QString const& worker_program,
                                           ...
                                           TaskIpcManagedProcessOptions const* process_options,
            ...
            if (process_options == nullptr) {
                qint64 worker_pid = 0;
                bool const started = QProcess::startDetached(worker_program, worker_args, working_dir, &worker_pid);
                if (!started) {
                    if (error_message != nullptr) {
                        *error_message = QStringLiteral("Failed to start task IPC worker process.");
                    }
                    return false;
                }
                if (out_worker_pid != nullptr) {
            ...
            if (out_process == nullptr) {
                if (error_message != nullptr) {
                    *error_message = QStringLiteral("Missing managed task IPC worker process output.");
                }
                return false;
            }
            ...
            if (!process->waitForStarted(30000)) {
                if (error_message != nullptr) {
                    QString const detail = process->errorString().trimmed();
                    *error_message =
                        detail.isEmpty() ? QStringLiteral("Failed to start task IPC worker process.") : detail;
                }
                return false;
            }
        ...
        bool dispatch_task_ipc_task_impl(QString const& worker_program,
                                         QString const& working_dir,
                                         QString const& owner_instance_id,
                                         TaskIpcPayload const& payload,
                                         TaskIpcManagedProcessOptions const* process_options,
            ...
            if (out_result == nullptr) {
                if (error_message != nullptr) {
                    *error_message = QStringLiteral("Missing task IPC dispatch result output.");
                }
                return false;
            }
            ...
            if (!validate_task_ipc_owner_instance_id(owner_instance_id, &normalized_owner_instance_id, error_message)) {
                return false;
            }

            QString payload_error;
            QByteArray const encoded_payload = serialize_task_payload(payload, &payload_error);
            if (encoded_payload.isEmpty()) {
                if (error_message != nullptr) {
                    *error_message = payload_error.isEmpty()
            ...
            worker_args
                << QStringLiteral("--task-ipc-session=%1").arg(session_id)
            ...
            qint64 worker_pid = 0;
            if (!start_task_ipc_worker_process(worker_program,
                                               worker_args,
                                               working_dir,
                                               process_options,
                                               out_process,
            ...
            TaskIpcRequestPoolHeaderRaw* request_pool = request_pool_raw(request_pool_memory.get());
            if (bootstrap == nullptr || request_pool == nullptr) {
                if (error_message != nullptr) {
                    *error_message = QStringLiteral("Task IPC shared memory payload pointer is null.");
                }
                return false;
            }
            ...
            QString const bootstrap_lock_wake_key = task_ipc_bootstrap_lock_wake_key();
            {
                SharedMemoryLock bootstrap_lock(&bootstrap->lock, bootstrap_lock_wake_key);
                if (!bootstrap_lock.ok()) {
                    if (error_message != nullptr) {
                        *error_message = bootstrap_lock.busy() ? QStringLiteral("Task IPC bootstrap is busy.")
                ...
                if (!ensure_task_ipc_semaphore_exists(task_ipc_cancel_semaphore_key_for_task(session_id, generation),
                                                      error_message)) {
                    return false;
                }
                ...
                if (!slot_lock.ok()) {
                    if (error_message != nullptr) {
                        *error_message = slot_lock.busy() ? QStringLiteral("Task IPC request-pool slot is busy.")
                                                          : slot_lock.error();
                    }
                    return false;
                }

                if (!write_request_payload_to_slot(
                        request_pool_memory.get(), slot_index, encoded_payload, error_message)) {
                    return false;
                }
                ...
                write_fixed_utf8(normalized_owner_instance_id, slot.owner_instance_id, kTaskIpcOwnerInstanceIdCapacity);
            ...
            post_task_ipc_semaphore(non_posix_task_ipc_event_semaphore_key(normalized_owner_instance_id), nullptr);
    ...
    bool dispatch_task_ipc_task(QString const& worker_program,
                                QString const& working_dir,
                                QString const& owner_instance_id,
                                TaskIpcPayload const& payload,
                                TaskIpcDispatchResult* out_result,
                                QString* error_message) {
        return dispatch_task_ipc_task_impl(
            worker_program, working_dir, owner_instance_id, payload, nullptr, out_result, nullptr, error_message);
    }

    bool dispatch_task_ipc_task_managed_process(QString const& worker_program,
                                                QString const& working_dir,
                                                QString const& owner_instance_id,
                                                TaskIpcPayload const& payload,
                                                TaskIpcManagedProcessOptions const& process_options,
    ...
    bool request_task_ipc_cancel(quint64 session_id, quint32 generation, QString* error_message) {
        using namespace task_ipc_internal;
        ...
        if (session_id == 0 || generation == 0U) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Task IPC cancel requires session and generation.");
            }
            return false;
        }
        ...
        std::atomic_ref<quint32> const state(slot.state);
        quint32 const state_value = state.load(std::memory_order_acquire);
        ...
        SharedMemoryLock bootstrap_lock(&bootstrap->lock, task_ipc_bootstrap_lock_wake_key());
        if (!bootstrap_lock.ok()) {
            if (error_message != nullptr) {
                *error_message =
                    bootstrap_lock.busy() ? QStringLiteral("Task IPC bootstrap is busy.") : bootstrap_lock.error();
            }
            return false;
        }
        ...
        for (int i = 0; i < kTaskIpcSlotCount; ++i) {
            ...
            slot.updated_msecs = now_msecs();
            return post_task_ipc_semaphore(task_ipc_cancel_semaphore_key_for_task(slot.session_id, slot.generation),
                                           error_message);
        }
        return true;
#endif
    ...
    bool claim_task_ipc_task_for_worker(quint64 session_id,
                                        quint32 generation,
        ...
        *out_task = TaskIpcClaimedTask{};
        ...
        if (session_id == 0 || generation == 0U) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Task IPC claim requires session and generation.");
            }
            return false;
        }
        ...
        QByteArray payload_bytes;
        {
            ...
            static_cast<void>(publish_task_ipc_stage_event(&slot, TaskIpcEventKind::kClaimed, now_msecs()));
            ...
            out_task->owner_instance_id = read_fixed_utf8(slot.owner_instance_id, kTaskIpcOwnerInstanceIdCapacity);
            out_task->launcher_pid = slot.launcher_pid;
            out_task->worker_pid = slot.worker_pid;
            payload_bytes = QByteArray(raw->payload, static_cast<int>(slot.request_payload_size));
        }
...

        {
            ...
            for (int i = 0; i < kTaskIpcSlotCount; ++i) {
                ...
                if (candidate.session_id == session_id
                    ...
                    && candidate.state == static_cast<quint32>(TaskIpcSlotState::kDispatched)) {
                    slot_index = i;
                    slot = &candidate;
                    break;
                }
            ...
            if (!slot_lock.ok()) {
                if (error_message != nullptr) {
                    *error_message =
                        slot_lock.busy() ? QStringLiteral("Task IPC request-pool slot is busy.") : slot_lock.error();
                }
                return false;
            }
            ...
            QString read_error;
            if (!read_request_payload_from_slot(request_pool_memory.get(),
        ...
        post_task_ipc_semaphore(non_posix_task_ipc_event_semaphore_key(out_task->owner_instance_id), nullptr);
        return true;
#endif
    }

    bool query_task_ipc_cancel_requested(TaskIpcClaimedTask const& task, bool* out_canceled, QString* error_message) {
        using namespace task_ipc_internal;
        ...
        if (out_canceled == nullptr) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Missing task IPC cancel status output.");
            }
            return false;
        }
        *out_canceled = false;
        ...
        if (task.session_id == 0 || task.generation == 0U) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Task IPC cancel query requires claimed task identity.");
            }
            return false;
        }
        ...
        std::atomic_ref<quint32> const cancel_requested(slot.cancel_requested);
        *out_canceled = cancel_requested.load(std::memory_order_acquire) != 0U;
        return true;
#else
        std::shared_ptr<QSharedMemory> bootstrap_memory;
        if (!open_bootstrap_memory(false, &bootstrap_memory, error_message)) {
        ...
        TaskIpcSlotRaw const& slot = bootstrap->slot_records[task.slot_index];
        if (!slot_matches_claim(slot, task)) {
            return true;
        }
