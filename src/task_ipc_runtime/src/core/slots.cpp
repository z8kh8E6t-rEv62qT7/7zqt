...
namespace z7::task_ipc_runtime::task_ipc_internal {
    ...
    TaskIpcBootstrapRaw const* bootstrap_raw(QSharedMemory const* memory) {
        ...
        return static_cast<TaskIpcBootstrapRaw const*>(memory->constData());
    }
    ...
    TaskIpcRequestPoolHeaderRaw const* request_pool_raw(QSharedMemory const* memory) {
        ...
        return static_cast<TaskIpcRequestPoolHeaderRaw const*>(memory->constData());
    }
    ...
    char const* request_pool_slot_payload(TaskIpcRequestPoolHeaderRaw const* raw, int slot_index) {
        ...
        char const* base = reinterpret_cast<char const*>(raw) + kTaskIpcRequestPoolPayloadOffset;
        return base + (slot_index * kTaskIpcRequestPoolSlotSize);
    }
    ...
    bool publish_task_ipc_stage_event(TaskIpcSlotRaw* slot, TaskIpcEventKind event_kind, qint64 now_msecs_value) {
        ...
        quint32 const event_sequence = task_ipc_event_sequence_for_kind(event_kind);
    ...
    bool publish_task_ipc_completion_event(TaskIpcSlotRaw* slot,
        ...
        return publish_task_ipc_stage_event(slot, TaskIpcEventKind::kCompleted, now_msecs_value);
    }
    ...
    bool publish_task_ipc_unclaimed_timeout_completion(TaskIpcSlotRaw* slot, qint64 now_msecs_value) {
        ...
        return publish_task_ipc_completion_event(
            slot, 2, QStringLiteral("7zG worker did not claim task before timing out."), now_msecs_value);
    }
    ...
    bool publish_task_ipc_worker_exit_completion(TaskIpcSlotRaw* slot, qint64 now_msecs_value) {
        if (slot == nullptr
            ...
            || slot->published_event_sequence >= kTaskIpcCompletedEventSequence
            || slot->worker_pid <= 0
            || process_is_alive(slot->worker_pid)) {
            return false;
        }
        return publish_task_ipc_completion_event(
    ...
    bool slot_matches_owner(TaskIpcSlotRaw const& slot, QString const& owner_instance_id) {
        return read_fixed_utf8(slot.owner_instance_id, kTaskIpcOwnerInstanceIdCapacity) == owner_instance_id;
    }

    bool slot_matches_claim(TaskIpcSlotRaw const& slot, TaskIpcClaimedTask const& task) {
        return task.slot_index >= 0 && slot.session_id == task.session_id && slot.generation == task.generation;
    }
