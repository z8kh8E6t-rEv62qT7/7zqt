...
namespace z7::task_ipc_runtime::task_ipc_internal {
    ...
    namespace {
        ...
        QString hashed_task_ipc_semaphore_key(QString const& prefix, QString const& value) {
        ...
        bool initialize_task_ipc_semaphore(QSystemSemaphore* semaphore,
            ...
            if (error_message != nullptr) {
                *error_message = semaphore->errorString().trimmed().isEmpty()
        ...
        public:
            explicit NonPosixEventWaiter(QString owner_instance_id) :
                owner_instance_id_(std::move(owner_instance_id)), semaphore_(QNativeIpcKey()) {}

            ~NonPosixEventWaiter() { stop(); }

            NonPosixEventWaiter(NonPosixEventWaiter const&) = delete;
            NonPosixEventWaiter& operator=(NonPosixEventWaiter const&) = delete;

            bool start(QString* error_message) {
                if (started_) {
                    return true;
                }
                if (!initialize_task_ipc_semaphore(&semaphore_,
                ...
                try {
                    ...
                    post_task_ipc_semaphore(non_posix_task_ipc_event_semaphore_key(owner_instance_id_), &ignored_error);
                    ...
                    if (error_message != nullptr) {
                        *error_message = QStringLiteral("Failed to start task IPC event waiter: %1")
        ...
        private:
            ...
            void scan_owner_events() {
                ...
                for (int i = 0; i < kTaskIpcSlotCount; ++i) {
                    ...
                    should_notify |= publish_task_ipc_unclaimed_timeout_completion(&slot, now);
                    should_notify |= publish_task_ipc_worker_exit_completion(&slot, now);
                }

                if (should_notify) {
                    post_task_ipc_semaphore(non_posix_task_ipc_event_semaphore_key(owner_instance_id_), nullptr);
                }
            }
    ...
    QString non_posix_task_ipc_event_semaphore_key(QString const& owner_instance_id) {
        return hashed_task_ipc_semaphore_key(QStringLiteral("z7.taskipc.owner."), owner_instance_id.trimmed());
    }

    QString task_ipc_cancel_semaphore_key_for_task(quint64 session_id, quint32 generation) {
        return hashed_task_ipc_semaphore_key(QStringLiteral("z7.taskipc.cancel.task."),
    ...
    bool post_task_ipc_semaphore(QString const& key, QString* error_message) {
        ...
        QSystemSemaphore semaphore{QNativeIpcKey()};
        if (!initialize_task_ipc_semaphore(&semaphore, key, QSystemSemaphore::Open, error_message)) {
            return false;
        }
    ...
    bool set_non_posix_event_notifier(QString const& owner_instance_id,
        ...
        bool should_start_waiter = false;
        {
            ...
            std::shared_ptr<NonPosixEventWaiter>& waiter_slot = non_posix_event_waiters()[trimmed_owner];
        ...
        if (waiter == nullptr) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Task IPC event waiter was not created.");
            }
            return false;
        }
    ...
    bool start_task_ipc_cancel_notification_thread(TaskIpcClaimedTask const& task,
        ...
        bool canceled = false;
        if (!z7::task_ipc_runtime::query_task_ipc_cancel_requested(task, &canceled, error_message)) {
            return false;
        }
        if (canceled) {
            notifier();
        ...
        try {
            std::thread([task, semaphore_key, notifier = std::move(notifier)]() mutable {
                ...
                if (!initialize_task_ipc_semaphore(
                        &semaphore, semaphore_key, QSystemSemaphore::Open, &semaphore_error)) {
                    return;
                }
