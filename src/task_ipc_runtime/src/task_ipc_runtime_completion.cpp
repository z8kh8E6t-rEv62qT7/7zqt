...
namespace z7::task_ipc_runtime {
    namespace {
        ...
        TaskIpcPayload decode_posix_payload(task_ipc_internal::TaskIpcPerTaskRaw const& raw) {
            ...
            QByteArray const payload_bytes(raw.payload, static_cast<int>(raw.slot.request_payload_size));
        ...
        TaskIpcPayload decode_non_posix_payload(QSharedMemory* request_pool_memory,
            ...
            TaskIpcPayload payload;
            if (request_pool == nullptr || request_pool_memory == nullptr || slot.request_payload_size == 0U) {
                return payload;
            }

            QString payload_error;
            SharedMemoryLock slot_lock(request_pool_slot_lock(request_pool, static_cast<int>(slot.request_pool_slot)));
            if (slot_lock.ok()) {
        ...
        TaskIpcEvent build_task_ipc_event(task_ipc_internal::TaskIpcSlotRaw const& slot,
            ...
            event.event_kind = task_ipc_internal::task_ipc_event_kind_for_sequence(event_sequence);
        ...
        void append_pending_events_for_slot(task_ipc_internal::TaskIpcSlotRaw const& slot,
                                            QString const& owner_instance_id,
                                            TaskIpcPayload const& payload,
            ...
            for (quint32 sequence = next_pending_task_ipc_event_sequence(slot);
                 sequence != 0U && sequence <= slot.published_event_sequence;
                 ++sequence) {
                out_events->push_back(build_task_ipc_event(slot, owner_instance_id, sequence, payload));
            }
        }

    } // namespace

    bool publish_task_ipc_completion(TaskIpcClaimedTask const& task,
                                     int result_code,
        ...
        if (task.slot_index < 0 || task.slot_index >= kTaskIpcSlotCount) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Task IPC claimed task slot index is invalid.");
            }
            return false;
        }
...

        {
            ...
            if (lock == nullptr) {
                if (error_message != nullptr) {
                    *error_message = error_message->trimmed().isEmpty() ? QStringLiteral("Task IPC task is busy.")
            ...
            if (!slot_matches_claim(slot, task)) {
                if (error_message != nullptr) {
                    *error_message = QStringLiteral("Task IPC task slot no longer matches claimed task.");
                }
                return false;
            }
        ...
        std::unique_ptr<SharedMemoryLock> bootstrap_lock =
            wait_for_shared_memory_lock(&bootstrap->lock,
                                        bootstrap_lock_wake_key,
                                        kCompletionPublishWaitMsecs,
                                        QStringLiteral("Task IPC bootstrap remained busy while publishing completion."),
                                        error_message);
        if (bootstrap_lock == nullptr) {
            if (error_message != nullptr) {
                *error_message = error_message->trimmed().isEmpty() ? QStringLiteral("Task IPC bootstrap is busy.")
                                                                    : error_message->trimmed();
            }
            return false;
        }
        ...
        return post_task_ipc_semaphore(non_posix_task_ipc_event_semaphore_key(task.owner_instance_id), error_message);
#endif
    }

    bool publish_task_ipc_completion_minimal(TaskIpcClaimedTask const& task, int result_code, QString* error_message) {
        using namespace task_ipc_internal;
    ...
    bool collect_task_ipc_events(QString const& owner_instance_id,
                                 QVector<TaskIpcEvent>* out_events,
                                 QString* error_message) {
        using namespace task_ipc_internal;
        ...
        for (std::shared_ptr<PosixTaskIpcMapping> const& mapping : mappings) {
            ...
            TaskIpcSlotRaw const& slot = raw->slot;
            ...
            append_pending_events_for_slot(slot, normalized_owner_instance_id, decode_posix_payload(*raw), out_events);
        }
        return true;
#else
        std::shared_ptr<QSharedMemory> bootstrap_memory;
        ...
        for (int i = 0; i < kTaskIpcSlotCount; ++i) {
            TaskIpcSlotRaw const& slot = bootstrap->slot_records[i];
            if (slot.state == static_cast<quint32>(TaskIpcSlotState::kEmpty)) {
                continue;
            }
            if (!slot_matches_owner(slot, normalized_owner_instance_id)) {
            ...
            append_pending_events_for_slot(slot,
    ...
    bool acknowledge_task_ipc_event(TaskIpcEvent const& event, QString* error_message) {
        using namespace task_ipc_internal;
        ...
        QString normalized_owner_instance_id;
        if (!validate_task_ipc_owner_instance_id(
                event.owner_instance_id, &normalized_owner_instance_id, error_message)) {
            return false;
        }
        quint32 const event_sequence = task_ipc_event_sequence_for_kind(event.event_kind);
        ...
        TaskIpcPerTaskRaw* raw = mapping->raw();
        {
            ...
            should_remove_mapping = slot.acknowledged_event_sequence >= kTaskIpcCompletedEventSequence;
        }
        ...
        for (int i = 0; i < kTaskIpcSlotCount; ++i) {
            ...
            if (slot.session_id != event.session_id || slot.generation != event.generation) {
                continue;
            }
            if (!slot_matches_event(slot, event)) {
                return true;
