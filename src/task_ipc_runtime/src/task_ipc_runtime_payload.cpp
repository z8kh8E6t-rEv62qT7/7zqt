...
namespace z7::task_ipc_runtime::task_ipc_internal {
    ...
    namespace {
        ...
        void write_optional_presence(QDataStream& stream, std::optional<TPayload> const& payload) {
            stream << payload.has_value();
        }

        template <typename TPayload>
        bool read_optional_presence(QDataStream& stream, bool* has_value) {
        ...
        quint32 encode_extract_path_remap_match_kind(TaskIpcExtractPathRemapMatchKind kind) {
            return static_cast<quint32>(kind);
        }

        TaskIpcExtractPathRemapMatchKind decode_extract_path_remap_match_kind(quint32 encoded) {
            switch (encoded) {
                case 1:
        ...
        void write_extract_path_remaps(QDataStream& stream, QVector<TaskIpcExtractPathRemap> const& remaps) {
            stream << static_cast<quint32>(remaps.size());
            for (TaskIpcExtractPathRemap const& remap : remaps) {
        ...
        bool read_extract_path_remaps(QDataStream& stream, QVector<TaskIpcExtractPathRemap>* out_remaps) {
        ...
        void write_add_payload(QDataStream& stream, std::optional<TaskIpcAddPayload> const& payload) {
        ...
        bool read_add_payload(QDataStream& stream, std::optional<TaskIpcAddPayload>* out_payload) {
        ...
        void write_extract_payload(QDataStream& stream, std::optional<TaskIpcExtractPayload> const& payload) {
            write_optional_presence(stream, payload);
            if (!payload.has_value()) {
                return;
            }
        ...
        bool read_extract_payload(QDataStream& stream, std::optional<TaskIpcExtractPayload>* out_payload) {
            if (out_payload == nullptr) {
                return false;
            }
            bool has_value = false;
        ...
        void write_archive_export_payload(QDataStream& stream,
                                          std::optional<TaskIpcArchiveExportPayload> const& payload) {
            write_optional_presence(stream, payload);
            if (!payload.has_value()) {
                return;
            }
        ...
        bool read_archive_export_payload(QDataStream& stream, std::optional<TaskIpcArchiveExportPayload>* out_payload) {
            if (out_payload == nullptr) {
                return false;
            }
            bool has_value = false;
        ...
        void write_test_payload(QDataStream& stream, std::optional<TaskIpcTestPayload> const& payload) {
            write_optional_presence(stream, payload);
            if (!payload.has_value()) {
                return;
            }
        ...
        bool read_test_payload(QDataStream& stream, std::optional<TaskIpcTestPayload>* out_payload) {
            if (out_payload == nullptr) {
                return false;
            }
            bool has_value = false;
        ...
        void write_hash_payload(QDataStream& stream, std::optional<TaskIpcHashPayload> const& payload) {
            write_optional_presence(stream, payload);
            if (!payload.has_value()) {
                return;
            }
        ...
        bool read_hash_payload(QDataStream& stream, std::optional<TaskIpcHashPayload>* out_payload) {
            if (out_payload == nullptr) {
                return false;
            }
            bool has_value = false;
        ...
        void write_benchmark_payload(QDataStream& stream, std::optional<TaskIpcBenchmarkPayload> const& payload) {
            write_optional_presence(stream, payload);
            if (!payload.has_value()) {
                return;
            }
        ...
        bool read_benchmark_payload(QDataStream& stream, std::optional<TaskIpcBenchmarkPayload>* out_payload) {
            if (out_payload == nullptr) {
                return false;
            }
            bool has_value = false;
        ...
        void write_open_payload(QDataStream& stream, std::optional<TaskIpcOpenPayload> const& payload) {
            write_optional_presence(stream, payload);
            if (!payload.has_value()) {
                return;
            }
        ...
        bool read_open_payload(QDataStream& stream, std::optional<TaskIpcOpenPayload>* out_payload) {
            if (out_payload == nullptr) {
                return false;
            }
            bool has_value = false;
        ...
        void write_cli_payload(QDataStream& stream, std::optional<TaskIpcCliPayload> const& payload) {
            write_optional_presence(stream, payload);
            if (!payload.has_value()) {
                return;
            }
        ...
        bool read_cli_payload(QDataStream& stream, std::optional<TaskIpcCliPayload>* out_payload) {
            if (out_payload == nullptr) {
                return false;
            }
            bool has_value = false;
    ...
    QByteArray serialize_task_payload(TaskIpcPayload const& payload, QString* error_message) {
        ...
        if (body_stream.status() != QDataStream::Ok) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Failed to serialize task IPC task payload.");
            }
            return QByteArray();
        }

        QByteArray const checksum = QCryptographicHash::hash(body, QCryptographicHash::Sha256);
        QByteArray encoded;
        ...
        stream << checksum;
        if (!body.isEmpty()) {
            int const bytes_written = stream.writeRawData(body.constData(), body.size());
            if (bytes_written != body.size()) {
                if (error_message != nullptr) {
                    *error_message = QStringLiteral("Failed to serialize task IPC task payload body.");
                }
                return QByteArray();
            }
        }
    ...
    bool deserialize_task_payload(QByteArray const& encoded, TaskIpcPayload* out_payload, QString* error_message) {
        ...
        if (magic != kTaskIpcPayloadMagic || version != kTaskIpcPayloadVersion) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Unsupported task IPC task payload format.");
            }
            return false;
        }
        if (body_size == 0 || body_size > static_cast<quint32>(encoded.size())) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Task IPC task payload body size is invalid.");
            }
            return false;
        }

        QByteArray body(static_cast<int>(body_size), Qt::Uninitialized);
        int const bytes_read = stream.readRawData(body.data(), body.size());
        if (bytes_read != body.size()) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Task IPC task payload body is truncated.");
            }
            return false;
        }
        if (stream.status() != QDataStream::Ok) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Task IPC task payload body is corrupted.");
            }
            return false;
        }

        QByteArray const expected_checksum = QCryptographicHash::hash(body, QCryptographicHash::Sha256);
        if (stored_checksum != expected_checksum) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Task IPC task payload checksum mismatch.");
            }
            return false;
        }
        ...
        *out_payload = TaskIpcPayload{};
        ...
        if (!read_add_payload(body_stream, &out_payload->add)
            ...
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Task IPC task payload command payload is corrupted.");
            }
            return false;
        }
        if (body_stream.status() != QDataStream::Ok) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Task IPC task payload fields are corrupted.");
            }
            return false;
        }
    ...
    bool write_request_payload_to_slot(QSharedMemory* request_pool_memory,
        ...
        if (request_pool_memory == nullptr) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Task IPC request-pool shared memory is null.");
            }
            return false;
        }
        if (slot_index < 0 || slot_index >= kTaskIpcSlotCount) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Task IPC request-pool slot index is invalid.");
            }
            return false;
        }
        if (payload.isEmpty()) {
        ...
        if (payload.size() > kTaskIpcRequestPoolSlotSize) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Task IPC request payload exceeds fixed request-pool slot capacity.");
            }
            return false;
        }
        if (request_pool_memory->size() < kTaskIpcRequestPoolSharedMemorySize) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Task IPC request-pool shared memory size is invalid.");
            }
            return false;
        }
        ...
        if (raw == nullptr) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Task IPC request-pool payload pointer is null.");
            }
            return false;
        }
        ...
        if (slot_bytes == nullptr) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Task IPC request-pool slot payload pointer is null.");
            }
            return false;
        }
        std::memset(slot_bytes, 0, static_cast<size_t>(kTaskIpcRequestPoolSlotSize));
        std::memcpy(slot_bytes, payload.constData(), static_cast<size_t>(payload.size()));
        return true;
    }
    ...
    bool read_request_payload_from_slot(QSharedMemory* request_pool_memory,
        ...
        if (payload_size == 0U || payload_size > static_cast<quint32>(kTaskIpcRequestPoolSlotSize)) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Task IPC request payload size is invalid.");
            }
            return false;
        }
        if (request_pool_memory->size() < kTaskIpcRequestPoolSharedMemorySize) {
        ...
        TaskIpcRequestPoolHeaderRaw const* raw = request_pool_raw(request_pool_memory);
        if (raw == nullptr) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Task IPC request-pool payload pointer is null.");
            }
        ...
        char const* slot_bytes = request_pool_slot_payload(raw, slot_index);
        if (slot_bytes == nullptr) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("Task IPC request-pool slot payload pointer is null.");
            }
        ...
        QByteArray const payload(slot_bytes, static_cast<int>(payload_size));
