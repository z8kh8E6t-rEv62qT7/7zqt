...
namespace z7::platform::qt {
    ...
    std::optional<std::string> SettingsStoreQt::load(std::string_view key) const {
        ...
        QString const key_qt = QString::fromUtf8(key.data(), static_cast<int>(key.size()));
        QVariant const value = settings.value(key_qt);
        if (!value.isValid()) {
            return std::nullopt;
        }

        QByteArray const utf8 = value.toString().toUtf8();
    ...
    void SettingsStoreQt::save(std::string_view key, std::string_view value) {
        PortableSettings settings(organization_, application_);
        QString const key_qt = QString::fromUtf8(key.data(), static_cast<int>(key.size()));
        QString const value_qt = QString::fromUtf8(value.data(), static_cast<int>(value.size()));
