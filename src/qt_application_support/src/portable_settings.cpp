...
namespace z7::platform::qt {
    ...
    PortableSettings::PortableSettings() : scope_(Scope::kApp), app_name_(app_name_or_default()) {}
    ...
    QVariant PortableSettings::value(QString const& key, QVariant const& default_value) const {
        QString init_error;
        {
        ...
        QString const file_path = portable_settings_file_path();
        ...
        with_locked_root(file_path, lock_path, &error_message, [&](QJsonObject* root, QString*) {
            QJsonObject const ns = namespace_object(*root, scope_, app_name_);
            QJsonValue const val = ns.value(key);
    ...
    void PortableSettings::setValue(QString const& key, QVariant const& value) {
        QString init_error;
        {
            QMutexLocker locker(&state_mutex());
            if (!ensure_initialized_locked(&init_error)) {
    ...
    void PortableSettings::remove(QString const& key) {
        QString init_error;
        {
            QMutexLocker locker(&state_mutex());
            if (!ensure_initialized_locked(&init_error)) {
    ...
    bool PortableSettings::contains(QString const& key) const {
        QString init_error;
        {
            QMutexLocker locker(&state_mutex());
            if (!ensure_initialized_locked(&init_error)) {
