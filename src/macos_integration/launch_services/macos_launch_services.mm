...
namespace z7::macos_integration::launch_services {
    namespace {
        ...
        QString app_bundle_path_for_executable(QString const& executable_path) {
            ...
            for (;;) {
                QString const dir_name = parent_dir.dirName();
        ...
        NSString* qstring_to_nsstring(QString const& value) {
        ...
        NSArray<NSString*>* qstringlist_to_nsarray(QStringList const& values) {
    ...
    FileManagerInstanceLaunchResult launch_new_filemanager_instance(FileManagerInstanceLaunchRequest const& request,
        ...
        QString const bundle_path = app_bundle_path_for_executable(request.program_path);
        if (bundle_path.isEmpty()) {
            return FileManagerInstanceLaunchResult::kFallbackToDetached;
        }

        @autoreleasepool {
            NSURL* const application_url = [NSURL fileURLWithPath:qstring_to_nsstring(bundle_path) isDirectory:YES];
            if (application_url == nil) {
                if (error_message != nullptr) {
                    *error_message = QStringLiteral("Failed to resolve 7zFM app bundle URL.");
                }
            ...
            [[NSWorkspace sharedWorkspace] openApplicationAtURL:application_url
