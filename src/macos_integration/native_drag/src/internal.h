...
namespace z7::macos_integration::native_drag::detail {
    ...
    struct SharedState {
        struct MetricsSnapshot {
            ...
            qint64 configured_large_file_threshold_bytes = kDefaultLargeFileThresholdBytes;
            qint64 effective_large_file_threshold_bytes = kDefaultLargeFileThresholdBytes;
            int configured_large_file_concurrency_limit = kDefaultLargeFileWriteConcurrency;
        ...
        explicit SharedState(MacOSIntegrationNativeDragRequest const& req, PromiseWriteConcurrencyPolicy const& policy);

        void on_promise_request_started(int item_index);
        void set_source_view_screen_rect(NSRect const& rect, bool valid);
        void set_own_app_window_screen_rects(QVector<NSRect> const& rects);
        void
        mark_drag_session_ended(Qt::DropAction result_action, bool ended_in_source_view, bool ended_in_own_app_window);
        ...
        void set_error_message_if_empty(QString const& value);
        int on_write_start();
        ...
        Qt::DropAction result_action = Qt::IgnoreAction;
        QString error_message;
        int const configured_concurrency_limit = kDefaultPromiseWriteConcurrency;
        int effective_concurrency_limit = kDefaultPromiseWriteConcurrency;
        qint64 const configured_large_file_threshold_bytes = kDefaultLargeFileThresholdBytes;
        qint64 effective_large_file_threshold_bytes = kDefaultLargeFileThresholdBytes;
        int const configured_large_file_concurrency_limit = kDefaultLargeFileWriteConcurrency;
        int effective_large_file_concurrency_limit = kDefaultLargeFileWriteConcurrency;
    ...
    public:
        ...
        ScopedWriteSlotAcquire(ScopedWriteSlotAcquire const&) = delete;
        ScopedWriteSlotAcquire& operator=(ScopedWriteSlotAcquire const&) = delete;
    ...
    public:
        ...
        ScopedLargeFileWriteSlotAcquire(ScopedLargeFileWriteSlotAcquire const&) = delete;
        ScopedLargeFileWriteSlotAcquire& operator=(ScopedLargeFileWriteSlotAcquire const&) = delete;
