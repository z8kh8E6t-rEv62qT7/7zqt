...
namespace z7::macos_integration::native_drag::detail {
    ...
    namespace {
        ...
        bool
        copy_directory_recursive(QString const& source_path, QString const& destination_path, QString* error_message) {
            ...
            QFileInfoList const entries =
                source_dir.entryInfoList(QDir::NoDotAndDotDot | QDir::AllEntries | QDir::Hidden | QDir::System);
            for (QFileInfo const& entry : entries) {
                QString const child_source = entry.absoluteFilePath();
                QString const child_destination = QDir(destination_path).filePath(entry.fileName());
                if (entry.isDir() && !entry.isSymLink()) {
                    if (!copy_directory_recursive(child_source, child_destination, error_message)) {
                        return false;
                    }
                    continue;
                }
                if (QFileInfo::exists(child_destination) && !QFile::remove(child_destination)) {
                    if (error_message != nullptr) {
        ...
        bool remove_existing_path(QString const& path, QString* error_message) {
            QFileInfo const existing(path);
            if (!existing.exists()) {
                return true;
            }
            ...
            if (error_message != nullptr) {
                *error_message =
                    QStringLiteral("Cannot replace existing destination: %1").arg(QDir::toNativeSeparators(path));
            }
            return false;
        }
    ...
    bool copy_source_path_to_destination(QString const& source_path,
                                         bool source_is_dir,
                                         QString const& target_directory,
                                         QString const& promised_name,
                                         QString* error) {
        QString const normalized_source = source_path.trimmed();
        QString const normalized_target_directory = target_directory.trimmed();
        QString const normalized_promised_name = promised_name.trimmed();
        ...
        QString const destination_path = QDir(normalized_target_directory).filePath(normalized_promised_name);
    ...
    bool write_to_destination_if_needed(MacOSIntegrationNativeDragItem* item,
        ...
        QString const normalized_destination = destination_path.trimmed();
        ...
        QFileInfo const destination_info(normalized_destination);
        QString const target_directory = destination_info.absolutePath().trimmed();
        QString const promised_name = destination_info.fileName().trimmed();
        ...
        if (!item->source_path.trimmed().isEmpty()) {
            return copy_source_path_to_destination(
                item->source_path, item->is_dir, target_directory, promised_name, error);
        }

        if (!item->write_to_destination) {
            if (error != nullptr) {
                *error =
                    QStringLiteral("Missing direct-export writer for drag item '%1'.").arg(default_item_name(*item));
            }
            return false;
        }
        ...
        auto const run_writer = [&]() {
            write_ok = item->write_to_destination(normalized_destination, &writer_error);
        };

        QCoreApplication* app = QCoreApplication::instance();
        if (app != nullptr && QThread::currentThread() != app->thread()) {
            bool const invoked = QMetaObject::invokeMethod(app, run_writer, Qt::BlockingQueuedConnection);
            if (!invoked) {
                if (error != nullptr) {
