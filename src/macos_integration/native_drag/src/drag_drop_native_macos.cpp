...
z7::macos_integration::native_drag::run_macos_integration_native_drag(
    z7::macos_integration::native_drag::MacOSIntegrationNativeDragRequest const& request) {
    Q_UNUSED(request);
    return {};
}
...
static constexpr auto kZ7NativeDragNoPromiseGrace = std::chrono::milliseconds(150);
...
static NSOperationQueue* Z7MakeFilePromiseOperationQueue(NSInteger item_index) {
    ...
    queue.name =
        [NSString stringWithFormat:@"app.sevenzip.native-drag.file-promise.%ld", static_cast<long>(item_index)];
    queue.qualityOfService = NSQualityOfServiceUserInitiated;
    return queue;
}

static NSString*
Z7ExplicitFileTypeIdentifier(z7::macos_integration::native_drag::MacOSIntegrationNativeDragItem const& item) {
    QString const explicit_identifier = item.file_type_identifier.trimmed();
    if (explicit_identifier.isEmpty()) {
        return nil;
    }

    UTType* explicit_type = [UTType typeWithIdentifier:z7nd_detail::to_ns_string(explicit_identifier)];
...
static NSString*
Z7FilePromiseTypeIdentifier(z7::macos_integration::native_drag::MacOSIntegrationNativeDragItem const& item) {
    if (item.is_dir) {
    ...
    QString const item_name = z7nd_detail::default_item_name(item);
    QString const extension = QFileInfo(item_name).suffix().trimmed();
    if (extension.isEmpty()) {
        return Z7FallbackFilePromiseTypeIdentifier();
    }

    UTType* content_type = [UTType typeWithFilenameExtension:z7nd_detail::to_ns_string(extension)];
...
static bool Z7TryGetViewScreenRect(NSView* view, NSRect* out_rect) {
    ...
    NSRect const view_bounds = [view bounds];
    NSRect const window_rect = [view convertRect:view_bounds toView:nil];
...
static QVector<NSRect> Z7OwnAppWindowScreenRects() {
    ...
    QWidgetList const top_level_widgets = QApplication::topLevelWidgets();
...
static NSDragOperation Z7NsDragOperationMaskForRequest(SharedState const& shared) {
    if (shared.request.kind == z7::macos_integration::native_drag::MacOSIntegrationNativeDragKind::kArchive) {
...
static bool Z7RemoveMovedSourcePath(QString const& source_path, bool is_dir, QString* error_message) {
    QString const normalized_source = source_path.trimmed();
    if (normalized_source.isEmpty()) {
        return true;
    }

    QFileInfo const source_info(normalized_source);
    ...
    if (error_message != nullptr) {
        *error_message =
            QStringLiteral("Failed to remove moved source path: %1").arg(QDir::toNativeSeparators(normalized_source));
    }
    return false;
}

static qint64
Z7EstimatedPayloadSizeBytes(z7::macos_integration::native_drag::MacOSIntegrationNativeDragItem const& item,
                            PromiseWriteConcurrencyPolicy const& policy) {
...
- (instancetype)initWithShared:(std::shared_ptr<SharedState>)shared retainedDelegates:(NSArray*)retainedDelegates;
...
- (instancetype)initWithShared:(std::shared_ptr<SharedState>)shared itemIndex:(NSInteger)itemIndex;
...
- (void)draggingSession:(NSDraggingSession*)session
    ...
    if (_shared->source_view_screen_rect_valid) {
        ended_in_source_view = NSMouseInRect(screenPoint, _shared->source_view_screen_rect, NO);
    }
    bool ended_in_own_app_window = false;
    for (NSRect const& rect : _shared->own_app_window_screen_rects) {
    ...
    _shared->mark_drag_session_ended(
        z7nd_detail::qt_action_from_ns_operation(operation), ended_in_source_view, ended_in_own_app_window);
}
...
- (NSString*)filePromiseProvider:(NSFilePromiseProvider*)filePromiseProvider fileNameForType:(NSString*)fileType {
    ...
    Q_UNUSED(fileType);
    qsizetype const item_index = static_cast<qsizetype>(_itemIndex);
    if (!_shared || item_index < 0 || item_index >= _shared->request.items.size()) {
        return @"7zFM-item";
    }

    auto const& item = _shared->request.items[static_cast<int>(item_index)];
...
- (void)filePromiseProvider:(NSFilePromiseProvider*)filePromiseProvider
    ...
    if (!_shared || item_index < 0 || item_index >= _shared->request.items.size()) {
        if (completionHandler != nullptr) {
            completionHandler(z7nd_detail::make_copy_error(QStringLiteral("Invalid drag promise state.")));
        }
        return;
    }
    ...
    [[maybe_unused]] int const active_write_count = _shared->on_write_start();
    ...
    auto finish_and_reply = [&](bool success, QString const& error_message) {
    ...
    PromiseWriteConcurrencyPolicy const concurrency_policy = PromiseWriteConcurrencyPolicy::from_defaults();
    payload_size_bytes = Z7EstimatedPayloadSizeBytes(item, concurrency_policy);
    large_file_throttle_applied = _shared->should_use_large_file_throttle(payload_size_bytes);
    ...
    if (!z7nd_detail::write_to_destination_if_needed(&item, destination_path, &write_error)) {
...
z7::macos_integration::native_drag::run_macos_integration_native_drag(
    ...
    source_widget->winId();
    NSView* native_view = (__bridge NSView*)reinterpret_cast<void*>(source_widget->winId());
    if (native_view == nil) {
    ...
    NSView* source_boundary_view = (__bridge NSView*)reinterpret_cast<void*>(request.source_widget->winId());
    if (source_boundary_view == nil) {
        source_boundary_view = native_view;
    }
    source_view_screen_rect_valid = Z7TryGetViewScreenRect(source_boundary_view, &source_view_screen_rect);

    PromiseWriteConcurrencyPolicy const concurrency_policy = PromiseWriteConcurrencyPolicy::from_defaults();
    std::shared_ptr<SharedState> shared = std::make_shared<SharedState>(request, concurrency_policy);
    shared->set_source_view_screen_rect(source_view_screen_rect, source_view_screen_rect_valid);
    ...
    for (int i = 0; i < request.items.size(); ++i) {
        Z7MacFilePromiseDelegate* delegate = [[Z7MacFilePromiseDelegate alloc] initWithShared:shared itemIndex:i];
        ...
        NSFilePromiseProvider* provider =
            [[NSFilePromiseProvider alloc] initWithFileType:file_type_identifier delegate:delegate];
        NSDraggingItem* dragging_item = [[NSDraggingItem alloc] initWithPasteboardWriter:provider];
    ...
    Z7MacDragSource* drag_source = [[Z7MacDragSource alloc] initWithShared:shared retainedDelegates:delegates];
    ...
    std::thread settle_waiter([shared, &loop, &stop_waiter]() {
        ...
        while (!stop_waiter.load(std::memory_order_acquire)) {
            SharedState::MetricsSnapshot const metrics = shared->metrics_snapshot();
            ...
            if (grace_active) {
                shared->completion_wait_condition.wait_until(
                    lock, grace_deadline, [shared, &observed_epoch, &stop_waiter]() {
                        return stop_waiter.load(std::memory_order_acquire)
                            || shared->completion_wait_epoch != observed_epoch;
                    });
            } else {
                shared->completion_wait_condition.wait(lock, [shared, &observed_epoch, &stop_waiter]() {
                    return stop_waiter.load(std::memory_order_acquire)
                        || shared->completion_wait_epoch != observed_epoch;
                });
            }
        ...
        if (!stop_waiter.load(std::memory_order_acquire)) {
            QMetaObject::invokeMethod(QCoreApplication::instance(), [&loop]() { loop.quit(); }, Qt::QueuedConnection);
        }
    });
    loop.exec();
    ...
    result.promise_writes_settled = metrics.drag_session_ended
    ...
    if (request.kind == z7::macos_integration::native_drag::MacOSIntegrationNativeDragKind::kFilesystem
        ...
        for (int i = 0; i < request.items.size(); ++i) {
            ...
            if (!Z7RemoveMovedSourcePath(item.source_path, item.is_dir, &remove_error)
    ...
    if (timeout.isActive()) {
        ...
        if (result.error_message.isEmpty()) {
            result.error_message = QStringLiteral("Native drag timed out waiting for promised file writes.");
        }
    }
    return result;
}
