...
namespace z7::macos_integration::native_drag::detail {
    ...
    SharedState::SharedState(MacOSIntegrationNativeDragRequest const& req,
        ...
        configured_concurrency_limit(
            qBound(kDefaultPromiseWriteConcurrency, policy.configured_limit, kMaxPromiseWriteConcurrency)),
        effective_concurrency_limit(configured_concurrency_limit),
        configured_large_file_threshold_bytes(
            qBound<qint64>(0, policy.large_file_threshold_bytes, kMaxLargeFileThresholdBytes)),
        configured_large_file_concurrency_limit(qBound(
            kDefaultLargeFileWriteConcurrency, policy.large_file_concurrency_limit, kMaxPromiseWriteConcurrency)),
        effective_large_file_concurrency_limit(configured_large_file_concurrency_limit) {
    ...
    void SharedState::set_source_view_screen_rect(NSRect const& rect, bool valid) {
        source_view_screen_rect = rect;
        source_view_screen_rect_valid = valid;
    }

    void SharedState::set_own_app_window_screen_rects(QVector<NSRect> const& rects) {
        own_app_window_screen_rects = rects;
    }

    void SharedState::mark_drag_session_ended(Qt::DropAction const action,
    ...
    void SharedState::acquire_write_slot() {
        ...
        write_gate_condition.wait(lock, [this]() { return active_write_slots < effective_concurrency_limit; });
        ++active_write_slots;
    }
    ...
    bool SharedState::should_use_large_file_throttle(qint64 payload_size_bytes) const {
        ...
        std::lock_guard<std::mutex> lock(write_gate_mutex);
        return effective_large_file_threshold_bytes > 0 && payload_size_bytes >= effective_large_file_threshold_bytes;
    }

    void SharedState::acquire_large_file_write_slot() {
        std::unique_lock<std::mutex> lock(large_file_write_gate_mutex);
        large_file_write_gate_condition.wait(
            lock, [this]() { return active_large_file_write_slots < effective_large_file_concurrency_limit; });
    ...
    void SharedState::set_error_message_if_empty(QString const& value) {
    ...
    void SharedState::on_write_finish(qint64 direct_export_ms,
        ...
        qint64 const normalized_payload_bytes = qMax<qint64>(0, payload_size_bytes);
        ...
        if (large_file_mode) {
            ...
            if (std::numeric_limits<qint64>::max() - large_file_total_bytes < normalized_payload_bytes) {
    ...
    SharedState::MetricsSnapshot SharedState::metrics_snapshot() const {
        ...
        snapshot.configured_large_file_threshold_bytes = configured_large_file_threshold_bytes;
        snapshot.configured_large_file_concurrency_limit = configured_large_file_concurrency_limit;
        ...
        snapshot.max_single_write_bytes = max_single_write_bytes;
        {
            ...
            snapshot.effective_large_file_threshold_bytes = effective_large_file_threshold_bytes;
        }
        {
            std::lock_guard<std::mutex> large_file_lock(large_file_write_gate_mutex);
            snapshot.effective_large_file_concurrency_limit = effective_large_file_concurrency_limit;
        }
        return snapshot;
    }

    ScopedWriteSlotAcquire::ScopedWriteSlotAcquire(SharedState* shared) : shared_(shared) {
        if (shared_ != nullptr) {
    ...
    ScopedLargeFileWriteSlotAcquire::ScopedLargeFileWriteSlotAcquire(SharedState* shared, bool enable) :
