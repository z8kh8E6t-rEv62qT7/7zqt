...
namespace z7::macos_integration::capi_internal {
    namespace {
        ...
        QString hash_method_for_action(QString const& action_id) {
        ...
        QString generated_sha256_file_name(QStringList const& selected_paths) {
            return z7::shell_integration::shell_integration_create_archive_name_from_paths(
                       selected_paths, true, nullptr)
    ...
    bool build_task_ipc_payload_for_action(QString const& action_id,
        ...
        if (action_id == QString::fromLatin1(kActionAddTo7z) || action_id == QString::fromLatin1(kActionAddToZip)) {
            if (plan.selected_paths.isEmpty()) {
                if (error_message != nullptr) {
                    *error_message = QStringLiteral("Add requires selected paths.");
                }
            ...
            QString const archive_type =
                action_id == QString::fromLatin1(kActionAddTo7z) ? QStringLiteral("7z") : QStringLiteral("zip");
            payload.command = z7::task_ipc_runtime::TaskIpcCommandKind::kAdd;
            payload.add = z7::task_ipc_runtime::TaskIpcAddPayload{};
            payload.add->archive_type = archive_type;
            payload.add->archive_path =
                QDir(plan.base_folder).filePath(plan.archive_name + QStringLiteral(".") + archive_type);
            payload.add->input_paths = plan.selected_paths;
            *out_payload = std::move(payload);
            return true;
        }
        ...
        if (action_id == QString::fromLatin1(kActionGenerateSha256)) {
            ...
            payload.add->archive_path =
                QDir(plan.base_folder).filePath(generated_sha256_file_name(plan.selected_paths));
            payload.add->input_paths = plan.selected_paths;
            *out_payload = std::move(payload);
            return true;
        }
        ...
        QString const hash_method = hash_method_for_action(action_id);
        ...
        if (error_message != nullptr) {
            *error_message = QStringLiteral("Action is not supported in task IPC mode: %1").arg(action_id);
        }
        return false;
    }
