...
extern "C" {
    ...
    void z7_mi_session_destroy(z7_mi_session_t* session) {
        ...
        if (auto const& state = session->state) {
            ...
            capi::cancel_all_pending_prompts(state);
            {
                ...
                for (auto const& pair : state->in_flight_tasks) {
            ...
            auto const cached_chains = capi::take_nested_session_cache_for_destroy(state);
            for (auto const& chain : cached_chains) {
