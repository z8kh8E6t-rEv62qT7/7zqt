...
namespace {
    ...
    public:
        QuickLookBatchExportDelegate(z7_mi_quicklook_batch_export_progress_callback_t progress_callback,
                                     void* user_data,
                                     size_t total_item_count) :
            progress_callback_(progress_callback), user_data_(user_data), total_item_count_(total_item_count) {}

        void set_item_context(size_t completed_item_count,
                              size_t current_item_index,
                              QString const& current_entry_path,
                              QString const& current_destination_path) {
        ...
        void emit_indeterminate_progress(QString const& message) {
            ...
            }
            {
                ...
                progress->current_destination_path = capi::duplicate_c_string(current_destination_path_);
        ...
        void on_progress(z7::app::ProgressSnapshot const& progress) override {
            ...
            dto->current_destination_path = capi::duplicate_c_string(current_destination_path);
            ...
            dto->current_path =
                capi::duplicate_c_string(z7::ui::archive_support::from_utf8_string(progress.current_path));
            dto->message = capi::duplicate_c_string(z7::ui::archive_support::from_utf8_string(progress.message));
    ...
    QString normalize_batch_export_entry_path(QString const& entry_path, QStringList const& nested_archive_entries) {
    ...
    z7_mi_status_t quicklook_batch_export_status_from_outcome(z7::app::OperationOutcome const& outcome,
        ...
        z7::app::OperationResult const result = z7::app::operation_result_from_outcome(outcome);
        z7::app::ArchiveErrorDomain const error_domain =
            result.error.domain != z7::app::ArchiveErrorDomain::kNone ? result.error.domain : outcome.error.domain;
        switch (error_domain) {
    ...
    bool quicklook_batch_export_is_password_failure(z7::app::OperationOutcome const& outcome) {
        z7::app::OperationResult const result = z7::app::operation_result_from_outcome(outcome);
    ...
    bool quicklook_batch_export_request_password(std::shared_ptr<z7::app::IArchiveDelegate> const& password_delegate,
                                                 std::shared_ptr<AsyncTaskState> const& task_state,
                                                 bool wrong_password,
        ...
        if (task_state
            && (task_state->password_prompt_canceled.load() || task_state->password_prompt_missing_callback.load())) {
            return false;
        }

        z7::app::PasswordPrompt prompt;
        prompt.reason_kind = wrong_password ? z7::app::PasswordPromptReason::kWrongPassword
        ...
        if (!reply.has_value() || reply->kind != z7::app::PasswordReplyKind::kProvide) {
            return false;
        }
    ...
    z7::app::OperationOutcome
    run_quicklook_extract_with_password_retry(z7::app::ExtractRequest request,
                                              std::shared_ptr<AsyncTaskState> const& task_state,
                                              std::shared_ptr<z7::app::IArchiveDelegate> const& password_delegate) {
        constexpr int kMaxPasswordPrompts = 3;
        int password_prompts = 0;

        for (;;) {
            uint32_t const prompt_count_before = task_state ? task_state->password_prompt_request_count.load() : 0;
            z7::app::OperationOutcome outcome =
                capi::run_archive_request_sync(z7::app::ArchiveRequest{request}, task_state, password_delegate);
            if (!quicklook_batch_export_is_password_failure(outcome)
                || (task_state && capi::should_report_canceled(outcome, task_state->cancel_requested.load()))) {
                return outcome;
            }
            if (task_state
            ...
            uint32_t const prompt_count_after = task_state ? task_state->password_prompt_request_count.load() : 0;
            bool const backend_prompted_this_attempt = prompt_count_after > prompt_count_before;
            bool const wrong_password_prompt =
                backend_prompted_this_attempt || password_prompts > 0 || !request.password.empty();
            if (!quicklook_batch_export_request_password(
                    password_delegate, task_state, wrong_password_prompt, &request)) {
                return outcome;
            }
            ++password_prompts;
        }
    ...
    z7_mi_quicklook_batch_export_result_t* build_batch_export_success_result(size_t completed_item_count,
                                                                             size_t total_item_count) {
        auto* out_result = capi::allocate_result<z7_mi_quicklook_batch_export_result_t>();
    ...
    void free_batch_export_progress_strings(z7_mi_quicklook_batch_export_progress_t* progress) {
        if (progress == nullptr) {
            return;
        }
    ...
    void free_batch_export_result_strings(z7_mi_quicklook_batch_export_result_t* result) {
        if (result == nullptr) {
            return;
        }
    ...
    bool prepare_batch_export_items(std::shared_ptr<z7_mi_session_state> const& state,
                                    QString const& archive_path,
                                    QString const& archive_type_hint,
                                    QStringList const& nested_archive_entries,
                                    std::shared_ptr<AsyncTaskState> const& task_state,
                                    std::vector<QuickLookBatchExportItem> const& items,
        ...
        if (out_chain == nullptr
            || out_prepared_items == nullptr
            || error_message == nullptr
            || failed_item_index == nullptr
            || failed_entry_path == nullptr
        ...
        *failed_item_index = -1;
        ...
        z7_mi_status_t const chain_status = capi::open_nested_session_chain(
            state, archive_path, archive_type_hint, nested_archive_entries, task_state, out_chain, error_message);
        ...
        for (size_t index = 0; index < items.size(); ++index) {
            QuickLookBatchExportItem const& item = items[index];
            ...
            prepared.normalized_entry_path = normalize_batch_export_entry_path(item.entry_path, nested_archive_entries);

            z7::app::GetEntryInfoRequest request;
            request.session_token = (*out_chain)->tokens.back();
            request.entry_path = z7::ui::archive_support::to_utf8_string(prepared.normalized_entry_path);
            z7::app::OperationOutcome const outcome =
                capi::run_archive_request_sync(z7::app::ArchiveRequest{request}, task_state);
            auto const payload = z7::app::outcome_payload_as<z7::app::GetEntryInfoResult>(outcome);
            ...
            if (!payload->exists) {
                ...
                *failed_entry_path = prepared.failure_entry_path;
                *failed_destination_path = prepared.failure_destination_path;
                *error_message = QStringLiteral("Quick Look batch export entry does not exist in archive.");
                if (out_status != nullptr) {
                    *out_status = Z7_MI_STATUS_INVALID_ARGUMENT;
                }
                return false;
            ...
            if (payload->is_directory != item.entry_is_directory) {
                ...
                *failed_entry_path = prepared.failure_entry_path;
                *failed_destination_path = prepared.failure_destination_path;
                *error_message = QStringLiteral("Quick Look batch export entry type does not match archive.");
                if (out_status != nullptr) {
                    *out_status = Z7_MI_STATUS_INVALID_ARGUMENT;
                }
                return false;
            ...
            if (payload->is_directory && !item.recursive) {
                ...
                *failed_entry_path = prepared.failure_entry_path;
                *failed_destination_path = prepared.failure_destination_path;
                *error_message = QStringLiteral("Quick Look batch export directory requires recursive extraction.");
                if (out_status != nullptr) {
                    *out_status = Z7_MI_STATUS_INVALID_ARGUMENT;
                }
                return false;
            ...
            if (payload->is_directory) {
                if (!payload->subtree_total_size.has_value() || !payload->subtree_file_count.has_value()) {
                    *failed_item_index = static_cast<int64_t>(index);
                    *failed_entry_path = prepared.failure_entry_path;
                    *failed_destination_path = prepared.failure_destination_path;
                    *error_message = QStringLiteral("Quick Look batch export directory statistics unavailable.");
                    if (out_status != nullptr) {
                        *out_status = Z7_MI_STATUS_INTERNAL_ERROR;
                    }
                    return false;
                ...
                if (prepared.known_size > capi::kQuicklookBatchExportMaxBytes) {
                    ...
                    *failed_entry_path = prepared.failure_entry_path;
                    *failed_destination_path = prepared.failure_destination_path;
                    *error_message = QStringLiteral("Quick Look batch export directory too large: more than 1 GiB.");
                    if (out_status != nullptr) {
                        *out_status = Z7_MI_STATUS_INVALID_ARGUMENT;
                    }
                    return false;
                ...
                if (aggregated_directory_file_count > capi::kQuicklookBatchExportDirectoryMaxFiles) {
                    *failed_item_index = static_cast<int64_t>(index);
                    *failed_entry_path = prepared.failure_entry_path;
                    *failed_destination_path = prepared.failure_destination_path;
                    *error_message =
                        QStringLiteral("Quick Look batch export selects more than 1000 files across directories.");
                    if (out_status != nullptr) {
                        *out_status = Z7_MI_STATUS_INVALID_ARGUMENT;
                    }
                    return false;
                ...
                if (prepared.known_size == 0) {
                    int const slash_index = prepared.normalized_entry_path.lastIndexOf(QLatin1Char('/'));
                    QString const parent_dir =
                        slash_index < 0 ? QString() : prepared.normalized_entry_path.left(slash_index);
                    QString const child_name = slash_index < 0 ? prepared.normalized_entry_path
                    ...
                    if (!parent_list_cache.contains(parent_dir)) {
                        ...
                        z7::app::OperationOutcome const list_outcome =
                            capi::run_archive_request_sync(z7::app::ArchiveRequest{list_request}, task_state);
                        auto const list_payload = z7::app::outcome_payload_as<z7::app::ListResult>(list_outcome);
                        if (!list_payload.has_value() || !list_payload->ok) {
                            if (out_status != nullptr) {
                                *out_status = quicklook_batch_export_status_from_outcome(list_outcome, task_state);
                            }
                            *failed_item_index = static_cast<int64_t>(index);
                            *failed_entry_path = prepared.failure_entry_path;
                            *failed_destination_path = prepared.failure_destination_path;
                    ...
                    std::optional<z7::app::ListResult> const& cached_list = parent_list_cache.value(parent_dir);
                    if (cached_list.has_value()) {
                        for (z7::app::ArchiveListEntry const& list_entry : cached_list->entries) {
                            QString const list_name = z7::ui::archive_support::from_native_string(list_entry.path);
                ...
                if (prepared.known_size > capi::kQuicklookBatchExportMaxBytes) {
                    ...
                    *failed_entry_path = prepared.failure_entry_path;
                    *failed_destination_path = prepared.failure_destination_path;
                    *error_message = QStringLiteral("Quick Look batch export file too large: more than 1 GiB.");
                    if (out_status != nullptr) {
                        *out_status = Z7_MI_STATUS_INVALID_ARGUMENT;
                    }
                    return false;
...
extern "C" {
    ...
    z7_mi_status_t z7_mi_quicklook_batch_export(z7_mi_session_t* session,
                                                z7_mi_quicklook_batch_export_request_t const* request,
        ...
        QString const archive_path = QFileInfo(capi::to_qstring(request->archive_path)).absoluteFilePath();
        if (archive_path.isEmpty()) {
            return Z7_MI_STATUS_INVALID_ARGUMENT;
        }
        QString const archive_type_hint = capi::to_qstring(request->archive_type_hint);
        QString nested_error;
        QStringList const nested_archive_entries = capi::normalize_nested_archive_entries(
            request->nested_archive_entries, request->nested_archive_entry_count, &nested_error);
        ...
        for (size_t index = 0; index < request->item_count; ++index) {
            z7_mi_quicklook_batch_export_item_t const& item = request->items[index];
            QuickLookBatchExportItem native_item;
            native_item.entry_path = capi::to_qstring(item.entry_path);
            native_item.destination_path = QFileInfo(capi::to_qstring(item.destination_path)).absoluteFilePath();
            ...
            if (!native_item.entry_is_directory && native_item.entry_path.trimmed().isEmpty()) {
                return Z7_MI_STATUS_INVALID_ARGUMENT;
            }
        ...
        uint64_t const task_id = capi::register_in_flight_task(state, task_state);
        if (task_id == 0) {
            return Z7_MI_STATUS_INTERNAL_ERROR;
        }
        ...
        *out_task = task_handle;
        ...
        std::weak_ptr<z7_mi_session_state> const weak_state(state);
        std::thread([callback,
                     progress_callback,
                     user_data,
                     state = state,
            ...
            QString failed_entry_path;
            QString failed_destination_path;
            bool const prepared = prepare_batch_export_items(state,
                                                             archive_path,
                                                             archive_type_hint,
                                                             nested_archive_entries,
                                                             task_state,
            ...
            if (!prepared) {
                ...
                if (out_result != nullptr) {
                    capi::init_quicklook_batch_export_error(
                        out_result,
                        preflight_status,
                        error_message.trimmed().isEmpty() ? QStringLiteral("Quick Look batch export preflight failed.")
                                                          : error_message,
                        0,
                        items.size(),
                        failed_item_index,
                ...
                auto delegate =
                    std::make_shared<QuickLookBatchExportDelegate>(progress_callback, user_data, prepared_items.size());
                QString const effective_archive_type =
                    capi::infer_archive_format(archive_path, archive_type_hint).trimmed().toLower();
                auto password_delegate = capi::make_quicklook_password_relay_delegate(
                    state, archive_path, effective_archive_type, nested_archive_entries, task_state, delegate);
                ...
                for (size_t index = 0; index < prepared_items.size(); ++index) {
                    if (task_state->cancel_requested.load()) {
                        out_result = capi::allocate_result<z7_mi_quicklook_batch_export_result_t>();
                        if (out_result != nullptr) {
                            capi::init_quicklook_batch_export_error(out_result,
                    ...
                    QuickLookBatchExportPreparedItem const& prepared_item = prepared_items[index];
                    delegate->set_item_context(completed_item_count,
                                               index,
                                               prepared_item.failure_entry_path,
                                               prepared_item.failure_destination_path);
                    delegate->emit_indeterminate_progress(
                        QStringLiteral("Extracting %1 of %2").arg(index + 1).arg(prepared_items.size()));

                    z7::ui::archive_support::QuickLookExtractPlan const extract_plan =
                        z7::ui::archive_support::build_quicklook_extract_plan(prepared_item.normalized_entry_path,
                    ...
                    extract_request.archive_type_hint = z7::ui::archive_support::to_utf8_string(archive_type_hint);
                    ...
                    extract_request.output_dir = z7::ui::archive_support::to_native_string(extract_plan.output_dir);
                    ...
                    if (!prepared_item.normalized_entry_path.isEmpty()) {
                        extract_request.entries.push_back(
                            z7::ui::archive_support::to_utf8_string(prepared_item.normalized_entry_path));
                    }
                    extract_request.path_remaps.push_back(extract_plan.path_remap);

                    z7::app::OperationOutcome const outcome = run_quicklook_extract_with_password_retry(
                        std::move(extract_request), task_state, password_delegate);
                    auto const payload = z7::app::outcome_payload_as<z7::app::ExtractResult>(outcome);
                    if (!payload.has_value() || !payload->ok) {
                        z7_mi_status_t const status = quicklook_batch_export_status_from_outcome(outcome, task_state);
                        out_result = capi::allocate_result<z7_mi_quicklook_batch_export_result_t>();
                        if (out_result != nullptr) {
                            capi::init_quicklook_batch_export_error(out_result,
                                                                    status,
                ...
                if (out_result == nullptr) {
                    out_result = build_batch_export_success_result(prepared_items.size(), prepared_items.size());
                }
            }
            ...
            bool const should_dispatch = capi::report_once(task_state);
    ...
    void z7_mi_free_quicklook_batch_export_progress(z7_mi_quicklook_batch_export_progress_t* progress) {
        free_batch_export_progress_strings(progress);
    }

    void z7_mi_destroy_quicklook_batch_export_progress(z7_mi_quicklook_batch_export_progress_t* progress) {
        if (progress == nullptr) {
            return;
        }
        free_batch_export_progress_strings(progress);
    ...
    void z7_mi_free_quicklook_batch_export_result(z7_mi_quicklook_batch_export_result_t* result) {
        free_batch_export_result_strings(result);
    }

    void z7_mi_destroy_quicklook_batch_export_result(z7_mi_quicklook_batch_export_result_t* result) {
        if (result == nullptr) {
            return;
        }
        free_batch_export_result_strings(result);
