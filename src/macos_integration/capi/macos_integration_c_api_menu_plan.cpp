...
extern "C" {
    ...
    z7_mi_build_menu_plan(z7_mi_session_t* session, z7_mi_selection_t const* selection, z7_mi_menu_plan_t* out_plan) {
        if (session == nullptr || selection == nullptr || out_plan == nullptr) {
            capi::init_menu_plan_error(out_plan,
        ...
        size_t const action_count = static_cast<size_t>(plan.actions.size());
        out_plan->action_count = action_count;
        if (action_count == 0) {
            return out_plan->status;
        }

        out_plan->actions = static_cast<z7_mi_menu_action_t*>(std::calloc(action_count, sizeof(z7_mi_menu_action_t)));
        if (out_plan->actions == nullptr) {
            z7_mi_free_menu_plan(out_plan);
            capi::init_menu_plan_error(
                out_plan, Z7_MI_STATUS_INTERNAL_ERROR, QStringLiteral("Out of memory allocating menu actions."));
            return Z7_MI_STATUS_INTERNAL_ERROR;
        }

        for (size_t i = 0; i < action_count; ++i) {
            out_plan->actions[i].action_id = capi::duplicate_c_string(plan.actions[static_cast<int>(i)].action_id);
            out_plan->actions[i].title = capi::duplicate_c_string(plan.actions[static_cast<int>(i)].title);
        }
        return out_plan->status;
    }
