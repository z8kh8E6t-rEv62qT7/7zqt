...
namespace z7::macos_integration::capi_internal {
    ...
    QString effective_archive_type_key(QString const& archive_path, QString const& type_hint) {
    ...
    z7_mi_status_t open_nested_session_chain(std::shared_ptr<z7_mi_session_state> const& state,
                ...
                while (state->nested_session_cache.size() > kCapacity) {
                    ...
                    for (auto it = state->nested_session_cache.begin(); it != state->nested_session_cache.end(); ++it) {
                        ...
                        if (oldest == state->nested_session_cache.end()
                            || chain->last_used < oldest.value()->last_used) {
                            oldest = it;
                        }
                    }
                    if (oldest == state->nested_session_cache.end()) {
        ...
        QString const effective_root_archive_type = effective_archive_type_key(archive_path, archive_type_hint);

        if (!nested_archive_entries.isEmpty() && state) {
            prune_cache();
            QString const cache_key =
                nested_session_cache_key(archive_path, effective_root_archive_type, nested_archive_entries);
        ...
        auto root_delegate = make_quicklook_password_relay_delegate(
            state, archive_path, effective_root_archive_type, QStringList{}, task_state);
        ...
        if (!root_payload.has_value() || !root_payload->ok || !root_payload->token.is_valid()) {
            if (error_message != nullptr) {
                *error_message = format_nested_error_message(0, fallback_archive_error_summary(open_root_outcome));
            }
        ...
        for (int i = 0; i < nested_archive_entries.size(); ++i) {
            ...
            auto child_delegate = make_quicklook_password_relay_delegate(
                state, archive_path, effective_root_archive_type, child_chain, task_state);
