...
namespace {
    ...
    bool dispatch_prompt_entry(std::shared_ptr<PromptDispatchEntry> const& entry) {
        if (!entry) {
            return false;
        }
...
extern "C" {
    ...
    void z7_mi_password_prompt_provide(z7_mi_password_prompt_t* handle, char const* password) {
        ...
        if (password == nullptr) {
            ...
            if (auto const state = handle->state.lock()) {
...
namespace z7::macos_integration::capi_internal {
    ...
    void finish_prompt_slot(std::shared_ptr<PromptSlot> const& slot, bool canceled, std::string password) {
        if (!slot) {
            return;
        }
        {
    ...
    void cancel_all_pending_prompts(std::shared_ptr<z7_mi_session_state> const& state) {
        if (!state) {
            return;
        }
        ...
        std::vector<std::shared_ptr<PromptDispatchEntry>> queued_dispatches;
        {
            ...
            for (auto const& weak_slot : state->orphan_prompt_slots) {
                if (auto const slot = weak_slot.lock()) {
        ...
        for (auto const& dispatch : queued_dispatches) {
            if (!dispatch) {
                continue;
            }
            {
        ...
        for (auto const& slot : prompt_slots) {
            finish_prompt_slot(slot, true);
        }
    }

    void clear_password_cache(std::shared_ptr<z7_mi_session_state> const& state) {
        if (!state) {
            return;
        }
    ...
    void queue_password_prompt_dispatch(std::shared_ptr<z7_mi_session_state> const& state,
                                        std::shared_ptr<PromptDispatchEntry> const& dispatch) {
        if (!state || !dispatch) {
            return;
        }
    ...
    void advance_password_prompt_dispatch(std::shared_ptr<z7_mi_session_state> const& state,
                                          std::shared_ptr<PromptDispatchEntry> const& completed_dispatch) {
        ...
        while (current_dispatch) {
            std::shared_ptr<PromptDispatchEntry> next_dispatch;
            {
                ...
                if (state->active_prompt_dispatch == current_dispatch) {
                    state->active_prompt_dispatch.reset();
                    while (!state->queued_prompt_dispatches.empty()) {
                        auto const candidate = state->queued_prompt_dispatches.front();
                        state->queued_prompt_dispatches.pop_front();
                        if (!candidate) {
                            continue;
                        }
