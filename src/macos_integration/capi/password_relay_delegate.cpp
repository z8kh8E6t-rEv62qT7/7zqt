...
namespace z7::macos_integration::capi_internal {
    namespace {
        ...
        public:
            QuickLookPasswordRelayDelegate(std::weak_ptr<z7_mi_session_state> state,
            ...
            std::optional<z7::app::PasswordReply> request_password(z7::app::PasswordPrompt const& prompt) override {
                auto const state = state_.lock();
                if (!state) {
                    return std::nullopt;
                }

                QString const cache_key =
                    nested_session_cache_key(archive_path_abs_, effective_archive_type_, nested_chain_);
                bool const wrong_password =
                    retry_count_ > 0 || prompt.reason_kind == z7::app::PasswordPromptReason::kWrongPassword;
                if (wrong_password) {
                    ...
                    std::lock_guard<std::mutex> lock(state->mutex);
                    auto const it = state->password_cache.constFind(cache_key);
                ...
                QString const reason_key = wrong_password          ? QStringLiteral("wrong_password")
                ...
                {
                    std::lock_guard<std::mutex> lock(state->mutex);
                    auto const it = state->pending_prompts.find(cache_key);
                    ...
                    if (!slot) {
                        ...
                        created = true;
                        auto const existing = state->pending_prompts.find(cache_key);
                ...
                if (created) {
                    ...
                    dispatch->cv.wait(dispatch_lock,
                                      [&dispatch]() { return dispatch->dispatched || dispatch->canceled; });
                    bool const dispatch_canceled = dispatch->canceled;
                    ...
                    QByteArray const archive_path_utf8 = archive_path_abs_.toUtf8();
                    QByteArray const reason_key_utf8 = reason_key.toUtf8();
                    std::vector<QByteArray> chain_storage;
                    std::vector<char const*> chain_ptrs;
                    chain_storage.reserve(static_cast<size_t>(nested_chain_.size()));
                    chain_ptrs.reserve(static_cast<size_t>(nested_chain_.size()));
                    for (QString const& entry : nested_chain_) {
                        chain_storage.push_back(entry.toUtf8());
                    }
                    for (QByteArray const& value : chain_storage) {
                ...
                bool const canceled = slot->canceled;
    ...
    make_quicklook_password_relay_delegate(std::shared_ptr<z7_mi_session_state> const& state,
        ...
        return std::make_shared<QuickLookPasswordRelayDelegate>(
            state, archive_path_abs, effective_archive_type, nested_chain, task_state, std::move(forward));
    }
