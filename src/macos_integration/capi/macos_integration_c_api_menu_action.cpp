...
namespace {
    ...
    void consume_task_ipc_completions(QString const& owner_instance_id) {
        QVector<z7::task_ipc_runtime::TaskIpcEvent> events;
        if (!z7::task_ipc_runtime::collect_task_ipc_events(owner_instance_id, &events, nullptr)) {
            return;
        }
    ...
    void register_task_ipc_completion_consumer(QString const& owner_instance_id) {
        z7::task_ipc_runtime::set_task_ipc_event_notifier(
            owner_instance_id,
            [](QString const& notified_owner_instance_id) { consume_task_ipc_completions(notified_owner_instance_id); },
            nullptr);
    }

    z7_mi_status_t dispatch_finder_sync_task_ipc(QString const& worker_program,
                                                 QString const& missing_program_name,
                                                 QString const& working_dir,
                                                 QString const& action_id,
                                                 z7::task_ipc_runtime::TaskIpcPayload const& payload,
                                                 z7_mi_action_result_t* out_result) {
        QFileInfo const worker_info(worker_program);
        ...
        QString const owner_instance_id = z7::task_ipc_runtime::ensure_task_ipc_owner_instance_id();
        ...
        bool const dispatched = z7::task_ipc_runtime::dispatch_task_ipc_task(
            worker_info.absoluteFilePath(), working_dir, owner_instance_id, payload, &dispatch_result, &dispatch_error);
        if (!dispatched) {
            capi::init_action_result_error(out_result,
                                           Z7_MI_STATUS_BACKEND_ERROR,
...
extern "C" {
    ...
    z7_mi_status_t z7_mi_execute_menu_action(z7_mi_session_t* session,
                                             char const* action_id,
                                             z7_mi_selection_t const* selection,
                                             z7_mi_action_result_t* out_result) {
        if (session == nullptr || selection == nullptr || out_result == nullptr) {
            capi::init_action_result_error(out_result,
                                           Z7_MI_STATUS_INVALID_ARGUMENT,
                                           QStringLiteral("Null argument passed to z7_mi_execute_menu_action."),
        ...
        native_selection.selected_paths =
            capi::string_list_from_utf8(selection->selected_paths, selection->selected_path_count);
        ...
        QString const q_action_id = capi::to_qstring(action_id);

        MacOSIntegrationConfig const settings = z7::macos_integration::load_macos_integration_config_from_settings();
        ShellIntegrationConfig config = capi::runtime_config_from_settings(settings);

        ShellIntegrationMenuPlan const plan =
            z7::shell_integration::build_shell_integration_menu_plan(native_selection, config);
        ...
        auto const action_it =
            std::find_if(plan.actions.cbegin(),
                         plan.actions.cend(),
                         [&q_action_id](z7::shell_integration::ShellIntegrationMenuAction const& action) {
        ...
        QString const first_path = plan.selected_paths.isEmpty() ? QString() : plan.selected_paths.front();
        QString const open_as_type = capi::open_as_type_for_action(q_action_id);
        bool const is_open_action = q_action_id == QString::fromLatin1(z7::shell_integration::kActionOpen);
        bool const is_open_as_action = !open_as_type.isEmpty();
        ...
        if (is_open_action || is_open_as_action) {
            ...
            QString const program = capi::bundled_program_path_from_process_dir(
                QCoreApplication::applicationDirPath(), QString::fromLatin1(z7::shell_integration::kProgram7zFM));
            QString launch_error;
            if (!z7::platform::qt::filemanager_instance_launcher::launch_open_request_for_program(
                    program,
                    first_path,
                ...
                capi::init_action_result_error(
                    ...
                    launch_error.trimmed().isEmpty()
                        ? z7::i18n::text(QStringLiteral("ui.navigation.task_ipc.launch_7zfm_failed"))
        ...
        if (!capi::build_task_ipc_payload_for_action(q_action_id, plan, &payload, &payload_error)) {
            capi::init_action_result_error(out_result, Z7_MI_STATUS_INVALID_ARGUMENT, payload_error, q_action_id);
            return Z7_MI_STATUS_INVALID_ARGUMENT;
        }

        QString const worker_program =
            capi::bundled_program_path_from_process_dir(QCoreApplication::applicationDirPath(), QStringLiteral("7zG"));
        return dispatch_finder_sync_task_ipc(worker_program,
                                             QStringLiteral("7zG"),
                                             capi::resolve_working_dir(plan, native_selection),
                                             q_action_id,
                                             payload,
