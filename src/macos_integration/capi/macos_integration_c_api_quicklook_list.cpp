...
namespace {
    ...
    build_quicklook_list_success_result(QString const& archive_path,
                                        QString const& virtual_dir,
                                        std::vector<z7::app::ArchiveListEntry> const& entries) {
        z7_mi_quicklook_list_result_t* out_result = capi::allocate_result<z7_mi_quicklook_list_result_t>();
        ...
        out_result->items =
            static_cast<z7_mi_quicklook_item_t*>(std::calloc(out_result->item_count, sizeof(z7_mi_quicklook_item_t)));
        if (out_result->items == nullptr) {
            z7_mi_free_quicklook_list_result(out_result);
            capi::init_quicklook_list_error(
                out_result, Z7_MI_STATUS_INTERNAL_ERROR, QStringLiteral("Out of memory allocating quicklook items."));
            return out_result;
        }

        for (size_t i = 0; i < out_result->item_count; ++i) {
            z7::app::ArchiveListEntry const& entry = entries[i];
            QString const name = z7::ui::archive_support::from_native_string(entry.path);
            out_result->items[i].name = capi::duplicate_c_string(name);
            out_result->items[i].path = capi::duplicate_c_string(capi::join_virtual_dir_with_name(virtual_dir, name));
            out_result->items[i].is_dir = entry.is_dir;
            out_result->items[i].size = entry.size;
            out_result->items[i].mtime_msecs_utc = entry.mtime_msecs_utc.has_value() ? *entry.mtime_msecs_utc : -1;
            out_result->items[i].is_archive_like = !entry.is_dir && capi::is_archive_like_by_name(name);
        }
        return out_result;
    }
...
extern "C" {
    ...
    z7_mi_status_t z7_mi_quicklook_list(z7_mi_session_t* session,
                                        z7_mi_quicklook_list_request_t const* request,
        ...
        if (session == nullptr || request == nullptr || callback == nullptr || out_task == nullptr) {
            (void)user_data;
        ...
        QString const virtual_dir = capi::normalize_virtual_entry_path(capi::to_qstring(request->virtual_dir));
        ...
        *out_task = task_handle;
        ...
        std::thread([callback,
            ...
            z7_mi_status_t const chain_status = capi::open_nested_session_chain(
                state, archive_path, archive_type_hint, nested_archive_entries, task_state, &chain, &chain_error);
            if (chain_status != Z7_MI_STATUS_OK) {
                ...
                if (out_result != nullptr) {
                    capi::init_quicklook_list_error(
                        out_result, Z7_MI_STATUS_INTERNAL_ERROR, QStringLiteral("Nested session chain was empty."));
                }
            } else {
                ...
                auto password_delegate = capi::make_quicklook_password_relay_delegate(
                    state, archive_path, effective_archive_type, nested_archive_entries, task_state, collector);

                z7::app::OperationOutcome const outcome = capi::run_archive_request_sync(
                ...
                if (capi::should_report_canceled(outcome, task_state->cancel_requested.load())) {
                    ...
                    if (out_result != nullptr) {
                        capi::init_quicklook_list_error(
                            out_result, Z7_MI_STATUS_CANCELED, capi::canceled_message(outcome));
                    }
                    ...
                    auto const payload = z7::app::outcome_payload_as<z7::app::ListResult>(outcome);
                    if (!payload.has_value() || !payload->ok) {
                        out_result = capi::allocate_result<z7_mi_quicklook_list_result_t>();
                        if (out_result != nullptr) {
                            capi::init_quicklook_list_error(
                                out_result, Z7_MI_STATUS_BACKEND_ERROR, capi::fallback_archive_error_summary(outcome));
                        }
                    } else {
                        ...
                        out_result = build_quicklook_list_success_result(archive_path, virtual_dir, entries);
                    }
                }
            }
