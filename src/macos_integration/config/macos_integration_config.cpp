...
namespace z7::macos_integration {
    namespace {
        ...
        inline constexpr char const* kSettingsOptionsIntegrateShell = "Options/IntegrateToShellMenu";
        inline constexpr char const* kSettingsOptionsCascadedMenu = "Options/CascadedMenu";
        inline constexpr char const* kSettingsOptionsMenuIcons = "Options/MenuIcons";
        inline constexpr char const* kSettingsOptionsContextMenu = "Options/ContextMenu";
        inline constexpr char const* kSettingsLanguage = "Lang";
        inline constexpr char const* kSettingsFileName = "settings.json";
        inline constexpr char const* kFileManagerAppName = "7zFM";
#ifdef Z7_TESTING
        inline constexpr char const* kTestSettingsRootEnv = "Z7_TEST_PORTABLE_SETTINGS_ROOT";
#endif

        QSet<QString> defaults_as_set() {
            QStringList const defaults = shell::default_shell_integration_visible_actions();
        ...
        std::uint32_t context_menu_flags_from_json(QJsonValue const& value, bool* ok_out) {
            bool ok = false;
            qulonglong raw = 0;
            if (value.isDouble()) {
        ...
        bool is_sandbox_container_home(QString const& path) {
        ...
        QString real_user_home_dir_from(QString value) {
            ...
            QString const cleaned = QDir::cleanPath(trimmed);
        ...
        QString posix_user_home_dir() {
            ...
            long const configured_size = sysconf(_SC_GETPW_R_SIZE_MAX);
...
#ifdef Z7_TESTING
            QString const test_root = qEnvironmentVariable(kTestSettingsRootEnv).trimmed();
            if (!test_root.isEmpty()) {
                return test_root;
            }
#endif

            QString const posix_home = real_user_home_dir_from(posix_user_home_dir());
            if (!posix_home.isEmpty()) {
                return QDir(posix_home).filePath(QStringLiteral(".config/7zqt"));
            }

            QString const environment_home = real_user_home_dir_from(qEnvironmentVariable("HOME"));
        ...
        QJsonObject read_7zfm_settings_object() {
            QString const settings_root = read_only_settings_root_dir();
        ...
        QString preferred_locale_from_settings(QJsonObject const& settings) {
            QString const stored = settings.value(QString::fromLatin1(kSettingsLanguage)).toString().trimmed();
    ...
    MacOSIntegrationConfig load_macos_integration_config_from_settings() {
        ...
        QJsonObject const settings = read_7zfm_settings_object();
        if (settings.isEmpty()) {
            return config;
        }

        config.enabled = settings.value(QString::fromLatin1(kSettingsOptionsIntegrateShell)).toBool(config.enabled);
        config.cascaded_menu =
            settings.value(QString::fromLatin1(kSettingsOptionsCascadedMenu)).toBool(config.cascaded_menu);
        config.show_menu_icons =
            settings.value(QString::fromLatin1(kSettingsOptionsMenuIcons)).toBool(config.show_menu_icons);

        QString const context_menu_key = QString::fromLatin1(kSettingsOptionsContextMenu);
        if (settings.contains(context_menu_key)) {
            bool ok = false;
            std::uint32_t const flags = context_menu_flags_from_json(settings.value(context_menu_key), &ok);
            if (ok) {
                QStringList const normalized = shell::shell_integration_visible_actions_from_context_menu_flags(flags);
