...
void populate_context_menu(QMenu* menu, SevenZipMenuState const& seven_zip_state);
void sync_path_bar_from_current_dir();
void rebuild_path_bar_popup_items();
bool navigate_to_path_from_bar(QString const& path_text);
bool can_open_parent_from_current_dir() const;
void rebuild_file_menu_seven_zip_section();
void remember_path_history(QString const& abs_path);
void rebuild_file_crc_menu();
void populate_crc_hash_menu(QMenu* menu, bool enabled, std::function<void(QString const&)> const& on_trigger);
void refresh_action_states();
void refresh_active_panel_chrome();
void update_status();
void update_status_for_panel(int panel_index);
void show_transient_status_message(QString const& message, int timeout_ms, int panel_index = -1);
...
void set_current_directory(QString const& path);
void set_current_directory_for_panel(int panel_index, QString const& path);
...
void set_folder_history(QStringList const& history);
...
void select_archive_entry_for_panel(int panel_index, QString const& archive_entry);
...
bool load_archive_virtual_directory_for_panel(int panel_index,
                                              ...
                                              std::function<void(int, QString const&)> const& failed_cb = {},
...
bool apply_archive_list_result_for_panel(int panel_index,
                                         QString const& archive_path,
                                         QString const& virtual_dir,
                                         QString const& origin_dir,
                                         QString const& archive_type_hint,
                                         z7::app::ListResult const& list_result,
                                         bool update_origin_dir,
                                         z7::app::ArchiveSessionToken session_token = {},
                                         QString const& virtual_display_source = {});
bool reload_archive_virtual_directory_for_panel(int panel_index);
bool reload_archive_virtual_directory_for_panel(int panel_index, std::function<void(bool)> const& finished_cb);
void reload_archive_virtual_directories_serially(QVector<int> panel_indexes,
                                                 std::function<void()> const& finished_cb = {});
...
bool export_archive_drag_entry_to_destination_for_panel(
    int panel_index, QString const& archive_entry, bool entry_is_dir, QString const& destination_path, QString* error);
void open_archive_entries_outside_for_panel(int panel_index, QStringList const& entries);
...
void open_archive_inside(QString const& archive_path, QString const& archive_type_hint = QString());
void open_archive_inside(QString const& archive_path, OpenArchiveInsideOptions options);
void open_archive_inside_for_panel(int panel_index,
                                   QString const& archive_path,
                                   QString const& archive_type_hint = QString());
void open_archive_inside_for_panel(int panel_index, QString const& archive_path, OpenArchiveInsideOptions options);
...
void open_item(QString const& path, bool try_internal, bool try_external, QString const& archive_type_hint = QString());
QString focused_path_for_panel(int panel_index) const;
bool open_path_externally_untracked(QString const& path);
bool confirm_external_open_targets_safe(QStringList const& paths, QString const& caption);
...
struct OpenOutsideLaunchResult {
    ...
    bool has_tracked_lifecycle() const {
        for (QPointer<QProcess> const& process : tracked_processes) {
            if (process != nullptr) {
                return true;
            }
        }
...
OpenOutsideLaunchResult launch_open_outside(QStringList const& paths, QString const& working_dir);
...
ArchiveTempFileSnapshot capture_archive_temp_file_snapshot(QString const& archive_entry,
                                                           QString const& extracted_path) const;
bool archive_temp_file_snapshot_changed(ArchiveTempFileSnapshot const& snapshot) const;
QString archive_update_format_for_session(ArchiveTempSession const& session) const;
void update_archive_entries_from_snapshots(ArchiveTempSession const& session,
                                           QVector<ArchiveTempFileSnapshot> const& changed_snapshots,
                                           std::function<void(bool, QString const&)> const& finished_cb);
ArchiveWritebackPlan build_archive_writeback_plan_for_panel(int panel_index) const;
bool can_add_external_files_to_archive_preview(int panel_index) const;
bool select_model_path_for_panel(int panel_index, QString const& model_path);
bool edit_focused_item_label_for_panel(int panel_index);
bool start_rename_item_for_panel(int panel_index, QString const& item_path, QString const& new_name, bool item_is_dir);
bool start_rename_filesystem_item_for_panel(int panel_index, QString const& source_path, QString const& new_name);
bool start_add_external_files_to_archive_preview(int panel_index,
                                                 QStringList const& input_paths,
                                                 QString const& archive_destination_virtual_dir,
                                                 QString const& caption);
bool start_add_mapped_files_to_archive_preview(
    ArchiveWritebackPlan const& plan,
    z7::app::ArchiveSessionToken session_token,
    QVector<ArchiveAddInputItem> const& input_items,
    QString const& caption,
...
bool start_rename_archive_entry_in_preview(int panel_index,
                                           QString const& archive_entry,
                                           QString const& new_name,
                                           bool entry_is_dir);
void restore_archive_writeback_plan_for_panel(int panel_index,
                                              ArchiveWritebackPlan const& plan,
                                              std::function<void(bool)> const& finished_cb = {});
...
bool payload_represents_archive_scoped_refresh(z7::task_ipc_runtime::TaskIpcPayload const& payload) const;
bool archive_refresh_target_matches_panel(z7::task_ipc_runtime::TaskIpcPayload const& payload, int panel_index) const;
bool refresh_archive_panels_for_task_ipc_payload(z7::task_ipc_runtime::TaskIpcPayload const& payload);
void finalize_archive_temp_session(QSharedPointer<ArchiveTempSession> const& session);
void on_open_outside_temp_session_tracking_finished(QSharedPointer<ArchiveTempSession> const& session);
void on_archive_temp_session_process_finished(QSharedPointer<ArchiveTempSession> const& session);
void on_open_outside_temp_session_finished(QSharedPointer<ArchiveTempSession> const& session);
...
struct PanelController {
    ...
    bool source_rows_contain_dir(QModelIndexList const& rows) const;
    QStringList real_item_paths_for_rows(QModelIndexList const& rows) const;
    ...
    bool auto_refresh_needs_rebind(QString const& current_dir) const;
    void enter_archive_view(QString const& source_archive,
    ...
    void commit_return_to_parent_archive(ParentArchiveReturnTransition const& transition);
    void rollback_return_to_parent_archive(ParentArchiveReturnTransition const& transition);
    ...
    void
    clear_archive_view_state(std::function<void(QSharedPointer<ArchiveTempSession> const&)> const& release_temp_session,
    ...
    QStringList selected_archive_entries() const;
    QStringList archive_entries_for_source_rows(QModelIndexList const& rows) const;
    ...
    bool matches_archive_writeback_target(QString const& source_archive, QString const& archive_display_source) const;
    ...
    void apply_restored_writeback_plan(ArchiveWritebackPlan const& plan,
                                       QVector<z7::app::ArchiveSessionToken> const& opened_tokens);
    ...
    void restore_scroll_position(ScrollPositionSnapshot const& snapshot) const;
    SelectionSnapshot capture_selection_snapshot() const;
    void restore_selection_snapshot(SelectionSnapshot const& snapshot) const;
    ...
    QModelIndex map_proxy_to_source(QModelIndex const& proxy_index) const;
    QModelIndex map_source_to_proxy(QModelIndex const& source_index) const;
...
PanelController const& panel_controller(int panel_index) const;
PanelController& active_panel_controller();
PanelController const& active_panel_controller() const;
...
QAbstractItemView* drop_target_view_for_panel(PanelController const& panel, QObject const* watched) const;
DropTargetInfo resolve_drop_target_info_for_panel(int panel_index,
                                                  QAbstractItemView* view,
                                                  QObject const* watched,
                                                  QDropEvent const* event,
                                                  QString const& fallback_directory) const;
int panel_index_for_view(QObject const* view_object) const;
void set_active_panel(int panel_index);
void bind_opposite_panel_to_same_folder();
void bind_opposite_panel_to_focused_folder();
bool bind_panel_to_filesystem_directory(int panel_index, QString const& directory);
bool bind_panel_to_archive_target(int panel_index, CrossPanelArchiveBindTarget const& target);
CrossPanelArchiveBindTarget archive_bind_target_from_panel(int panel_index, QString const& virtual_dir) const;
std::optional<CrossPanelBindTarget> focused_archive_bind_target_for_panel(int panel_index) const;
bool archive_session_token_referenced_outside_panel(int panel_index, z7::app::ArchiveSessionToken token) const;
QVector<z7::app::ArchiveSessionToken>
filter_archive_session_tokens_for_panel_close(int panel_index, QVector<z7::app::ArchiveSessionToken> tokens) const;
bool archive_temp_session_referenced_outside_panel(int panel_index,
                                                   QSharedPointer<ArchiveTempSession> const& session) const;
void release_archive_temp_session_for_panel_close(int panel_index, QSharedPointer<ArchiveTempSession> const& session);
...
void on_hash_with_method_requested(QString const& method);
...
bool handle_favorite_slot_shortcut(QKeyEvent const& event, Qt::KeyboardModifiers modifiers);
void set_favorite_slot(int slot);
void open_favorite_slot(int slot);
void open_favorite_path(QString const& path);
...
DropSourceState resolve_drop_source_state(int panel_index,
                                          bool window_drop_target,
                                          QDropEvent const* drop_event,
                                          QStringList const& dropped_paths,
                                          QString const& drop_target_directory) const;
bool handle_panel_drag_enter_or_move(QObject* watched,
                                     int panel_index,
                                     bool window_drop_target,
                                     QDropEvent* drop_like_event);
bool handle_panel_drop(QObject* watched, int panel_index, bool window_drop_target, QDropEvent* drop_event);
void on_panel_drag_finished(z7::ui::filemanager::DragExecutionReport const& report);
void run_copy_or_move(bool move, bool copy_to_same = false);
QString copy_move_info_text_for_source_rows(int panel_index, QModelIndexList const& rows) const;
bool run_filesystem_transfer_to_archive_panel(
    int target_panel_index, QStringList const& sources, bool move, QString const& history_path, QString const& caption);
...
void show_properties_table(QString const& title, QVector<PropertiesDialogRow> const& rows);
void run_view_or_edit_requested(bool use_editor_command);
void run_archive_view_or_edit_with_command(QString const& command_line, QString const& caption);
bool run_external_command_with_targets(QString const& command_line,
...
QMenu* append_seven_zip_submenu(QMenu* menu, SevenZipMenuState const& state, QAction* insert_before = nullptr);
QStringList selected_file_paths() const;
QString create_archive_name(bool is_hash, QString* base_name = nullptr) const;
bool build_archive_scoped_open_payload_for_panel(int panel_index,
                                                 z7::task_ipc_runtime::TaskIpcOpenPayload* out_payload) const;
QString archive_export_info_text_for_panel(int panel_index, QStringList const& archive_entries) const;
...
void run_sevenzip_open_archive_as(QString const& type);
...
void run_sevenzip_add_to_type(QString const& type);
void run_sevenzip_hash(QString const& method);
void run_sevenzip_generate_sha256();
void run_sevenzip_checksum_test();
bool launch_7zfm_open_request(QString const& caption, QString const& target_path, QString const& archive_type_hint);
bool launch_gui_subprocess_task(QString const& caption, z7::task_ipc_runtime::TaskIpcPayload const& payload);
void start_split_task(QString const& source_file_path, QString const& output_dir, QString const& volume_size_spec);
void start_combine_task(QString const& source_part_path, QString const& output_dir);
void start_hash_task(QStringList const& inputs, QString const& hash_method, bool recursive_dirs = true);
QVector<QPair<QString, QString>> build_hash_result_rows(z7::app::HashSummary const& summary) const;
...
QStringList extracted_archive_entry_paths(QString const& temp_dir_path, QStringList const& archive_entries) const;
QVector<ArchiveTempFileSnapshot> extracted_archive_entry_snapshots(QString const& temp_dir_path,
                                                                   QStringList const& archive_entries) const;
bool start_task_with_runner(
    QString const& header,
    QString const& failure_caption,
    std::function<bool(ArchiveProcessRunner*)> const& start_fn,
    std::function<void(bool, int, int, QString const&, z7::app::OperationOutcome const&)> const& finished_cb = {},
    RunnerTaskUiMode task_ui_mode = RunnerTaskUiMode::kDefault,
    std::function<bool(int, QString const&)> const& should_show_failure = {});
...
QAction *open_action_ = nullptr, *open_inside_action_ = nullptr, *open_inside_one_action_ = nullptr,
        *open_inside_parser_action_ = nullptr, *open_outside_action_ = nullptr, *view_action_ = nullptr,
        *edit_action_ = nullptr, *rename_action_ = nullptr, *copy_to_action_ = nullptr, *move_to_action_ = nullptr,
        *delete_action_ = nullptr, *split_action_ = nullptr, *combine_action_ = nullptr, *properties_action_ = nullptr,
        *comment_action_ = nullptr, *diff_action_ = nullptr, *version_edit_action_ = nullptr,
        *version_commit_action_ = nullptr, *version_revert_action_ = nullptr, *version_diff_action_ = nullptr,
        *create_folder_action_ = nullptr, *create_file_action_ = nullptr, *exit_action_ = nullptr,
        *link_action_ = nullptr, *alternate_streams_action_ = nullptr;
QAction *compress_action_ = nullptr, *extract_action_ = nullptr, *test_action_ = nullptr;
QAction *select_all_action_ = nullptr, *deselect_all_action_ = nullptr, *invert_selection_action_ = nullptr,
        *select_action_ = nullptr, *deselect_action_ = nullptr, *select_by_type_action_ = nullptr,
        *deselect_by_type_action_ = nullptr;
QAction *large_icons_action_ = nullptr, *small_icons_action_ = nullptr, *list_mode_action_ = nullptr,
        *details_mode_action_ = nullptr, *sort_name_action_ = nullptr, *sort_type_action_ = nullptr,
        *sort_date_action_ = nullptr, *sort_size_action_ = nullptr, *unsorted_action_ = nullptr,
        *flat_view_action_ = nullptr, *two_panels_action_ = nullptr, *open_root_action_ = nullptr,
        *open_parent_action_ = nullptr, *folders_history_action_ = nullptr, *refresh_action_ = nullptr,
        *auto_refresh_action_ = nullptr, *archive_toolbar_action_ = nullptr, *standard_toolbar_action_ = nullptr,
        *large_buttons_action_ = nullptr, *show_buttons_text_action_ = nullptr, *time_day_action_ = nullptr,
...
std::function<bool(QString const&)> external_opener_;
std::function<bool(QString const&, QStringList const&, QString const&, qint64*)> external_command_launcher_;
std::function<QMessageBox::StandardButton(
    QString const&, QString const&, QMessageBox::StandardButtons, QMessageBox::StandardButton)>
    question_box_;
