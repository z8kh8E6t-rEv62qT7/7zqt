// src/ui/filemanager/src/archive_process_runner/core_prompts.cpp
// Role: Default interaction dialogs for overwrite/password/choice/memory limits.
...
namespace z7::ui::filemanager {
    ...
    namespace {
        ...
        QString localized_strip(uint32_t id) {
        ...
        QString format_overwrite_time(std::optional<int64_t> const& msecs_utc) {
            if (!msecs_utc.has_value()) {
                return QStringLiteral("?");
            }
            QDateTime const utc = QDateTime::fromMSecsSinceEpoch(*msecs_utc, QTimeZone::UTC);
            if (!utc.isValid()) {
                return QStringLiteral("?");
            }
        ...
        QString memory_limit_dialog_message(z7::app::MemoryLimitPrompt const& prompt, uint64_t persisted_limit_bytes) {
            if (prompt.required_usage_bytes != 0
                || prompt.current_limit_defined
                || !prompt.archive_path.empty()
                || !prompt.file_path.empty()) {
                QString message =
                    QStringLiteral("The extraction operation requires a large amount of memory.\nRequired: %1 MB")
                        .arg(prompt.required_usage_bytes / kBytesPerMegabyte);
                if (prompt.current_limit_defined) {
                    message +=
                        QStringLiteral("\nAllowed limit: %1 MB").arg(prompt.current_limit_bytes / kBytesPerMegabyte);
                }
                if (persisted_limit_bytes != 0) {
                    message +=
                        QStringLiteral("\nConfigured limit: %1 MB").arg(persisted_limit_bytes / kBytesPerMegabyte);
                }
                if (!prompt.archive_path.empty()) {
                    message += QStringLiteral("\nArchive: %1")
                                   .arg(QDir::toNativeSeparators(QString::fromLocal8Bit(
                                       prompt.archive_path.data(), static_cast<int>(prompt.archive_path.size()))));
                }
                if (!prompt.file_path.empty()) {
                    message += QStringLiteral("\nFile: %1")
                                   .arg(QDir::toNativeSeparators(QString::fromLocal8Bit(
                                       prompt.file_path.data(), static_cast<int>(prompt.file_path.size()))));
                }
                return message;
            }
            ...
            if (prompt.safe_limit_defined) {
                message += QStringLiteral("\nSafe limit: %1 MB").arg(prompt.safe_limit_bytes / kBytesPerMegabyte);
            }
            if (persisted_limit_bytes != 0) {
                message += QStringLiteral("\nConfigured limit: %1 MB").arg(persisted_limit_bytes / kBytesPerMegabyte);
            }
        ...
        bool is_extract_memory_limit_prompt(z7::app::MemoryLimitPrompt const& prompt) {
            return prompt.required_usage_bytes != 0
                || prompt.current_limit_defined
                || !prompt.archive_path.empty()
                || !prompt.file_path.empty()
                || prompt.test_mode;
        ...
        uint64_t load_persisted_memory_limit_bytes(z7::app::MemoryLimitPrompt const& prompt) {
        ...
        void save_persisted_memory_limit_bytes(z7::app::MemoryLimitPrompt const& prompt, uint64_t bytes) {
            if (is_extract_memory_limit_prompt(prompt)) {
        ...
        bool prompt_for_updated_memory_limit_bytes(QWidget* parent,
            ...
            uint64_t const suggested_mb = (suggested + kBytesPerMegabyte - 1) / kBytesPerMegabyte;
            while (true) {
                bool ok = false;
                QString const entered = QInputDialog::getText(parent,
                                                              title,
                ...
                qulonglong const mb = entered.trimmed().toULongLong(&parsed_ok);
                if (!parsed_ok || mb == 0) {
                    QMessageBox::warning(parent, title, QStringLiteral("Please enter a positive integer value in MB."));
                    continue;
                }

                uint64_t const bytes = static_cast<uint64_t>(mb) * kBytesPerMegabyte;
    ...
    z7::app::OverwriteDecision show_default_overwrite_prompt(z7::app::OverwritePrompt const& prompt) {
        ...
        box.setWindowTitle(localized_strip(3500));

        QString const existing = QDir::toNativeSeparators(
            QString::fromLocal8Bit(prompt.existing_path.data(), static_cast<int>(prompt.existing_path.size())));
        QString const incoming = QDir::toNativeSeparators(
            QString::fromLocal8Bit(prompt.incoming_path.data(), static_cast<int>(prompt.incoming_path.size())));
        box.setText(localized_strip(3501));
        box.setInformativeText(QStringLiteral("Existing: %1\nNew: %2\n\nExisting size: %3\nNew size: %4\n"
                                              "Existing modified: %5\nNew modified: %6")
                                   .arg(existing,
                                        incoming,
        ...
        QPushButton* yes_all_button = box.addButton(z7::ui::runtime_support::L(440), QMessageBox::AcceptRole);
        QPushButton* no_all_button = box.addButton(z7::ui::runtime_support::L(441), QMessageBox::DestructiveRole);
        QPushButton* rename_button = box.addButton(localized_strip(3424), QMessageBox::ActionRole);
    ...
    z7::app::PasswordReply show_default_password_prompt(QWidget* parent, z7::app::PasswordPrompt const& prompt) {
        ...
        std::optional<QString> const value = z7::ui::runtime_support::show_password_prompt_dialog(parent, prompt);
    ...
    z7::app::ChoiceReply show_default_choice_prompt(z7::app::ChoicePrompt const& prompt) {
        ...
        QString title = QString::fromLocal8Bit(prompt.title.data(), static_cast<int>(prompt.title.size()));
        if (title.trimmed().isEmpty()) {
            title = QStringLiteral("Choose");
        }
        box.setWindowTitle(title);

        QString message = QString::fromLocal8Bit(prompt.message.data(), static_cast<int>(prompt.message.size()));
        if (message.trimmed().isEmpty()) {
        ...
        for (std::string const& choice : prompt.choices) {
            choice_buttons.push_back(box.addButton(
                QString::fromLocal8Bit(choice.data(), static_cast<int>(choice.size())), QMessageBox::ActionRole));
        }
        QPushButton* cancel_button = box.addButton(QMessageBox::Cancel);

        if (prompt.default_index >= 0 && static_cast<size_t>(prompt.default_index) < choice_buttons.size()) {
    ...
    z7::app::MemoryLimitReply show_default_memory_limit_prompt(z7::app::MemoryLimitPrompt const& prompt) {
        ...
        uint64_t const required_bytes =
            prompt.required_usage_bytes != 0 ? prompt.required_usage_bytes : prompt.estimated_usage_bytes;

        uint64_t const persisted_limit_bytes = load_persisted_memory_limit_bytes(prompt);
        if (persisted_limit_bytes != 0 && required_bytes <= persisted_limit_bytes) {
        ...
        if (prompt.required_usage_bytes != 0 || !prompt.archive_path.empty()) {
            title = localized_strip(3400);
        } else {
            title = localized_strip(7600);
            if (title.trimmed().isEmpty()) {
        ...
        while (true) {
            ...
            QPushButton* continue_button = box.addButton(QStringLiteral("Continue Once"), QMessageBox::AcceptRole);
#ifdef Z7_TESTING
            continue_button->setObjectName(QStringLiteral("memoryLimitContinueOnceButton"));
#endif
            QPushButton* update_button = box.addButton(QStringLiteral("Update Limit..."), QMessageBox::ActionRole);
#ifdef Z7_TESTING
            ...
            QPushButton* skip_button =
                box.addButton(prompt.skip_archive_supported ? QStringLiteral("Skip Archive") : QStringLiteral("Skip"),
            ...
            uint64_t const seed = persisted_limit_bytes != 0
                                    ? persisted_limit_bytes
                                    : (prompt.current_limit_defined
                                           ? prompt.current_limit_bytes
                                           : (prompt.safe_limit_defined ? prompt.safe_limit_bytes : required_bytes));
            if (!prompt_for_updated_memory_limit_bytes(parent, title, required_bytes, seed, &updated_limit_bytes)) {
                continue;
            }
