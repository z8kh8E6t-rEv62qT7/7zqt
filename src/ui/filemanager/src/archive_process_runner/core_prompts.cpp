...
QString memory_limit_dialog_message(const z7::app::MemoryLimitPrompt& prompt,
  ...
  if (prompt.required_usage_bytes != 0 || prompt.current_limit_defined ||
    ...
    if (!prompt.archive_path.empty()) {
      message += QStringLiteral("\nArchive: %1").arg(QDir::toNativeSeparators(
          QString::fromLocal8Bit(prompt.archive_path.data(),
                                 static_cast<int>(prompt.archive_path.size()))));
    }
    if (!prompt.file_path.empty()) {
      message += QStringLiteral("\nFile: %1").arg(QDir::toNativeSeparators(
          QString::fromLocal8Bit(prompt.file_path.data(),
                                 static_cast<int>(prompt.file_path.size()))));
    }
    return message;
  }
...
z7::app::OverwriteDecision show_default_overwrite_prompt(
  ...
  const QString existing = QDir::toNativeSeparators(QString::fromLocal8Bit(
      prompt.existing_path.data(), static_cast<int>(prompt.existing_path.size())));
  const QString incoming = QDir::toNativeSeparators(QString::fromLocal8Bit(
      prompt.incoming_path.data(), static_cast<int>(prompt.incoming_path.size())));
...
z7::app::ChoiceReply show_default_choice_prompt(
  ...
  QString title = QString::fromLocal8Bit(
      prompt.title.data(), static_cast<int>(prompt.title.size()));
  ...
  QString message = QString::fromLocal8Bit(
      prompt.message.data(), static_cast<int>(prompt.message.size()));
