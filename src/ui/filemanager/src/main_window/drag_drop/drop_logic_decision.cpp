...
namespace z7::ui::filemanager {
    ...
    namespace {
...
#ifdef Z7_TESTING
        bool parse_drop_command_override(QVariant const& value, DropCommand* command) {
        ...
        DropCommand choose_internal_fs_left_drop_command(QDropEvent const* event, bool source_target_same_volume) {
            Qt::KeyboardModifiers const modifiers = event != nullptr ? event->modifiers() : Qt::NoModifier;
            bool const has_ctrl = modifiers.testFlag(Qt::ControlModifier);
            bool const has_shift = modifiers.testFlag(Qt::ShiftModifier);
            bool const has_alt = modifiers.testFlag(Qt::AltModifier);
        ...
        DropCommand constrain_copy_move_command_by_possible_actions(DropCommand command,
                                                                    Qt::DropActions possible_actions,
                                                                    bool source_target_same_volume) {
            bool const allow_copy = possible_actions.testFlag(Qt::CopyAction);
            bool const allow_move = possible_actions.testFlag(Qt::MoveAction);
        ...
        DropCommand normalize_drop_command(DropCommand command, bool in_archive_view, bool allow_copy_move) {
            ...
            if ((command == DropCommand::kCopy || command == DropCommand::kMove) && !allow_copy_move) {
                return DropCommand::kAddToArchive;
            }
            return command;
        }

        bool is_right_button_drop(QDropEvent const* event) {
            if (event == nullptr) {
                return false;
            }
        ...
        QPoint drop_event_pos_in_target(QObject const* watched, QDropEvent const* event) {
            if (event == nullptr) {
                return QPoint();
            }
            QWidget const* target = qobject_cast<QWidget const*>(watched);
            if (target == nullptr) {
        ...
        QPoint drop_event_global_pos(QObject const* watched, QDropEvent const* event) {
            QWidget const* target = qobject_cast<QWidget const*>(watched);
            if (target == nullptr) {
                return QPoint();
            }
    ...
    DropCommand choose_drop_command(MainWindow* window,
        ...
        bool const right_button = is_right_button_drop(event);
        Qt::DropActions const possible_actions =
            event != nullptr ? event->possibleActions() : (Qt::CopyAction | Qt::MoveAction);
        bool const allow_copy_effect = possible_actions.testFlag(Qt::CopyAction);
        bool const allow_move_effect = possible_actions.testFlag(Qt::MoveAction);
#ifdef Z7_TESTING
        DropCommand command_override = DropCommand::kCancel;
        if (parse_drop_command_override(window->property("z7.fm.drop.command.override"), &command_override)) {
            if (command_override == DropCommand::kCopy || command_override == DropCommand::kMove) {
                command_override = constrain_copy_move_command_by_possible_actions(
                    command_override, possible_actions, source_target_same_volume);
            }
        ...
        if (!right_button) {
            ...
            DropCommand const default_command =
        ...
        if (in_archive_view) {
            if (allow_copy_effect) {
                ...
                QString const archive_suffix = z7::ui::runtime_support::strip_mnemonic(lang_or(2321));
        ...
        if (parse_drop_command_override(window->property("z7.fm.drop.menu.selection.override"),
                                        &menu_selection_override)) {
            if (menu_selection_override == DropCommand::kCopy && copy_action != nullptr) {
                return normalize_drop_command(constrain_copy_move_command_by_possible_actions(
                                                  DropCommand::kCopy, possible_actions, source_target_same_volume),
                                              in_archive_view,
                                              allow_copy_move);
            }
            if (menu_selection_override == DropCommand::kMove && move_action != nullptr) {
                return normalize_drop_command(constrain_copy_move_command_by_possible_actions(
                                                  DropCommand::kMove, possible_actions, source_target_same_volume),
                                              in_archive_view,
                                              allow_copy_move);
            }
            if (menu_selection_override == DropCommand::kAddToArchive && add_action != nullptr) {
                return DropCommand::kAddToArchive;
            }
            if (menu_selection_override == DropCommand::kCopyToArchive && copy_to_archive_action != nullptr) {
                return DropCommand::kCopyToArchive;
            }
            return DropCommand::kCancel;
        }
        ...
        QPoint const global_pos = drop_event_global_pos(watched, event);
    ...
    DropCommand choose_drop_preview_command(MainWindow* window,
        ...
        if (app_target) {
            ...
            return in_archive_view ? DropCommand::kCopyToArchive : DropCommand::kAddToArchive;
        }
        if (in_archive_view) {
            if (!allow_copy_effect) {
                return DropCommand::kCancel;
        ...
        if (right_button && allow_copy_move) {
            ...
            DropCommand const default_right_command =
                source_target_same_volume ? DropCommand::kMove : DropCommand::kCopy;
            return normalize_drop_command(constrain_copy_move_command_by_possible_actions(
                                              default_right_command, possible_actions, source_target_same_volume),
                                          in_archive_view,
                                          allow_copy_move);
        }
        if (!allow_copy_effect) {
        ...
        return normalize_drop_command(DropCommand::kAddToArchive, in_archive_view, allow_copy_move);
    }
