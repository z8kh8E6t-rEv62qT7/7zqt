...
namespace z7::ui::filemanager {
    ...
    namespace {
        ...
        QString archive_drop_destination_virtual_dir(QString const& virtual_dir) {
            return z7::ui::archive_support::normalize_virtual_dir(virtual_dir);
        }

        QString archive_drop_destination_display_path(QString const& normalized_virtual_dir) {
        ...
        QString archive_drop_destination_display_path(QString const& display_source,
            ...
            return z7::ui::archive_support::virtual_display_path(display_source, normalized_virtual_dir);
        }

    } // namespace

    bool
    MainWindow::handle_panel_drop(QObject* watched, int panel_index, bool window_drop_target, QDropEvent* drop_event) {
        ...
        bool const has_trusted_archive_source =
            read_internal_archive_source_marker(drop_event->mimeData(), &archive_payload, &archive_marker_trusted)
            && archive_marker_trusted
            && !archive_payload.entries.isEmpty();
        QStringList const dropped_paths =
            has_trusted_archive_source ? QStringList() : local_existing_drop_paths(drop_event->mimeData());
        ...
        DropTargetInfo const drop_target =
            resolve_drop_target_info_for_panel(panel_index, target_view, watched, drop_event, panel_fs_directory);
        QString const drop_target_directory = drop_target.directory;

        DropSourceState const source_state = resolve_drop_source_state(
            panel_index, window_drop_target, drop_event, dropped_paths, drop_target_directory);
        if (source_state.same_panel_source) {
            apply_windows_drop_effect_feedback(drop_event, false, Qt::IgnoreAction, false);
            drop_event->setDropAction(Qt::IgnoreAction);
            drop_event->ignore();
            return true;
        }

        DropCommand const command = choose_drop_command(this,
                                                        archive_view,
        ...
        auto const reject_drop = [drop_event]() {
            apply_windows_drop_effect_feedback(drop_event, false, Qt::IgnoreAction, false);
            drop_event->setDropAction(Qt::IgnoreAction);
            drop_event->ignore();
            return true;
            ...
            case DropCommand::kAddToArchive:
                {
                    ...
                    QString const archive_path = QDir(base_folder).filePath(archive_name);
                    QString const caption = add_caption();
            ...
            case DropCommand::kCopyToArchive:
                {
...
#ifdef Z7_TESTING
                    DropCopyToArchiveConfirmOverride const confirm_override =
                        parse_copy_to_archive_confirm_override(property("z7.fm.drop.copy_to_archive.confirm.override"));
                    if (confirm_override == DropCopyToArchiveConfirmOverride::kNo) {
...
#endif
                        QString const confirm_title = z7::ui::runtime_support::strip_mnemonic(lang_or(6010));
                        QString const confirm_text =
                            QStringLiteral("%1\n%2\n%3 ?")
                                .arg(z7::ui::runtime_support::strip_mnemonic(lang_or(6002)),
                                     archive_drop_destination_display_path(panel.archive_display_source(),
                        ...
                        QMessageBox confirm_box(QMessageBox::Question,
                                                ...
                                                this);
#ifdef Z7_TESTING
                        confirm_box.setProperty("z7.fm.semantic.window_title", confirm_title);
#endif
            ...
            case DropCommand::kMove:
                {
                    ...
                    if (source_state.internal_archive_source) {
                        ...
                        started = start_archive_source_extract_task(
                            QStringLiteral("%1: %2").arg(z7::ui::runtime_support::strip_mnemonic(lang_or(6000)),
                    ...
                    bool const move_mode = command == DropCommand::kMove;
                    QString const op_caption =
                        z7::ui::runtime_support::strip_mnemonic(lang_or(move_mode ? 6001 : 6000));
                    QString const destination_display = QDir::toNativeSeparators(drop_target_directory);
                    started = start_task_with_runner(
                        QStringLiteral("%1: %2").arg(op_caption, destination_display),
                        op_caption,
                        [move_mode, dropped_paths, drop_target_directory](ArchiveProcessRunner* runner) {
                            if (runner == nullptr) {
                                return false;
                            }
                            return move_mode ? runner->start_move_paths(
                                                   dropped_paths, drop_target_directory, OverwriteMode::kOverwrite)
        ...
        if (started) {
            if (source_state.internal_archive_source) {
                if (QMimeData* mutable_mime_data = const_cast<QMimeData*>(drop_event->mimeData());
                    mutable_mime_data != nullptr) {
                    mutable_mime_data->setData(QString::fromLatin1(kMimeTypeZ7FmArchiveInternalDropHandled),
                                               QByteArrayLiteral("1"));
                }
            }
            Qt::DropAction const reported_action =
                reported_drop_action_for_source(command, source_state.trusted_internal_fs_source);
            apply_windows_drop_effect_feedback(
                drop_event, true, reported_action, source_state.trusted_internal_fs_source);
