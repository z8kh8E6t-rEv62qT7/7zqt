...
namespace z7::ui::filemanager {
    ...
    bool MainWindow::eventFilter(QObject* watched, QEvent* event) {
        if (path_combo_ != nullptr
            && watched == path_combo_->lineEdit()
            && event != nullptr
            && event->type() == QEvent::ShortcutOverride) {
            auto* key_event = static_cast<QKeyEvent*>(event);
            bool const no_modifiers = key_event->modifiers() == Qt::NoModifier;
            if (no_modifiers
        ...
        if (event != nullptr) {
            ...
            bool const panel_view_target = panel_index >= 0;
            bool const window_drop_target = watched == this || watched == centralWidget();
            bool const is_drop_event = event->type() == QEvent::DragEnter
            ...
            if (panel_index >= 0) {
                ...
                if (event->type() == QEvent::DragEnter || event->type() == QEvent::DragMove) {
                    return handle_panel_drag_enter_or_move(
                        watched, panel_index, window_drop_target, static_cast<QDropEvent*>(event));
                }
                if (event->type() == QEvent::Drop) {
                    return handle_panel_drop(watched, panel_index, window_drop_target, static_cast<QDropEvent*>(event));
                }
                if (event->type() == QEvent::KeyPress) {
                    set_active_panel(panel_index);
                    auto* key_event = static_cast<QKeyEvent*>(event);
                    Qt::KeyboardModifiers const modifiers = key_event->modifiers() & ~Qt::KeypadModifier;
                    ...
                    if (key_event->key() == Qt::Key_A && modifiers == Qt::ControlModifier) {
                        set_operable_rows_selected_for_panel(panel_index, true);
                        return true;
                    }
                    if (key_event->key() == Qt::Key_N && modifiers == Qt::ControlModifier) {
                        on_create_file_requested();
                        return true;
                    }
                    if (key_event->key() == Qt::Key_R && modifiers == Qt::ControlModifier) {
                        on_refresh_requested();
                        return true;
                    }
                    if (key_event->key() == Qt::Key_W && modifiers == Qt::ControlModifier) {
                        close();
                        return true;
                    }
                    ...
                    if (key_event->key() == Qt::Key_Plus && modifiers == Qt::AltModifier) {
                        on_select_by_type_requested();
                        return true;
                    }
                    if (key_event->key() == Qt::Key_Plus && modifiers == Qt::ShiftModifier) {
                        set_operable_rows_selected_for_panel(panel_index, true);
                        return true;
                    }
                    if (key_event->key() == Qt::Key_Plus && modifiers == Qt::NoModifier) {
                        on_select_requested();
                        return true;
                    }
                    if (key_event->key() == Qt::Key_Minus && modifiers == Qt::AltModifier) {
                        on_deselect_by_type_requested();
                        return true;
                    }
                    if (key_event->key() == Qt::Key_Minus && modifiers == Qt::ShiftModifier) {
                        set_operable_rows_selected_for_panel(panel_index, false);
                        return true;
                    }
                    if (key_event->key() == Qt::Key_Minus && modifiers == Qt::NoModifier) {
                        on_deselect_requested();
                        return true;
                    }
                    if (key_event->key() == Qt::Key_Asterisk && modifiers == Qt::NoModifier) {
                    ...
                    if (key_event->key() == Qt::Key_F7 && modifiers == Qt::NoModifier) {
                        on_create_folder_requested();
                        return true;
                    }
                    if (key_event->key() == Qt::Key_F12 && modifiers == Qt::AltModifier) {
                    ...
                    if (key_event->key() == Qt::Key_F9 && modifiers == Qt::NoModifier) {
                        on_two_panels_action_triggered();
                        return true;
                    }
                    QAbstractItemView const* details_view = active_panel_controller().ui.details_view;
                    bool const details_view_key_event =
                        watched == details_view || (details_view != nullptr && watched == details_view->viewport());
