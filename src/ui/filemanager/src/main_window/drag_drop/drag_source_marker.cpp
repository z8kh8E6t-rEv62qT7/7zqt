...
namespace z7::ui::filemanager {
    ...
    namespace {
        ...
        QString ensure_drag_source_instance_id() {
            ...
            QString instance_id = app->property(kDragSourceInstanceIdProperty).toString().trimmed();
        ...
        bool parse_marker_v1(QByteArray const& payload, qint64* pid, QString* sid) {
            if (pid == nullptr || sid == nullptr) {
                return false;
            }

            QString const text = QString::fromUtf8(payload).trimmed();
            if (!text.startsWith(QStringLiteral("v1;"))) {
                return false;
            }

            QStringList const parts = text.split(QLatin1Char(';'), Qt::SkipEmptyParts);
            ...
            for (int i = 1; i < parts.size(); ++i) {
                ...
                int const eq = part.indexOf(QLatin1Char('='));
                if (eq <= 0 || eq >= part.size() - 1) {
                    continue;
                }
                QString const key = part.left(eq).trimmed().toLower();
                QString const value = part.mid(eq + 1).trimmed();
            ...
            bool ok = false;
            qint64 const parsed_pid = pid_text.toLongLong(&ok);
            if (!ok) {
                return false;
            }
        ...
        bool parse_archive_marker(QByteArray const& payload,
                                  qint64* pid,
                                  QString* sid,
            ...
            QJsonDocument const document = QJsonDocument::fromJson(payload);
            ...
            QJsonValue const pid_value = object.value(QStringLiteral("pid"));
            ...
            *sid = object.value(QStringLiteral("sid")).toString().trimmed();
            ...
            archive_payload->archive_path = object.value(QStringLiteral("archive_path")).toString().trimmed();
            archive_payload->archive_type_hint = object.value(QStringLiteral("archive_type_hint")).toString().trimmed();
            archive_payload->session_token_value = 0;

            QJsonValue const session_token_value = object.value(QStringLiteral("session_token"));
            if (session_token_value.isDouble()) {
                archive_payload->session_token_value = static_cast<quint64>(session_token_value.toInteger());
            } else if (session_token_value.isString()) {
                bool ok = false;
                archive_payload->session_token_value = session_token_value.toString().trimmed().toULongLong(&ok);
                if (!ok) {
                    archive_payload->session_token_value = 0;
                }
            }

            QJsonArray const entries_array = object.value(QStringLiteral("entries")).toArray();
            QSet<QString> dedup;
            archive_payload->entries.clear();
            archive_payload->entries.reserve(entries_array.size());
            for (QJsonValue const& value : entries_array) {
                QString const entry = value.toString().trimmed();
    ...
    QByteArray make_internal_fs_source_marker() {
        QString const instance_id = ensure_drag_source_instance_id();
        if (instance_id.trimmed().isEmpty()) {
            return QByteArrayLiteral("1");
        }

        return QStringLiteral("v1;pid=%1;sid=%2").arg(QCoreApplication::applicationPid()).arg(instance_id).toUtf8();
    }
    ...
    bool is_trusted_internal_fs_source_marker(QMimeData const* mime_data) {
        ...
        if (!parse_marker_v1(
                mime_data->data(QString::fromLatin1(kMimeTypeZ7FmFsSource)), &marker_pid, &marker_instance_id)) {
            return false;
        }

        QString const current_instance_id = ensure_drag_source_instance_id();
    ...
    bool read_internal_archive_source_marker(QMimeData const* mime_data,
        ...
        qint64 marker_pid = -1;
        QString marker_instance_id;
        if (!parse_archive_marker(mime_data->data(QString::fromLatin1(kMimeTypeZ7FmArchiveSource)),
                                  &marker_pid,
                                  &marker_instance_id,
                                  &parsed_payload)) {
            return false;
        ...
        QString const current_instance_id = ensure_drag_source_instance_id();
        bool const marker_is_trusted = !current_instance_id.isEmpty()
