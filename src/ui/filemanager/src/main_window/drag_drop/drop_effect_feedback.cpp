...
namespace z7::ui::filemanager {
    ...
    namespace {
        ...
        constexpr char const* kClipboardNamePerformedDropEffect = "Performed DropEffect";
        constexpr char const* kClipboardNameLogicalPerformedDropEffect = "Logical Performed DropEffect";
        ...
        void set_windows_clipboard_format_payload(QMimeData* mime_data,
            ...
            QString const quoted_key =
                QStringLiteral("application/x-qt-windows-mime;value=\"%1\"").arg(QString::fromLatin1(clipboard_name));
            QString const unquoted_key =
                QStringLiteral("application/x-qt-windows-mime;value=%1").arg(QString::fromLatin1(clipboard_name));
    ...
    void apply_windows_drop_effect_feedback(QDropEvent const* event,
        ...
        quint32 const performed = performed_drop_effect_value(operation_succeeded);
        quint32 const logical =
            logical_drop_effect_value(operation_succeeded, reported_action, trusted_internal_source);

        set_windows_clipboard_format_payload(
            mime_data, kClipboardNamePerformedDropEffect, encode_windows_drop_effect_dword(performed));
        set_windows_clipboard_format_payload(
            mime_data, kClipboardNameLogicalPerformedDropEffect, encode_windows_drop_effect_dword(logical));
    }
