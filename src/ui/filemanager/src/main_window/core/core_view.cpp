...
namespace z7::ui::filemanager {
    ...
    namespace {
        ...
        QString parent_archive_virtual_dir(QString const& virtual_dir) {
        ...
        QString external_open_suspicious_name_message(QString name) {
            ...
            while (suffix_start > 0) {
                QChar const c = displayed_name.at(suffix_start - 1);
            ...
            if (suffix_start != displayed_name.size()) {
                QString const trimmed_name = displayed_name.left(suffix_start);
                QString const suffix = QFileInfo(trimmed_name).suffix().toLower();
            ...
            if (!long_space) {
                qsizetype const left = message.indexOf(QLatin1Char('('));
                qsizetype const right = left >= 0 ? message.indexOf(QLatin1Char(')'), left + 1) : -1;
        ...
        public:
            explicit FoldersHistoryDialog(QStringList const& history, QWidget* parent) :
                QDialog(parent), list_(new QListWidget(this)) {
#ifdef Z7_TESTING
                setObjectName(QStringLiteral("foldersHistoryDialog"));
#endif
                setWindowTitle(z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(736)));
                resize(520, 360);
...
#endif
                delete_button_ =
                    buttons->addButton(z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(7205)),
                ...
                connect(list_, &QListWidget::itemSelectionChanged, this, [this]() { update_delete_button(); });
                connect(list_, &QListWidget::itemDoubleClicked, this, [this](QListWidgetItem*) { accept(); });
                connect(delete_button_, &QPushButton::clicked, this, [this]() { delete_selected_items(); });
            ...
            FoldersHistoryDialogResult result() const {
                ...
                for (int row = 0; row < list_->count(); ++row) {
                    QListWidgetItem const* item = list_->item(row);
                    if (item != nullptr) {
        ...
        protected:
            bool eventFilter(QObject* watched, QEvent* event) override {
                if (watched == list_ && event != nullptr && event->type() == QEvent::KeyPress) {
        ...
        private:
            ...
            void update_delete_button() { delete_button_->setEnabled(!list_->selectedItems().isEmpty()); }
    ...
    void MainWindow::apply_archive_preview_columns_visibility_for_panel(int panel_index) {
        ...
        bool const archive_mode = panel.in_archive_view();
        std::array<int, 11> const archive_only_columns = {DirectoryListModel::kPackedSizeColumn,
        ...
        for (int const column : archive_only_columns) {
    ...
    MainWindow::archive_bind_target_from_panel(int panel_index, QString const& virtual_dir) const {
        PanelController const& panel = panel_controller(panel_index);
        CrossPanelArchiveBindTarget target;
        target.source_archive = panel.archive.source_archive;
        target.virtual_dir = z7::ui::archive_support::normalize_virtual_dir(virtual_dir);
    ...
    MainWindow::focused_archive_bind_target_for_panel(int panel_index) const {
        ...
        if (panel.model->is_parent_link_for_row(focused.row())) {
            ...
            if (!current_virtual.isEmpty()) {
                ...
                out.archive = true;
                out.archive_target =
                    archive_bind_target_from_panel(panel_index, parent_archive_virtual_dir(current_virtual));
                return out;
            }

            if (!panel.archive.parent_stack.isEmpty()) {
                QVector<PanelController::ArchiveState::ParentContext> parent_stack = panel.archive.parent_stack;
                PanelController::ArchiveState::ParentContext const parent = parent_stack.back();
                parent_stack.removeLast();

                CrossPanelBindTarget out;
                out.archive = true;
                out.archive_target.source_archive =
                    parent.archive_path.isEmpty() ? panel.archive.source_archive : parent.archive_path;
                out.archive_target.virtual_dir = z7::ui::archive_support::normalize_virtual_dir(parent.virtual_dir);
                out.archive_target.origin_dir = parent.origin_dir;
                out.archive_target.type_hint = parent.type_hint;
                out.archive_target.virtual_display_source = parent.virtual_display_source.isEmpty()
                                                              ? out.archive_target.source_archive
            ...
            out.filesystem_dir = panel.archive.origin_dir.isEmpty()
                                   ? QFileInfo(panel.archive.source_archive).absolutePath()
        ...
        QString const focused_virtual_dir =
        ...
        CrossPanelBindTarget out;
        out.archive = true;
        out.archive_target = archive_bind_target_from_panel(panel_index, focused_virtual_dir);
        return out;
    }

    bool MainWindow::bind_panel_to_filesystem_directory(int panel_index, QString const& directory) {
        if (panel_index < 0 || panel_index > 1) {
            return false;
        }

        QFileInfo const info(directory);
        if (!info.exists() || !info.isDir()) {
            return false;
        }
        QString const target_dir = info.absoluteFilePath();

        if (in_archive_view_for_panel(panel_index)) {
            close_archive_view_for_panel(panel_index, [this, panel_index, target_dir](bool ok) {
                if (ok) {
    ...
    bool MainWindow::bind_panel_to_archive_target(int panel_index, CrossPanelArchiveBindTarget const& target) {
        if (panel_index < 0
            || panel_index > 1
        ...
        auto const start_load = [this, panel_index, target]() -> bool {
        ...
        if (in_archive_view_for_panel(panel_index)) {
            close_archive_view_for_panel(panel_index, [start_load](bool ok) {
                if (ok) {
                    (void)start_load();
                }
            });
    ...
    void MainWindow::bind_opposite_panel_to_same_folder() {
        ...
        int const source_panel = active_panel_index_;
        int const target_panel = 1 - source_panel;
        PanelController const& source = panel_controller(source_panel);
        if (source.in_archive_view()) {
            (void)bind_panel_to_archive_target(
                target_panel, archive_bind_target_from_panel(source_panel, source.archive.virtual_dir));
            return;
        }

        (void)bind_panel_to_filesystem_directory(target_panel, source.current_directory());
    }
    ...
    void MainWindow::bind_opposite_panel_to_focused_folder() {
        ...
        if (source.in_archive_view()) {
            std::optional<CrossPanelBindTarget> const target = focused_archive_bind_target_for_panel(source_panel);
            if (!target.has_value()) {
                return;
            }
            if (target->archive) {
                (void)bind_panel_to_archive_target(target_panel, target->archive_target);
            } else {
                (void)bind_panel_to_filesystem_directory(target_panel, target->filesystem_dir);
            }
            return;
        }

        QModelIndex const focused = source.focused_source_index();
        if (!focused.isValid() || !source.model->is_dir_for_row(focused.row())) {
            return;
        }

        (void)bind_panel_to_filesystem_directory(target_panel, source.model->path_for_row(focused.row()));
    }
    ...
    void MainWindow::apply_sort_mode_to_panel(int panel_index, int sort_action, bool toggle_if_same) {
        ...
        SortColumnAndOrder const target = resolve_sort_action(sort_action);
        auto* header = panel.ui.details_view->horizontalHeader();
        panel.runtime.active_sort_action = sort_action;
        PanelController::SelectionSnapshot const selection_snapshot = panel.capture_selection_snapshot();
        ...
        if (toggle_if_same && header != nullptr && header->sortIndicatorSection() == target.column) {
    ...
    void MainWindow::update_view_menu_checks() {
        ...
        int const view_mode = panel.view_mode;
        ...
        if (active_sort_action < kSortActionName || active_sort_action > kSortActionUnsorted) {
        ...
        if (time_day_action_ != nullptr) {
            time_day_action_->setChecked(display_settings_.timestamp_level
                                         == DirectoryListModel::kTimestampPrintLevelDay);
        }
        if (time_min_action_ != nullptr) {
            time_min_action_->setChecked(display_settings_.timestamp_level
                                         == DirectoryListModel::kTimestampPrintLevelMin);
        }
        if (time_sec_action_ != nullptr) {
            time_sec_action_->setChecked(display_settings_.timestamp_level
                                         == DirectoryListModel::kTimestampPrintLevelSec);
        }
        if (time_ntfs_action_ != nullptr) {
            time_ntfs_action_->setChecked(display_settings_.timestamp_level
                                          == DirectoryListModel::kTimestampPrintLevelNtfs);
        }
        if (time_ns_action_ != nullptr) {
            time_ns_action_->setChecked(display_settings_.timestamp_level
                                        == DirectoryListModel::kTimestampPrintLevelNs);
        }
    ...
    QString MainWindow::format_sample_time_for_level(int timestamp_level) const {
        QDateTime const now =
            display_settings_.timestamp_show_utc ? QDateTime::currentDateTimeUtc() : QDateTime::currentDateTime();
        QString const suffix = display_settings_.timestamp_show_utc ? QStringLiteral("Z") : QString();
        switch (timestamp_level) {
            ...
            case DirectoryListModel::kTimestampPrintLevelNs:
                {
                    int const digits = (timestamp_level == DirectoryListModel::kTimestampPrintLevelNtfs) ? 7 : 9;
                    QString frac = QString::number(now.time().msec()).rightJustified(3, QLatin1Char('0'));
                    ...
                    return now.toString(QStringLiteral("yyyy-MM-dd HH:mm:ss")) + QStringLiteral(".") + frac + suffix;
                }
            default:
                return now.toString(QStringLiteral("yyyy-MM-dd HH:mm:ss")) + suffix;
        }
    ...
    void MainWindow::on_flat_view_action_triggered() {
        ...
        bool const enabled = !panel.model->flat_view();
    ...
    void MainWindow::on_two_panels_action_triggered() {
        ...
        write_fm_panels_state(
            &settings, two_panels_visible_, active_panel_index_, current_fm_splitter_pos(panels_splitter_));
        update_view_menu_checks();
    }

    void MainWindow::on_folders_history_requested() {
        int const panel_index = active_panel_index_;
        QStringList const history = folder_history_;
        if (history.isEmpty()) {
            return;
        }
        ...
        FoldersHistoryDialogResult const result = dialog.result();
    ...
    bool MainWindow::activate_archive_parent_link_for_panel(int panel_index, QModelIndex const& view_index) {
        PanelController& panel = panel_controller(panel_index);
        if (!panel.in_archive_view() || panel.model == nullptr) {
            return false;
        }
        ...
        auto source_index_for_view_index = [&panel](QModelIndex const& index) {
            if (!index.isValid()) {
                return QModelIndex{};
            }
        ...
        QModelIndex const source_index = source_index_for_view_index(view_index);
        ...
        if (selected_rows.size() == 1 && panel.model->is_parent_link_for_row(selected_rows.front().row())) {
            return open_parent();
        }
        return false;
    }
    ...
    void MainWindow::activate_panel_selection(Qt::KeyboardModifiers modifiers) {
        ...
        if (archive_mode && activate_archive_parent_link_for_panel(active_panel_index_)) {
            return;
        }

        bool const shift = (modifiers & Qt::ShiftModifier) != 0;
        bool const alt = (modifiers & Qt::AltModifier) != 0;
        bool const ctrl = (modifiers & Qt::ControlModifier) != 0;
        if (!shift && alt && !ctrl) {
            show_properties_dialog();
            return;
        }
        bool const try_internal = !shift || alt || ctrl;
        if (archive_mode) {
    ...
    bool MainWindow::open_path_externally_untracked(QString const& path) {
        if (path.isEmpty()) {
            return false;
        }
        if (!confirm_external_open_targets_safe(
                QStringList{path}, z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(540)))) {
            return false;
        }
        return external_opener_(path);
    }

    bool MainWindow::confirm_external_open_targets_safe(QStringList const& paths, QString const& caption) {
        for (QString const& path : paths) {
            QString const name = QFileInfo(path).fileName();
            QString const message = external_open_suspicious_name_message(name);
            if (message.isEmpty()) {
                continue;
            }
            QMessageBox::warning(this, caption.trimmed().isEmpty() ? QStringLiteral("7-Zip") : caption, message);
            return false;
        }
        return true;
    }
