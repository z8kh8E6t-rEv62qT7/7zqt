...
namespace z7::ui::filemanager {
    ...
    namespace {
        ...
        bool source_rows_contain_row(QModelIndexList const& rows, int source_row) {
    ...
    MainWindow::ArchiveOpenSelectionTarget MainWindow::resolve_archive_open_selection_target(int panel_index) const {
        ...
        for (QModelIndex const& row : rows) {
            ...
            if (target.entries.size() == 1) {
                ...
                target.single_entry_archive_index = panel.model->archive_index_for_row(row_index);
    ...
    void
    MainWindow::open_item(QString const& path, bool try_internal, bool try_external, QString const& archive_type_hint) {
        ...
        if (try_internal) {
            bool const force_internal = !archive_type_hint.isEmpty();
            bool const prefer_external =
    ...
    void MainWindow::open_focused_item_as_internal(QString const& archive_type_hint) {
        if (in_archive_view()) {
            ...
            QString const entry_path = panel.model->path_for_row(focused.row());
            ...
            load_archive_virtual_directory_for_panel(active_panel_index_,
        ...
        QString const focused_path = focused_path_for_panel(active_panel_index_);
    ...
    void MainWindow::open_selected_archive_entries(bool try_internal) {
        ...
        ArchiveOpenSelectionTarget const selection = resolve_archive_open_selection_target(active_panel_index_);
        QStringList const& entries = selection.entries;
        if (entries.isEmpty()) {
            return;
        }
        if (entries.size() > 20) {
        ...
        bool const prefer_external = should_always_start_externally(selection.single_entry_path);
        if (prefer_external) {
            open_archive_entries_outside_for_panel(active_panel_index_, entries);
            return;
        }
    ...
    void MainWindow::open_selected_filesystem_paths_including_parent_link(bool try_internal) {
        ...
        QModelIndexList const real_rows = panel.selected_real_item_rows();
        ...
        QModelIndex const focused = panel.focused_source_index();
        bool const focused_is_selected = focused.isValid() && source_rows_contain_row(selected_rows, focused.row());
        bool const focused_parent_selected = focused_is_selected && panel.model->is_parent_link_for_row(focused.row());
        if (focused_parent_selected && try_internal) {
            on_open_parent_requested();
            return;
        }
        if (focused_parent_selected && real_rows.isEmpty()) {
            QString const parent_path = panel.model->path_for_row(focused.row());
            if (!parent_path.trimmed().isEmpty()) {
                paths << parent_path;
            }
        }

        for (QModelIndex const& row : real_rows) {
            if (!row.isValid()) {
                continue;
            }
