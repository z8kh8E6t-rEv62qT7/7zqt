...
namespace z7::ui::filemanager {
    ...
    bool MainWindow::can_open_parent_from_current_dir() const {
        ...
        QString const current_dir = panel.current_directory();
    ...
    void MainWindow::remember_path_history(QString const& abs_path) {
        QString const normalized = QDir(abs_path).absolutePath();
    ...
    void MainWindow::sync_path_bar_from_current_dir() {
        ...
        if (in_archive_view()) {
            QString const display = archive_virtual_display_path_for_panel(active_panel_index_);
            QSignalBlocker const blocker(path_combo_);
            QSignalBlocker const edit_blocker(path_combo_->lineEdit());
            path_combo_->setEditText(display);
            return;
        }

        QString const absolute = QDir(current_directory()).absolutePath();
        QString const native = QDir::toNativeSeparators(absolute);
    ...
    void MainWindow::rebuild_path_bar_popup_items() {
        ...
        if (in_archive_view()) {
            PanelController const& panel = active_panel_controller();
            QString const virtual_dir = z7::ui::archive_support::normalize_virtual_dir(panel.current_directory());
            QString const current_data = QString::fromLatin1(kArchivePathBarDataPrefix) + virtual_dir;
            ...
            dirs << QString();
            QString acc;
            QStringList const parts = virtual_dir.split(QLatin1Char('/'), Qt::SkipEmptyParts);
            for (QString const& part : parts) {
                acc = acc.isEmpty() ? part : acc + QLatin1Char('/') + part;
                dirs << acc;
            }

            QIcon const dir_icon = style()->standardIcon(QStyle::SP_DirIcon);
            for (QString const& dir : dirs) {
                QString const display = z7::ui::archive_support::virtual_display_path(display_source, dir);
                QString const data = QString::fromLatin1(kArchivePathBarDataPrefix) + dir;
                path_combo_->addItem(dir_icon, display, data);
            }

            int const current_index = path_combo_->findData(current_data, Qt::UserRole);
            if (current_index >= 0) {
        ...
        QString const current = QDir(current_directory()).absolutePath();
        QString const current_native = QDir::toNativeSeparators(current);
        ...
        auto add_path = [&](QString const& path) {
            QString const absolute = QDir(path).absolutePath();
            if (absolute.isEmpty() || seen.contains(absolute)) {
                return;
            }
            seen.insert(absolute);

            QFileInfo const info(absolute);
            QIcon const icon = info.exists() ? icon_provider.icon(info) : style()->standardIcon(QStyle::SP_DirIcon);
            path_combo_->addItem(icon, QDir::toNativeSeparators(absolute), absolute);
        };

        for (QString const& path : ancestor_paths(current)) {
            add_path(path);
        }
        for (QString const& path : path_history_) {
            add_path(path);
        }

        int const current_index = path_combo_->findData(current, Qt::UserRole);
        if (current_index >= 0) {
            path_combo_->setCurrentIndex(current_index);
        }
    ...
    bool MainWindow::navigate_to_path_from_bar(QString const& path_text) {
        if (in_archive_view()) {
            PanelController& panel = active_panel_controller();
            QString normalized = QDir::fromNativeSeparators(path_text.trimmed());
            QString const prefix = QString::fromLatin1(kArchivePathBarDataPrefix);
            if (normalized.startsWith(prefix)) {
                normalized = normalized.mid(prefix.size());
            } else {
                QString const display_source = panel.archive_display_source();
                QString const source = QDir::fromNativeSeparators(display_source.trimmed());
...
#endif
                if (normalized.compare(source, kPathCase) == 0 || normalized.compare(source_prefix, kPathCase) == 0) {
                    normalized.clear();
            ...
            return load_archive_virtual_directory_for_panel(active_panel_index_,
        ...
        QString const normalized = QDir::fromNativeSeparators(path_text.trimmed());
        if (normalized.isEmpty()) {
            return false;
        }

        QFileInfo const info(normalized);
    ...
    QStringList MainWindow::selected_file_paths() const {
        QStringList files;
        for (QString const& path : active_panel_controller().selected_real_item_paths()) {
    ...
    QString MainWindow::suggested_extract_subdir_for_menu() const {
        ...
        if (paths.size() == 1) {
            return z7::shell_integration::shell_integration_extract_subfolder_name(QFileInfo(paths.front()).fileName())
                 + QDir::separator();
        }
        return QStringLiteral("*") + QDir::separator();
    }
