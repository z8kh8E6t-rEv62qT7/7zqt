...
namespace z7::ui::filemanager {
    namespace {
        ...
        QString normalize_existing_directory_for_add_sources_dialog(QString const& value) {
            QFileInfo const info(QDir::fromNativeSeparators(value.trimmed()));
        ...
        QString archive_virtual_dir_display_for_add_sources_dialog(QString const& value) {
            QString const normalized = z7::ui::archive_support::normalize_virtual_dir(value);
        ...
        public:
            ArchiveAddSourcesDialog(QWidget* parent,
                ...
                QDialog(parent),
                current_directory_(normalize_existing_directory_for_add_sources_dialog(initial_directory)) {
#ifdef Z7_TESTING
        ...
        private:
            void build_ui(QString const& target_virtual_dir_display) {
                auto* root_layout = new QVBoxLayout(this);
                root_layout->setContentsMargins(8, 8, 8, 8);
                root_layout->setSpacing(6);

                auto* prompt_label =
                    new QLabel(z7::ui::runtime_support::J(QStringLiteral("ui.archive.add_sources.prompt")), this);
#ifdef Z7_TESTING
                prompt_label->setObjectName(QStringLiteral("archiveAddSourcesPromptLabel"));
#endif
                ...
                up_button_->setToolTip(z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(735)));
...
#ifdef Z7_TESTING
                browse_button_->setObjectName(QStringLiteral("archiveAddSourcesBrowseButton"));
#endif
                browse_button_->setIcon(style()->standardIcon(QStyle::SP_DirOpenIcon));
                browse_button_->setToolTip(
                    z7::ui::runtime_support::J(QStringLiteral("ui.archive.add_sources.browse_folder")));
                ...
                header->setSectionResizeMode(DirectoryListModel::kNameColumn, QHeaderView::Stretch);
                header->setSectionResizeMode(DirectoryListModel::kSizeColumn, QHeaderView::Interactive);
                header->setSectionResizeMode(DirectoryListModel::kModifiedColumn, QHeaderView::Interactive);
                hide_non_picker_columns();
                view_->sortByColumn(DirectoryListModel::kNameColumn, Qt::AscendingOrder);
                root_layout->addWidget(view_, 1);

                auto* target_label =
                    new QLabel(z7::ui::runtime_support::JF(
                                   QStringLiteral("ui.archive.add_sources.target_folder"),
                                   {archive_virtual_dir_display_for_add_sources_dialog(target_virtual_dir_display)}),
                               this);
#ifdef Z7_TESTING
                target_label->setObjectName(QStringLiteral("archiveAddSourcesTargetLabel"));
#endif
                ...
                cancel_button_ = new QPushButton(z7::ui::runtime_support::L(402), buttons_row);
#ifdef Z7_TESTING
                cancel_button_->setObjectName(QStringLiteral("archiveAddSourcesCancelButton"));
#endif
            ...
            void wire_ui() {
                connect(ok_button_, &QPushButton::clicked, this, [this]() { accept_selected_paths(); });
                connect(cancel_button_, &QPushButton::clicked, this, &QDialog::reject);

                path_combo_->before_show_popup = [this]() {
                    rebuild_path_combo();
                };
                connect(path_combo_, qOverload<int>(&QComboBox::activated), this, [this](int index) {
                    QString const data = path_combo_->itemData(index, Qt::UserRole).toString();
                    QString const next = data.trimmed().isEmpty() ? path_combo_->itemText(index) : data;
                    set_current_directory(next);
                });
                if (path_combo_->lineEdit() != nullptr) {
                    connect(path_combo_->lineEdit(), &QLineEdit::returnPressed, this, [this]() {
                        set_current_directory(path_combo_->currentText());
                    });
                }
                ...
                connect(view_,
                        ...
                        this,
                        [this](QModelIndex const& index) { activate_index(index); });
                connect(view_,
                ...
                if (view_->selectionModel() != nullptr) {
                    ...
                    connect(view_->selectionModel(), &QItemSelectionModel::currentChanged, this, [this]() {
                        update_ok_enabled();
                    });
                }
            }
            ...
            void hide_non_picker_columns() {
                ...
                for (int column = 0; column < DirectoryListModel::kColumnCount; ++column) {
                    bool const visible = column == DirectoryListModel::kNameColumn
            ...
            void rebuild_path_combo() {
                QString const current_absolute = QDir(current_directory_).absolutePath();
                QStringList const items = ancestor_paths(current_absolute);
                QString const current_native = QDir::toNativeSeparators(current_absolute);

                QSignalBlocker const blocker(path_combo_);
                path_combo_->clear();
                for (QString const& item : items) {
                    path_combo_->addItem(QDir::toNativeSeparators(item), item);
                }
                int const current_index = path_combo_->findData(current_absolute, Qt::UserRole);
                if (current_index >= 0) {
            ...
            bool set_current_directory(QString const& raw_path) {
                QString const normalized = normalize_existing_directory_for_add_sources_dialog(raw_path);
                if (normalized.isEmpty()) {
                    rebuild_path_combo();
                    return false;
                }
            ...
            QModelIndex source_index_for_proxy_index(QModelIndex const& proxy_index) const {
            ...
            QModelIndexList selected_source_rows() const {
                if (view_ == nullptr || view_->selectionModel() == nullptr || model_ == nullptr) {
                    return {};
                }

                QModelIndexList out;
                QSet<int> seen_rows;
                auto add_proxy_index = [&](QModelIndex const& proxy_index) {
                    QModelIndex const source = source_index_for_proxy_index(proxy_index);
                ...
                int const primary = view_->primary_column();
                for (QModelIndex const& index : view_->selectionModel()->selectedRows(primary)) {
                    add_proxy_index(index);
                }
                for (QModelIndex const& index : view_->selectionModel()->selectedIndexes()) {
                    add_proxy_index(index);
                }
                return out;
            }
            ...
            QStringList selected_real_filesystem_paths() const {
                QStringList paths;
                QModelIndexList const rows = selected_source_rows();
                paths.reserve(rows.size());
                for (QModelIndex const& row : rows) {
                    if (!row.isValid() || model_->is_parent_link_for_row(row.row())) {
                        continue;
                    }
                    QString const path = model_->path_for_row(row.row());
            ...
            void activate_index(QModelIndex const& proxy_index) {
                QModelIndex const source = source_index_for_proxy_index(proxy_index);
            ...
            void accept_selected_paths() {
                QStringList const selected_paths = selected_real_filesystem_paths();
    ...
    ArchiveAddSourcesDialogResult show_archive_add_sources_dialog(QWidget* parent,
                                                                  ...
                                                                  QString const& initial_directory,
                                                                  QString const& target_virtual_dir_display) {
        ArchiveAddSourcesDialog dialog(parent, title, initial_directory, target_virtual_dir_display);
