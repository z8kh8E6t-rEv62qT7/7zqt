...
namespace z7::ui::filemanager {
    ...
    namespace {
        ...
        constexpr quint32 kToolbarMaskDefault = kToolbarMaskDefaultSentinel
        ...
        ToolbarSettings decode_toolbar_settings(QVariant const& stored_value) {
            bool ok = false;
            quint64 const raw_mask = stored_value.toULongLong(&ok);
            quint32 const mask = ok ? static_cast<quint32>(raw_mask) : kToolbarMaskDefault;
        ...
        quint32 encode_toolbar_settings(ToolbarSettings const& settings) {
            quint32 mask = 0;
    ...
    void MainWindow::apply_model_display_settings_to_panel(int panel_index) {
        ...
        panel.model->set_timestamp_display(display_settings_.timestamp_level, display_settings_.timestamp_show_utc);
    ...
    void MainWindow::retranslate_ui() {
        ...
        crc_menu_->setTitle(z7::ui::runtime_support::J(QStringLiteral("ui.view.crc_menu_title")));
        diff_action_->setText(z7::ui::runtime_support::L(554));
        QString const version_control_suffix = QStringLiteral(" (Unsupported)");
        QString const version_control_tooltip =
            QStringLiteral("7-Zip version-control commands are not implemented in this Qt File Manager.");
        version_edit_action_->setText(QStringLiteral("Ver Edit (&1)") + version_control_suffix);
        version_commit_action_->setText(QStringLiteral("Ver Commit") + version_control_suffix);
        version_revert_action_->setText(QStringLiteral("Ver Revert") + version_control_suffix);
        version_diff_action_->setText(QStringLiteral("Ver Diff (&0)") + version_control_suffix);
        ...
        auto const alternate_streams_platform = z7::ui::runtime_support::PlatformSupport::kWindowsOnly;
        alternate_streams_action_->setText(z7::ui::runtime_support::with_platform_suffix_if_unsupported(
            z7::ui::runtime_support::L(559), alternate_streams_platform));
        alternate_streams_action_->setProperty(kActionCapabilityKeyProperty,
        ...
        time_utc_action_->setText(z7::ui::runtime_support::J(QStringLiteral("ui.view.time_utc")));
        ...
        benchmark2_action_->setText(QStringLiteral("%1 2").arg(lang_or(901)));
    ...
    void MainWindow::load_runtime_settings() {
        ...
        ToolbarSettings const toolbar_settings =
            decode_toolbar_settings(settings.value(QString::fromLatin1(kSettingsFmToolbars), kToolbarMaskDefault));
        bool const auto_refresh = settings.value(QString::fromLatin1(kSettingsFmAutoRefresh), true).toBool();
    ...
    void MainWindow::apply_runtime_settings() {
        bool const show_archive_toolbar = archive_toolbar_action_ != nullptr && archive_toolbar_action_->isChecked();
        bool const show_standard_toolbar = standard_toolbar_action_ != nullptr && standard_toolbar_action_->isChecked();
        bool const large_buttons = large_buttons_action_ != nullptr && large_buttons_action_->isChecked();
        bool const show_text = show_buttons_text_action_ == nullptr || show_buttons_text_action_->isChecked();

        archive_toolbar_->setVisible(show_archive_toolbar);
        standard_toolbar_->setVisible(show_standard_toolbar);
        int const toolbar_icon_size = z7::platform::qt::toolbar_icon_extent(large_buttons, this);
        QSize const icon_size(toolbar_icon_size, toolbar_icon_size);
        archive_toolbar_->setIconSize(icon_size);
        standard_toolbar_->setIconSize(icon_size);
        Qt::ToolButtonStyle const button_style = show_text ? Qt::ToolButtonTextUnderIcon : Qt::ToolButtonIconOnly;
        archive_toolbar_->setToolButtonStyle(button_style);
        standard_toolbar_->setToolButtonStyle(button_style);
        apply_toolbar_action_texts();

        QAbstractItemView::SelectionMode const selection_mode = display_settings_.alternative_selection_mode
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            if (panel.model != nullptr) {
                should_reload_archive_view =
                    panel.model->show_dots() != display_settings_.show_dots && in_archive_view_for_panel(i);
                apply_model_display_settings_to_panel(i);
            }
            if (panel.ui.details_view != nullptr) {
                panel.ui.details_view->set_alternative_selection_mode(display_settings_.alternative_selection_mode);
                ...
                QVector<int> const current_widths = column_width_persistence::capture_widths(
                    panel.ui.details_view->horizontalHeader(), DirectoryListModel::kColumnCount);
                auto config = panel.ui.details_view->config();
                config.style.row_hover_bg = display_settings_.full_row_select ? QColor(0, 0, 0, 14) : QColor();
                config.style.grid_line = display_settings_.show_grid_lines ? QColor(0, 0, 0, 22) : QColor();
                panel.ui.details_view->set_config(config);
                if (current_widths.size() == DirectoryListModel::kColumnCount) {
                    column_width_persistence::apply_widths(panel.ui.details_view->horizontalHeader(), current_widths);
                }
                panel.ui.details_view->viewport()->update();
            }
            if (panel.ui.icon_list_view != nullptr) {
                if (auto* icon_view = dynamic_cast<DragAwareListView*>(panel.ui.icon_list_view)) {
        ...
        if (panels_splitter_ != nullptr) {
            if (two_panels_visible_) {
                QList<int> const current_sizes = panels_splitter_->sizes();
                bool const has_two_visible_sizes =
                    current_sizes.size() >= 2 && current_sizes.at(0) > 0 && current_sizes.at(1) > 0;
        ...
        settings.setValue(QString::fromLatin1(kSettingsFmToolbars), encode_toolbar_settings(toolbar_settings));
        settings.setValue(QString::fromLatin1(kSettingsFmAutoRefresh), auto_refresh_action_->isChecked());
        save_display_settings(settings, display_settings_);

        refresh_action_states();
        update_status();
        reload_archive_virtual_directories_serially(std::move(panels_to_reload_archive_view));
    }
