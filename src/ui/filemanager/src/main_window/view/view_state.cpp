...
namespace z7::ui::filemanager {
    ...
    namespace {
        ...
        quint64 model_row_size(DirectoryListModel const& model, int row) {
            QVariant const size_key = model.data(model.index(row, DirectoryListModel::kSizeColumn),
                                                 z7::ui::widgets::StructuredListSortFilterProxy::kSortKeyRole);
            bool ok = false;
            quint64 const size = size_key.toULongLong(&ok);
            return ok ? size : 0;
        }

        int non_parent_row_count(DirectoryListModel const& model) {
            int count = 0;
    ...
    void MainWindow::show_context_menu(QPoint const& pos) {
        auto* source_view = qobject_cast<QAbstractItemView*>(sender());
        if (source_view != nullptr) {
            int const panel_index = panel_index_for_view(source_view);
            if (panel_index >= 0) {
        ...
        bool const shift_pressed = (QApplication::keyboardModifiers() & Qt::ShiftModifier) != 0;
        SevenZipMenuState const seven_zip_state = compute_seven_zip_menu_state(shift_pressed);
    ...
    void MainWindow::populate_context_menu(QMenu* menu, SevenZipMenuState const& seven_zip_state) {
        if (menu == nullptr) {
            return;
        }
        ...
        QMenu* crc_sub = menu->addMenu(z7::ui::runtime_support::J(QStringLiteral("ui.view.crc_menu_title")));
        populate_crc_hash_menu(crc_sub, has_oper_smart_real_items(), [this](QString const& method) {
            on_hash_with_method_requested(method);
        });
    ...
    void MainWindow::update_status_for_panel(int panel_index) {
        ...
        if (panel.model == nullptr
            || panel.ui.status_selected_count == nullptr
        ...
        bool const has_selected_list_row = !selected_rows.isEmpty();
        int selected_count = 0;
        quint64 selected_size = 0;
        for (QModelIndex const& row : selected_rows) {
        ...
        panel.ui.status_selected_count->setText(z7::ui::runtime_support::LF(
            3002,
            {QStringLiteral("%1 / %2").arg(QString::number(selected_count),
                                           QString::number(non_parent_row_count(*panel.model)))}));
        panel.ui.status_selected_size->setText(selected_count > 0 ? format_original_status_size(selected_size)
                                                                  : QString());

        QString focused_size;
        QString focused_modified;
        if (has_selected_list_row) {
            QAbstractItemView const* view = panel.current_item_view();
            QModelIndex focused_proxy = view != nullptr ? view->currentIndex() : QModelIndex();
            if (!focused_proxy.isValid() && panel.selection_model() != nullptr) {
                focused_proxy = panel.selection_model()->currentIndex();
            }
            QModelIndex const focused_source = panel.map_proxy_to_source(focused_proxy);
            if (focused_source.isValid() && !panel.model->is_parent_link_for_row(focused_source.row())) {
                focused_modified =
                    panel.model
                        ->data(panel.model->index(focused_source.row(), DirectoryListModel::kModifiedColumn),
                               Qt::DisplayRole)
                        .toString();
                focused_size = format_original_status_size(model_row_size(*panel.model, focused_source.row()));
            }
        }
    ...
    void MainWindow::show_transient_status_message(QString const& message, int timeout_ms, int panel_index) {
        ...
        quint64 const generation = transient_status_generation_;
    ...
    bool MainWindow::refresh_directory_for_panel(int panel_index) {
        ...
        PanelController::SelectionSnapshot const snapshot = panel.capture_selection_snapshot();
    ...
    void MainWindow::ensure_auto_refresh_watcher_for_panel(int panel_index) {
        ...
        connect(panel.runtime.auto_refresh_watcher,
                ...
                this,
                [this, panel_index](QString const&) { mark_panel_auto_refresh_dirty(panel_index); });
        connect(panel.runtime.auto_refresh_watcher,
    ...
    void MainWindow::rebind_auto_refresh_watcher_for_panel(int panel_index) {
        ...
        bool const auto_refresh_enabled = auto_refresh_action_ != nullptr && auto_refresh_action_->isChecked();
        if (!auto_refresh_enabled || panel.model == nullptr || panel.in_archive_view()) {
            return;
        }

        QString const watched_dir = QFileInfo(panel.model->directory()).absoluteFilePath();
        QFileInfo const watched_info(watched_dir);
    ...
    void MainWindow::on_auto_refresh_timer_tick() {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            QString const current_dir = QFileInfo(panel.model->directory()).absoluteFilePath();
            bool const was_dirty = panel.runtime.auto_refresh_dirty;
    ...
    void MainWindow::set_current_directory_for_panel(int panel_index, QString const& path) {
        PanelController& panel = panel_controller(panel_index);
        if (panel.model == nullptr) {
            return;
        }
        ...
        QString const abs_path = info.absoluteFilePath();
    ...
    void MainWindow::set_current_directory(QString const& path) {
    ...
    QString MainWindow::focused_comment_for_panel(int panel_index) const {
        ...
        for (int row = 0; row < row_count; ++row) {
            if (panel.model->path_for_row(row) == focused_path) {
                return panel.model->data(panel.model->index(row, DirectoryListModel::kCommentColumn), Qt::DisplayRole)
                    .toString();
            }
        }
        return {};
    ...
    QString MainWindow::selected_archive_path() const {
        QStringList const paths = selected_filesystem_paths_including_parent_link();
        if (paths.size() != 1) {
            return QString();
        }
