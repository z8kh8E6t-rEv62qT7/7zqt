...
namespace z7::ui::filemanager {
    namespace {
        ...
        QString archive_icon_name_for_suffix(QString const& suffix) {
            QString const normalized = suffix.trimmed().toLower();
            ...
            if (z7::common::is_gzip_family_archive_suffix(normalized.toStdString())) {
                return QString::fromStdString(z7::common::preferred_archive_output_suffix_copy("gzip"));
            }
            if (z7::common::is_bzip2_family_archive_suffix(normalized.toStdString())) {
                return QString::fromStdString(z7::common::preferred_archive_output_suffix_copy("bzip2"));
            }
            if (normalized == QStringLiteral("xz") || normalized == QStringLiteral("txz")) {
                return QStringLiteral("xz");
            }
            if (normalized == QStringLiteral("zst") || normalized == QStringLiteral("tzst")) {
            ...
            if (normalized == QStringLiteral("z") || normalized == QStringLiteral("taz")) {
                return QStringLiteral("z");
            }
            if (normalized == QStringLiteral("lzh") || normalized == QStringLiteral("lha")) {
            ...
            if (normalized == QStringLiteral("xar") || normalized == QStringLiteral("pkg")) {
                return QStringLiteral("xar");
            }
            if (normalized == QStringLiteral("vhd") || normalized == QStringLiteral("vhdx")) {
    ...
    void DirectoryListModel::set_directory(QString const& directory) {
    ...
    void DirectoryListModel::set_virtual_entries(QString const& virtual_dir, QVector<VirtualEntry> const& entries) {
        beginResetModel();
        ...
        for (VirtualEntry const& virtual_entry : entries) {
            entries_.push_back(make_virtual_entry(virtual_entry));
        }
        endResetModel();
    }

    void DirectoryListModel::set_archive_drag_source(QString const& archive_path,
    ...
    void DirectoryListModel::set_archive_drag_materializer(ArchiveDragMaterializer materializer) {
    ...
    void DirectoryListModel::set_rename_enabled(bool enabled) {
        ...
        if (!entries_.isEmpty()) {
            emit dataChanged(index(0, kNameColumn), index(entries_.size() - 1, kNameColumn), {Qt::EditRole});
        }
    }
    ...
    bool DirectoryListModel::set_filesystem_directory_stats(QString const& path,
                                                            uint64_t size,
        ...
        QString const normalized = QFileInfo(path).absoluteFilePath();
        ...
        for (int row = 0; row < entries_.size(); ++row) {
            ...
            if (entry.is_parent_link || !entry.is_dir || QFileInfo(entry.path).absoluteFilePath() != normalized) {
                continue;
            }
            ...
            emit dataChanged(index(row, kSizeColumn),
                             ...
                             {Qt::DisplayRole, z7::ui::widgets::StructuredListSortFilterProxy::kSortKeyRole});
            return true;
        }
        return false;
    }
    ...
    void DirectoryListModel::set_show_real_file_icons(bool enabled) {
        ...
        if (!entries_.isEmpty()) {
            emit dataChanged(index(0, kNameColumn), index(entries_.size() - 1, kNameColumn), {Qt::DecorationRole});
        }
    }
    ...
    void DirectoryListModel::set_timestamp_display(int timestamp_level, bool show_utc) {
        int const normalized_level = normalize_timestamp_level(timestamp_level);
        ...
        if (!entries_.isEmpty()) {
            emit dataChanged(index(0, kModifiedColumn), index(entries_.size() - 1, kAccessedColumn));
        }
    }
    ...
    void DirectoryListModel::reload() {
        ...
        QDir dir(directory_);
        std::map<std::string, std::string> const comments_by_key = load_filesystem_comments_for_directory(dir);
        if (flat_view_) {
            ...
            QFileInfoList const list =
                dir.entryInfoList(QDir::NoDotAndDotDot | QDir::AllEntries | QDir::Hidden | QDir::System, QDir::NoSort);

            entries_.reserve(list.size());
            for (QFileInfo const& info : list) {
                Entry entry = make_filesystem_entry(info, info.fileName());
                if (std::optional<QString> const comment =
                        filesystem_comment_for_entry(comments_by_key, dir, info, info.fileName());
                    comment.has_value()) {
        ...
        if (flat_view_) {
            for (Entry& entry : entries_) {
                if (std::optional<QString> const comment =
                        filesystem_comment_for_entry(comments_by_key, dir, entry.info, entry.display_name);
                    comment.has_value()) {
                    entry.comment = *comment;
                }
            }
    ...
    bool DirectoryListModel::icon_has_renderable_pixels(QIcon const& icon) {
        if (icon.isNull()) {
            return false;
        }
    ...
    bool DirectoryListModel::is_numeric_split_extension(QString const& extension) {
        if (extension.isEmpty()) {
            return false;
        }
        for (QChar const ch : extension) {
            if (!ch.isDigit()) {
                return false;
            }
        }
    ...
    QHash<QString, QString> const& DirectoryListModel::archive_icon_name_by_extension() {
        static QHash<QString, QString> const kIconNames = [] {
            QHash<QString, QString> out;
            out.insert(QStringLiteral("split"), QStringLiteral("split"));
            for (QString const& suffix : z7::ui::common::ordered_archive_suffixes()) {
                QString const icon_name = archive_icon_name_for_suffix(suffix);
    ...
    QString DirectoryListModel::extension_for_icon_lookup(QString const& display_name) {
        if (display_name.isEmpty()) {
            return QString();
        }

        QString file_name = display_name;
        int const separator_pos =
            std::max(file_name.lastIndexOf(QLatin1Char('/')), file_name.lastIndexOf(QLatin1Char('\\')));
        if (separator_pos >= 0 && separator_pos + 1 < file_name.size()) {
            file_name = file_name.mid(separator_pos + 1);
        }

        int const dot_pos = file_name.lastIndexOf(QLatin1Char('.'));
