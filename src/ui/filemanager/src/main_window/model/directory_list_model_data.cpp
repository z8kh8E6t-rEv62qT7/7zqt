...
namespace z7::ui::filemanager {
    ...
    namespace {
        ...
        public:
            ArchiveLazyMimeData(QString const& source_archive,
                ...
                QByteArray const marker = make_internal_archive_source_marker(
                    source_archive_, archive_type_hint_, entries_, session_token_value_);
                if (!marker.isEmpty()) {
                    setData(QString::fromLatin1(kMimeTypeZ7FmArchiveSource), marker);
                }
                if (z7::platform::qt::mac_archive_native_drag_enabled()) {
                    QByteArray const promise_payload = z7::platform::qt::encode_mac_archive_promise_payload(
                        source_archive_, archive_type_hint_, entries_);
                    if (!promise_payload.isEmpty()) {
                        setData(z7::platform::qt::mac_archive_promise_mime_type(), promise_payload);
                    }
                }
            }
        ...
        protected:
            QStringList formats() const override {
                ...
                if (z7::platform::qt::mac_archive_native_drag_enabled()) {
                    QString const promise_format = z7::platform::qt::mac_archive_promise_mime_type();
                    if (!promise_format.isEmpty() && !out.contains(promise_format)) {
                        out.push_back(promise_format);
                    }
                }
                QString const uri_format = QStringLiteral("text/uri-list");
            ...
            bool hasFormat(QString const& mime_type) const override {
            ...
            QVariant retrieveData(QString const& mime_type, QMetaType type) const override {
                if (mime_type == z7::platform::qt::mac_archive_promise_mime_type()) {
        ...
        private:
            void ensure_urls_materialized() const {
                ...
                bool finished = false;
                materializer_(
                    entries_, [this, &wait_loop, &finished](QStringList const& paths, QString const& error_message) {
                        QList<QUrl> urls;
                        ...
                        for (QString const& path : paths) {
                            ...
                            urls.push_back(QUrl::fromLocalFile(QFileInfo(trimmed_path).absoluteFilePath()));
                        }
                        const_cast<ArchiveLazyMimeData*>(this)->setUrls(urls);
                        QString const trimmed_error = error_message.trimmed();
                        if (!trimmed_error.isEmpty()) {
                            const_cast<ArchiveLazyMimeData*>(this)->setData(
                                QString::fromLatin1(kMimeTypeZ7FmArchiveMaterializationError), trimmed_error.toUtf8());
                        }
                        finished = true;
            ...
            void mark_transfer_requested() const {
                const_cast<ArchiveLazyMimeData*>(this)->setData(
                    QString::fromLatin1(kMimeTypeZ7FmArchiveTransferRequested), QByteArrayLiteral("1"));
            }
    ...
    DirectoryListModel::DirectoryListModel(QObject* parent) : QAbstractTableModel(parent) {}

    void DirectoryListModel::set_icon_style_context(QWidget* context) {
        icon_style_context_ = context;
    }

    int DirectoryListModel::rowCount(QModelIndex const& parent) const {
        if (parent.isValid()) {
            return 0;
        }
    ...
    int DirectoryListModel::columnCount(QModelIndex const& parent) const {
        if (parent.isValid()) {
            return 0;
        }
        return kColumnCount;
    ...
    Qt::ItemFlags DirectoryListModel::flags(QModelIndex const& index) const {
        Qt::ItemFlags item_flags = QAbstractTableModel::flags(index);
        if (!index.isValid() || index.row() < 0 || index.row() >= entries_.size()) {
            return item_flags;
        }

        Entry const& entry = entries_[index.row()];
        bool const can_drag_fs = mode_ == DataMode::kFilesystem;
        bool const can_drag_archive = mode_ == DataMode::kVirtualArchive
                                   && !archive_drag_source_archive_.isEmpty()
                                   && static_cast<bool>(archive_drag_materializer_);
        if ((can_drag_fs || can_drag_archive) && !entry.is_parent_link && !entry.path.trimmed().isEmpty()) {
            item_flags |= Qt::ItemIsDragEnabled;
        }
        if (rename_enabled_
            && index.column() == kNameColumn
            && !entry.is_parent_link
    ...
    QMimeData* DirectoryListModel::mimeData(QModelIndexList const& indexes) const {
        if (indexes.isEmpty()) {
        ...
        for (QModelIndex const& index : indexes) {
            ...
            QFileInfo const info(entry.path);
            if (!info.exists()) {
                continue;
            }
        ...
        if (!urls.isEmpty()) {
            mime_data->setData(QString::fromLatin1(kMimeTypeZ7FmFsSource), make_internal_fs_source_marker());
        }
        return mime_data;
    }
    ...
    QVariant DirectoryListModel::data(QModelIndex const& index, int role) const {
        ...
        if (role == Qt::TextAlignmentRole) {
            switch (index.column()) {
                ...
                case kEncryptedColumn:
                    return QVariant::fromValue(static_cast<int>(Qt::AlignCenter));
                default:
                    break;
            }
        }
                ...
                case kModifiedColumn:
                    {
                        if (entry.mtime_msecs_utc.has_value()) {
                            return QDateTime::fromMSecsSinceEpoch(*entry.mtime_msecs_utc, QTimeZone::UTC);
                        }
                        QDateTime const fs = entry.info.lastModified();
                        return fs.isValid() ? fs : QDateTime();
                    }
                case kCreatedColumn:
                    {
                        if (entry.ctime_msecs_utc.has_value()) {
                            return QDateTime::fromMSecsSinceEpoch(*entry.ctime_msecs_utc, QTimeZone::UTC);
                        }
                        if (!entry.is_virtual) {
                            QDateTime const fs =
                                entry.info.birthTime().isValid() ? entry.info.birthTime() : entry.info.lastModified();
                            return fs;
                        }
                        return QDateTime();
                    }
                case kAccessedColumn:
                    {
                        if (entry.atime_msecs_utc.has_value()) {
                            return QDateTime::fromMSecsSinceEpoch(*entry.atime_msecs_utc, QTimeZone::UTC);
                        }
                        QDateTime const fs = entry.info.lastRead();
                        return fs.isValid() ? fs : QDateTime();
                    }
                case kCrcColumn:
        ...
        if (role != Qt::DisplayRole && role != z7::ui::widgets::StructuredListSortFilterProxy::kSortKeyRole) {
            return QVariant();
        }
        ...
        switch (index.column()) {
            ...
            case kModifiedColumn:
                return format_timestamp_for_entry(entry.mtime_msecs_utc, entry.info.lastModified());
            case kCreatedColumn:
                {
                    QDateTime created;
                    if (!entry.is_virtual) {
                        created = entry.info.birthTime().isValid() ? entry.info.birthTime() : entry.info.lastModified();
                    }
                    return format_timestamp_for_entry(entry.ctime_msecs_utc, created);
                }
            case kAccessedColumn:
                return format_timestamp_for_entry(entry.atime_msecs_utc, entry.info.lastRead());
            ...
            case kCrcColumn:
                ...
                return QStringLiteral("%1").arg(static_cast<qulonglong>(*entry.crc), 8, 16, QLatin1Char('0')).toUpper();
            case kMethodColumn:
                return entry.method;
    ...
    bool DirectoryListModel::setData(QModelIndex const& index, QVariant const& value, int role) {
        if (role != Qt::EditRole
            || !index.isValid()
            || index.column() != kNameColumn
            || index.row() < 0
