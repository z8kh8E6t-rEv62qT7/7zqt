...
namespace z7::ui::filemanager {
    ...
    QIcon DirectoryListModel::mapped_archive_icon(QString const& extension) const {
        ...
        auto const cached = archive_icon_cache_.constFind(key);
        if (cached != archive_icon_cache_.cend()) {
            return *cached;
        }

        auto const& icon_names = archive_icon_name_by_extension();
        auto const icon_name_it = icon_names.constFind(key);
        if (icon_name_it == icon_names.cend()) {
            archive_icon_cache_.insert(key, QIcon());
            return QIcon();
        }

        QIcon const icon(QStringLiteral(":/z7/archive-icons/%1.ico").arg(*icon_name_it));
        archive_icon_cache_.insert(key, icon);
        return icon;
    }

    QIcon DirectoryListModel::standard_style_icon(bool is_dir) const {
        QStyle::StandardPixmap const pixmap = is_dir ? QStyle::SP_DirIcon : QStyle::SP_FileIcon;
        if (icon_style_context_ != nullptr && icon_style_context_->style() != nullptr) {
            return icon_style_context_->style()->standardIcon(pixmap, nullptr, icon_style_context_);
        }
    ...
    QIcon DirectoryListModel::resolve_entry_icon(Entry const& entry) const {
        if (show_real_file_icons_ && !entry.is_virtual) {
            QIcon const real_icon = icon_provider_.icon(entry.info);
        ...
        if (!entry.is_dir) {
            ...
            QIcon const mapped_icon = mapped_archive_icon(icon_extension);
            if (icon_has_renderable_pixels(mapped_icon)) {
                return mapped_icon;
            }
        }

        QIcon const generic_icon =
            icon_provider_.icon(entry.is_dir ? QFileIconProvider::Folder : QFileIconProvider::File);
        if (icon_has_renderable_pixels(generic_icon)) {
            return generic_icon;
        }

        QIcon const style_icon = standard_style_icon(entry.is_dir);
    ...
    DirectoryListModel::Entry DirectoryListModel::make_filesystem_entry(QFileInfo const& info,
    ...
    DirectoryListModel::load_filesystem_comments_for_directory(QDir const& root) const {
        z7::app::DescriptIonDocument document;
        std::string error_message;
        if (!z7::app::load_descript_ion_document(
                z7::ui::archive_support::to_native_string(root.absolutePath()), &document, &error_message)) {
            return {};
        }

        std::map<std::string, std::string> out;
        for (auto const& [id, value] : document.entries) {
            std::optional<std::string> const display_value =
    ...
    DirectoryListModel::filesystem_comment_for_entry(std::map<std::string, std::string> const& comments_by_key,
                                                     QDir const&,
        ...
        auto const it = comments_by_key.find(z7::ui::archive_support::to_native_string(key));
    ...
    DirectoryListModel::Entry DirectoryListModel::make_parent_entry(QDir const& dir) {
        Entry entry;
        QDir parent_dir(dir);
        if (!parent_dir.cdUp()) {
            parent_dir = dir;
    ...
    DirectoryListModel::Entry DirectoryListModel::make_virtual_entry(VirtualEntry const& virtual_entry) {
        Entry entry;
    ...
    void DirectoryListModel::append_entries_recursive(QDir const& root, QDir const& dir) {
    ...
    QString DirectoryListModel::format_timestamp_for_entry(std::optional<int64_t> const& msecs_utc,
                                                           QDateTime const& source_time) const {
    ...
    QString DirectoryListModel::format_timestamp(QDateTime const& input) const {
        if (!input.isValid()) {
            return QString();
        }
        ...
        QString const suffix = timestamp_show_utc_ ? QStringLiteral("Z") : QString();
        switch (timestamp_level_) {
            ...
            case kTimestampPrintLevelNs:
                {
                    int const digits = (timestamp_level_ == kTimestampPrintLevelNtfs) ? 7 : 9;
                    QString frac = QString::number(value.time().msec()).rightJustified(3, QLatin1Char('0'));
                    ...
                    return value.toString(QStringLiteral("yyyy-MM-dd HH:mm:ss")) + QStringLiteral(".") + frac + suffix;
                }
            default:
                return value.toString(QStringLiteral("yyyy-MM-dd HH:mm:ss")) + suffix;
        }
