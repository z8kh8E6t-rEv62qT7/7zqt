...
namespace z7::ui::filemanager {
    ...
    namespace {
        ...
        FilesystemDirectoryStats collect_filesystem_directory_stats(QString const& root_path) {
    ...
    bool MainWindow::run_external_command_with_targets(QString const& command_line,
        ...
        ExternalCommandParts const command = parse_external_command_line(command_line);
    ...
    void MainWindow::run_archive_view_or_edit_with_command(QString const& command_line, QString const& caption) {
        ...
        QString const focused_entry = z7::ui::archive_support::normalize_virtual_dir(panel.focused_path());
        if (focused_entry.isEmpty()) {
            return;
        }
        QStringList const entries{focused_entry};
        ...
        start_archive_source_extract_task(
            ...
            [this,
             ...
             caption](bool ok, int, int, QString const&, z7::app::OperationOutcome const&) {
                ...
                QStringList const targets = extracted_archive_entry_paths(temp_dir->path(), entries);
                QVector<ArchiveTempFileSnapshot> const snapshots =
    ...
    void MainWindow::run_view_or_edit_requested(bool use_editor_command) {
        QString const caption = z7::ui::runtime_support::strip_mnemonic(
            use_editor_command ? z7::ui::runtime_support::L(544) : z7::ui::runtime_support::L(543));
        QString const missing_command_message =
            use_editor_command
                ? QStringLiteral("No editor command is configured. Configure Editor in Options before using Edit.")
                : QStringLiteral("No viewer command is configured. Configure Viewer in Options before using View.");

        if (!use_editor_command && !in_archive_view()) {
            PanelController& panel = active_panel_controller();
            if (panel.model != nullptr) {
                bool calculated = false;
                for (QModelIndex const& row : panel.selected_real_item_rows()) {
                    if (!row.isValid() || !panel.model->is_dir_for_row(row.row())) {
                        continue;
                    }
                    QString const path = panel.model->path_for_row(row.row());
                    QFileInfo const info(path);
                    if (!info.exists() || !info.isDir()) {
                        continue;
                    }
                    FilesystemDirectoryStats const stats = collect_filesystem_directory_stats(info.absoluteFilePath());
                    calculated = panel.model->set_filesystem_directory_stats(
                                     info.absoluteFilePath(), stats.size, stats.folders, stats.files)
                              || calculated;
                }
                if (calculated) {
                    update_status();
        ...
        QString const command_line =
            settings.value(QString::fromLatin1(use_editor_command ? kSettingsFmEditor : kSettingsFmViewer), QString())
                .toString()
                .trimmed();
        ...
        QString const focused_path = panel.focused_path();
        if (focused_path.trimmed().isEmpty()) {
            return;
        }

        QFileInfo const info(focused_path);
        if (!info.exists() || !info.isFile()) {
            return;
        }
        QStringList const targets{info.absoluteFilePath()};

        QString error_message;
        if (!run_external_command_with_targets(command_line, targets, current_directory(), &error_message)) {
            if (!error_message.trimmed().isEmpty()) {
                QMessageBox::warning(this, caption, error_message);
            }
        }
    ...
    bool MainWindow::resolve_diff_targets(QString* path1, QString* path2) const {
        ...
        QStringList const active_selected =
            selected_filesystem_paths_including_parent_link_for_panel(active_panel_index_);
        ...
        if (active_selected.size() != 1 || !two_panels_visible_ || in_archive_view_for_panel(1 - active_panel_index_)) {
            return false;
        }

        *path1 = QFileInfo(active_selected.front()).absoluteFilePath();
        QStringList const opposite_selected =
            selected_filesystem_paths_including_parent_link_for_panel(1 - active_panel_index_);
        if (opposite_selected.size() == 1) {
            *path2 = QFileInfo(opposite_selected.front()).absoluteFilePath();
            return !path1->isEmpty() && !path2->isEmpty();
        }

        QString relative_path = QDir(current_directory_for_panel(active_panel_index_)).relativeFilePath(*path1);
        DirectoryListModel const* active_model = panel_controller(active_panel_index_).model;
        DirectoryListModel const* opposite_model = panel_controller(1 - active_panel_index_).model;
        ...
        *path2 = QFileInfo(QDir(current_directory_for_panel(1 - active_panel_index_)).filePath(relative_path))
                     .absoluteFilePath();
        return !path1->isEmpty() && !path2->isEmpty();
    }
    ...
    void MainWindow::on_diff_requested() {
        ...
        if (!run_external_command_with_targets(diff_command,
            ...
            if (!error_message.trimmed().isEmpty()) {
                QMessageBox::warning(
                    this, z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(554)), error_message);
            }
        }
    }

    void MainWindow::on_compress_requested() {
        QString const caption = z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(7200));
        ...
        if (in_archive_view()) {
            ...
            ArchiveWritebackPlan const plan = build_archive_writeback_plan_for_panel(active_panel_index_);
            ...
            ArchiveAddSourcesDialogResult const dialog_result =
                show_archive_add_sources_dialog(this, caption, initial_dir, plan.current_virtual_dir());
            if (!dialog_result.accepted || dialog_result.selected_paths.isEmpty()) {
                return;
            }

            if (!start_add_external_files_to_archive_preview(
                    active_panel_index_, dialog_result.selected_paths, plan.current_virtual_dir(), caption)) {
        ...
        QStringList const inputs = active_panel_controller().selected_real_item_paths();
        if (inputs.isEmpty()) {
            QMessageBox::information(this, caption, z7::ui::runtime_support::L(3015));
            return;
        }
