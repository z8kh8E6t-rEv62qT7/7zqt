...
namespace z7::ui::filemanager {
    namespace {
...
#ifdef Q_OS_WIN
        constexpr char const* k7zGProgramName = "7zG.exe";
#else
        constexpr char const* k7zGProgramName = "7zG";
#endif

        QStringList unique_non_empty_paths(QStringList const& paths) {
            QStringList out;
            QSet<QString> seen;
            for (QString const& path : paths) {
                ...
                QString const dedup_key = normalized.toLower();
        ...
        QStringList unique_non_empty_archive_entries(QStringList const& paths) {
            QStringList out;
            QSet<QString> seen;
            for (QString const& path : paths) {
                QString const normalized = z7::ui::archive_support::normalize_virtual_dir(path);
        ...
        bool command_supports_archive_scoped_refresh(z7::task_ipc_runtime::TaskIpcCommandKind command) {
            switch (command) {
                case z7::task_ipc_runtime::TaskIpcCommandKind::kExtract:
                case z7::task_ipc_runtime::TaskIpcCommandKind::kTest:
                case z7::task_ipc_runtime::TaskIpcCommandKind::kHash:
    ...
    QString MainWindow::locate_7zg_program() const {
        QString const self_program = QCoreApplication::applicationFilePath().trimmed();
        if (self_program.isEmpty()) {
            return QString();
        }
        QFileInfo const self_info(self_program);
        if (!self_info.exists()) {
            return QString();
        }
        QString const worker_path = self_info.dir().filePath(QString::fromLatin1(k7zGProgramName));
        QFileInfo const worker_info(worker_path);
    ...
    void MainWindow::ensure_task_ipc_completion_notifier() {
        if (task_ipc_completion_notifier_registered_ || task_ipc_owner_instance_id_.trimmed().isEmpty()) {
            return;
        }
    ...
    void MainWindow::poll_task_ipc_completions() {
        ...
        if (!z7::task_ipc_runtime::collect_task_ipc_events(task_ipc_owner_instance_id_, &events, &error_message)) {
            return;
        }
        if (events.isEmpty()) {
            return;
        ...
        for (z7::task_ipc_runtime::TaskIpcEvent const& task_event : events) {
            z7::task_ipc_runtime::acknowledge_task_ipc_event(task_event, nullptr);
            if (task_event.event_kind != z7::task_ipc_runtime::TaskIpcEventKind::kCompleted) {
                continue;
            }
            ...
            bool const archive_refresh_handled = payload_represents_archive_scoped_refresh(task_event.payload)
                                              && refresh_archive_panels_for_task_ipc_payload(task_event.payload);
            if (!archive_refresh_handled && task_event.refresh_after_finish) {
                should_refresh = true;
            }
            QString const event_summary = task_event.summary.trimmed();
            if (task_event.result_code != 0 || !event_summary.isEmpty()) {
                bool const canceled = task_event.result_code == 5;
                ...
                show_transient_status_message(
                    summary, task_event.result_code == 0 ? 5000 : 8000, completion_panel_index);
            }
        }
        if (should_refresh) {
            refresh_directory();
    ...
    bool MainWindow::archive_refresh_target_matches_panel(z7::task_ipc_runtime::TaskIpcPayload const& payload,
        ...
        return QFileInfo(plan.source_archive).absoluteFilePath()
                == QFileInfo(payload.open.has_value() ? payload.open->archive_path : QString()).absoluteFilePath()
    ...
    bool MainWindow::launch_gui_subprocess_task(QString const& caption,
                                                z7::task_ipc_runtime::TaskIpcPayload const& source_payload) {
        int const origin_panel_index = active_panel_index_;
        ...
        if (payload.extract.has_value() && !payload.open.has_value()) {
            payload.extract->archive_inputs = unique_non_empty_paths(payload.extract->archive_inputs);
        }
        if (payload.test.has_value() && !payload.open.has_value()) {
            payload.test->archive_inputs = unique_non_empty_paths(payload.test->archive_inputs);
        }
        ...
        bool const archive_aware_hash_test_or_extract =
        ...
        if (worker_program.isEmpty()) {
            QMessageBox::warning(
                this,
                caption,
                z7::ui::runtime_support::J(QStringLiteral("ui.navigation.task_ipc.worker_program_missing")));
            return false;
        }
        ...
        if (payload.command == z7::task_ipc_runtime::TaskIpcCommandKind::kExtract
            ...
            QMessageBox::warning(
                this, caption, z7::ui::runtime_support::J(QStringLiteral("ui.navigation.task_ipc.no_archive_path")));
            return false;
        }
        ...
        if ((payload.command == z7::task_ipc_runtime::TaskIpcCommandKind::kHash
             || payload.command == z7::task_ipc_runtime::TaskIpcCommandKind::kTest)
            && archive_aware_hash_test_or_extract
            && (!payload.hash.has_value() ? (payload.test.has_value() ? payload.test->archive_inputs.isEmpty() : true)
                                          : payload.hash->input_paths.isEmpty())) {
            QMessageBox::warning(
                this, caption, z7::ui::runtime_support::J(QStringLiteral("ui.navigation.task_ipc.no_archive_entries")));
            return false;
        }
        if (payload.command == z7::task_ipc_runtime::TaskIpcCommandKind::kHash
            && !archive_aware_hash_test_or_extract
        ...
        if (!z7::task_ipc_runtime::dispatch_task_ipc_task(
                worker_program, working_dir, task_ipc_owner_instance_id_, payload, &dispatch_result, &dispatch_error)) {
            QMessageBox::warning(this,
                                 caption,
                                 dispatch_error.trimmed().isEmpty() ? z7::ui::runtime_support::J(QStringLiteral(
                                                                          "ui.navigation.task_ipc.dispatch_failed"))
                                                                    : dispatch_error);
            return false;
        }
        ...
        show_transient_status_message(
            QStringLiteral("%1: 7zG %2 (session %3)")
                .arg(caption, task_ipc_command_name(payload.command), QString::number(dispatch_result.session_id)),
            5000,
            origin_panel_index);
        return true;
    }

    MainWindow::SevenZipMenuState MainWindow::compute_seven_zip_menu_state(bool shift_pressed) const {
    ...
    bool MainWindow::launch_7zfm_open_request(QString const& caption,
        ...
        if (target_path.trimmed().isEmpty()) {
            QMessageBox::warning(
                this,
                caption,
                z7::ui::runtime_support::J(QStringLiteral("ui.navigation.task_ipc.open_requires_one_target")));
            return false;
        }

        QString const working_dir = QFileInfo(QDir::fromNativeSeparators(target_path.trimmed())).absolutePath();
        QString launch_error;
        if (!z7::platform::qt::filemanager_instance_launcher::launch_open_request_for_current_app(
                target_path, archive_type_hint, working_dir, &launch_error)) {
            QMessageBox::warning(this,
                                 caption,
                                 launch_error.trimmed().isEmpty() ? z7::ui::runtime_support::J(QStringLiteral(
                                                                        "ui.navigation.task_ipc.launch_7zfm_failed"))
                                                                  : launch_error);
            return false;
        }
        show_transient_status_message(QStringLiteral("%1: launched 7zFM open").arg(caption), 5000, origin_panel_index);
        return true;
    }
    ...
    void MainWindow::run_sevenzip_open_archive() {
        ...
        QString const target_path = state.selected_real_item_paths.front();
        QString const caption = z7::ui::runtime_support::strip_mnemonic(lang_or(2322));
        launch_7zfm_open_request(caption, target_path, QString());
    }

    void MainWindow::run_sevenzip_open_archive_as(QString const& type) {
        SevenZipMenuState const state = compute_seven_zip_menu_state(false);
        ...
        QString const target_path = state.selected_real_item_paths.front();
        QString const caption = z7::ui::runtime_support::strip_mnemonic(lang_or(2322));
        QString const type_hint = type.trimmed();
        launch_7zfm_open_request(caption, target_path, type_hint);
    }

    void MainWindow::run_sevenzip_extract_files_dialog() {
        SevenZipMenuState const state =
            compute_seven_zip_menu_state((QApplication::keyboardModifiers() & Qt::ShiftModifier) != 0);
    ...
    void MainWindow::run_sevenzip_extract_here() {
        ...
        QString const output_dir = state.base_folder;
        QString const caption = z7::ui::runtime_support::strip_mnemonic(lang_or(2326));
        z7::task_ipc_runtime::TaskIpcPayload payload;
        payload.command = z7::task_ipc_runtime::TaskIpcCommandKind::kExtract;
        payload.refresh_after_finish = true;
        payload.extract = z7::task_ipc_runtime::TaskIpcExtractPayload{};
    ...
    void MainWindow::run_sevenzip_extract_to() {
        ...
        QString const output_dir = state.base_folder;
        QString const caption = z7::ui::runtime_support::strip_mnemonic(lang_or(2327));
        z7::task_ipc_runtime::TaskIpcPayload payload;
        payload.command = z7::task_ipc_runtime::TaskIpcCommandKind::kExtract;
        payload.refresh_after_finish = true;
        payload.extract = z7::task_ipc_runtime::TaskIpcExtractPayload{};
        ...
        payload.extract->split_dest_enabled = true;
        payload.extract->split_dest_name = state.extract_subdir;
        payload.extract->eliminate_root_duplication = extract_eliminate_root_duplication_from_settings();
        payload.extract->zone_id_mode = extract_zone_id_mode_from_settings();
        payload.extract->archive_inputs = state.selected_files;
        launch_gui_subprocess_task(caption, payload);
    }
    ...
    void MainWindow::run_sevenzip_test_archive() {
        ...
        QString const caption = z7::ui::runtime_support::strip_mnemonic(lang_or(2325));
        z7::task_ipc_runtime::TaskIpcPayload payload;
        payload.command = z7::task_ipc_runtime::TaskIpcCommandKind::kTest;
