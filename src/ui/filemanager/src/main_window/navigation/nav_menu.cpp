...
namespace z7::ui::filemanager {
    ...
    QMenu* MainWindow::append_seven_zip_submenu(QMenu* menu, SevenZipMenuState const& state, QAction* insert_before) {
        ...
        if (state.show_open) {
            ...
            if (state.show_open_as) {
                ...
                static QStringList const kOpenTypes = {QStringLiteral("*"),
                                                       QStringLiteral("#"),
                                                       QStringLiteral("#:e"),
                                                       QStringLiteral("7z"),
                                                       QStringLiteral("zip"),
                                                       QStringLiteral("cab"),
                                                       QStringLiteral("rar")};
                for (QString const& type : kOpenTypes) {
        ...
        if (state.show_extract_group) {
            ...
            connect(extract_files_action, &QAction::triggered, this, &MainWindow::run_sevenzip_extract_files_dialog);

            QAction* extract_here_action = seven->addAction(z7::ui::runtime_support::strip_mnemonic(lang_or(2326)));
            connect(extract_here_action, &QAction::triggered, this, &MainWindow::run_sevenzip_extract_here);

            QString const extract_to_label = format_menu_name(2327, state.extract_subdir);
            QAction* extract_to_action = seven->addAction(extract_to_label);
            connect(extract_to_action, &QAction::triggered, this, &MainWindow::run_sevenzip_extract_to);

            if (state.show_test) {
                QAction* test_action = seven->addAction(z7::ui::runtime_support::strip_mnemonic(lang_or(2325)));
                connect(test_action, &QAction::triggered, this, &MainWindow::run_sevenzip_test_archive);
            }
        ...
        if (state.show_compress_group) {
            ...
            connect(add_dialog_action, &QAction::triggered, this, &MainWindow::run_sevenzip_add_to_archive);

            QString const add_7z_label = format_menu_name(2328, state.archive_name_7z);
            QAction* add_7z_action = seven->addAction(add_7z_label);
            connect(
                add_7z_action, &QAction::triggered, this, [this]() { run_sevenzip_add_to_type(QStringLiteral("7z")); });

            QString const add_zip_label = format_menu_name(2328, state.archive_name_zip);
        ...
        if (state.show_crc_group) {
            QMenu* crc_menu = seven->addMenu(z7::ui::runtime_support::J(QStringLiteral("shell.actions.crc_sha_menu")));
            for (z7::shell_integration::ShellIntegrationHashMethodDef const& item :
                ...
                connect(action, &QAction::triggered, this, [this, method]() { run_sevenzip_hash(method); });
            }

            crc_menu->addSeparator();

            QStringList const crc_paths = active_panel_controller().oper_smart_real_item_paths();
            QString dynamic_sha_name =
                z7::shell_integration::shell_integration_create_archive_name_from_paths(
                    crc_paths.isEmpty() ? state.selected_real_item_paths : crc_paths, true, nullptr)
                + QStringLiteral(".sha256");
            reduce_menu_string(&dynamic_sha_name);
            QAction* generate_sha256 = crc_menu->addAction(
                z7::ui::runtime_support::JF(QStringLiteral("shell.actions.generate_sha256"), {dynamic_sha_name}));
            connect(generate_sha256, &QAction::triggered, this, &MainWindow::run_sevenzip_generate_sha256);

            QAction* checksum_test =
                crc_menu->addAction(z7::ui::runtime_support::J(QStringLiteral("shell.actions.checksum_test")));
            connect(checksum_test, &QAction::triggered, this, &MainWindow::run_sevenzip_checksum_test);
        }
    ...
    void MainWindow::rebuild_file_menu_seven_zip_section() {
        ...
        QAction* first_action = file_menu_->actions().isEmpty() ? nullptr : file_menu_->actions().front();
        file_menu_seven_zip_menu_ = append_seven_zip_submenu(file_menu_, seven_zip_state, first_action);
        if (file_menu_seven_zip_menu_ == nullptr) {
            return;
        }

        if (first_action != nullptr) {
            file_menu_seven_zip_separator_action_ = file_menu_->insertSeparator(first_action);
        } else {
