...
namespace z7::ui::filemanager {
    ...
    namespace {
        ...
        bool is_filesystem_comment_target_editable(QString const& panel_directory, QString const& selected_path) {
            if (panel_directory.trimmed().isEmpty() || selected_path.trimmed().isEmpty()) {
                return false;
            }
            QFileInfo const selected_info(selected_path);
    ...
    void MainWindow::refresh_action_states() {
        if (open_action_ == nullptr || extract_action_ == nullptr) {
            return;
        }

        QStringList const selected_paths_including_parent_link = selected_filesystem_paths_including_parent_link();
        bool const has_selection = !selected_paths_including_parent_link.isEmpty();
        PanelController const& active_panel = active_panel_controller();
        bool const selected_rows_include_parent = active_panel.selected_rows_include_parent_link();
        bool const has_selected_real_items = has_selected_real_items_for_panel(active_panel_index_);
        bool const has_selected_real_items_without_parent_link =
            has_selected_real_items && !selected_rows_include_parent;
        bool const has_oper_smart_real_items = has_oper_smart_real_items_for_panel(active_panel_index_);
        bool const single_selection = selected_paths_including_parent_link.size() == 1;
        bool const archive_mode = active_panel.in_archive_view();
        QModelIndexList const selected_real_rows = active_panel.selected_real_item_rows();
        bool const has_selected_real_dirs = active_panel.source_rows_contain_dir(selected_real_rows);
        bool const has_extract_operands = !selected_real_rows.isEmpty() && !has_selected_real_dirs;
        bool all_files = has_selected_real_items_without_parent_link;
        for (QString const& path : selected_paths_including_parent_link) {
            if (!QFileInfo(path).isFile()) {
                all_files = false;
                break;
            }
        }
        bool const archive_preview_test_enabled =
            archive_mode && build_archive_writeback_plan_for_panel(active_panel_index_).is_valid();
        bool const archive_preview_add_enabled =
            archive_mode && can_add_external_files_to_archive_preview(active_panel_index_);
        bool const archive_preview_properties_enabled = archive_mode
        ...
        bool const has_oper_smart_archive_entries =
            archive_preview_test_enabled && !active_panel.oper_smart_archive_entries().isEmpty();
        bool const focused_is_parent_link = active_panel.focused_item_is_parent_link();
        QString const focused_path = active_panel.focused_path();
        bool const has_focused_item = focused_is_parent_link || !focused_path.isEmpty();
        int const row_count = active_panel.model != nullptr ? active_panel.model->rowCount() : 0;
        bool const has_rows = row_count > 0;
        bool const has_focused_non_parent = has_focused_item && !focused_is_parent_link;
        QString const favorite_folder_prefix = active_panel.favorite_folder_prefix();
        bool const can_add_to_favorites = !favorite_folder_prefix.trimmed().isEmpty();
        bool const can_comment =
            has_focused_non_parent
            && (archive_mode ? active_panel.archive.current_token.is_valid()
                             : is_filesystem_comment_target_editable(active_panel.current_directory(), focused_path));
        bool const can_link = single_selection && has_selected_real_items_without_parent_link && !archive_mode;
#if defined(Q_OS_WIN)
        bool const supports_alternate_streams = true;
#else
        bool const supports_alternate_streams = false;
#endif
        bool const can_open_alternate_streams = supports_alternate_streams
                                             && single_selection
                                             && has_selected_real_items_without_parent_link
                                             && !archive_mode;
        bool const archive_preview_rename_enabled =
            archive_mode && has_focused_non_parent && active_panel.archive.current_token.is_valid();
        bool const can_open_parent = can_open_parent_from_current_dir();
        z7::platform::qt::PortableSettings settings;
        QString const diff_command =
            settings.value(QString::fromLatin1(kSettingsFmDiff), QString()).toString().trimmed();
        bool const has_diff_command = !diff_command.isEmpty();
        QString const version_control_command =
            settings.value(QString::fromLatin1(kSettingsFmVersionControl), QString()).toString().trimmed();
        bool const has_version_control_command = !version_control_command.isEmpty();
        bool show_version_edit = false;
        bool show_version_commit = false;
        bool show_version_revert = false;
        bool show_version_diff = false;
        if (has_diff_command
            && has_version_control_command
            && !archive_mode
            && single_selection
            && has_selected_real_items_without_parent_link) {
            QFileInfo const version_control_file(selected_paths_including_parent_link.at(0));
        ...
        bool const can_edit_fs = !archive_mode;
        bool const writable_current_dir = QFileInfo(current_directory()).isWritable();
        bool const filesystem_rename_enabled = !archive_mode && has_focused_non_parent && writable_current_dir;
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            bool const panel_can_rename = panel.in_archive_view() ? panel.archive.current_token.is_valid()
        ...
        rename_action_->setEnabled(archive_mode ? archive_preview_rename_enabled : filesystem_rename_enabled);
        ...
        split_action_->setEnabled(
            single_selection && all_files && !archive_mode && backend_capabilities_.supports_split);
        combine_action_->setEnabled(
            single_selection && all_files && !archive_mode && backend_capabilities_.supports_combine);
        properties_action_->setEnabled(archive_mode ? archive_preview_properties_enabled
                                                    : has_selected_real_items_without_parent_link);
        bool const diff_with_two_selected = selected_paths_including_parent_link.size() == 2;
        bool const diff_with_dual_panel_single_selection =
            single_selection && two_panels_visible_ && !in_archive_view_for_panel(1 - active_panel_index_);
        diff_action_->setVisible(has_diff_command);
        diff_action_->setEnabled(
            has_diff_command && !archive_mode && (diff_with_two_selected || diff_with_dual_panel_single_selection));
        ...
        compress_action_->setEnabled(archive_mode ? archive_preview_add_enabled
                                                  : has_selected_real_items_without_parent_link);
        extract_action_->setEnabled(archive_mode ? has_oper_smart_archive_entries : has_extract_operands);
        test_action_->setEnabled(archive_mode ? archive_preview_test_enabled : has_oper_smart_real_items);
    ...
    void MainWindow::setup_ui() {
        ...
        int const small_icon_extent = z7::platform::qt::small_icon_extent(this);
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            panel.model->set_archive_drag_materializer(
                [this, i](QStringList const& entries, DirectoryListModel::ArchiveDragMaterializedCallback finished_cb) {
                    materialize_archive_drag_entries_for_panel(i, entries, std::move(finished_cb));
                });
            panel.model->set_rename_handler(
                [this, i](QString const& item_path, QString const& new_name, bool item_is_dir) {
                    return start_rename_item_for_panel(i, item_path, new_name, item_is_dir);
                });
            ...
            connect(header, &QHeaderView::sectionPressed, this, [this, i](int section) {
                PanelController& panel = panel_controller(i);
                panel.runtime.pending_layout_selection = panel.capture_selection_snapshot();
                panel.runtime.has_pending_layout_selection = true;
                Q_UNUSED(section);
            });
            connect(header, &QHeaderView::sortIndicatorChanged, this, [this, i](int section, Qt::SortOrder order) {
                PanelController& panel = panel_controller(i);
                int const detected_sort_action = [&panel, section]() -> int {
                    if (panel.ui.details_view == nullptr || !panel.ui.details_view->isSortingEnabled() || section < 0) {
                ...
                PanelController::SelectionSnapshot const selection_snapshot = panel.runtime.has_pending_layout_selection
            ...
            details_view->set_drag_finished_callback(
                [this](DragExecutionReport const& report) { on_panel_drag_finished(report); });
            details_view->set_archive_drag_materializer(
                [this, i](QStringList const& entries,
                          DragAwareStructuredListView::ArchiveDragMaterializedCallback finished_cb) {
                    materialize_archive_drag_entries_for_panel(i, entries, std::move(finished_cb));
                });
            details_view->set_archive_drag_direct_exporter(
                [this, i](QString const& archive_entry, bool is_dir, QString const& destination_path, QString* error) {
            ...
            icon_list_view->set_archive_drag_materializer(
                [this, i](QStringList const& entries, DragAwareListView::ArchiveDragMaterializedCallback finished_cb) {
                    materialize_archive_drag_entries_for_panel(i, entries, std::move(finished_cb));
                });
            ...
            QList<QLabel*> const status_labels = {
                selected_count,
                selected_size,
                focused_size,
                focused_modified,
            ...
            transient_message->setSizePolicy(QSizePolicy::Ignored, QSizePolicy::Preferred);
            ...
            connect(details_view,
                    ...
                    this,
                    [this, i](QModelIndex const&) {
                        set_active_panel(i);
                        activate_panel_selection(QApplication::keyboardModifiers());
                    });
            connect(details_view, &z7::ui::widgets::StructuredListView::backspace_pressed, this, [this, i]() {
                set_active_panel(i);
                on_open_parent_requested();
            });
            connect(details_view, &z7::ui::widgets::StructuredListView::delete_pressed, this, [this, i]() {
                set_active_panel(i);
                on_delete_requested();
            });
            connect(details_view,
                    ...
                    this,
                    [this, i](QModelIndex const& /*index*/, QPoint const& viewport_pos, QPoint const& /*global_pos*/) {
                        set_active_panel(i);
                        show_context_menu(viewport_pos);
                    });

            // Icon view keeps Qt's default click/double-click/context-menu signals.
            auto on_item_activated = [this, i](QModelIndex const& index) {
                set_active_panel(i);
                if (!index.isValid())
                    return;
                if (activate_archive_parent_link_for_panel(i, index)) {
            ...
            auto on_item_clicked = [this, i](QModelIndex const& index) {
                set_active_panel(i);
                if (!display_settings_.single_click_open || !index.isValid())
                    return;
                auto* sm = panel_controller(i).ui.details_view->selectionModel();
            ...
            connect(
                panel.ui.icon_list_view, &QWidget::customContextMenuRequested, this, &MainWindow::show_context_menu);

            connect(panel.ui.details_view->selectionModel(), &QItemSelectionModel::selectionChanged, this, [this, i]() {
                set_active_panel(i);
                update_status();
                refresh_action_states();
            });
            connect(panel.ui.details_view->selectionModel(), &QItemSelectionModel::currentChanged, this, [this, i]() {
                set_active_panel(i);
                update_status();
            });
            connect(panel.model, &QAbstractItemModel::modelReset, this, [this, i]() { update_status_for_panel(i); });
            connect(
                panel.model,
                &QAbstractItemModel::dataChanged,
                this,
                [this, i](QModelIndex const&, QModelIndex const&, QList<int> const&) { update_status_for_panel(i); });

            apply_view_mode_to_panel(i, PanelController::kViewModeDetails);
            panel.ui.details_view->sortByColumn(DirectoryListModel::kNameColumn, Qt::AscendingOrder);
