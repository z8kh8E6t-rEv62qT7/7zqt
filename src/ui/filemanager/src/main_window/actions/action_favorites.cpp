...
namespace z7::ui::filemanager {
    namespace {
        ...
        QStringList load_folder_shortcuts(z7::platform::qt::PortableSettings const& settings) {
            QStringList shortcuts =
                settings.value(QString::fromLatin1(kFolderShortcutsKey), QStringList()).toStringList();
        ...
        bool key_event_has_original_right_control_modifier(QKeyEvent const& event) {
#if defined(Q_OS_MACOS)
    ...
    void MainWindow::rebuild_favorites_menu() {
        ...
        QStringList const shortcuts = load_folder_shortcuts(settings);
        for (int slot = 0; slot < kFavoriteSlotCount; ++slot) {
            QString const path = shortcuts.value(slot);
    ...
    bool MainWindow::handle_favorite_slot_shortcut(QKeyEvent const& event, Qt::KeyboardModifiers modifiers) {
        std::optional<int> const slot = favorite_slot_from_digit_key(event.key());
        if (!slot.has_value()) {
            return false;
        }

        bool const set_slot = (modifiers & Qt::ShiftModifier) != 0;
        modifiers &= ~Qt::ShiftModifier;

        bool const alt_shortcut = modifiers == Qt::AltModifier;
        bool const ctrl_shortcut = modifiers == Qt::ControlModifier
    ...
    void MainWindow::set_favorite_slot(int slot) {
        ...
        if (folder_prefix.isEmpty()) {
            QMessageBox::warning(this,
                                 ...
                                 z7::ui::runtime_support::J(QStringLiteral("ui.state.favorites.no_filesystem_folder")));
            return;
        }
    ...
    void MainWindow::open_favorite_path(QString const& raw_path) {
        QString const path = raw_path.trimmed();
        if (path.isEmpty()) {
            return;
        }
        ...
        if (!open_folder_prefix_for_panel(panel_index, path)) {
            QMessageBox::warning(this,
                                 ...
                                 z7::ui::runtime_support::JF(QStringLiteral("ui.state.favorites.folder_missing"),
    ...
    void MainWindow::on_add_to_favorites_requested() {
        ...
        bool slot_ok = false;
        int const slot = action->data().toInt(&slot_ok);
        if (!slot_ok) {
            return;
        }
        set_favorite_slot(slot);
