...
namespace z7::ui::filemanager {
    namespace {
        ...
        QString resolve_link_target_path(QString const& input, QString const& base_dir) {
        ...
        QString link_dialog_text(uint32_t lang_id, QStringView fallback) {
            QString text = z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(lang_id));
        ...
        QString default_link_from_path(QFileInfo const& source_info,
                                       QString const& source_panel_base_dir,
                                       QString const& opposite_panel_dir) {
            ...
            return QDir(base_dir).absoluteFilePath(source_info.fileName() + QStringLiteral(".link"));
        }

        class LinkDialog final : public QDialog {
        public:
            LinkDialog(QString const& initial_from,
                       QString const& initial_to,
...
#endif
                setWindowTitle(z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(558)));
                resize(520, 300);
                ...
                auto* type_box = new QGroupBox(link_dialog_text(7710, QStringLiteral("Link Type")), this);
#ifdef Z7_TESTING
                ...
                add_type_radio(type_layout,
                               ...
                               false);
                file_symlink_radio_ = add_type_radio(type_layout,
                                                     1,
...
#endif
                link_button_ =
                    buttons->addButton(link_dialog_text(7701, QStringLiteral("Link")), QDialogButtonBox::AcceptRole);
#ifdef Z7_TESTING
                ...
                connect(from_browse, &QPushButton::clicked, this, [this]() { browse_for_combo(from_combo_); });
                connect(to_browse, &QPushButton::clicked, this, [this]() { browse_for_combo(to_combo_); });
                connect(buttons, &QDialogButtonBox::accepted, this, &QDialog::accept);
                connect(buttons, &QDialogButtonBox::rejected, this, &QDialog::reject);
                connect(from_combo_, &QComboBox::editTextChanged, this, [this]() { update_link_button(); });
                connect(to_combo_, &QComboBox::editTextChanged, this, [this]() { update_link_button(); });
                update_link_button();
            }
        ...
        private:
            QRadioButton*
            add_type_radio(QVBoxLayout* layout, int id, QString const& text, QString const& object_name, bool enabled) {
            ...
            void browse_for_combo(QComboBox* combo) {
                ...
                QString const current = combo->currentText().trimmed();
                ...
                QString const selected = QFileDialog::getExistingDirectory(
                    this, z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(1003)), start_dir);
                if (!selected.isEmpty()) {
    ...
    void MainWindow::on_comment_requested() {
        ...
        QString const source_path = panel.focused_path();
        ...
        QString const current_comment = focused_comment_for_panel(active_panel_index_);
        bool ok = false;
        ...
        start_task_with_runner(
            ...
            [source_path,
             ...
             archive_token = panel.archive.current_token](ArchiveProcessRunner* runner) {
                ...
                if (archive_mode) {
                    return runner->start_archive_comment(archive_path, source_path, archive_token, comment);
                }
                QFileInfo const source_info(source_path);
                return runner->start_filesystem_comment(source_info.absolutePath(), source_info.fileName(), comment);
            },
            [this](bool ok_result, int, int, QString const&, z7::app::OperationOutcome const&) {
                if (ok_result) {
                    refresh_directory();
                }
            });
    ...
    void MainWindow::on_link_requested() {
        ...
        QStringList const paths = panel_controller(source_panel_index).selected_real_item_paths();
        if (paths.size() != 1) {
            QMessageBox::information(this,
        ...
        QString const source_panel_base_dir = current_directory_for_panel(source_panel_index).trimmed();
        if (source_panel_base_dir.isEmpty()) {
            return;
        }

        QFileInfo const source_info(paths.front());
        ...
        int const opposite_panel_index = source_panel_index == 0 ? 1 : 0;
        ...
        QString const initial_link_from =
            default_link_from_path(source_info, source_panel_base_dir, opposite_panel_dir);
        QString const initial_link_to = source_info.absoluteFilePath();
        LinkDialogType const initial_type =
            source_info.isDir() ? LinkDialogType::kDirectorySymlink : LinkDialogType::kFileSymlink;
        LinkDialog dialog(initial_link_from, initial_link_to, initial_type, this);
        if (dialog.exec() != QDialog::Accepted) {
            return;
        }

        LinkDialogResult const result = dialog.result();
        QString const link_from_path = resolve_link_target_path(result.link_from, source_panel_base_dir);
        QString const link_to_path = resolve_link_target_path(result.link_to, source_panel_base_dir);
        if (link_from_path.isEmpty() || link_to_path.isEmpty()) {
            return;
        }

        QFileInfo const link_from_info(link_from_path);
        if (link_from_info.exists()) {
            QMessageBox::warning(
                this,
                z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(558)),
                QStringLiteral("Target already exists:\n%1").arg(QDir::toNativeSeparators(link_from_path)));
            return;
        }

        QFileInfo const link_to_info(link_to_path);
        bool const directory_symlink = result.type == LinkDialogType::kDirectorySymlink;
        ...
        QString const parent_dir = link_from_info.absolutePath();
        if (!QDir().mkpath(parent_dir)) {
            QMessageBox::warning(
                this,
                z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(558)),
                QStringLiteral("Failed to prepare target directory:\n%1").arg(QDir::toNativeSeparators(parent_dir)));
            return;
        }

        std::error_code ec;
        std::filesystem::path const link_to_native(z7::ui::archive_support::to_native_string(link_to_path));
        std::filesystem::path const link_from_native(z7::ui::archive_support::to_native_string(link_from_path));
        ...
        if (ec) {
            QMessageBox::warning(
                this,
                z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(558)),
                QStringLiteral("Failed to create link:\n%1").arg(QString::fromLocal8Bit(ec.message())));
            return;
        }
...
#if defined(Q_OS_WIN)
        QString const stream_view_path = source_info.absoluteFilePath() + QStringLiteral(":");
