...
namespace z7::ui::filemanager {
    namespace {
        ...
        QString display_name_for_row(DirectoryListModel const* model, int row) {
            if (model == nullptr || row < 0 || row >= model->rowCount()) {
                return QString();
            }
            return model->data(model->index(row, DirectoryListModel::kNameColumn), Qt::DisplayRole).toString();
        }
        ...
        QString leaf_name_for_path(QString path) {
            ...
            int const slash_pos = path.lastIndexOf(QLatin1Char('/'));
            int const backslash_pos = path.lastIndexOf(QLatin1Char('\\'));
            int const separator_pos = slash_pos > backslash_pos ? slash_pos : backslash_pos;
        ...
        QString leaf_name_for_row(DirectoryListModel const* model, int row) {
            if (model == nullptr || row < 0 || row >= model->rowCount()) {
                return {};
            }
        ...
        QModelIndex focused_source_index_for_panel(PanelLike const& panel) {
            if (panel.model == nullptr) {
                return {};
            }

            if (QAbstractItemView* view = panel.current_item_view()) {
                QModelIndex const focused_index = panel.map_proxy_to_source(view->currentIndex());
            ...
            if (QItemSelectionModel* selection_model = panel.selection_model()) {
                QModelIndex const focused_index = panel.map_proxy_to_source(selection_model->currentIndex());
                if (focused_index.isValid()) {
                    return focused_index;
                }
            }
        ...
        bool matches_original_type_name(QString const& focused_leaf_name, QString const& candidate_leaf_name) {
            int const focused_dot_pos = focused_leaf_name.lastIndexOf(QLatin1Char('.'));
        ...
        template <typename PanelLike>
        void apply_row_selection_source(PanelLike const& panel, int source_row, bool select_mode) {
            QItemSelectionModel* selection_model = panel.selection_model();
            if (selection_model == nullptr || panel.model == nullptr || source_row < 0) {
                return;
            }
            QModelIndex const source_index = panel.model->index(source_row, DirectoryListModel::kNameColumn);
            ...
            QItemSelectionModel::SelectionFlags const flags =
                select_mode ? QItemSelectionModel::Select : (QItemSelectionModel::Deselect | QItemSelectionModel::Rows);
            selection_model->select(proxy_index, flags);
        }

        template <typename PanelLike>
        void set_operable_row_selection_source(PanelLike const& panel, int source_row, bool selected) {
            if (panel.model == nullptr
                || source_row < 0
        ...
        template <typename PanelLike>
        void clear_parent_link_selection_source(PanelLike const& panel) {
        ...
        template <typename PanelLike>
        void apply_type_selection_by_original_name(PanelLike const& panel, bool select_mode) {
            if (panel.model == nullptr || panel.selection_model() == nullptr) {
                return;
            }

            QModelIndex const focused_index = focused_source_index_for_panel(panel);
            if (!focused_index.isValid()) {
                return;
            }

            int const focused_row = focused_index.row();
            if (focused_row < 0
                || focused_row >= panel.model->rowCount()
                || panel.model->is_parent_link_for_row(focused_row)) {
                return;
            }

            bool const focused_is_dir = panel.model->is_dir_for_row(focused_row);
            QString const focused_leaf_name = leaf_name_for_row(panel.model, focused_row);

            for (int row = 0; row < panel.model->rowCount(); ++row) {
                if (panel.model->is_parent_link_for_row(row) || panel.model->is_dir_for_row(row) != focused_is_dir) {
                    continue;
                }

                bool const type_match =
                    focused_is_dir
    ...
    void MainWindow::set_operable_rows_selected_for_panel(int panel_index, bool selected) {
    ...
    void MainWindow::invert_operable_selection_for_panel(int panel_index) {
        ...
        for (int row = 0; row < panel.model->rowCount(); ++row) {
            ...
            QModelIndex const proxy_index =
                panel.map_source_to_proxy(panel.model->index(row, DirectoryListModel::kNameColumn));
            if (!proxy_index.isValid()) {
                continue;
            }
    ...
    void MainWindow::on_select_requested() {
        ...
        QRegularExpression const regex(QRegularExpression::wildcardToRegularExpression(mask),
        ...
        for (int row = 0; row < panel.model->rowCount(); ++row) {
            ...
            QString const name = display_name_for_row(panel.model, row);
