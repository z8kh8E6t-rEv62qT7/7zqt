...
namespace z7::ui::filemanager {
    ...
    namespace {
        ...
        QStringList build_archive_move_delete_entries(z7::app::ExtractResult const& extract_result) {
            QSet<QString> selected_entries;
            QStringList ordered_directories;
            QStringList delete_entries;

            for (z7::app::ExtractMaterializedEntry const& entry : extract_result.materialized_entries) {
                QString const archive_entry = z7::ui::archive_support::normalize_virtual_dir(
            ...
            for (QString const& directory_entry : ordered_directories) {
                QString const child_prefix = directory_entry + QLatin1Char('/');
                bool has_materialized_child = false;
                for (QString const& candidate : selected_entries) {
                    if (candidate.size() > child_prefix.size() && candidate.startsWith(child_prefix)) {
        ...
        QString archive_entry_leaf_name(QString const& archive_entry) {
            QString const normalized = z7::ui::archive_support::normalize_virtual_dir(archive_entry);
            int const slash = normalized.lastIndexOf(QLatin1Char('/'));
            if (slash < 0) {
                return normalized;
            }
        ...
        QString archive_entry_parent_dir(QString const& archive_entry) {
            QString const normalized = z7::ui::archive_support::normalize_virtual_dir(archive_entry);
            int const slash = normalized.lastIndexOf(QLatin1Char('/'));
            if (slash <= 0) {
                return QString();
        ...
        QString existing_entry_identity_path(QString const& path) {
        ...
        bool paths_refer_to_same_existing_entry(QString const& left, QString const& right) {
            QString const left_identity = existing_entry_identity_path(left);
            QString const right_identity = existing_entry_identity_path(right);
        ...
        QString localized_size_bytes(uint64_t value) {
            QString bytes_format = z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(3504)).trimmed();
            QString const number = QString::number(value);
        ...
        void append_items_info_pair(
            QStringList* lines, QString const& label, uint64_t count, bool size_defined, uint64_t size) {
            if (lines == nullptr || count == 0) {
                return;
            }
            if (!size_defined) {
                lines->append(QStringLiteral("%1: %2").arg(label, QString::number(count)));
                return;
            }
            lines->append(
                QStringLiteral("%1: %2    ( %3 )").arg(label, QString::number(count), localized_size_bytes(size)));
        }

        QString normalize_transfer_destination_for_compare(QString const& value) {
        ...
        bool transfer_destination_matches_archive_panel(PanelLike const& panel, QString const& raw_destination) {
            if (!panel.in_archive_view()) {
                return false;
            }
            QString const requested = normalize_transfer_destination_for_compare(raw_destination);
            QString const panel_path = normalize_transfer_destination_for_compare(panel.archive_virtual_display_path());
    ...
    QString MainWindow::copy_move_info_text_for_source_rows(int panel_index, QModelIndexList const& rows) const {
        ...
        for (QModelIndex const& row_index : rows) {
            if (!row_index.isValid() || panel.model->is_parent_link_for_row(row_index.row())) {
                continue;
            }

            ++real_row_count;
            bool const is_dir = panel.model->is_dir_for_row(row_index.row());
            if (is_dir) {
                ++dir_count;
            } else {
                ++file_count;
            ...
            uint64_t const item_size =
                panel.model->data(panel.model->index(row_index.row(), DirectoryListModel::kSizeColumn), Qt::DisplayRole)
                    .toULongLong(&ok_size);
            if (!ok_size) {
                if (is_dir) {
            ...
            if (preview_lines.size() < kPreviewLimit) {
                ...
                if (panel.in_archive_view()) {
                    ...
                    if (!current_virtual.isEmpty()) {
                        QString const prefix = current_virtual + QLatin1Char('/');
                ...
                if (is_dir && !line.endsWith(QLatin1Char('/')) && !line.endsWith(QLatin1Char('\\'))) {
                    line += QDir::separator();
                }
                preview_lines << line;
            }
        ...
        QString prefix_line = panel.in_archive_view() ? panel.archive_virtual_display_path()
        ...
        QStringList lines;
        append_items_info_pair(&lines, non_empty_label(1031), dir_count, folders_size_defined, folders_size);
        append_items_info_pair(&lines, non_empty_label(1032), file_count, files_size_defined, files_size);
        bool const include_total_size =
            folders_size_defined && files_size_defined && folders_size != 0 && files_size != 0;
        if (include_total_size) {
            lines << QStringLiteral("%1: %2").arg(non_empty_label(1007),
                                                  localized_size_bytes(folders_size + files_size));
        }
        lines << QString();
        lines << prefix_line;
        for (QString const& preview : preview_lines) {
    ...
    bool MainWindow::select_model_path_for_panel(int panel_index, QString const& model_path) {
        ...
        int target_row = -1;
        int const row_count = panel.model->rowCount();
        ...
        QModelIndex const proxy_index =
            panel.map_source_to_proxy(panel.model->index(target_row, DirectoryListModel::kNameColumn));
    ...
    bool MainWindow::edit_focused_item_label_for_panel(int panel_index) {
        ...
        bool const can_rename = panel.in_archive_view() ? panel.archive.current_token.is_valid()
                                                        : QFileInfo(panel.current_directory()).isWritable();
        if (!can_rename) {
            return false;
        }

        QModelIndex const source_index = panel.model->index(focused.row(), DirectoryListModel::kNameColumn);
        ...
        QModelIndex const proxy_index = panel.map_source_to_proxy(source_index);
        if (!proxy_index.isValid()) {
            return false;
        }
    ...
    bool MainWindow::start_rename_item_for_panel(int panel_index,
                                                 QString const& item_path,
                                                 QString const& new_name,
                                                 bool item_is_dir) {
        if (in_archive_view_for_panel(panel_index)) {
            return start_rename_archive_entry_in_preview(panel_index, item_path, new_name, item_is_dir);
        }
        return start_rename_filesystem_item_for_panel(panel_index, item_path, new_name);
    }

    bool MainWindow::start_rename_filesystem_item_for_panel(int panel_index,
                                                            QString const& source_path,
                                                            QString const& new_name) {
        if (in_archive_view_for_panel(panel_index)) {
            return false;
        }
        ...
        QFileInfo const src(source_path);
        ...
        QString const title = z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(545));
        ...
        QString const dst_path = src.dir().filePath(normalized_name);
        if (QFileInfo::exists(dst_path)) {
            QMessageBox::warning(this, title, z7::ui::runtime_support::LF(3008, {dst_path}));
            return false;
        }

        QString const absolute_source_path = src.absoluteFilePath();
        QString const old_path = absolute_source_path;
        QString const new_path = QFileInfo(dst_path).absoluteFilePath();
        return start_task_with_runner(
            ...
            [absolute_source_path, normalized_name](ArchiveProcessRunner* runner) {
                return runner != nullptr && runner->start_rename_path(absolute_source_path, normalized_name);
            },
            [this, panel_index, old_path, new_path](
                bool ok, int, int, QString const&, z7::app::OperationOutcome const&) {
                refresh_directory();
                (void)select_model_path_for_panel(panel_index, ok ? new_path : old_path);
            });
    }

    void MainWindow::select_archive_entry_for_panel(int panel_index, QString const& archive_entry) {
        if (!in_archive_view_for_panel(panel_index)) {
            return;
        }
        QString const normalized_entry = z7::ui::archive_support::normalize_virtual_dir(archive_entry);
        (void)select_model_path_for_panel(panel_index, normalized_entry);
    }

    bool MainWindow::start_rename_archive_entry_in_preview(int panel_index,
        ...
        QString const new_entry =
            z7::ui::archive_support::join_virtual_path(archive_entry_parent_dir(normalized_entry), normalized_name);
        int const row_count = panel.model->rowCount();
        for (int row = 0; row < row_count; ++row) {
            if (panel.model->is_parent_link_for_row(row)) {
                continue;
            }
            QString const candidate = z7::ui::archive_support::normalize_virtual_dir(panel.model->path_for_row(row));
            if (candidate == new_entry) {
                QMessageBox::warning(this, title, z7::ui::runtime_support::LF(3008, {normalized_name}));
                return false;
            }
        }
        ...
        return start_task_with_runner(
            ...
            [archive_path, session_token, normalized_entry, normalized_name, entry_is_dir](
                ...
                return runner != nullptr
                    && runner->start_rename_archive_entry(
                        archive_path, session_token, normalized_entry, normalized_name, entry_is_dir);
            },
            [this, panel_index, archive_path, archive_display_source, session_token, normalized_entry, new_entry](
                bool ok, int, int, QString const&, z7::app::OperationOutcome const&) {
                QString const focus_entry = ok ? new_entry : normalized_entry;
                if (!reload_archive_virtual_directory_for_panel(
                        panel_index, [this, panel_index, focus_entry](bool loaded) {
                            if (loaded) {
    ...
    void MainWindow::on_delete_requested() {
        ...
        if (in_archive_view()) {
            ...
            QModelIndexList const rows = panel.selected_rows_including_parent_link();
            ...
            for (QModelIndex const& row : rows) {
                ...
                int const row_index = row.row();
                if (panel.model->is_parent_link_for_row(row_index)) {
                    continue;
                }

                QString const rel =
                    z7::ui::archive_support::normalize_virtual_dir(panel.model->path_for_row(row_index));
                ...
                if (entries.size() == 1) {
                    ...
                    single_entry_name = panel.model->index(row_index, 0).data(Qt::DisplayRole).toString().trimmed();
            ...
            auto const reply = ask_question(
                title, message, QMessageBox::Yes | QMessageBox::No | QMessageBox::Cancel, QMessageBox::Yes);
            ...
            start_task_with_runner(
                ...
                [archive_path, entries, session_token](ArchiveProcessRunner* runner) {
                    return runner != nullptr && runner->start_delete_entries(archive_path, entries, session_token);
                },
                [this, panel_index, session_token](
                    bool ok_delete, int, int, QString const&, z7::app::OperationOutcome const&) {
                    if (!ok_delete) {
                        return;
                    }
                    PanelController const& panel = panel_controller(panel_index);
        ...
        QStringList const paths = active_panel_controller().selected_real_item_paths();
        if (paths.isEmpty()) {
            return;
        }
        bool const to_recycle_bin = (QApplication::keyboardModifiers() & Qt::ShiftModifier) == 0;

        QString title;
        QString message;

        if (paths.size() == 1) {
            QFileInfo const info(paths.front());
            if (info.isDir()) {
                title = z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(6101));
        ...
        if (confirm_delete_) {
            auto const reply = ask_question(title, message, QMessageBox::Yes | QMessageBox::No, QMessageBox::No);
            if (reply != QMessageBox::Yes) {
                return;
            }
        }
        ...
        start_task_with_runner(
            ...
            [paths, to_recycle_bin](ArchiveProcessRunner* runner) {
                return runner != nullptr && runner->start_delete_paths(paths, to_recycle_bin);
            },
            [this](bool ok, int, int, QString const&, z7::app::OperationOutcome const&) {
                if (ok) {
                    refresh_directory();
                }
            });
    ...
    bool MainWindow::run_filesystem_transfer_to_archive_panel(int target_panel_index,
                                                              QStringList const& sources,
                                                              bool move,
                                                              QString const& history_path,
                                                              QString const& caption) {
        if (sources.isEmpty() || !can_add_external_files_to_archive_preview(target_panel_index)) {
            return false;
        }

        PanelController const& target_panel = panel_controller(target_panel_index);
        ArchiveWritebackPlan const writeback_plan = build_archive_writeback_plan_for_panel(target_panel_index);
        if (!writeback_plan.is_valid() || !target_panel.archive.current_token.is_valid()) {
            return false;
        }

        QVector<ArchiveAddInputItem> input_items;
        input_items.reserve(sources.size());
        QString const target_virtual_dir =
            z7::ui::archive_support::normalize_virtual_dir(target_panel.archive.virtual_dir);
        for (QString const& source : sources) {
            QFileInfo const source_info(source);
            if (!source_info.exists() || (!source_info.isFile() && !source_info.isDir())) {
                continue;
            }
            QString const target_entry =
                z7::ui::archive_support::join_virtual_path(target_virtual_dir, source_info.fileName());
            if (target_entry.isEmpty()) {
                continue;
            }
            input_items.push_back(ArchiveAddInputItem{source_info.absoluteFilePath(), target_entry});
        }
        if (input_items.isEmpty()) {
            return false;
        }

        QString const normalized_history = history_path.trimmed();
        if (!normalized_history.isEmpty()) {
            save_copy_history(normalize_copy_history(read_copy_history(), normalized_history));
        }

        QStringList const sources_to_delete = sources;
        z7::app::ArchiveSessionToken const session_token = target_panel.archive.current_token;
        return start_add_mapped_files_to_archive_preview(
            ...
            [this, move, sources_to_delete, caption](
                ...
                QTimer::singleShot(0, this, [this, sources_to_delete, caption]() {
                    start_task_with_runner(
                        QStringLiteral("%1: %2").arg(caption, QString::number(sources_to_delete.size())),
                        caption,
                        [sources_to_delete](ArchiveProcessRunner* runner) {
                            return runner != nullptr && runner->start_delete_paths(sources_to_delete, false);
                        },
                        [this](bool ok, int, int, QString const&, z7::app::OperationOutcome const&) {
                            if (ok) {
                                refresh_directory();
    ...
    void MainWindow::run_copy_or_move(bool move, bool copy_to_same) {
        ...
        if (copy_to_same) {
            ...
            QString const focused_path = panel.model != nullptr ? panel.model->path_for_row(focused.row()) : QString();
            if (focused_path.isEmpty()) {
                return;
            }
            source_rows << panel.model->index(focused.row(), DirectoryListModel::kNameColumn);
        ...
        if (sources.isEmpty()) {
            QMessageBox::information(this,
                                     z7::ui::runtime_support::strip_mnemonic(move ? z7::ui::runtime_support::L(6001)
                                                                                  : z7::ui::runtime_support::L(6000)),
                                     z7::ui::runtime_support::L(3015));
            return;
        }

        QString const operation_caption = z7::ui::runtime_support::strip_mnemonic(
            move ? z7::ui::runtime_support::L(6001) : z7::ui::runtime_support::L(6000));
        CopyMoveDialogResult const dialog_result =
            show_copy_move_dialog(this,
                                  operation_caption,
                                  z7::ui::runtime_support::strip_mnemonic(move ? z7::ui::runtime_support::L(6003)
        ...
        if (!copy_to_same && two_panels_visible_) {
            int const other_panel_index = 1 - active_panel_index_;
            PanelController const& other_panel = panel_controller(other_panel_index);
            if (transfer_destination_matches_archive_panel(other_panel, dialog_result.destination_path)) {
                if (!run_filesystem_transfer_to_archive_panel(other_panel_index,
                                                              sources,
                                                              move,
                                                              other_panel.archive_virtual_display_path(),
                                                              operation_caption)) {
                    QMessageBox::information(this, operation_caption, z7::ui::runtime_support::L(3015));
                }
                return;
            }
        }

        CopyTransferDestinationPlan const destination_plan = build_copy_transfer_destination_plan(
            dialog_result.destination_path, current_directory_for_panel(active_panel_index_), sources.size(), false);
        if (!destination_plan.valid) {
            return;
        }

        auto destination_for_source = [&destination_plan](QString const& source) {
            if (!destination_plan.destination_path.isEmpty()) {
                return destination_plan.destination_path;
            }
            return QDir(destination_plan.destination_dir).filePath(QFileInfo(source).fileName());
        };

        QStringList sources_to_apply = sources;
        for (QString const& src : sources_to_apply) {
            if (paths_refer_to_same_existing_entry(src, destination_for_source(src))) {
                QMessageBox::warning(
                    this, operation_caption, QStringLiteral("Cannot copy or move files onto themselves."));
                return;
            }
        }
        ...
        QString const destination_display = QDir::toNativeSeparators(destination_plan.display_path);
        start_task_with_runner(
            QStringLiteral("%1: %2").arg(operation_caption, destination_display),
            operation_caption,
            [move, sources_to_apply, destination_plan](ArchiveProcessRunner* runner) {
                if (runner == nullptr) {
                    return false;
                }
    ...
    void MainWindow::run_copy_or_move_archive_same_folder(bool move) {
        ...
        QString const source_entry =
            z7::ui::archive_support::normalize_virtual_dir(panel.model->path_for_row(focused.row()));
        QString const source_leaf = archive_entry_leaf_name(source_entry);
        ...
        bool const source_is_dir = panel.model->is_dir_for_row(focused.row());
        QString const archive_path = panel.archive.source_archive;
        QString const archive_type_hint = panel.archive.type_hint.trimmed();
        QString const current_virtual_dir = z7::ui::archive_support::normalize_virtual_dir(panel.archive.virtual_dir);
        z7::app::ArchiveSessionToken const session_token = panel.archive.current_token;
        QString const operation_caption = z7::ui::runtime_support::strip_mnemonic(
            move ? z7::ui::runtime_support::L(6001) : z7::ui::runtime_support::L(6000));

        CopyMoveDialogResult const dialog_result = show_copy_move_dialog(
            this,
            operation_caption,
            z7::ui::runtime_support::strip_mnemonic(move ? z7::ui::runtime_support::L(6003)
                                                         : z7::ui::runtime_support::L(6002)),
            copy_move_info_text_for_source_rows(
                panel_index, QModelIndexList{panel.model->index(focused.row(), DirectoryListModel::kNameColumn)}),
            source_leaf);
        if (!dialog_result.accepted) {
            return;
        }
        ...
        QString target_leaf;
        QString error_message;
        if (!validate_basename_only_name(dialog_result.destination_path, &target_leaf, &error_message)) {
            QMessageBox::warning(this, operation_caption, error_message);
            return;
        }

        QString const target_entry = z7::ui::archive_support::join_virtual_path(current_virtual_dir, target_leaf);
        if (target_entry.isEmpty()) {
            return;
        }
        ...
        for (int row = 0; row < row_count; ++row) {
            ...
            if (candidate == target_entry) {
                QMessageBox::warning(this, operation_caption, z7::ui::runtime_support::LF(3008, {target_leaf}));
                return;
            }
        }
        ...
        QStringList const entries{source_entry};
        start_archive_source_extract_task(
            ...
            [this,
             ...
             archive_path,
             session_token,
             operation_caption](bool ok_extract, int, int, QString const&, z7::app::OperationOutcome const& outcome) {
                if (!ok_extract) {
                    return;
                }

                QStringList const extracted_paths = extracted_archive_entry_paths(temp_dir->path(), {source_entry});
                ...
                QStringList delete_entries;
                if (move) {
                    if (auto const extract_result = z7::app::outcome_payload_as<z7::app::ExtractResult>(outcome);
                ...
                QVector<ArchiveAddInputItem> input_items;
                input_items.push_back(ArchiveAddInputItem{extracted_paths.front(), target_entry});
                QString const archive_display_source = writeback_plan.current_display_source();
                bool const add_started = start_add_mapped_files_to_archive_preview(
                    writeback_plan,
                    session_token,
                    input_items,
                    operation_caption,
                    !move,
                    [this, temp_dir, move, archive_path, archive_display_source, delete_entries, session_token](
                        bool ok_add, int, int, QString const&, z7::app::OperationOutcome const&) {
                        if (!move || !ok_add) {
                            return;
                        }
                            ...
                            0, this, [this, archive_path, archive_display_source, delete_entries, session_token]() {
                                start_task_with_runner(
                                    QStringLiteral("%1: %2").arg(
                                        ...
                                        archive_path),
                                    z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(6106)),
                                    [archive_path, delete_entries, session_token](ArchiveProcessRunner* runner) {
                                        return runner != nullptr
    ...
    void MainWindow::run_copy_or_move_archive_context(bool move) {
        ...
        int const panel_index = active_panel_index_;
        PanelController const& panel = active_panel_controller();
        QModelIndexList const source_rows = panel.selected_real_item_rows();
        QStringList const entries = panel.archive_entries_for_source_rows(source_rows);
        if (entries.isEmpty()) {
            QMessageBox::information(this,
                                     z7::ui::runtime_support::strip_mnemonic(move ? z7::ui::runtime_support::L(6001)
                                                                                  : z7::ui::runtime_support::L(6000)),
        ...
        QString destination_dir = QDir::fromNativeSeparators(dialog_result.destination_path.trimmed());
        if (destination_dir.isEmpty()) {
            return;
        }
        if (QDir::isRelativePath(destination_dir)) {
            destination_dir = QDir(current_directory_for_panel(active_panel_index_)).absoluteFilePath(destination_dir);
        }

        save_copy_history(normalize_copy_history(read_copy_history(), destination_dir));

        QStringList const items = {z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(3421)),
        ...
        int const choice_index = items.indexOf(choice);
        if (choice_index == 1) {
        ...
        start_archive_source_extract_task(
            ...
            mode,
            entries,
            [this, panel_index, move, archive_path, session_token](
                bool ok_copy, int, int, QString const&, z7::app::OperationOutcome const& outcome) {
                if (!move || !ok_copy) {
                    return;
                }

                auto const extract_result = z7::app::outcome_payload_as<z7::app::ExtractResult>(outcome);
                if (!extract_result.has_value()) {
                    return;
                }

                QStringList const delete_entries = build_archive_move_delete_entries(*extract_result);
                if (delete_entries.isEmpty()) {
                    return;
                }
                ...
                QTimer::singleShot(0, this, [this, panel_index, archive_path, delete_entries, session_token]() {
                    start_task_with_runner(
                        ...
                        [archive_path, delete_entries, session_token](ArchiveProcessRunner* runner) {
                            return runner != nullptr
                                && runner->start_delete_entries(archive_path, delete_entries, session_token);
                        },
                        [this, panel_index, session_token](
                            bool ok_delete, int, int, QString const&, z7::app::OperationOutcome const&) {
                            if (!ok_delete) {
    ...
    QString MainWindow::default_target_directory_for_transfer() const {
        QString default_dir;
        int const other_panel = 1 - active_panel_index_;
        if (two_panels_visible_) {
            if (!in_archive_view() && in_archive_view_for_panel(other_panel)) {
                QString const other_archive_dir = panel_controller(other_panel).archive_virtual_display_path();
                if (!other_archive_dir.isEmpty()) {
                    default_dir = other_archive_dir;
                }
            } else if (!in_archive_view_for_panel(other_panel)) {
                QString const other_dir = current_directory_for_panel(other_panel);
