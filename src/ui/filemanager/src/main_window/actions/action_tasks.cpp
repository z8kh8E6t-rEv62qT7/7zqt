...
namespace z7::ui::filemanager {
    ...
    namespace {
        ...
        std::optional<qulonglong> first_split_volume_size_bytes(QString const& volume_size_spec) {
            QVector<qulonglong> values;
            bool prev_is_number = false;
            QString const input = volume_size_spec.trimmed();

            for (qsizetype i = 0; i < input.size();) {
                QChar const c = input.at(i++);
                if (c.isSpace()) {
                    continue;
                }
                if (c == QLatin1Char('-')) {
                ...
                --i;
                qsizetype const start = i;
                qulonglong value = 0;
                while (i < input.size() && input.at(i).isDigit()) {
                    int const digit = input.at(i).digitValue();
                    if (value > (std::numeric_limits<qulonglong>::max() - static_cast<qulonglong>(digit)) / 10ULL) {
                        return std::nullopt;
                    }
    ...
    void MainWindow::on_create_folder_requested() {
        ...
        if (!validate_basename_only_name(name, &normalized_name, &error_message)) {
            QMessageBox::warning(
                this, z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(6300)), error_message);
            return;
        }

        QString const parent_dir = current_directory_for_panel(active_panel_index_);
        start_task_with_runner(
            QStringLiteral("%1: %2").arg(z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(6300)),
                                         normalized_name),
            z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(6306)),
            [parent_dir, normalized_name](ArchiveProcessRunner* runner) {
                return runner != nullptr && runner->start_create_directory(parent_dir, normalized_name);
            },
            [this](bool ok_create, int, int, QString const&, z7::app::OperationOutcome const&) {
                if (ok_create) {
                    refresh_directory();
                }
            });
    ...
    void MainWindow::on_create_file_requested() {
        ...
        if (!validate_basename_only_name(name, &normalized_name, &error_message)) {
            QMessageBox::warning(
                this, z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(6301)), error_message);
            return;
        }

        QString const parent_dir = current_directory_for_panel(active_panel_index_);
        start_task_with_runner(
            QStringLiteral("%1: %2").arg(z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(6301)),
                                         normalized_name),
            z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(6307)),
            [parent_dir, normalized_name](ArchiveProcessRunner* runner) {
                return runner != nullptr && runner->start_create_file(parent_dir, normalized_name);
            },
            [this](bool ok_create, int, int, QString const&, z7::app::OperationOutcome const&) {
                if (ok_create) {
                    refresh_directory();
    ...
    void MainWindow::on_benchmark_requested() {
        QString const caption = z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(901));
    ...
    void MainWindow::on_temp_files_requested() {
        QString const temp_path = QDir::cleanPath(QDir::tempPath());
    ...
    bool MainWindow::start_task_with_runner(
        ...
        auto const background_mode = std::make_shared<z7::ui::common::TaskBackgroundModeController>();
        auto const remove_task = [this, task]() {
        ...
        if (created_dialog != nullptr) {
...
#ifdef Z7_TESTING
            QStringList task_headers = property("z7.fm.task.headers").toStringList();
            task_headers << header;
            setProperty("z7.fm.task.headers", task_headers);
#endif
            created_dialog->set_test_mode(false);
            created_runner->set_prompt_parent_provider([dialog_guard, progress_presenter]() -> QWidget* {
                if (!dialog_guard) {
                    return nullptr;
                }
            ...
            connect(
                created_dialog, &TaskProgressDialog::cancel_requested, created_runner, &ArchiveProcessRunner::cancel);
            connect(created_dialog, &TaskProgressDialog::pause_requested, created_runner, &ArchiveProcessRunner::pause);
            connect(
                created_dialog, &TaskProgressDialog::resume_requested, created_runner, &ArchiveProcessRunner::resume);
            connect(created_dialog,
                    &TaskProgressDialog::background_requested,
                    this,
                    [background_mode](bool backgrounded) { background_mode->set_backgrounded(backgrounded); });
            created_dialog->set_pause_available(created_runner->supports_pause());

            connect(created_runner,
                    &ArchiveProcessRunner::started,
                    created_dialog,
                    [dialog_guard](QString const& backend, QString const& operation, QStringList const& targets) {
                        if (!dialog_guard) {
                            return;
                        }
                        dialog_guard->append_log(
                            QStringLiteral("%1 %2 %3").arg(backend, operation, targets.join(QLatin1Char(' '))));
                    });

            connect(created_runner, &ArchiveProcessRunner::log_line, created_dialog, &TaskProgressDialog::append_log);
            ...
            connect(created_runner,
                    ...
                    created_dialog,
                    [dialog_guard, runner_guard](QString const& stage) {
                        if (!dialog_guard) {
                            return;
                        }
                        if (runner_guard) {
            ...
            [this,
             ...
             origin_panel_index](bool ok, int exit_code, int error_domain, QString const& summary) {
                ...
                if (dialog_guard) {
                    ...
                    dialog_guard->append_log(QStringLiteral("%1 (exit=%2)").arg(summary).arg(exit_code));
                }
                ...
                z7::app::OperationOutcome const finished_outcome =
        ...
        if (created_dialog != nullptr) {
            ...
            if (task_ui_mode == RunnerTaskUiMode::kDelayed) {
                progress_presenter->schedule(this, z7::ui::runtime_support::kOriginal7ZipProgressDialogDelayMs);
            } else {
                progress_presenter->show_now();
            }
        }
        ...
        bool const started = start_fn(created_runner);
        if (!started) {
            background_mode->restore();
            remove_task();
            if (created_dialog != nullptr) {
    ...
    void MainWindow::start_split_task(QString const& source_file_path,
                                      QString const& output_dir,
                                      QString const& volume_size_spec) {
        QFileInfo const source_info(source_file_path);
        std::optional<qulonglong> const first_volume_size = first_split_volume_size_bytes(volume_size_spec);
        if (first_volume_size.has_value()
            && source_info.exists()
            && source_info.isFile()
        ...
        start_task_with_runner(
            QStringLiteral("%1: %2").arg(z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(549)),
                                         source_file_path),
            z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(549)),
            [source_file_path, output_dir, volume_size_spec](ArchiveProcessRunner* runner) {
                return runner != nullptr && runner->start_split(source_file_path, output_dir, volume_size_spec);
            },
