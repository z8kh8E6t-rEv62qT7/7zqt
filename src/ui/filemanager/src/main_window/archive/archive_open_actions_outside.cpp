...
namespace z7::ui::filemanager {
    ...
    namespace {
        ...
        QString build_open_outside_failure_message(QStringList const& failed_paths) {
            if (failed_paths.isEmpty()) {
                return z7::ui::runtime_support::J(QStringLiteral("ui.archive.open_outside.failed_launch"));
            }

            QStringList display_paths;
            display_paths.reserve(failed_paths.size());
            for (QString const& path : failed_paths) {
                display_paths << QDir::toNativeSeparators(path);
            }

            return z7::ui::runtime_support::JF(QStringLiteral("ui.archive.open_outside.failed_launch_for_paths"),
                                               {display_paths.join(QLatin1Char('\n'))});
        }

    } // namespace

    MainWindow::OpenOutsideLaunchResult MainWindow::launch_open_outside(QStringList const& paths,
                                                                        QString const& working_dir) {
        OpenOutsideLaunchResult result;
        if (paths.isEmpty()) {
            result.message = z7::ui::runtime_support::J(QStringLiteral("ui.archive.open_outside.no_external_paths"));
            return result;
        }
        ...
        if (!result.has_success()) {
            result.message =
                build_open_outside_failure_message(result.failed_paths.isEmpty() ? paths : result.failed_paths);
            return result;
        }
    ...
    void MainWindow::launch_archive_temp_session_outside(QSharedPointer<ArchiveTempSession> const& session,
                                                         QStringList const& paths,
                                                         QString const& working_dir) {
        if (session == nullptr || paths.isEmpty()) {
            return;
        }
        OpenOutsideLaunchResult const launch_result = launch_open_outside(paths, working_dir);
        if (!launch_result.has_success()) {
            QMessageBox::warning(
                this,
                session->command_caption,
                launch_result.message.trimmed().isEmpty()
                    ? z7::ui::runtime_support::J(QStringLiteral("ui.archive.open_outside.failed_launch"))
                    : launch_result.message);
            return;
        }

        session->extracted_paths = launch_result.launched_paths;
...
#endif
        session->pending_open_outside_trackers = session->tracked_processes.size()
#if defined(Q_OS_WIN)
                                               + session->tracked_process_handles.size()
#endif
            ;
        session->open_outside_cleanup_policy = launch_result.has_tracked_lifecycle()
        ...
        for (QPointer<QProcess> const& process : launch_result.tracked_processes) {
            if (process == nullptr) {
            ...
            if (process->state() == QProcess::NotRunning) {
                QTimer::singleShot(
                    0, this, [this, session]() { on_open_outside_temp_session_tracking_finished(session); });
            }
        }
            ...
            if (!session->tracked_process_handles.isEmpty()) {
                ...
                *poll_handles = [this, session, poll_handles]() {
                    ...
                    if (any_pending_handle) {
                        QTimer::singleShot(200, this, [poll_handles]() { (*poll_handles)(); });
                    }
                };
                QTimer::singleShot(200, this, [poll_handles]() { (*poll_handles)(); });
            }
    ...
    void MainWindow::open_archive_entries_outside_for_panel(int panel_index, QStringList const& entries) {
        if (entries.isEmpty()) {
            return;
        }

        PanelController const& panel = panel_controller(panel_index);
        if (!in_archive_view_for_panel(panel_index) || panel.archive.source_archive.isEmpty()) {
            return;
        }
        ...
        QString const command_caption = z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(540));
        start_archive_source_extract_task(
            ...
            entries,
            [this, temp_dir, entries, command_caption](
                bool ok, int, int, QString const&, z7::app::OperationOutcome const&) {
                if (!ok) {
                    return;
                }

                QStringList const extracted_paths = extracted_archive_entry_paths(temp_dir->path(), entries);
                ...
                launch_archive_temp_session_outside(session, extracted_paths, temp_dir->path());
            });
    }
