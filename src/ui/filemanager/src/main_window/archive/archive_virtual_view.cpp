...
namespace z7::ui::filemanager {
    ...
    bool MainWindow::load_archive_virtual_directory_for_panel(int panel_index,
        ...
        QString const type_hint = archive_type_hint.trimmed();
        QString const effective_display_source =
            virtual_display_source.isEmpty() ? source_archive : virtual_display_source;
        bool const recursive_dirs = panel.model->flat_view();
        ...
        bool const started = start_task_with_runner(
            ...
            [source_archive, normalized_virtual_dir, type_hint, recursive_dirs, out_list_result, session_token](
                ...
                if (session_token.is_valid()) {
                    return runner->start_list_in_session(
                        session_token, normalized_virtual_dir, recursive_dirs, true, out_list_result);
                }
                return runner->start_open_archive(
                    source_archive, normalized_virtual_dir, type_hint, recursive_dirs, true, out_list_result);
            },
            [this,
             panel_index,
             source_archive,
    ...
    bool MainWindow::apply_archive_list_result_for_panel(int panel_index,
                                                         QString const& archive_path,
                                                         QString const& virtual_dir,
                                                         QString const& origin_dir,
                                                         QString const& archive_type_hint,
        ...
        if (!list_result.ok && !list_result.listing_completed) {
            ...
            if (list_result.error.domain != z7::app::ArchiveErrorDomain::kCanceled) {
                QString const message = z7::ui::runtime_support::localize_archive_failure_message(summary).trimmed();
                QMessageBox::warning(
                    this,
                    z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(541)),
                    message.isEmpty() ? stable_error_message(static_cast<int>(list_result.error.domain)) : message);
            }
            return false;
        }
        ...
        for (z7::app::ArchiveListEntry const& item : list_result.entries) {
            QString const name =
                z7::ui::archive_support::normalize_virtual_dir(z7::ui::archive_support::from_utf8_string(item.path));
            if (name.isEmpty()) {
                continue;
            }

            DirectoryListModel::VirtualEntry entry;
            entry.path = z7::ui::archive_support::join_virtual_path(normalized_virtual_dir, name);
        ...
        panel.archive.virtual_display_source =
            virtual_display_source.isEmpty() ? source_archive : virtual_display_source;
        panel.enter_archive_view(
            source_archive, normalized_virtual_dir, origin_dir, archive_type_hint.trimmed(), update_origin_dir);

        QString const model_dir = panel.current_directory();
    ...
    bool MainWindow::reload_archive_virtual_directory_for_panel(int panel_index,
        ...
        if (panel.model != nullptr) {
            ...
            if (view != nullptr) {
                QModelIndex const current_index = view->currentIndex();
        ...
        PanelController::ScrollPositionSnapshot const scroll_snapshot = panel.capture_scroll_position();
        ...
        return load_archive_virtual_directory_for_panel(
            ...
            [this,
             panel_index,
             selected_paths_snapshot,
             current_path_snapshot,
             current_column_snapshot,
             scroll_snapshot,
             finished_cb](bool loaded) {
                if (!loaded) {
                    if (finished_cb) {
                        finished_cb(false);
                    }
                ...
                auto find_row_by_path = [&current_panel](QString const& path) -> int {
                    if (path.isEmpty() || current_panel.model == nullptr) {
                        return -1;
                    }
                    int const row_count = current_panel.model->rowCount();
                ...
                QItemSelectionModel::SelectionFlags const restore_flags = QItemSelectionModel::Select;
                QSet<int> restored_rows;
                for (QString const& selected_path : selected_paths_snapshot) {
                    int const row = find_row_by_path(selected_path);
                    if (row < 0 || restored_rows.contains(row)) {
                        continue;
                    }
                    restored_rows.insert(row);
                    QModelIndex const proxy_index = current_panel.map_source_to_proxy(
                ...
                if (current_row >= 0) {
                    ...
                    QModelIndex const proxy_index = current_panel.map_source_to_proxy(
                        current_panel.model->index(current_row, DirectoryListModel::kNameColumn));
                    if (proxy_index.isValid()) {
    ...
    void MainWindow::reload_archive_virtual_directories_serially(QVector<int> panel_indexes,
                                                                 std::function<void()> const& finished_cb) {
        panel_indexes.erase(std::remove_if(panel_indexes.begin(),
                                           panel_indexes.end(),
                                           [this](int panel_index) { return !in_archive_view_for_panel(panel_index); }),
                            panel_indexes.end());
        ...
        struct ArchiveReloadQueue final : public std::enable_shared_from_this<ArchiveReloadQueue> {
            ...
            void start_next() {
                ...
                int const panel_index = pending_panels.front();
                pending_panels.removeFirst();
                MainWindow* const window = owner.data();
                bool const started = window->reload_archive_virtual_directory_for_panel(
                    panel_index, [self = shared_from_this()](bool) { self->start_next(); });
                if (!started) {
                    start_next();
                }
            }
    ...
    bool MainWindow::archive_session_token_referenced_outside_panel(int panel_index,
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            PanelController const& panel = panel_controller(i);
            ...
            for (PanelController::ArchiveState::ParentContext const& parent : panel.archive.parent_stack) {
    ...
    MainWindow::filter_archive_session_tokens_for_panel_close(int panel_index,
        ...
        tokens.erase(std::remove_if(tokens.begin(),
                                    ...
                                    [this, panel_index](z7::app::ArchiveSessionToken token) {
                                        return !token.is_valid()
                                            || archive_session_token_referenced_outside_panel(panel_index, token);
                                    }),
                     tokens.end());
        return tokens;
    }
    ...
    bool
    MainWindow::archive_temp_session_referenced_outside_panel(int panel_index,
    ...
    void MainWindow::close_archive_view_for_panel(int panel_index, std::function<void(bool)> const& finished_cb) {
        if (!in_archive_view_for_panel(panel_index)) {
            if (finished_cb) {
                finished_cb(true);
            }
        ...
        for (auto it = panel.archive.parent_stack.rbegin(); it != panel.archive.parent_stack.rend(); ++it) {
            if (it->session_token.is_valid()) {
                tokens.push_back(it->session_token);
            }
        }
        tokens = filter_archive_session_tokens_for_panel_close(panel_index, std::move(tokens));

        close_archive_sessions_async(std::move(tokens), [this, panel_index, finished_cb](bool ok) {
            if (!ok) {
                if (finished_cb) {
                    finished_cb(false);
                }
            ...
            panel_controller(panel_index)
                .clear_archive_view_state([this, panel_index](QSharedPointer<ArchiveTempSession> const& session) {
    ...
    void MainWindow::close_archive_sessions_async(QVector<z7::app::ArchiveSessionToken> tokens,
                                                  std::function<void(bool)> const& finished_cb) {
        QSet<quint64> seen;
        for (int i = tokens.size() - 1; i >= 0; --i) {
            z7::app::ArchiveSessionToken const token = tokens[i];
        ...
        auto remaining = std::make_shared<QVector<z7::app::ArchiveSessionToken>>(std::move(tokens));
        ...
        *start_next = [this, remaining, start_next, done]() {
            ...
            z7::app::ArchiveSessionToken const token = remaining->first();
            remaining->remove(0);
            bool const started = start_task_with_runner(
