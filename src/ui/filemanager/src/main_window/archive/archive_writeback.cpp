...
namespace z7::ui::filemanager {
    ...
    namespace {
        ...
        QString normalized_archive_display_source(QString const& value) {
            return QDir::cleanPath(QDir::fromNativeSeparators(value));
        }

        QString normalize_archive_update_format_token(QString const& value) {
        ...
        QString canonical_archive_update_format_from_suffix(QString const& value) {
            QString const lowered = value.trimmed().toLower();
            if (lowered == QStringLiteral("*") || lowered == QStringLiteral("#")) {
                return QString();
            }
        ...
        QString resolve_archive_update_format(QString const& type_hint,
                                              QString const& current_display_source,
            ...
            format = canonical_archive_update_format_from_suffix(QFileInfo(current_display_source).suffix());
            if (!format.isEmpty()) {
                return format;
            }

            format = canonical_archive_update_format_from_suffix(QFileInfo(source_archive).suffix());
            if (!format.isEmpty()) {
                return format;
            }
        ...
        QString default_archive_writeback_copying_caption() {
            return z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(6004));
        }

        QStringList normalize_archive_add_input_paths(QStringList const& input_paths,
                                                      QString const& blocked_archive_path) {
            QStringList out;
            QSet<QString> seen;
            QString const blocked_absolute = QFileInfo(blocked_archive_path).absoluteFilePath().toLower();
            for (QString const& raw_path : input_paths) {
                QString const trimmed = raw_path.trimmed();
                if (trimmed.isEmpty()) {
                    continue;
                }
                ...
                QString const absolute = QDir::cleanPath(QDir::fromNativeSeparators(info.absoluteFilePath()));
                if (absolute.isEmpty()) {
                    continue;
                }

                QString const dedup_key = absolute.toLower();
        ...
        QVector<ArchiveAddInputItem> normalize_archive_add_input_items(QVector<ArchiveAddInputItem> const& input_items,
                                                                       QString const& blocked_archive_path) {
            ...
            QString const blocked_absolute = QFileInfo(blocked_archive_path).absoluteFilePath().toLower();
            for (ArchiveAddInputItem const& raw_item : input_items) {
                QString const source_path = raw_item.filesystem_path.trimmed();
                QString const archive_entry = z7::ui::archive_support::normalize_virtual_dir(raw_item.archive_entry);
                if (source_path.isEmpty() || archive_entry.isEmpty()) {
                    continue;
                }

                QFileInfo const info(source_path);
                if (!info.exists() || (!info.isFile() && !info.isDir())) {
                    continue;
                }
                ...
                QString const source_key = absolute.toLower();
    ...
    MainWindow::ArchiveWritebackPlan MainWindow::build_archive_writeback_plan_for_panel(int panel_index) const {
    ...
    bool MainWindow::start_add_external_files_to_archive_preview(int panel_index,
        ...
        PanelController const& panel = panel_controller(panel_index);
        ArchiveWritebackPlan const plan = build_archive_writeback_plan_for_panel(panel_index);
        if (!plan.is_valid()) {
            return false;
        }

        QStringList const normalized_input_paths = normalize_archive_add_input_paths(input_paths, plan.source_archive);
        ...
        options.format =
            resolve_archive_update_format(panel.archive.type_hint, plan.current_display_source(), plan.source_archive);
        options.session_token = panel.archive.current_token;
        options.input_paths = normalized_input_paths;
        options.directory = z7::ui::archive_support::normalize_virtual_dir(archive_destination_virtual_dir);

        Q_UNUSED(caption);
        QString const trimmed_caption = archive_writeback_copying_caption();
    ...
    bool MainWindow::start_add_mapped_files_to_archive_preview(
        ...
        QVector<ArchiveAddInputItem> const normalized_input_items =
        ...
        options.archive_path = plan.source_archive;
        options.format =
            resolve_archive_update_format(plan.current_type_hint(), plan.current_display_source(), plan.source_archive);
        ...
        return start_task_with_runner(
            ...
            [this, archive_path, archive_display_source, session_token, reload_on_success, finished_cb](
                bool ok, int code, int native_code, QString const& summary, z7::app::OperationOutcome const& outcome) {
    ...
    bool MainWindow::PanelController::matches_archive_writeback_target(QString const& source_archive,
                                                                       QString const& target_display_source) const {
        return in_archive_view()
            && QFileInfo(archive.source_archive).absoluteFilePath() == QFileInfo(source_archive).absoluteFilePath()
    ...
    void MainWindow::PanelController::apply_restored_writeback_plan(
        ArchiveWritebackPlan const& plan, QVector<z7::app::ArchiveSessionToken> const& opened_tokens) {
        archive.parent_stack.clear();

        int const parent_count = plan.reopen_frames.isEmpty() ? 0 : static_cast<int>(plan.reopen_frames.size()) - 1;
        int const opened_parent_count = opened_tokens.isEmpty() ? 0 : static_cast<int>(opened_tokens.size()) - 1;
        int const restored_parent_count = std::min(parent_count, opened_parent_count);
    ...
    void MainWindow::restore_archive_writeback_plan_for_panel(int panel_index,
                                                              ArchiveWritebackPlan const& plan,
        ...
        panel_controller(panel_index)
            .clear_archive_view_state(
                [this, panel_index](QSharedPointer<ArchiveTempSession> const& session) {
        ...
        close_archive_sessions_async(
            filter_archive_session_tokens_for_panel_close(panel_index, std::move(tokens_to_close)));

        struct ArchiveWritebackRestoreTask final : public std::enable_shared_from_this<ArchiveWritebackRestoreTask> {
            ...
            void start_root_open() {
                ...
                QString const root_display_source = plan.root_display_source();
                QString const header = QStringLiteral("%1: %2").arg(
                    z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(541)), root_display_source);
            ...
            void start_list_reload() {
                ...
                auto out_list_result = std::make_shared<std::optional<z7::app::ListResult>>();
                bool const recursive_dirs = !owner.isNull()
                ...
                bool const started = owner->start_task_with_runner(
                    ...
                    [this, recursive_dirs, out_list_result](ArchiveProcessRunner* runner) {
                        return runner != nullptr
                            && runner->start_list_in_session(opened_tokens.back(),
                    ...
                    [self = shared_from_this(),
                     out_list_result](bool ok, int, int, QString const&, z7::app::OperationOutcome const&) {
            ...
            void apply_list_result(z7::app::ListResult const& list_result) {
                if (owner.isNull() || opened_tokens.isEmpty()) {
                    finish(false, false);
                    return;
                }

                bool const loaded = owner->apply_archive_list_result_for_panel(panel_index,
                                                                               plan.source_archive,
                                                                               plan.current_virtual_dir(),
                                                                               plan.origin_dir,
            ...
            void finish(bool ok, bool show_warning) {
                if (!ok) {
                    if (!owner.isNull()) {
                        ...
                        int const restore_panel_index = panel_index;
                            ...
                            window->panel_controller(restore_panel_index)
                                .begin_exit_archive_view_to_filesystem(
                                    ...
                                    [window, restore_panel_index](
                                        QSharedPointer<MainWindow::ArchiveTempSession> const& session) {
                                        if (!window.isNull()) {
                                            window->release_archive_temp_session_for_panel_close(restore_panel_index,
                                                                                                 session);
                                        }
                                    });
                        QVector<z7::app::ArchiveSessionToken> tokens_to_close = opened_tokens;
                        tokens_to_close += exit_transition.tokens_to_close;
                        tokens_to_close = owner->filter_archive_session_tokens_for_panel_close(
                            restore_panel_index, std::move(tokens_to_close));
                        owner->close_archive_sessions_async(
                            std::move(tokens_to_close), [window, restore_panel_index, exit_transition](bool) {
                                if (window.isNull()) {
                                    return;
                                }
                                if (!exit_transition.target_directory.isEmpty()) {
                                    window->set_current_directory_for_panel(restore_panel_index,
                        ...
                        owner->apply_archive_preview_columns_visibility_for_panel(restore_panel_index);
                        if (show_warning) {
                            QMessageBox::warning(
                                owner,
                                z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(541)),
