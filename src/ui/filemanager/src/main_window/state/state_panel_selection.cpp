...
namespace z7::ui::filemanager {
    ...
    namespace {
        ...
        QPoint viewport_pos_for_watched(QAbstractItemView* view, QObject const* watched, QPoint const& watched_pos) {
            if (view == nullptr) {
                return watched_pos;
            }
    ...
    bool MainWindow::PanelController::source_rows_contain_dir(QModelIndexList const& rows) const {
        if (model == nullptr) {
            return false;
        }
    ...
    QStringList MainWindow::PanelController::real_item_paths_for_rows(QModelIndexList const& rows) const {
        if (model == nullptr) {
            return {};
        }
        ...
        for (QModelIndex const& row : rows) {
            ...
            QString const path = model->path_for_row(row.row());
    ...
    QStringList MainWindow::PanelController::archive_entries_for_source_rows(QModelIndexList const& rows) const {
        if (!in_archive_view() || model == nullptr) {
            return {};
        }
    ...
    MainWindow::PanelController::SelectionSnapshot MainWindow::PanelController::capture_selection_snapshot() const {
        SelectionSnapshot snapshot;
        if (model == nullptr) {
            return snapshot;
        }

        snapshot.selected_filesystem_paths_including_parent_link = selected_filesystem_paths_including_parent_link();
        snapshot.current_path = focused_path();
        if (snapshot.current_path.isEmpty() && !snapshot.selected_filesystem_paths_including_parent_link.isEmpty()) {
            snapshot.current_path = snapshot.selected_filesystem_paths_including_parent_link.front();
        }
        return snapshot;
    }

    void MainWindow::PanelController::restore_selection_snapshot(SelectionSnapshot const& snapshot) const {
        if (model == nullptr || ui.details_view == nullptr || ui.details_view->selectionModel() == nullptr) {
            return;
        }
        ...
        for (QString const& selected_path : snapshot.selected_filesystem_paths_including_parent_link) {
            ...
            QModelIndex const proxy_index = map_source_to_proxy(model->index(row, DirectoryListModel::kNameColumn));
            if (proxy_index.isValid()) {
                if (first_selected) {
                    selection_model->setCurrentIndex(
                        proxy_index, QItemSelectionModel::ClearAndSelect | QItemSelectionModel::Current);
        ...
        if (current_row >= 0) {
            QModelIndex const proxy_index =
                map_source_to_proxy(model->index(current_row, DirectoryListModel::kNameColumn));
            if (proxy_index.isValid()) {
    ...
    QModelIndex MainWindow::hovered_source_index_for_panel(int panel_index,
    ...
    DropTargetInfo MainWindow::resolve_drop_target_info_for_panel(int panel_index,
                                                                  QAbstractItemView* view,
                                                                  QObject const* watched,
        ...
        if (model != nullptr && model->is_virtual_mode()) {
            info.archive_virtual_dir = z7::ui::archive_support::normalize_virtual_dir(panel.archive.virtual_dir);
        ...
        QModelIndex const hovered =
            hovered_source_index_for_panel(panel_index, view, watched, event->position().toPoint());
        if (!hovered.isValid()) {
            return info;
        }
        ...
        if (model->is_virtual_mode()) {
            info.archive_virtual_dir = z7::ui::archive_support::normalize_virtual_dir(hovered_path);
            info.allow_copy_move = true;
