...
namespace z7::ui::filemanager {
        ...
        std::optional<ArchivePrefixTarget> resolve_archive_prefix_target(QString const& path) {
            ...
            while (!candidate.isEmpty()) {
                ...
                if (info.exists() && info.isFile() && is_archive_file(info.absoluteFilePath())) {
                    ...
                    return ArchivePrefixTarget{info.absoluteFilePath(),
    ...
    void MainWindow::load_folder_history() {
        ...
        folder_history_ =
            normalize_folder_history(settings.value(QString::fromLatin1(kSettingsFmFolderHistory)).toStringList());
    }

    void MainWindow::save_folder_history() const {
        z7::platform::qt::PortableSettings settings;
        settings.setValue(QString::fromLatin1(kSettingsFmFolderHistory), normalize_folder_history(folder_history_));
        settings.sync();
    }
    ...
    void MainWindow::restore_panel_paths_from_settings() {
        ...
        bool const two_panels_configured = read_fm_panels_state(settings).two_panels;
    ...
    bool MainWindow::open_folder_prefix_for_panel(int panel_index, QString const& path) {
        if (auto const archive_target = resolve_archive_prefix_target(path)) {
            auto open_archive_target = [this, panel_index, archive_target]() {
                return load_archive_virtual_directory_for_panel(panel_index,
            ...
            if (in_archive_view_for_panel(panel_index)) {
                close_archive_view_for_panel(panel_index, [open_archive_target](bool ok) {
                    if (ok) {
                        open_archive_target();
                    }
                });
        ...
        if (in_archive_view_for_panel(panel_index)) {
            close_archive_view_for_panel(panel_index, [this, panel_index, selected_dir](bool ok) {
                if (ok) {
