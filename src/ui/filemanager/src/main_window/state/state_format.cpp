...
namespace z7::ui::filemanager {
    namespace {
        ...
        bool ends_with_path_separator(QString const& value) {
            return value.endsWith(QLatin1Char('/')) || value.endsWith(QLatin1Char('\\'));
        }

        bool is_root_path(QString const& path) {
            return QDir::cleanPath(path) == QDir::cleanPath(QDir(path).rootPath());
        }

        QString append_path_separator(QString path) {
            if (!path.isEmpty() && !path.endsWith(QLatin1Char('/')) && !is_root_path(path)) {
                path += QLatin1Char('/');
            }
            return path;
        }
        ...
        QString clean_path_preserving_double_slash(QString value) {
            bool const preserve_double_slash =
                value.startsWith(QStringLiteral("//")) && !value.startsWith(QStringLiteral("///"));
        ...
        QString normalize_copy_history_entry(QString value) {
            ...
            bool const preserve_trailing_separator = ends_with_path_separator(value);
        ...
        QString trim_trailing_dots_and_spaces(QString component) {
            while (!component.isEmpty()) {
                QChar const tail = component.back();
        ...
        QString path_root_prefix(QString const& path) {
            if (path.startsWith(QStringLiteral("//"))) {
                QString const rest = path.mid(2);
                QStringList const parts = rest.split(QLatin1Char('/'));
        ...
        QString correct_copy_destination_path(QString raw_destination,
            ...
            bool const trailing_separator = ends_with_path_separator(absolute);
            absolute = clean_path_preserving_double_slash(absolute);
            if (trailing_separator) {
                absolute = append_path_separator(absolute);
            }

            QString const root = path_root_prefix(absolute);
            QString suffix = absolute.mid(root.size());
            bool const corrected_trailing_separator = suffix.endsWith(QLatin1Char('/'));
            while (suffix.endsWith(QLatin1Char('/'))) {
                suffix.chop(1);
            }

            QStringList const components = suffix.split(QLatin1Char('/'), Qt::SkipEmptyParts);
            QString result = root;
            bool check_existing = true;
            for (QString component : components) {
                QString const candidate =
                    result.endsWith(QLatin1Char('/')) ? result + component : result + QLatin1Char('/') + component;
                if (check_existing) {
                    QFileInfo const info(candidate);
                    if (info.exists()) {
                        if (!info.isDir()) {
                            return corrected_trailing_separator ? append_path_separator(absolute) : absolute;
                        }
                    } else {
    ...
    QStringList normalize_copy_history(QStringList history, QString const& new_item, int max_items) {
        QStringList out;
        QStringList keys;
        out.reserve(history.size() + 1);
        keys.reserve(history.size() + 1);

        QString const normalized_new = normalize_copy_history_entry(new_item);
        QString const new_key = copy_history_compare_key(normalized_new);
        if (!normalized_new.isEmpty()) {
            out << normalized_new;
            keys << new_key;
        }

        for (QString const& raw : history) {
            QString const normalized = normalize_copy_history_entry(raw);
            QString const key = copy_history_compare_key(normalized);
            if (normalized.isEmpty() || keys.contains(key, copy_history_case_sensitivity())) {
                continue;
            }
            out << normalized;
            keys << key;
    ...
    QStringList read_copy_history() {
        ...
        return normalize_copy_history(
            settings.value(QString::fromLatin1(kSettingsFmCopyToHistory)).toStringList(), QString(), 20);
    }
    ...
    CopyTransferDestinationPlan build_copy_transfer_destination_plan(QString const& raw_destination,
        ...
        QString const corrected_destination =
            correct_copy_destination_path(raw_destination, relative_base, &ends_with_separator);
        if (corrected_destination.isEmpty()) {
            return plan;
        }

        QFileInfo const destination_info(corrected_destination);
        bool const directory_mode = force_directory_mode
                                 || source_count != 1
                                 || ends_with_separator
    ...
    bool ensure_copy_transfer_destination_directories(CopyTransferDestinationPlan const& plan) {
        ...
        QString const directory_to_create =
            plan.destination_path.isEmpty() ? plan.destination_dir : QFileInfo(plan.destination_path).absolutePath();
    ...
    bool validate_basename_only_name(QString const& raw_name, QString* normalized_name, QString* error_message) {
        ...
        QByteArray const raw_name_utf8 = raw_name.toUtf8();
        z7::common::BasenameValidationResult const validation = z7::common::validate_basename_only_name(
            std::string_view(raw_name_utf8.constData(), static_cast<size_t>(raw_name_utf8.size())));
        if (!validation.ok) {
            if (error_message != nullptr) {
        ...
        if (normalized_name != nullptr) {
            *normalized_name = QString::fromUtf8(validation.normalized_name.data(),
                                                 static_cast<qsizetype>(validation.normalized_name.size()));
        }
        return true;
    }
    ...
    CopyMoveDialogResult show_copy_move_dialog(QWidget* parent,
        ...
        QStringList const history = read_copy_history();
        ...
        QObject::connect(browse_button, &QPushButton::clicked, &dialog, [&dialog, destination_combo]() {
            QString const current = QDir::fromNativeSeparators(destination_combo->currentText().trimmed());
            QString browse_base = current;
            if (!browse_base.isEmpty()) {
                QFileInfo const info(browse_base);
                if (!info.isDir()) {
        ...
        QString const destination = QDir::fromNativeSeparators(destination_combo->currentText());
