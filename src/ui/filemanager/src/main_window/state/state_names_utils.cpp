...
namespace z7::ui::filemanager {
    ...
    QString format_menu_name(uint32_t lang_id, QString const& value) {
        QString const quoted = quoted_reduced_menu_name(value);
    ...
    QSet<QString> const& always_start_extensions() {
        static QSet<QString> const kExts = [] {
            QSet<QString> out;
            QString const joined =
                QStringLiteral("exe bat ps1 com lnk "
                               "chm "
            ...
            QStringList const parts = joined.split(QLatin1Char(' '), Qt::SkipEmptyParts);
            for (QString const& item : parts) {
    ...
    bool should_always_start_externally(QString const& path) {
        QString const ext = QFileInfo(path).suffix().toLower();
    ...
    QString stable_error_message(int error_domain) {
        auto const domain = static_cast<z7::app::ArchiveErrorDomain>(error_domain);
        switch (domain) {
