...
namespace z7::ui::filemanager {
    ...
    namespace {
        ...
        std::optional<int> read_int_setting(z7::platform::qt::PortableSettings const& settings, char const* key) {
            bool ok = false;
            int const value = settings.value(QString::fromLatin1(key)).toInt(&ok);
            if (!ok) {
                return std::nullopt;
            }
            return value;
        ...
        std::optional<QRect> fit_window_geometry_to_available_screens(QRect const& requested) {
            ...
            QSize const size(qMin(requested.width(), available.width()), qMin(requested.height(), available.height()));
        ...
        QByteArray encode_details_column_state(QHeaderView const* header, int expected_count) {
            ...
            append_u32_le(&encoded, sort_section < 0 ? kNoSortColumn : static_cast<quint32>(sort_section));
            append_u32_le(&encoded, header->sortIndicatorOrder() == Qt::AscendingOrder ? 1u : 0u);
            ...
            for (int logical = 0; logical < expected_count; ++logical) {
                ...
                section.width = column_width_persistence::clamp_column_width(header->sectionSize(logical));
                section.visual = header->visualIndex(logical);
                sections.push_back(section);
            }
            std::sort(sections.begin(),
                      sections.end(),
                      [](DetailsColumnSectionState const& left, DetailsColumnSectionState const& right) {
        ...
        DetailsColumnState default_details_column_state(int expected_count, QVector<int> const& defaults) {
            ...
            for (int logical = 0; logical < expected_count; ++logical) {
                ...
                if (logical < defaults.size()) {
                    section.width = column_width_persistence::clamp_column_width(defaults.at(logical));
                }
        ...
        bool decode_details_column_state(QByteArray const& encoded,
            ...
            DetailsColumnState state = default_details_column_state(expected_count, defaults);
            state.sort_section = sort_section;
            state.sort_order = *ascending != 0u ? Qt::AscendingOrder : Qt::DescendingOrder;
            ...
            int const stored_count = (encoded.size() - kListViewHeaderSize) / kColumnInfoSize;
            for (int i = 0; i < stored_count; ++i) {
                ...
                int const stored_width = *width > static_cast<quint32>(std::numeric_limits<int>::max())
                                           ? std::numeric_limits<int>::max()
                                           : static_cast<int>(*width);
                section.width = column_width_persistence::clamp_column_width(stored_width);
        ...
        void apply_details_column_state(DragAwareStructuredListView* view,
            ...
            if (view == nullptr || view->horizontalHeader() == nullptr || expected_count <= 0) {
                return;
            }
            ...
            for (DetailsColumnSectionState const& section : state.sections) {
                ...
                bool const visible = section.logical == DirectoryListModel::kNameColumn ? true : section.visible;
            ...
            for (int target_visual = 0; target_visual < ordered.size(); ++target_visual) {
            ...
            if (active_sort_action_out != nullptr) {
                *active_sort_action_out = default_sort_action_for_column(state.sort_section);
            }
        ...
        dedupe_archive_session_tokens(QVector<z7::app::ArchiveSessionToken> tokens) {
            QSet<quint64> seen;
    ...
    FmPanelsState read_fm_panels_state(z7::platform::qt::PortableSettings const& settings) {
        FmPanelsState state;
        QByteArray const payload = settings.value(QString::fromLatin1(kSettingsFmPanels), QByteArray()).toByteArray();
        ...
        if (!num_panels.has_value() || !current_panel.has_value() || !splitter_pos.has_value()) {
            return state;
        }
        ...
        state.splitter_pos = *splitter_pos > static_cast<quint32>(std::numeric_limits<int>::max())
                               ? std::numeric_limits<int>::max()
    ...
    void write_fm_panels_state(z7::platform::qt::PortableSettings* settings,
        ...
        append_u32_le(&payload, static_cast<quint32>(qMax(0, splitter_pos)));
    ...
    void MainWindow::restore_main_window_geometry() {
        ...
        QByteArray const payload = settings.value(QString::fromLatin1(kSettingsFmPosition), QByteArray()).toByteArray();
        ...
        if (!left_raw.has_value()
            || !top_raw.has_value()
            || !right_raw.has_value()
            || !bottom_raw.has_value()
        ...
        QRect const requested(QPoint(left, top), QSize(right - left, bottom - top));
        std::optional<QRect> const restored = fit_window_geometry_to_available_screens(requested);
    ...
    void MainWindow::load_details_column_state() {
        ...
        std::array<char const*, 2> const keys = {kSettingsFmColumnsPanel0, kSettingsFmColumnsPanel1};
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            QVector<int> widths;
            QVariant const encoded = settings.value(QString::fromLatin1(keys[static_cast<size_t>(i)]), QVariant());
            DetailsColumnState state;
            if (decode_details_column_state(
                    encoded.toByteArray(), DirectoryListModel::kColumnCount, defaults, &state)) {
    ...
    void MainWindow::save_details_column_state() const {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            QByteArray const encoded =
                encode_details_column_state(details_view->horizontalHeader(), DirectoryListModel::kColumnCount);
            if (encoded.isEmpty()) {
                continue;
            }
            settings.setValue(QString::fromLatin1(keys[static_cast<size_t>(i)]), encoded);
        }
    }
    ...
    void MainWindow::restore_panel_ui_state_from_settings() {
        ...
        if (std::optional<int> const list_mode = read_int_setting(settings, kSettingsFmListMode);
            list_mode.has_value()) {
            for (int i = 0; i < 2; ++i) {
        ...
        std::array<char const*, 2> const flat_keys = {kSettingsFmFlatViewArc0, kSettingsFmFlatViewArc1};
        for (int i = 0; i < 2; ++i) {
        ...
        if (two_panels_visible_ && panels_splitter_ != nullptr) {
            ...
            if (splitter_pos > 0) {
                int const total_width =
                    panels_splitter_->width() > splitter_pos ? panels_splitter_->width() : kDefaultMainWindowWidth;
                panels_splitter_->setSizes({splitter_pos, qMax(1, total_width - splitter_pos)});
            }
        }
    ...
    void MainWindow::save_panel_ui_state() const {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            settings.setValue(QString::fromLatin1(flat_keys[static_cast<size_t>(i)]), panels_[i].model->flat_view());
        }
    ...
    QVector<z7::app::ArchiveSessionToken> MainWindow::run_shutdown_cleanup_once() {
        ...
        for (int i = 0; i < 2; ++i) {
            panels_[i].clear_archive_view_state(
                [this](QSharedPointer<ArchiveTempSession> const& session) { release_archive_temp_session(session); },
