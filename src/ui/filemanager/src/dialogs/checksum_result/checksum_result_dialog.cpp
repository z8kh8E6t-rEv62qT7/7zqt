...
namespace z7::ui::filemanager {
    ...
    namespace {
        ...
        constexpr char const* kSettingsFmChecksumResultColumns = "FM/View/ChecksumResultColumns";

    } // namespace

    ChecksumResultDialog::ChecksumResultDialog(QWidget* parent) : QDialog(parent) {
    ...
    void ChecksumResultDialog::set_rows(QVector<QPair<QString, QString>> const& rows) {
        table_->setRowCount(rows.size());
        for (int row = 0; row < rows.size(); ++row) {
            QPair<QString, QString> const& entry = rows.at(row);
    ...
    void ChecksumResultDialog::load_table_column_widths() {
        ...
        QVector<int> const default_widths = {280, 620};

        z7::platform::qt::PortableSettings settings;
        QString const encoded =
            settings.value(QString::fromLatin1(kSettingsFmChecksumResultColumns), QString()).toString().trimmed();
        QVector<int> widths;
        has_persisted_column_widths_ = column_width_persistence::decode_widths(encoded, kExpectedColumns, &widths);
    ...
    void ChecksumResultDialog::save_table_column_widths() const {
        ...
        QVector<int> const widths =
            column_width_persistence::capture_widths(table_->horizontalHeader(), kExpectedColumns);
        if (widths.size() != kExpectedColumns) {
            return;
        }

        z7::platform::qt::PortableSettings settings;
        settings.setValue(QString::fromLatin1(kSettingsFmChecksumResultColumns),
