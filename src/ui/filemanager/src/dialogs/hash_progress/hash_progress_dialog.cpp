...
namespace z7::ui::filemanager {
    ...
    HashProgressDialog::HashProgressDialog(QWidget* parent) : QDialog(parent) {
#ifdef Z7_TESTING
    ...
    void HashProgressDialog::set_operation_name(QString const& method_name) {
        operation_name_ = method_name;
        refresh_metrics();
    }

    void HashProgressDialog::set_stage(QString const& text) {
    ...
    void HashProgressDialog::set_progress(bool totals_known,
        ...
        if (!totals_known_) {
            ...
            if (denom == 0) {
                ...
                quint64 const value = std::min<quint64>(1000, (numer * 1000) / denom);
    ...
    void HashProgressDialog::set_backgrounded(bool backgrounded) {
        ...
        background_button_->setText(backgrounded_ ? lang(445) : lang(444));
        update_title();
    }
    ...
    void HashProgressDialog::refresh_metrics() {
        ...
        quint64 const elapsed_sec = static_cast<quint64>(elapsed_ms_ / 1000);
        elapsed_value_->setText(format_hhmmss(elapsed_sec));

        quint64 const remaining_num = total_bytes_ != 0 ? total_bytes_ : total_files_;
        quint64 const remaining_den = total_bytes_ != 0 ? completed_bytes_ : completed_files_;
        if (totals_known_ && remaining_num != 0 && remaining_den != 0 && remaining_den < remaining_num) {
            quint64 const remaining_ms =
                (static_cast<quint64>(elapsed_ms_) * (remaining_num - remaining_den)) / remaining_den;
        ...
        if (total_files_ != 0) {
            files_value_->setText(QStringLiteral("%1 / %2").arg(completed_files_).arg(total_files_));
        } else {
    ...
    void HashProgressDialog::update_title() {
        ...
        if (totals_known_) {
            ...
            quint64 const numer = total_bytes_ != 0 ? completed_bytes_ : completed_files_;
            if (denom != 0) {
                quint64 const percent = std::min<quint64>(100, (numer * 100) / denom);
