...
namespace z7::ui::filemanager {
    ...
    void TempFilesDialog::on_delete_requested() {
        ...
        for (TempFilesListEntry const& entry : entries) {
            bool const remove_as_dir = entry.is_dir && !entry.is_symlink;
            TempFilesDeletePathResult const remove_result =
    ...
    void TempFilesDialog::on_context_menu(QPoint const& pos) {
        if (context_menu_ == nullptr
            || context_delete_action_ == nullptr
        ...
        QModelIndex const index = table_->indexAt(pos);
    ...
    void TempFilesDialog::update_context_menu_actions() {
        if (context_delete_action_ == nullptr
            || context_open_outside_action_ == nullptr
            || context_open_outside_7zip_action_ == nullptr
            || context_properties_action_ == nullptr
            || context_separator_after_delete_ == nullptr
            || context_separator_before_properties_ == nullptr) {
            return;
        }

        int const selected_count = selected_entries().size();
        bool const has_selection = selected_count > 0;
        bool const has_single_selection = selected_count == 1;
        bool const allow_open_variants = selected_count <= 1;
    ...
    void TempFilesDialog::on_open_outside_7zip_requested() {
        ...
        if (!launch_path_in_new_7zfm(target_path, model_.current_path())) {
            QMessageBox::warning(
                this,
                localized_text(542),
                QStringLiteral("Failed to launch 7zFM:\n%1").arg(QDir::toNativeSeparators(target_path)));
        }
    }
    ...
    void TempFilesDialog::show_selected_entry_properties() {
        ...
        QString const text = build_temp_entry_properties_text(entries.front());
        if (text.isEmpty()) {
            return;
        }
        QMessageBox::information(this, localized_text(551), text);
    }

    void TempFilesDialog::show_blocked_operation_warning() {
        QMessageBox::warning(this, QStringLiteral("7-Zip"), z7::ui::runtime_support::L(3014));
    }

    bool TempFilesDialog::open_path_externally_with_warning(QString const& path) {
        ...
        QMessageBox::warning(
            this, localized_text(542), QStringLiteral("Failed to open:\n%1").arg(QDir::toNativeSeparators(path)));
        return false;
    }
    ...
    QVector<TempFilesListEntry> TempFilesDialog::selected_entries() const {
        ...
        std::sort(selected.begin(), selected.end(), [](QModelIndex const& a, QModelIndex const& b) {
            return a.row() < b.row();
        });
