...
namespace z7::ui::filemanager {
    ...
    TempFilesDialogModel::TempFilesDialogModel(QString const& temp_root_path) :
        temp_root_path_(normalized_absolute_path(temp_root_path)), current_path_(temp_root_path_) {}
    ...
    void TempFilesDialogModel::open_parent() {
        ...
        QString const parent_path = normalized_absolute_path(QFileInfo(current_path_).dir().path());
    ...
    bool TempFilesDialogModel::set_current_path(QString const& path) {
        QString const normalized = normalized_absolute_path(path);
        QString const old_path = current_path_;
    ...
    QString TempFilesDialogModel::normalized_absolute_path(QString const& path) {
        return QDir::cleanPath(QFileInfo(path).absoluteFilePath());
    }

    bool TempFilesDialogModel::is_path_inside_temp_root(QString const& path) const {
        QString const normalized = normalized_absolute_path(path);
