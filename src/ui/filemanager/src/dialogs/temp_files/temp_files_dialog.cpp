...
namespace z7::ui::filemanager {
    ...
    TempFilesDialog::TempFilesDialog(QString const& temp_root_path, QWidget* parent) :
        QDialog(parent), model_(temp_root_path) {
#ifdef Z7_TESTING
...
#endif
        context_delete_action_ = context_menu_->addAction(QStringLiteral("%1\tDelete").arg(localized_text(7205)));
#ifdef Z7_TESTING
        ...
        context_open_outside_action_ =
            context_menu_->addAction(QStringLiteral("%1\tShift+Enter").arg(localized_text(542)));
#ifdef Z7_TESTING
        context_open_outside_action_->setObjectName(QStringLiteral("tempFilesContextOpenOutsideAction"));
#endif
        context_open_outside_7zip_action_ =
            context_menu_->addAction(QStringLiteral("%1 : 7-Zip").arg(localized_text(542)));
#ifdef Z7_TESTING
        context_open_outside_7zip_action_->setObjectName(QStringLiteral("tempFilesContextOpenOutside7ZipAction"));
#endif
        context_separator_before_properties_ = context_menu_->addSeparator();
        context_properties_action_ = context_menu_->addAction(QStringLiteral("%1\tAlt+Enter").arg(localized_text(551)));
#ifdef Z7_TESTING
        context_properties_action_->setObjectName(QStringLiteral("tempFilesContextPropertiesAction"));
#endif

        connect(context_open_outside_action_, &QAction::triggered, this, [this]() { on_open_outside_requested(); });
        connect(context_open_outside_7zip_action_, &QAction::triggered, this, [this]() {
            on_open_outside_7zip_requested();
        });
        connect(context_delete_action_, &QAction::triggered, this, [this]() { on_delete_requested(); });
        connect(context_properties_action_, &QAction::triggered, this, [this]() { on_properties_requested(); });
        ...
        connect(table_, &QTableWidget::customContextMenuRequested, this, [this](QPoint const& pos) {
            on_context_menu(pos);
        });
    ...
    void TempFilesDialog::reload() {
        QStringList const selected_names = selected_entry_names();
        QString const focused_name = focused_entry_name();
        model_.reload();
        repopulate_table();
        ...
        if (table_header_ != nullptr) {
            table_header_->setSortIndicator(model_.sort_column(),
    ...
    void TempFilesDialog::repopulate_table() {
        ...
        for (int i = 0; i < model_.entries().size(); ++i) {
            TempFilesListEntry const& entry = model_.entries().at(i);

            QString const modified_text = format_datetime(entry.modified);
            QString const size_text = QString::number(entry.size);
            ...
            name_item->setIcon(style()->standardIcon(entry.is_dir ? QStyle::SP_DirIcon : QStyle::SP_FileIcon));
    ...
    void TempFilesDialog::restore_selection_and_focus(QStringList const& selected_names, QString const& focused_name) {
        ...
        int focused_row = -1;
        for (QString const& name : selected_names) {
            int const row = row_for_entry_name(name);
            if (row < 0) {
                continue;
            }
        ...
        if (!focused_name.isEmpty()) {
            int const row = row_for_entry_name(focused_name);
            if (row >= 0) {
                focused_row = row;
            }
        }
    ...
    int TempFilesDialog::row_for_entry_name(QString const& name) const {
        if (table_ == nullptr) {
            return -1;
        }
    ...
    QStringList TempFilesDialog::selected_entry_names() const {
        QStringList names;
        QVector<TempFilesListEntry> const entries = selected_entries();
        names.reserve(entries.size());
        for (TempFilesListEntry const& entry : entries) {
    ...
    QString TempFilesDialog::focused_entry_name() const {
        ...
        int const row = table_->currentRow();
        if (row < 0) {
            return {};
        }
        if (QTableWidgetItem* item = table_->item(row, 0)) {
    ...
    void TempFilesDialog::activate_current_row(Qt::KeyboardModifiers modifiers) {
        ...
        TempFilesListEntry const& entry = entries.front();
        if (entry.is_symlink) {
