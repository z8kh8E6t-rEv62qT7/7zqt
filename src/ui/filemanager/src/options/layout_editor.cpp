...
namespace z7::ui::filemanager {
    ...
    QWidget* OptionsDialog::create_editor_page() {
        ...
        auto add_editor_row = [group, group_layout](QString const& label_name,
                                                    QString const& edit_name,
        ...
        if (viewer_browse_button_ != nullptr) {
            connect(viewer_browse_button_, &QPushButton::clicked, this, [this]() { browse_editor_path(viewer_edit_); });
        }
        if (editor_browse_button_ != nullptr) {
            connect(editor_browse_button_, &QPushButton::clicked, this, [this]() { browse_editor_path(editor_edit_); });
        }
        if (diff_browse_button_ != nullptr) {
            connect(diff_browse_button_, &QPushButton::clicked, this, [this]() { browse_editor_path(diff_edit_); });
        }
    ...
    void OptionsDialog::load_editor_settings() {
        ...
        initial_viewer_command_ = settings.value(QString::fromLatin1(kSettingsFmViewer), QString()).toString();
        initial_editor_command_ = settings.value(QString::fromLatin1(kSettingsFmEditor), QString()).toString();
        initial_diff_command_ = settings.value(QString::fromLatin1(kSettingsFmDiff), QString()).toString();

        QSignalBlocker const block_viewer(viewer_edit_);
        QSignalBlocker const block_editor(editor_edit_);
        QSignalBlocker const block_diff(diff_edit_);
    ...
    bool OptionsDialog::apply_editor_settings() {
        ...
        QString const current_viewer = viewer_edit_->text();
        QString const current_editor = editor_edit_->text();
        QString const current_diff = diff_edit_->text();

        bool const viewer_changed = current_viewer != initial_viewer_command_;
        bool const editor_changed = current_editor != initial_editor_command_;
        bool const diff_changed = current_diff != initial_diff_command_;
    ...
    void OptionsDialog::browse_editor_path(QLineEdit* line_edit) {
        ...
        QString const program_path = command_program_part(line_edit->text());
        QString const selected = QFileDialog::getOpenFileName(this, QString(), program_path);
        if (selected.isEmpty()) {
            return;
        }
    ...
    void OptionsDialog::build_settings_page() {
...
#ifdef Z7_TESTING
        alternative_selection_checkbox_->setObjectName(QStringLiteral("settingsAlternativeSelectionCheckBox"));
#endif
        ...
        QString const win_only_tip = windows_only_supported() ? QString() : windows_only_tooltip();
        bool const large_pages_available = large_pages_supported();
        QString const large_pages_tip = large_pages_available ? QString() : large_pages_tooltip();
        bool const mem_supported = extract_memory_limit_supported();
        QString const mem_tip = mem_supported ? QString() : extract_memory_limit_tooltip();
        ...
        int const max_gb = max_mem_limit_gb(ram_bytes);
        ...
        auto const implemented_controls = {show_dots_checkbox_,
        ...
        connect(use_large_pages_checkbox_, &QCheckBox::toggled, this, &OptionsDialog::on_large_pages_toggled);
        connect(mem_limit_spin_, qOverload<int>(&QSpinBox::valueChanged), this, [this](int) {
