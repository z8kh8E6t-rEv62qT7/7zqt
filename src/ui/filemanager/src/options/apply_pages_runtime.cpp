...
namespace z7::ui::filemanager {
    ...
    namespace {
        ...
        WorkDirMode normalized_work_dir_mode(int raw_mode, QString const& path) {
    ...
    void OptionsDialog::load_runtime_settings() {
        ...
        DisplaySettings const display_settings = load_display_settings(settings);
        z7::ui::runtime_support::ExtractMemoryLimitSettings const mem_settings =
            z7::ui::runtime_support::load_extract_memory_limit_settings();

        QString const work_path =
            settings.value(QString::fromLatin1(kSettingsFmFoldersWorkDirPath), QString()).toString();
        WorkDirMode const work_mode = normalized_work_dir_mode(
            settings.value(QString::fromLatin1(kSettingsFmFoldersWorkDirMode), static_cast<int>(WorkDirMode::kSystem))
                .toInt(),
            work_path);
        bool const for_removable_only =
            settings.value(QString::fromLatin1(kSettingsFmFoldersWorkForRemovableOnly), true).toBool();
        QString const normalized_work_path = QDir::fromNativeSeparators(work_path.trimmed());

        QSignalBlocker const block_show_dots(show_dots_checkbox_);
        QSignalBlocker const block_show_real_icons(show_real_icons_checkbox_);
        QSignalBlocker const block_full_row(full_row_checkbox_);
        QSignalBlocker const block_show_grid(show_grid_checkbox_);
        QSignalBlocker const block_single_click(single_click_checkbox_);
        QSignalBlocker const block_alternative(alternative_selection_checkbox_);
        QSignalBlocker const block_large_pages(use_large_pages_checkbox_);
        QSignalBlocker const block_mem_enabled(mem_limit_enable_checkbox_);
        QSignalBlocker const block_mem_spin(mem_limit_spin_);
        QSignalBlocker const block_work_system(folders_work_system_radio_);
        QSignalBlocker const block_work_current(folders_work_current_radio_);
        QSignalBlocker const block_work_specified(folders_work_specified_radio_);
        QSignalBlocker const block_work_path(folders_work_path_edit_);
        QSignalBlocker const block_removable_only(folders_work_removable_only_checkbox_);
        ...
        if (use_large_pages_checkbox_ != nullptr) {
            ...
            use_large_pages_checkbox_->setChecked(large_pages_supported()
                                                  && z7::ui::runtime_support::load_large_pages_enabled());
        }
        if (mem_limit_spin_ != nullptr) {
            quint64 const ram_bytes = detect_total_ram_bytes();
        ...
        if (mem_limit_enable_checkbox_ != nullptr) {
            mem_limit_enable_checkbox_->setChecked(extract_memory_limit_supported() && mem_settings.enabled);
        }
    ...
    void OptionsDialog::save_runtime_settings() const {
        if (show_dots_checkbox_ == nullptr
            || show_real_icons_checkbox_ == nullptr
            || full_row_checkbox_ == nullptr
            || show_grid_checkbox_ == nullptr
            || single_click_checkbox_ == nullptr
            || alternative_selection_checkbox_ == nullptr) {
            return;
        }
        ...
        display_settings.alternative_selection_mode = alternative_selection_checkbox_->isChecked();
        ...
        } else if (folders_work_specified_radio_ != nullptr && folders_work_specified_radio_->isChecked()) {
            work_dir_mode = static_cast<int>(WorkDirMode::kSpecified);
        }
        QString const work_path = folders_work_path_edit_ != nullptr
                                    ? QDir::fromNativeSeparators(folders_work_path_edit_->text().trimmed())
                                    : QString();
        bool const removable_only =
            folders_work_removable_only_checkbox_ != nullptr && folders_work_removable_only_checkbox_->isChecked();
        ...
        settings.setValue(QString::fromLatin1(kSettingsFmFoldersWorkForRemovableOnly), removable_only);
        ...
        if (extract_memory_limit_supported() && mem_limit_enable_checkbox_ != nullptr && mem_limit_spin_ != nullptr) {
        ...
        if (large_pages_supported() && use_large_pages_checkbox_ != nullptr) {
            z7::ui::runtime_support::save_large_pages_enabled(use_large_pages_checkbox_->isChecked());
    ...
    void OptionsDialog::update_memory_limit_controls_enabled() {
        bool const supported = extract_memory_limit_supported();
        bool const enabled =
            supported && mem_limit_enable_checkbox_ != nullptr && mem_limit_enable_checkbox_->isChecked();
