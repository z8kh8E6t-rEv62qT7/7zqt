...
namespace z7::ui::filemanager {
    ...
    namespace {
        ...
        QVector<bool> capture_context_item_checks(QListWidget const* list) {
            QVector<bool> checks;
            if (list == nullptr) {
                return checks;
            }
        ...
        QStringList capture_context_visible_actions(QListWidget const* list) {
            QStringList actions;
            if (list == nullptr) {
                return actions;
            }
            ...
            for (int i = 0; i < list->count(); ++i) {
                ...
                QString const action_id = item->data(Qt::UserRole).toString().trimmed();
        ...
        std::uint32_t context_menu_flags_from_variant(QVariant const& value) {
            bool ok = false;
            qulonglong const raw = value.toULongLong(&ok);
            return ok ? static_cast<std::uint32_t>(raw) : 0;
        }

        QString combine_tooltips(QString const& first, QString const& second) {
            QString const left = first.trimmed();
            QString const right = second.trimmed();
            if (left.isEmpty()) {
                return right;
            }
            if (right.isEmpty()) {
    ...
    void OptionsDialog::build_qt_page() {
        ...
        auto add_combo_row =
            [startup_group, group_layout](QString const& label_name, QString const& combo_name, QComboBox** combo_out) {
        ...
        add_combo_row(
            QStringLiteral("qtHiDpiPolicyLabel"), QStringLiteral("qtHiDpiPolicyCombo"), &qt_hidpi_policy_combo_);

        QStringList const styles = z7::platform::qt::available_qt_styles();
        if (qt_preferred_style_combo_ != nullptr) {
            qt_preferred_style_combo_->addItem(QStringLiteral("Fusion (default)"), QStringLiteral("Fusion"));
            qt_preferred_style_combo_->addItem(QStringLiteral("System default"), QString());
            for (QString const& style : styles) {
        ...
        if (qt_hidpi_policy_combo_ != nullptr) {
            ...
            add_hidpi_policy_combo_item(
                qt_hidpi_policy_combo_, QStringLiteral("Round"), Qt::HighDpiScaleFactorRoundingPolicy::Round);
            add_hidpi_policy_combo_item(qt_hidpi_policy_combo_,
                                        QStringLiteral("RoundPreferFloor"),
                                        Qt::HighDpiScaleFactorRoundingPolicy::RoundPreferFloor);
            add_hidpi_policy_combo_item(
                qt_hidpi_policy_combo_, QStringLiteral("Ceil"), Qt::HighDpiScaleFactorRoundingPolicy::Ceil);
            add_hidpi_policy_combo_item(
                qt_hidpi_policy_combo_, QStringLiteral("Floor"), Qt::HighDpiScaleFactorRoundingPolicy::Floor);
        }
        ...
        if (qt_preferred_style_combo_ != nullptr) {
            connect(qt_preferred_style_combo_, qOverload<int>(&QComboBox::currentIndexChanged), this, [this](int) {
                on_qt_setting_control_changed();
            });
        }
        if (qt_hidpi_policy_combo_ != nullptr) {
            connect(qt_hidpi_policy_combo_, qOverload<int>(&QComboBox::currentIndexChanged), this, [this](int) {
                on_qt_setting_control_changed();
            });
        }
    }
    ...
    void OptionsDialog::populate_context_menu_items() {
        ...
        QString const folder_label = z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(2320));
        QString const archive_label = z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(2321));
        auto const label_for_action = [&](QString const& action_id) -> QString {
            ...
            if (action_id == QString::fromLatin1(shell::kActionAddTo7z)) {
                return z7::ui::runtime_support::strip_mnemonic(
                    z7::ui::runtime_support::LF(2328, {archive_label + QStringLiteral(".7z")}));
            }
            if (action_id == QString::fromLatin1(shell::kActionAddToZip)) {
                return z7::ui::runtime_support::strip_mnemonic(
                    z7::ui::runtime_support::LF(2328, {archive_label + QStringLiteral(".zip")}));
            }
            if (action_id == QString::fromLatin1(shell::kActionCrcShaMenu)) {
                return z7::ui::runtime_support::J(QStringLiteral("shell.actions.crc_sha_menu")) + QStringLiteral(" >");
            }
            return action_id;
        };

        QString const unsupported_tip = combine_tooltips(finder_shell_supported() ? QString() : finder_shell_tooltip(),
        ...
        auto const push_option_item = [&option_items](QString const& action_id, QString const& label, bool supported) {
        ...
        for (QString const& action_id : shell::shell_integration_context_menu_action_ids()) {
            if (action_id == QString::fromLatin1(shell::kActionCrcShaMenu)) {
                push_option_item(QString(),
                                 z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(2329))
                                     + unsupported_suffix(),
                                 false);
                push_option_item(QString(),
                                 z7::ui::runtime_support::strip_mnemonic(
                                     z7::ui::runtime_support::LF(2330, {archive_label + QStringLiteral(".7z")}))
                                     + unsupported_suffix(),
                                 false);
                push_option_item(QString(),
                                 z7::ui::runtime_support::strip_mnemonic(
                                     z7::ui::runtime_support::LF(2330, {archive_label + QStringLiteral(".zip")}))
                                     + unsupported_suffix(),
                                 false);
            }
            push_option_item(action_id, with_finder_shell_suffix_if_unsupported(label_for_action(action_id)), true);
        }

        for (ContextMenuOptionItem const& option : option_items) {
    ...
    void OptionsDialog::load_seven_zip_settings() {
        ...
        initial_integrate_shell_ = settings.value(QString::fromLatin1(kSettingsOptionsIntegrateShell), true).toBool();
        initial_integrate_shell_32_ =
            settings.value(QString::fromLatin1(kSettingsOptionsIntegrateShell32), true).toBool();
        initial_cascaded_menu_ = settings.value(QString::fromLatin1(kSettingsOptionsCascadedMenu), true).toBool();
        initial_menu_icons_ = settings.value(QString::fromLatin1(kSettingsOptionsMenuIcons), false).toBool();
        ...
        if (initial_zone_id_policy_index_ < 0 || initial_zone_id_policy_index_ >= zone_id_combo_->count()) {
            initial_zone_id_policy_index_ = 0;
        }

        QString const context_menu_key = QString::fromLatin1(kSettingsOptionsContextMenu);
        QStringList const visible_actions = settings.contains(context_menu_key)
        ...
        QSignalBlocker const block_shell(integrate_shell_checkbox_);
        QSignalBlocker const block_shell32(integrate_shell_32_checkbox_);
        QSignalBlocker const block_cascaded(cascaded_menu_checkbox_);
        QSignalBlocker const block_icons(menu_icons_checkbox_);
        QSignalBlocker const block_elim(eliminate_dup_roots_checkbox_);
        QSignalBlocker const block_zone(zone_id_combo_);
        QSignalBlocker const block_context(context_items_list_);
        ...
        for (int i = 0; i < context_items_list_->count(); ++i) {
            if (QListWidgetItem* item = context_items_list_->item(i)) {
                QString const action_id = item->data(Qt::UserRole).toString();
                item->setCheckState(visible_actions.contains(action_id) ? Qt::Checked : Qt::Unchecked);
            }
        }
    ...
    bool OptionsDialog::apply_seven_zip_settings() {
        ...
        bool const current_integrate_shell = integrate_shell_checkbox_->isChecked();
        bool const current_integrate_shell_32 = integrate_shell_32_checkbox_->isChecked();
        bool const current_cascaded_menu = cascaded_menu_checkbox_->isChecked();
        bool const current_menu_icons = menu_icons_checkbox_->isChecked();
        bool const current_eliminate_dup_roots = eliminate_dup_roots_checkbox_->isChecked();
        int const current_zone_policy = zone_id_combo_->currentIndex();
        QVector<bool> const current_checks = capture_context_item_checks(context_items_list_);

        bool const changed = current_integrate_shell != initial_integrate_shell_
        ...
        settings.setValue(QString::fromLatin1(kSettingsOptionsIntegrateShell32), current_integrate_shell_32);
        ...
        settings.setValue(QString::fromLatin1(kSettingsOptionsElimDupExtract), current_eliminate_dup_roots);
        settings.setValue(QString::fromLatin1(kSettingsOptionsWriteZoneIdExtract), current_zone_policy);
        settings.setValue(QString::fromLatin1(kSettingsOptionsContextMenu),
                          QVariant::fromValue(
                              static_cast<qulonglong>(shell::shell_integration_context_menu_flags_from_visible_actions(
    ...
    void OptionsDialog::on_seven_zip_setting_control_changed() {
        ...
        QVector<bool> const current_checks = capture_context_item_checks(context_items_list_);
        seven_zip_settings_dirty_ = integrate_shell_checkbox_->isChecked() != initial_integrate_shell_
    ...
    void OptionsDialog::populate_languages() {
        ...
        QList<z7::ui::runtime_support::LangInfo> const langs =
            z7::ui::runtime_support::OfficialLangCatalog::instance().available_languages();
        int current_index = 0;

        for (z7::ui::runtime_support::LangInfo const& lang : langs) {
