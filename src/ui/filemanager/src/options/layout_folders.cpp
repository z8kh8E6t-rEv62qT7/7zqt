...
namespace z7::ui::filemanager {
    ...
    QWidget* OptionsDialog::create_folders_page() {
...
#ifdef Z7_TESTING
        folders_work_removable_only_checkbox_->setObjectName(QStringLiteral("foldersWorkRemovableOnlyCheckBox"));
#endif
        ...
        auto update_path_controls = [this]() {
            bool const enable_path =
                folders_work_specified_radio_ != nullptr && folders_work_specified_radio_->isChecked();
        ...
        if (folders_work_specified_radio_ != nullptr) {
            connect(folders_work_specified_radio_, &QRadioButton::toggled, this, [this, update_path_controls](bool) {
                update_path_controls();
                on_folders_setting_control_changed();
            });
        }
        if (folders_work_path_edit_ != nullptr) {
            connect(folders_work_path_edit_, &QLineEdit::textChanged, this, [this](QString const&) {
                on_folders_setting_control_changed();
            });
        }
        if (folders_work_path_browse_button_ != nullptr) {
        ...
        if (folders_work_removable_only_checkbox_ != nullptr) {
            connect(folders_work_removable_only_checkbox_, &QCheckBox::toggled, this, [this](bool) {
                on_folders_setting_control_changed();
            });
        }
    ...
    void OptionsDialog::browse_folders_work_path() {
        ...
        QString start_dir = QDir::fromNativeSeparators(folders_work_path_edit_->text().trimmed());
    ...
    void OptionsDialog::on_folders_setting_control_changed() {
        ...
        bool const enable_path = folders_work_specified_radio_->isChecked();
        folders_work_path_edit_->setEnabled(enable_path);
        folders_work_path_browse_button_->setEnabled(enable_path);
        ...
        QString const current_path = QDir::fromNativeSeparators(folders_work_path_edit_->text().trimmed());
        bool const current_removable_only = folders_work_removable_only_checkbox_->isChecked();

        folders_settings_dirty_ = current_mode != initial_folders_work_mode_
