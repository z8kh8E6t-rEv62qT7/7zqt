...
namespace z7::ui::filemanager {
    ...
    void OptionsDialog::load_qt_startup_settings() {
        ...
        z7::platform::qt::AppStartupConfig const config =
            z7::platform::qt::startup_config_with_persisted_overrides(z7::platform::qt::StartupAppKind::kFileManager);
        initial_qt_preferred_style_ = config.preferred_style;
        initial_qt_hidpi_policy_ = config.hidpi.scale_factor_rounding;

        QSignalBlocker const block_preferred(qt_preferred_style_combo_);
        QSignalBlocker const block_hidpi(qt_hidpi_policy_combo_);

        select_combo_value_or_insert(
            qt_preferred_style_combo_, initial_qt_preferred_style_, QStringLiteral(" (currently unavailable)"));

        int hidpi_index = qt_hidpi_policy_combo_->findData(static_cast<int>(initial_qt_hidpi_policy_), Qt::UserRole);
        if (hidpi_index < 0) {
    ...
    bool OptionsDialog::apply_qt_startup_settings() {
        ...
        QString const preferred = qt_preferred_style_combo_->currentData(Qt::UserRole).toString();
        Qt::HighDpiScaleFactorRoundingPolicy const hidpi_policy =
            hidpi_policy_from_combo(qt_hidpi_policy_combo_, initial_qt_hidpi_policy_);

        bool const changed = preferred != initial_qt_preferred_style_ || hidpi_policy != initial_qt_hidpi_policy_;
        if (!changed) {
            qt_settings_dirty_ = false;
            return false;
        }
        ...
        z7::platform::qt::AppStartupConfig startup =
            z7::platform::qt::default_startup_config(z7::platform::qt::StartupAppKind::kFileManager);
    ...
    void OptionsDialog::update_language_info() {
        ...
        QString const selected_id = language_combo_->currentData(Qt::UserRole).toString();
        ...
        for (z7::ui::runtime_support::LangInfo const& lang : langs) {
            ...
            for (QString const& comment : lang.comments) {
    ...
    void OptionsDialog::update_apply_button_state() {
        ...
        QString const selected_language_id =
            language_combo_ != nullptr ? language_combo_->currentData(Qt::UserRole).toString() : QString();
        bool const language_dirty =
        ...
        if (QPushButton* apply_button = button_box_->button(QDialogButtonBox::Apply)) {
            apply_button->setEnabled(runtime_settings_dirty_
    ...
    void OptionsDialog::load_associations_table_column_widths() {
        ...
        QVector<int> const default_widths = {120, 220, 220};

        z7::platform::qt::PortableSettings settings;
        QString const encoded =
            settings.value(QString::fromLatin1(kSettingsFmOptionsAssociationsColumns), QString()).toString().trimmed();
    ...
    void OptionsDialog::save_associations_table_column_widths() const {
        ...
        constexpr int kExpectedColumns = 3;
        QVector<int> const widths =
            column_width_persistence::capture_widths(associations_table_->horizontalHeader(), kExpectedColumns);
        if (widths.size() != kExpectedColumns) {
            return;
        }

        z7::platform::qt::PortableSettings settings;
        settings.setValue(QString::fromLatin1(kSettingsFmOptionsAssociationsColumns),
