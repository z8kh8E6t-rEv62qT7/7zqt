...
namespace z7::ui::filemanager {
    ...
    void OptionsDialog::on_apply() {
        ...
        QString const current_language_id = z7::ui::runtime_support::OfficialLangCatalog::instance().current_language();
    ...
    void OptionsDialog::on_editor_command_changed() {
        editor_commands_dirty_ = viewer_edit_ != nullptr
                              && editor_edit_ != nullptr
                              && diff_edit_ != nullptr
    ...
    void OptionsDialog::on_qt_setting_control_changed() {
        ...
        qt_settings_dirty_ = preferred != initial_qt_preferred_style_ || policy != initial_qt_hidpi_policy_;
        update_apply_button_state();
    }
    ...
    void OptionsDialog::setup_ui() {
        ...
        auto const mark_with_tip = [](QWidget* widget, QString const& tip) {
            if (widget == nullptr) {
                return;
            }
        ...
        connect(language_combo_, qOverload<int>(&QComboBox::currentIndexChanged), this, [this](int) {
            on_language_selection_changed();
        });

        if (integrate_shell_checkbox_ != nullptr) {
            connect(integrate_shell_checkbox_, &QCheckBox::toggled, this, [this](bool) {
                on_seven_zip_setting_control_changed();
            });
        }
        if (integrate_shell_32_checkbox_ != nullptr) {
            connect(integrate_shell_32_checkbox_, &QCheckBox::toggled, this, [this](bool) {
                on_seven_zip_setting_control_changed();
            });
        }
        if (cascaded_menu_checkbox_ != nullptr) {
            connect(cascaded_menu_checkbox_, &QCheckBox::toggled, this, [this](bool) {
                on_seven_zip_setting_control_changed();
            });
        }
        if (menu_icons_checkbox_ != nullptr) {
            connect(menu_icons_checkbox_, &QCheckBox::toggled, this, [this](bool) {
                on_seven_zip_setting_control_changed();
            });
        }
        if (eliminate_dup_roots_checkbox_ != nullptr) {
            connect(eliminate_dup_roots_checkbox_, &QCheckBox::toggled, this, [this](bool) {
                on_seven_zip_setting_control_changed();
            });
        }
        if (zone_id_combo_ != nullptr) {
            connect(zone_id_combo_, qOverload<int>(&QComboBox::currentIndexChanged), this, [this](int) {
                on_seven_zip_setting_control_changed();
            });
        }
        if (context_items_list_ != nullptr) {
            connect(context_items_list_, &QListWidget::itemChanged, this, [this](QListWidgetItem*) {
                on_seven_zip_setting_control_changed();
            });
        }
