...
namespace z7::ui::filemanager {
    ...
    void OptionsDialog::update_texts() {
        bool const finder_supported = finder_shell_supported();
        bool const mem_supported = extract_memory_limit_supported();
        bool const large_pages_available = large_pages_supported();
        QString const win_only_tip = windows_only_tooltip();
        QString const win_only_tip_or_empty = windows_only_supported() ? QString() : win_only_tip;
        QString const finder_tip = finder_shell_tooltip();
        QString const finder_tip_or_empty = finder_supported ? QString() : finder_tip;
        QString const mem_tip = extract_memory_limit_tooltip();
        QString const mem_tip_or_empty = mem_supported ? QString() : mem_tip;
        QString const large_pages_tip = large_pages_tooltip();
        QString const large_pages_tip_or_empty = large_pages_available ? QString() : large_pages_tip;
        QString const associations_tip = qt_filemanager_unsupported_tooltip();
        QString const show_system_menu_tip = [&]() {
        ...
        tabs_->setTabText(0,
                          with_windows_only_suffix_if_unsupported(
        ...
        associations_table_->setHorizontalHeaderLabels(
            {z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(1020)),
             current_user_label(),
             z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(2202))});

        if (auto* integrate_shell = findChild<QCheckBox*>(QStringLiteral("integrateShellCheckBox"))) {
            integrate_shell->setText(with_finder_shell_suffix_if_unsupported(
                z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(2301))));
            integrate_shell->setToolTip(finder_tip_or_empty);
        }
        if (auto* integrate_shell_32 = findChild<QCheckBox*>(QStringLiteral("integrateShell32CheckBox"))) {
        ...
        if (auto* cascaded_menu = findChild<QCheckBox*>(QStringLiteral("cascadedMenuCheckBox"))) {
            cascaded_menu->setText(with_finder_shell_suffix_if_unsupported(
                z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(2302))));
            cascaded_menu->setToolTip(finder_tip_or_empty);
        }
        if (auto* menu_icons = findChild<QCheckBox*>(QStringLiteral("menuIconsCheckBox"))) {
            menu_icons->setText(with_finder_shell_suffix_if_unsupported(
                z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(2304))));
            menu_icons->setToolTip(finder_tip_or_empty);
        }
        if (auto* eliminate_dup_roots = findChild<QCheckBox*>(QStringLiteral("eliminateDupRootsCheckBox"))) {
            eliminate_dup_roots->setText(with_finder_shell_suffix_if_unsupported(
                z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(3430))));
            eliminate_dup_roots->setToolTip(finder_tip_or_empty);
        }
        if (auto* zone_label = findChild<QLabel*>(QStringLiteral("zoneIdLabel"))) {
            zone_label->setText(with_windows_only_suffix_if_unsupported(
                z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(3440))));
        ...
        if (auto* group = findChild<QGroupBox*>(QStringLiteral("contextItemsGroup"))) {
            group->setTitle(with_finder_shell_suffix_if_unsupported(
                z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(2303))));
        ...
        if (show_real_icons_checkbox_ != nullptr) {
            show_real_icons_checkbox_->setText(
                z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(2502)));
        }
        ...
        if (alternative_selection_checkbox_ != nullptr) {
            alternative_selection_checkbox_->setText(
                z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(2507)));
        }
        if (show_system_menu_checkbox_ != nullptr) {
            show_system_menu_checkbox_->setText(with_windows_only_suffix_if_unsupported(
                z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(2503))));
            show_system_menu_checkbox_->setToolTip(show_system_menu_tip);
        }
        if (use_large_pages_checkbox_ != nullptr) {
            use_large_pages_checkbox_->setText(with_large_pages_suffix_if_unsupported(
                z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(2508))));
            use_large_pages_checkbox_->setToolTip(large_pages_tip_or_empty);
        }
        if (mem_usage_label_ != nullptr) {
            mem_usage_label_->setText(with_extract_memory_limit_suffix_if_unsupported(
                z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(7816))));
        ...
        if (auto* group = findChild<QGroupBox*>(QStringLiteral("qtStartupGroup"))) {
            group->setTitle(z7::ui::runtime_support::J(QStringLiteral("ui.options.qt_startup.group_title")));
        }
        if (auto* label = findChild<QLabel*>(QStringLiteral("qtPreferredStyleLabel"))) {
            label->setText(z7::ui::runtime_support::J(QStringLiteral("ui.options.qt_startup.preferred_style_label")));
        }
        if (auto* label = findChild<QLabel*>(QStringLiteral("qtHiDpiPolicyLabel"))) {
            label->setText(z7::ui::runtime_support::J(QStringLiteral("ui.options.qt_startup.hidpi_policy_label")));
        }
        if (qt_restart_hint_label_ != nullptr) {
            qt_restart_hint_label_->setText(
                z7::ui::runtime_support::J(QStringLiteral("ui.options.qt_startup.restart_hint")));
        }

        if (viewer_label_ != nullptr) {
            viewer_label_->setText(
                ensure_colon_suffix(z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(543))));
        }
        ...
        if (folders_working_folder_label_ != nullptr) {
            folders_working_folder_label_->setText(
                z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(2401)));
        }
        if (folders_work_system_radio_ != nullptr) {
            folders_work_system_radio_->setText(
                z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(2402)));
        }
        if (folders_work_current_radio_ != nullptr) {
            folders_work_current_radio_->setText(
                z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(2403)));
        }
        if (folders_work_specified_radio_ != nullptr) {
            folders_work_specified_radio_->setText(
                z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(2404)));
        }
        if (folders_work_removable_only_checkbox_ != nullptr) {
            folders_work_removable_only_checkbox_->setText(
                z7::ui::runtime_support::strip_mnemonic(z7::ui::runtime_support::L(2405)));
        }
        ...
        if (QPushButton* apply_button = button_box_->button(QDialogButtonBox::Apply)) {
            apply_button->setText(z7::ui::runtime_support::J(QStringLiteral("ui.options.buttons.apply")));
        }
