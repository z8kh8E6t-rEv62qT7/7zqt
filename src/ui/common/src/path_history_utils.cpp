...
namespace z7::ui::common {
    ...
    QStringList normalized_path_history(QStringList history, QString const& new_item, int max_items) {
        ...
        QString const normalized_new = normalize_path_history_entry(new_item);
        if (!normalized_new.isEmpty()) {
            out << normalized_new;
        }

        for (QString const& raw : history) {
            QString const normalized = normalize_path_history_entry(raw);
            if (normalized.isEmpty() || out.contains(normalized, path_history_case_sensitivity())) {
                continue;
            }
            out << normalized;
        }
