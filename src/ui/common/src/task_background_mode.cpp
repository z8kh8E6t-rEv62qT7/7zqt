...
namespace z7::ui::common {
    namespace {
        ...
        int current_priority(int which, bool* ok) {
            ...
            errno = 0;
            int const value = getpriority(which, 0);
            if (value == -1 && errno != 0) {
                return 0;
            }
            if (ok != nullptr) {
    ...
    TaskBackgroundModeController::TaskBackgroundModeController() {
        ...
        bool ok = false;
        int const background_state = current_priority(PRIO_DARWIN_PROCESS, &ok);
        if (ok) {
            has_original_state_ = true;
...
#elif defined(Q_OS_UNIX)
        bool ok = false;
        int const nice_value = current_priority(PRIO_PROCESS, &ok);
        if (ok) {
            has_original_state_ = true;
    ...
    void TaskBackgroundModeController::set_backgrounded(bool backgrounded) {
        ...
        const DWORD target = backgrounded
                               ? IDLE_PRIORITY_CLASS
                               : static_cast<DWORD>(has_original_state_ ? original_state_ : NORMAL_PRIORITY_CLASS);
        ::SetPriorityClass(::GetCurrentProcess(), target);
#elif defined(Q_OS_DARWIN) && defined(PRIO_DARWIN_PROCESS) && defined(PRIO_DARWIN_BG)
        int const target = backgrounded ? PRIO_DARWIN_BG : 0;
        setpriority(PRIO_DARWIN_PROCESS, 0, target);
#elif defined(Q_OS_UNIX)
        int const target = backgrounded ? std::max(has_original_state_ ? original_state_ : 0, 10)
    ...
    void TaskBackgroundModeController::restore() {
        ...
        if (has_original_state_) {
            ::SetPriorityClass(::GetCurrentProcess(), static_cast<DWORD>(original_state_));
        }
#elif defined(Q_OS_DARWIN) && defined(PRIO_DARWIN_PROCESS) && defined(PRIO_DARWIN_BG)
        if (has_original_state_) {
            setpriority(PRIO_DARWIN_PROCESS, 0, original_state_ != 0 ? PRIO_DARWIN_BG : 0);
        }
#elif defined(Q_OS_UNIX)
        if (has_original_state_) {
