...
namespace z7::ui::gui {
        ...
        QString memory_limit_dialog_message(z7::app::MemoryLimitPrompt const& prompt, uint64_t persisted_limit_bytes) {
            if (prompt.required_usage_bytes != 0
                ...
                if (!prompt.file_path.empty()) {
                    message += QStringLiteral("\nFile: %1")
                                   .arg(QDir::toNativeSeparators(
                                       z7::ui::archive_support::from_native_string(prompt.file_path)));
                }
                return message;
            }
    ...
    z7::app::OverwriteDecision show_overwrite_prompt_dialog(QWidget* parent, z7::app::OverwritePrompt const& prompt) {
        ...
        QString const existing =
            QDir::toNativeSeparators(z7::ui::archive_support::from_native_string(prompt.existing_path));
        QString const incoming =
            QDir::toNativeSeparators(z7::ui::archive_support::from_native_string(prompt.incoming_path));
