...
namespace z7::ui::gui {
    namespace {
        ...
        QStringList
        normalized_history(QStringList history, QString const& new_item, int max_items = kExtractHistoryMax) {
            return z7::ui::common::normalized_path_history(history, new_item, max_items);
        }
        ...
        int original_value_from_path_mode_data(QString const& value) {
            QString const normalized = to_lower_ascii(value.trimmed());
        ...
        int original_value_from_overwrite_data(QString const& value) {
        ...
        QString planned_extract_history_path(ExtractCommandOptions const& options) {
            QString const output_dir = normalized_path(z7::ui::archive_support::from_native_string(options.output_dir));
            if (!options.split_dest_enabled) {
                return output_dir;
            }
            QString const split_name =
                normalized_path(z7::ui::archive_support::from_native_string(options.split_dest_name));
    ...
    ExtractDialog::ExtractDialog(ExtractCommandOptions const& initial, QWidget* parent) : QDialog(parent) {
        QString title = lang_or(3400);
        QString const archive_path = z7::ui::archive_support::from_native_string(initial.archive_name).trimmed();
        QString const archive_file_name = QFileInfo(archive_path).fileName();
        ...
        eliminate_dup_checkbox_ = new QCheckBox(lang_or(3430), this);
#ifdef Z7_TESTING
        ...
        overwrite_combo_->addItem(lang_or(3422), QStringLiteral("-aoa"));
        overwrite_combo_->addItem(lang_or(3423), QStringLiteral("-aos"));
        overwrite_combo_->addItem(lang_or(3424), QStringLiteral("-aou"));
        overwrite_combo_->addItem(lang_or(3425), QStringLiteral("-aot"));
        ...
        show_password_checkbox_ = new QCheckBox(lang_or(3803), password_group_);
#ifdef Z7_TESTING
        ...
        restore_security_checkbox_ = new QCheckBox(lang_or(3431), this);
#ifdef Z7_TESTING
        restore_security_checkbox_->setObjectName(QStringLiteral("extractRestoreSecurityCheckBox"));
#endif
        z7::ui::runtime_support::PlatformRestrictionUi const restore_security_ui =
            z7::ui::runtime_support::platform_restriction_ui(restore_security_checkbox_->text(),
                                                             z7::ui::runtime_support::PlatformSupport::kWindowsOnly);
        ...
        connect(path_mode_combo_, qOverload<int>(&QComboBox::currentIndexChanged), this, [this](int) {
            bool const no_paths = path_mode_combo_->currentData().toString() == QStringLiteral("no");
        ...
        connect(browse_button_, &QPushButton::clicked, this, [this]() {
            QString const selected =
                QFileDialog::getExistingDirectory(this, lang_or(3402), output_dir_combo_->currentText());
            if (!selected.isEmpty()) {
                output_dir_combo_->setEditText(QDir::toNativeSeparators(selected));
            }
        });
        connect(buttons_, &QDialogButtonBox::helpRequested, this, [this]() {
            QMessageBox::information(this, lang_or(3400), lang_or(3402));
        });
        connect(buttons_, &QDialogButtonBox::accepted, this, [this]() {
            ExtractCommandOptions const current = options();
        ...
        if (split_name.isEmpty() && !archive_file_name.isEmpty()) {
            QString const expected = z7::shell_integration::shell_integration_extract_subfolder_name(archive_file_name);
            QString normalized_output = output_dir;
            while (normalized_output.endsWith(QLatin1Char('/'))) {
                normalized_output.chop(1);
            }
            QFileInfo const out_info(normalized_output);
        ...
        if (!initial.overwrite_switch.empty()) {
            int const index = find_combo_data(overwrite_combo_,
                                              z7::ui::archive_support::from_native_string(initial.overwrite_switch));
            if (index >= 0) {
        ...
        if (!initial.path_mode.empty()) {
            int const index =
                find_combo_data(path_mode_combo_, z7::ui::archive_support::from_native_string(initial.path_mode));
            if (index >= 0) {
    ...
    void ExtractDialog::load_settings() {
        ...
        QStringList const history = normalized_history(
            settings.value(QString::fromLatin1(kSettingsExtractPathHistory)).toStringList(), QString());
        ...
        split_dest_checkbox_->setChecked(settings.value(QString::fromLatin1(kSettingsExtractSplitDest), true).toBool());

        QString const path_mode = path_mode_data_from_original_value(
            settings.value(QString::fromLatin1(kSettingsExtractMode), kOriginalPathModeFullPaths).toInt());
        int const path_mode_index = find_combo_data(path_mode_combo_, path_mode);
        if (path_mode_index >= 0) {
            path_mode_combo_->setCurrentIndex(path_mode_index);
        }

        QString const overwrite = overwrite_data_from_original_value(
            settings.value(QString::fromLatin1(kSettingsExtractOverwriteMode), kOriginalOverwriteAsk).toInt());
        int const overwrite_index = find_combo_data(overwrite_combo_, overwrite);
    ...
    void ExtractDialog::save_settings(ExtractCommandOptions const& options) const {
        z7::platform::qt::PortableSettings settings;
        QStringList const history = settings.value(QString::fromLatin1(kSettingsExtractPathHistory)).toStringList();
        settings.setValue(QString::fromLatin1(kSettingsExtractPathHistory),
                          normalized_history(history, planned_extract_history_path(options)));
        ...
        settings.setValue(QString::fromLatin1(kSettingsExtractMode),
                          original_value_from_path_mode_data(QString::fromStdString(options.path_mode)));
        settings.setValue(QString::fromLatin1(kSettingsExtractOverwriteMode),
                          original_value_from_overwrite_data(QString::fromStdString(options.overwrite_switch)));
        settings.setValue(QString::fromLatin1(kSettingsExtractElimDup), options.eliminate_root_duplication);
        settings.setValue(QString::fromLatin1(kSettingsExtractShowPassword), options.show_password);
        settings.setValue(QString::fromLatin1(kSettingsExtractSecurity), options.restore_file_security);
        settings.sync();
    }

    void ExtractDialog::update_password_echo_mode() {
        bool const show = show_password_checkbox_->isChecked();
