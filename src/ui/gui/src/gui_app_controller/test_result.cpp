...
namespace z7::ui::gui::gui_app_controller_helpers {
    namespace {
        ...
        void merge_hash_summary(z7::app::OperationResult const& result, ParsedTestSummary* summary) {
            if (summary == nullptr || !result.hash_summary.has_value()) {
                return;
            }
            z7::app::HashSummary const& hash = *result.hash_summary;
        ...
        ParsedTestSummary build_test_summary(z7::app::OperationResult const& result, uint64_t archive_count_hint) {
        ...
        QString build_test_result_message_from_summary(ParsedTestSummary const& summary) {
            QStringList lines;
            lines << QStringLiteral("%1: %2").arg(localized(3907)).arg(summary.archives.value_or(1));
            if (summary.physical_size.has_value()) {
                lines << QStringLiteral("%1: %2")
                             .arg(localized(1044))
                             .arg(z7::ui::archive_support::format_dual_size(*summary.physical_size));
            }
            if (summary.folders.has_value()) {
                lines << QStringLiteral("%1: %2").arg(localized(1031)).arg(*summary.folders);
            }
            if (summary.files.has_value()) {
                lines << QStringLiteral("%1: %2").arg(localized(1032)).arg(*summary.files);
            }
        ...
        void show_compact_result_dialog(QWidget* parent, QString const& title, QString const& text) {
            if (parent == nullptr) {
    ...
    QString build_test_result_message(z7::app::OperationResult const& result, uint64_t archive_count_hint) {
        ParsedTestSummary const summary = build_test_summary(result, archive_count_hint);
