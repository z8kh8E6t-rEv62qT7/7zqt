...
namespace z7::ui::gui::benchmark_internal {
    ...
    void set_metrics(QLabel* size,
                     ...
                     QLabel* rpu,
                     QLabel* rating,
                     QString const& size_text,
                     QString const& speed_text,
                     QString const& usage_text,
                     QString const& rpu_text,
                     QString const& rating_text) {
        if (size != nullptr) {
    ...
    void add_metric_header(QGridLayout* grid, int row) {
        QStringList const columns = {lang_or(1007), lang_or(3903), lang_or(7608), lang_or(7609), lang_or(7604)};
    ...
    bool parse_uint64_decimal(QString const& text, uint64_t& out) {
        QString const t = text.trimmed();
        if (t.isEmpty()) {
            return false;
        }
        uint64_t value = 0;
    ...
    uint64_t parse_size_to_bytes_or_default(QString const& raw, uint64_t default_value) {
        ...
        uint64_t mult = 1;
        QChar const suffix = t.back();
        if (suffix == QLatin1Char('k')
            || suffix == QLatin1Char('m')
            || suffix == QLatin1Char('g')
            || suffix == QLatin1Char('t')) {
            t.chop(1);
            if (suffix == QLatin1Char('k')) {
                mult = 1ULL << 10;
    ...
    uint32_t parse_uint32_or_default(QString const& raw, uint32_t default_value) {
        uint64_t parsed = 0;
        if (!parse_uint64_decimal(raw, parsed)
            || parsed == 0
    ...
    QString mips_to_gips(QString const& mips) {
        bool ok = false;
        qulonglong const value = mips.toULongLong(&ok);
        if (!ok) {
            return mips;
        }
        qulonglong const whole = value / 1000ULL;
        qulonglong const rem = value % 1000ULL;
        return QStringLiteral("%1.%2").arg(whole).arg(rem, 3, 10, QLatin1Char('0'));
    }

    double parse_double_or_zero(QString const& text) {
        bool ok = false;
        double const value = text.toDouble(&ok);
        return ok ? value : 0.0;
    }
    ...
    QString format_elapsed_text(qint64 elapsed_ms, bool finished) {
        ...
        qint64 const secs = elapsed_ms / 1000;
        qint64 const rem = elapsed_ms % 1000;
        return QStringLiteral("%1.%2 s").arg(secs).arg(rem, 3, 10, QLatin1Char('0'));
    }
