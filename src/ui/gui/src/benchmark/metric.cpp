...
namespace z7::ui::gui {
    ...
    void BenchmarkDialog::apply_typed_snapshot(z7::app::BenchmarkTypedSnapshot const& snapshot) {
        if (total_mode_ui_ && snapshot.kind != z7::app::BenchmarkSnapshotKind::kFrequency) {
            return;
        }
            ...
            case z7::app::BenchmarkSnapshotKind::kFrequency:
                {
                    if (!total_mode_ui_) {
                        QString const line =
                            z7::ui::archive_support::from_native_string(snapshot.frequency_line).trimmed();
            ...
            case z7::app::BenchmarkSnapshotKind::kDictionaryPass:
                {
                    ...
                    QString const compress_size_text = snapshot.metrics.has_compress_size
                                                         ? bytes_to_display_size(snapshot.metrics.compress_size_bytes)
                                                         : QStringLiteral("...");
                    QString const decompress_size_text =
                        snapshot.metrics.has_decompress_size
                            ? bytes_to_display_size(snapshot.metrics.decompress_size_bytes)
                            : QStringLiteral("...");
                    set_metrics(
                        compress_current_size_,
                        compress_current_speed_,
                        compress_current_usage_,
                        compress_current_rpu_,
                        compress_current_rating_,
                        compress_size_text,
                        QStringLiteral("%1 KB/s").arg(snapshot.metrics.compress_speed_kb_per_sec),
                        QStringLiteral("%1%").arg(snapshot.metrics.compress_cpu_usage_percent),
                        mips_to_gips(QString::number(snapshot.metrics.compress_rpu_mips)) + QStringLiteral(" GIPS"),
                        mips_to_gips(QString::number(snapshot.metrics.compress_rating_mips)) + QStringLiteral(" GIPS"));
                    set_metrics(decompress_current_size_,
                                ...
                                QStringLiteral("%1 KB/s").arg(snapshot.metrics.decompress_speed_kb_per_sec),
                                QStringLiteral("%1%").arg(snapshot.metrics.decompress_cpu_usage_percent),
            ...
            case z7::app::BenchmarkSnapshotKind::kAveragePass:
                {
                    ...
                    QString const compress_size_text =
                        snapshot.metrics.has_compress_size
                            ? bytes_to_display_size(snapshot.metrics.compress_size_bytes)
                            : (compress_current_size_ == nullptr ? QStringLiteral("...")
                                                                 : compress_current_size_->text());
                    QString const decompress_size_text =
                        snapshot.metrics.has_decompress_size
                            ? bytes_to_display_size(snapshot.metrics.decompress_size_bytes)
                            : (decompress_current_size_ == nullptr ? QStringLiteral("...")
                                                                   : decompress_current_size_->text());
                    set_metrics(
                        compress_result_size_,
                    ...
                    current_pass_compr_rating_ = mips_to_gips(QString::number(snapshot.metrics.compress_rating_mips));
            ...
            case z7::app::BenchmarkSnapshotKind::kTotalRating:
                {
                    if (total_usage_label_ != nullptr) {
                        total_usage_label_->setText(QStringLiteral("%1%").arg(snapshot.total_cpu_usage_percent));
                    }
                    if (total_rpu_label_ != nullptr) {
                        total_rpu_label_->setText(mips_to_gips(QString::number(snapshot.total_rpu_mips))
                                                  + QStringLiteral(" GIPS"));
                    }
                    if (total_rating_label_ != nullptr) {
                        total_rating_label_->setText(mips_to_gips(QString::number(snapshot.total_rating_mips))
                                                     + QStringLiteral(" GIPS"));
                    }
            ...
            case z7::app::BenchmarkSnapshotKind::kRamSize:
                {
                    ...
                    QString const value = QStringLiteral("%1 MB").arg(snapshot.ram_size_mb);
                    QString const current = memory_label_->text();
                    if (current.isEmpty() || current == QStringLiteral("...")) {
                        memory_label_->setText(value);
                    } else if (current.contains(QLatin1Char('/'))) {
                        QString const usage = current.split(QLatin1Char('/')).front().trimmed();
            ...
            case z7::app::BenchmarkSnapshotKind::kRamUsage:
                {
                    ...
                    QString const value = QStringLiteral("%1 MB").arg(snapshot.ram_usage_mb);
                    QString const current = memory_label_->text();
                    if (current.isEmpty() || current == QStringLiteral("...")) {
                        memory_label_->setText(value);
                    } else if (current.contains(QLatin1Char('/'))) {
                        QString const total = current.split(QLatin1Char('/')).back().trimmed();
    ...
    void BenchmarkDialog::apply_typed_summary(z7::app::BenchmarkTypedSummary const& summary) {
        if (summary.has_total_rating) {
            if (total_usage_label_ != nullptr) {
                total_usage_label_->setText(QStringLiteral("%1%").arg(summary.total_cpu_usage_percent));
            }
            if (total_rpu_label_ != nullptr) {
                total_rpu_label_->setText(mips_to_gips(QString::number(summary.total_rpu_mips))
                                          + QStringLiteral(" GIPS"));
            }
            if (total_rating_label_ != nullptr) {
                total_rating_label_->setText(mips_to_gips(QString::number(summary.total_rating_mips))
                                             + QStringLiteral(" GIPS"));
            }
        }
        if (memory_label_ != nullptr && summary.has_ram_usage && summary.has_ram_size) {
            memory_label_->setText(QStringLiteral("%1 MB / %2 MB").arg(summary.ram_usage_mb).arg(summary.ram_size_mb));
        }
    }
    ...
    void BenchmarkDialog::update_elapsed_label() {
        ...
        elapsed_label_->setText(format_elapsed_text(elapsed_clock_.elapsed(), !benchmark_running_));
    }
    ...
    void BenchmarkDialog::refresh_memory_estimate() {
        ...
        request.thread_count =
            threads_combo_ == nullptr
                ? std::string("auto")
                : z7::ui::archive_support::to_native_string(threads_combo_->currentData().toString().trimmed());
        request.dictionary_size =
            dictionary_combo_ == nullptr
                ? std::string("32m")
                : z7::ui::archive_support::to_native_string(dictionary_combo_->currentData().toString().trimmed());
        request.total_mode = false;

        z7::app::BenchmarkMemoryEstimate const estimate = z7::app::estimate_benchmark_memory(request);
        if (!estimate.ok) {
        ...
        if (estimate.ram_defined) {
            memory_label_->setText(QStringLiteral("%1 / %2").arg(mb_text(estimate.estimated_usage_bytes),
        ...
        if (error_label_ != nullptr && !benchmark_running_) {
            if (!estimate.within_limit) {
                error_label_->setText(z7::ui::runtime_support::J(QStringLiteral("ui.benchmark.memory_limit_exceeded")));
                error_label_->setVisible(true);
            } else if (error_label_->text()
                       == z7::ui::runtime_support::J(QStringLiteral("ui.benchmark.memory_limit_exceeded"))) {
                error_label_->clear();
    ...
    uint32_t BenchmarkDialog::selected_iterations() const {
        ...
        bool ok = false;
        int const data_value = iterations_combo_->currentData().toInt(&ok);
        if (ok && data_value > 0) {
    ...
    void BenchmarkDialog::append_pass_row() {
        ...
        QString const row = QStringLiteral("%1 %2 %3 %4")
        ...
        QString const cpu_number = current_pass_cpu_usage_.endsWith(QLatin1Char('%'))
    ...
    void BenchmarkDialog::rebuild_log_view() {
        ...
        if (!pass_rows_.isEmpty()) {
            ...
            if (benchmark_finished_ && summed_pass_count_ > 0) {
                ...
                double const inv = 1.0 / static_cast<double>(summed_pass_count_);
                QString const avg_row = QStringLiteral("%1 %2 %3 %4%")
