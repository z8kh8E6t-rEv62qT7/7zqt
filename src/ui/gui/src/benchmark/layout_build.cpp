...
namespace z7::ui::gui {
    ...
    BenchmarkDialog::BenchmarkDialog(BenchmarkCommandOptions const& initial, QWidget* parent) :
        ...
        z7::app::BenchmarkSystemInfo const sys = z7::app::query_benchmark_system_info();
        ...
        if (total_mode_ui_) {
            ...
            uint64_t const initial_dict_bytes = parse_size_to_bytes_or_default(
                z7::ui::archive_support::from_native_string(initial.dictionary_size), kDefaultDictBytes);
            int dict_cur = 0;
            for (uint32_t i = (kMinDicLogSize - 1) * 2; i <= (32 - 1) * 2; ++i) {
                uint64_t const dict = (static_cast<uint64_t>(2 + (i & 1)) << (i / 2));
                int const index = dictionary_combo_->count();
                dictionary_combo_->addItem(bytes_to_display_size(dict), bytes_to_switch_size(dict));
                dictionary_combo_->setItemData(index, QVariant::fromValue<qulonglong>(dict), Qt::UserRole + 1);
            ...
            uint32_t const cpu_threads = std::max<uint32_t>(1, sys.process_threads);
            uint32_t num_threads = cpu_threads;
            QString const initial_thread_count =
                z7::ui::archive_support::from_native_string(initial.thread_count).trimmed();
            if (!initial_thread_count.isEmpty() && initial_thread_count.toLower() != QStringLiteral("auto")) {
            ...
            uint32_t const num_threads_combo =
            ...
            for (; v <= num_threads_combo;) {
                ...
                uint32_t const v_next = v + (v < 2 ? 1 : 2);
                if (v <= num_threads && (num_threads < v_next || v_next > num_threads_combo)) {
                    if (v != num_threads) {
                        index = threads_combo_->count();
                        threads_combo_->addItem(QString::number(num_threads), QString::number(num_threads));
                    }
                    cur = index;
                }
                v = v_next;
            ...
            QString const hardware_threads =
                z7::ui::archive_support::from_native_string(sys.hardware_threads).trimmed();
            hardware_threads_label_ = new QLabel(
                hardware_threads.isEmpty() ? QStringLiteral("/ %1").arg(cpu_threads) : hardware_threads, this);
            ...
            compress_grid->addWidget(new QLabel(lang_or(7606), compress_group), 1, 0);
            ...
            compress_grid->addWidget(new QLabel(lang_or(7607), compress_group), 2, 0);
            ...
            decompress_grid->addWidget(new QLabel(lang_or(7606), decompress_group), 1, 0);
            ...
            decompress_grid->addWidget(new QLabel(lang_or(7607), decompress_group), 2, 0);
            ...
            if (iter_index < 0) {
                ...
                iterations_combo_->addItem(QString::number(target_iterations), static_cast<int>(target_iterations));
            }
            ...
            auto* cpu_label = new QLabel(z7::ui::archive_support::from_native_string(sys.cpu).trimmed(), this);
            cpu_label->setAlignment(Qt::AlignRight | Qt::AlignVCenter);
            left_col->addWidget(cpu_label);

            auto* feature_row = new QHBoxLayout();
            auto* feature_label =
                new QLabel(z7::ui::archive_support::from_native_string(sys.cpu_feature).trimmed(), this);
            auto* ver_label = new QLabel(z7::ui::archive_support::from_native_string(sys.version).trimmed(), this);
            ...
            if (!sys.sys1.empty()) {
                left_col->addWidget(new QLabel(z7::ui::archive_support::from_native_string(sys.sys1).trimmed(), this));
            }
            if (!sys.sys2.empty() && sys.sys2 != sys.sys1) {
                left_col->addWidget(new QLabel(z7::ui::archive_support::from_native_string(sys.sys2).trimmed(), this));
            }
            ...
            connect(dictionary_combo_, &QComboBox::currentTextChanged, this, &BenchmarkDialog::on_selector_changed);
            connect(threads_combo_, &QComboBox::currentTextChanged, this, &BenchmarkDialog::on_selector_changed);
            connect(iterations_combo_, &QComboBox::currentTextChanged, this, &BenchmarkDialog::on_selector_changed);
