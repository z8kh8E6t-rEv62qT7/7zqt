...
namespace z7::ui::gui {
    ...
    namespace {
        ...
        class BenchmarkOperationDelegate final : public z7::ui::archive_support::OwnerRelayDelegate<BenchmarkDialog> {
        public:
            BenchmarkOperationDelegate(BenchmarkDialog* owner,
                                       std::function<void(z7::app::OperationOutcome const&)> finished_cb) :
                z7::ui::archive_support::OwnerRelayDelegate<BenchmarkDialog>(
                    owner, std::move(finished_cb), nullptr, z7::ui::archive_support::MissingTargetPolicy::kDrop) {}
            ...
            void on_log(z7::app::ArchiveLog const& log) override {
                ...
                post_to_owner(
                    [event = std::move(event)](BenchmarkDialog* owner) { owner->dispatch_operation_event(event); });
            }
    ...
    void BenchmarkDialog::dispatch_operation_event(z7::app::OperationEvent const& event) {
    ...
    BenchmarkCommandOptions BenchmarkDialog::options() const {
        ...
        if (threads_combo_ != nullptr) {
            ...
            if (out.thread_count.empty()) {
                out.thread_count = z7::ui::archive_support::to_native_string(threads_combo_->currentText().trimmed());
            }
        }
        ...
        if (dictionary_combo_ != nullptr) {
            ...
            if (out.dictionary_size.empty()) {
                out.dictionary_size =
                    z7::ui::archive_support::to_native_string(dictionary_combo_->currentText().trimmed());
            }
        }
    ...
    void BenchmarkDialog::start_benchmark() {
        ...
        BenchmarkCommandOptions const current = options();
        ...
        uint32_t const run_iterations = std::max<uint32_t>(1, request.iterations);
        if (!total_mode_ui_) {
    ...
    void BenchmarkDialog::start_benchmark_pass(z7::app::BenchmarkRequest const& request) {
        auto delegate = std::make_shared<BenchmarkOperationDelegate>(
            this, [this](z7::app::OperationOutcome const& outcome) { on_benchmark_pass_finished(outcome); });
    ...
    void BenchmarkDialog::on_benchmark_pass_finished(z7::app::OperationOutcome const& outcome) {
        active_task_ = z7::app::ArchiveSession();
        active_delegate_.reset();

        auto const pass_result = z7::app::outcome_payload_as<z7::app::BenchmarkResult>(outcome);
        bool const pass_ok = pass_result.has_value() && pass_result->ok;
        ...
        if (!total_mode_ui_ && pass_ok && pending_passes_ > 0) {
            QTimer::singleShot(0, this, [this]() { start_benchmark_pass(pending_benchmark_request_); });
            return;
        }
    ...
    void BenchmarkDialog::finalize_benchmark_run(z7::app::OperationOutcome const& outcome) {
        auto const benchmark_result = z7::app::outcome_payload_as<z7::app::BenchmarkResult>(outcome);
        z7::app::OperationResult const result_base = z7::app::operation_result_from_outcome(outcome);
        ...
        bool const canceled = z7::app::is_operation_canceled(result_base.error);
        benchmark_finished_ = result_base.ok && !canceled;
        rebuild_log_view();

        if (!result_base.ok && !canceled) {
            QString const summary =
                z7::ui::archive_support::from_native_string(z7::app::describe_archive_error(result_base.error))
                    .trimmed();
            if (!summary.isEmpty()) {
                if (error_label_ != nullptr) {
                ...
                QMessageBox::warning(this, lang_or(7600), summary);
            }
    ...
    void BenchmarkDialog::reset_result_view() {
        QString const dots = QStringLiteral("...");
    ...
    void BenchmarkDialog::append_log_line(QString const& line) {
    ...
    void BenchmarkDialog::handle_operation_event(z7::app::OperationEvent const& event) {
        ...
        QString const line = z7::ui::archive_support::from_native_string(event.message).trimmed();
        if (line.isEmpty()) {
            return;
        }
