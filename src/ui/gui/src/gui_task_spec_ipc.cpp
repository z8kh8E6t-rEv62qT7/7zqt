...
namespace z7::ui::gui {
    ...
    namespace {
        ...
        QString unsupported_email_payload_message() {
            return QStringLiteral("Email archive sending is not supported in this Qt build.");
        }

        ExtractPathRemapMatchKind
        convert_remap_match_kind(z7::task_ipc_runtime::TaskIpcExtractPathRemapMatchKind kind) {
            switch (kind) {
    ...
    build_task_spec_from_task_ipc_payload(z7::task_ipc_runtime::TaskIpcPayload const& payload, QString* error_message) {
        ...
        switch (payload.command) {
            case TaskIpcCommandKind::kAdd:
                {
                    ...
                    spec.path_mode = z7::ui::archive_support::to_native_string(payload.add->path_mode.trimmed());
                    ...
                    spec.compression_level =
                        z7::ui::archive_support::to_native_string(payload.add->compression_level.trimmed());
                    spec.method_value = z7::ui::archive_support::to_native_string(payload.add->method_value.trimmed());
                    spec.dictionary_size =
                        z7::ui::archive_support::to_native_string(payload.add->dictionary_size.trimmed());
                    spec.word_size = z7::ui::archive_support::to_native_string(payload.add->word_size.trimmed());
                    ...
                    for (QString const& value : payload.add->extra_parameters) {
            ...
            case TaskIpcCommandKind::kExtract:
                {
                    ...
                    spec.overwrite_switch =
                        z7::ui::archive_support::to_native_string(payload.extract->overwrite_switch.trimmed());
                    spec.archive_type =
                        z7::ui::archive_support::to_native_string(payload.extract->archive_type.trimmed());
                    spec.eliminate_root_duplication = payload.extract->eliminate_root_duplication;
            ...
            case TaskIpcCommandKind::kArchiveExport:
                {
                    if (!payload.archive_export.has_value()) {
                        if (error_message != nullptr) {
                            *error_message = QStringLiteral("Archive export command payload is missing.");
                        }
                        return std::nullopt;
                    }
                    ...
                    spec.path_mode =
                        z7::ui::archive_support::to_native_string(payload.archive_export->path_mode.trimmed());
                    spec.eliminate_root_duplication = payload.archive_export->eliminate_root_duplication;
                    spec.restore_file_security = payload.archive_export->restore_file_security;
                    spec.zone_id_mode =
            ...
            case TaskIpcCommandKind::kBenchmark:
                {
                    ...
                    spec.method_value =
                        z7::ui::archive_support::to_native_string(payload.benchmark->method_value.trimmed());
                    spec.dictionary_size =
                        z7::ui::archive_support::to_native_string(payload.benchmark->dictionary_size.trimmed());
                    spec.thread_count =
                        z7::ui::archive_support::to_native_string(payload.benchmark->thread_count.trimmed());
                    for (QString const& value : payload.benchmark->operands) {
                        spec.operands.push_back(z7::ui::archive_support::to_native_string(value.trimmed()));
                    }
                    return GuiTaskSpec{std::move(spec)};
                }
            ...
            case TaskIpcCommandKind::kCli:
                if (error_message != nullptr) {
                    *error_message = QStringLiteral("CLI task IPC commands are handled by the 7zG entrypoint.");
                }
                return std::nullopt;
