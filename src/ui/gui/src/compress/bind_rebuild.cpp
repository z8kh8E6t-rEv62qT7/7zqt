...
namespace z7::ui::gui {
    ...
    void CompressDialog::populate_from_initial(CompressCommandOptions const& initial) {
        ...
        set_format_combo_data_or_text(format_combo_, initial_or_saved_archive_type(initial));
    ...
    void CompressDialog::recompute_state(bool keep_current_method,
        ...
        bool const prev_updating = updating_controls_;
    ...
    void CompressDialog::rebuild_level_combo() {
        ...
        QString const previous_level = level_combo_->currentData().toString();

        QSignalBlocker const blocker(level_combo_);
        level_combo_->clear();

        QStringList const level_values = level_values_for_mask(rule.levels_mask);
        for (QString const& level : level_values) {
        ...
        QString const default_level =
            level_values.contains(QStringLiteral("5")) ? QStringLiteral("5") : level_values.front();
    ...
    void CompressDialog::rebuild_method_combo(bool keep_current_method) {
        FormatRule const rule = rule_for_format_id(current_format_id());
        QString const previous_method = method_combo_->currentData().toString();
        int const level = method_level_or_default(level_combo_);

        QString default_method = rule.default_method;
        QStringList methods = method_list_for_rule(rule, level, rule.sfx && create_sfx_checkbox_->isChecked());

        QSignalBlocker const blocker(method_combo_);
        method_combo_->clear();
        for (QString const& method : methods) {
        ...
        set_combo_data_or_default(method_combo_, keep_current_method ? previous_method : QString(), default_method);
        set_combo_enabled_if_multiple(method_combo_);
    }

    void CompressDialog::rebuild_dictionary_combo(bool keep_current_dictionary) {
        QString const previous_dictionary = dictionary_combo_->currentData().toString();
        QString const format_id = current_format_id();
        QString const method = method_combo_->currentData().toString();
        QString const effective_method = effective_method_for_format(format_id, method);
        int const level = method_level_or_default(level_combo_);

        QSignalBlocker const blocker(dictionary_combo_);
        dictionary_combo_->clear();

        auto add_dict = [this](uint64_t bytes, bool is_default = false) {
            add_combo_item(dictionary_combo_, dictionary_size_label(bytes, is_default), dictionary_size_data(bytes));
        };

        if (effective_method == QStringLiteral("LZMA") || effective_method == QStringLiteral("LZMA2")) {
            add_dict(lzma_auto_dict_for_level(level), true);
            static std::array<uint64_t, 23> const kLzmaDictionaries = {static_cast<uint64_t>(64) << 10,
        ...
        } else if (effective_method == QStringLiteral("PPMd") || effective_method == QStringLiteral("PPMdZip")) {
            uint64_t const auto_dict = static_cast<uint64_t>(1) << (std::clamp(level, 0, 9) + 19);
        ...
        if (dictionary_combo_->count() > 0) {
            set_combo_data_or_default(
                dictionary_combo_, keep_current_dictionary ? previous_dictionary : QString(), QString());
        }
        set_combo_enabled_if_multiple(dictionary_combo_);
    }

    void CompressDialog::rebuild_word_size_combo(bool keep_current_word_size) {
        QString const previous_order = word_size_combo_->currentData().toString();
        QString const format_id = current_format_id();
        QString const method = method_combo_->currentData().toString();
        QString const effective_method = effective_method_for_format(format_id, method);
        int const level = method_level_or_default(level_combo_);

        QSignalBlocker const blocker(word_size_combo_);
        word_size_combo_->clear();

        auto add_order = [this](int value, bool is_default = false) {
            QString const text = is_default ? QStringLiteral("* %1").arg(value) : QString::number(value);
        ...
        if (effective_method == QStringLiteral("LZMA") || effective_method == QStringLiteral("LZMA2")) {
        ...
        } else if (effective_method == QStringLiteral("Deflate") || effective_method == QStringLiteral("Deflate64")) {
            int auto_order = 32;
            if (level >= 9) {
                auto_order = 128;
            } else if (level >= 7) {
            ...
            for (int v : {32, 48, 64, 96, 128, 192, effective_method == QStringLiteral("Deflate64") ? 257 : 258}) {
                add_order(v);
            }
        ...
        if (word_size_combo_->count() > 0) {
            set_combo_data_or_default(word_size_combo_, keep_current_word_size ? previous_order : QString(), QString());
        }
    ...
    void CompressDialog::rebuild_solid_combo(bool keep_current_solid) {
        FormatRule const rule = rule_for_format_id(current_format_id());
        QString const previous_solid = solid_combo_->currentData().toString();
        int const level = method_level_or_default(level_combo_);

        QSignalBlocker const blocker(solid_combo_);
        solid_combo_->clear();
        ...
        QString const method = method_combo_->currentData().toString();
        QString const effective_method = effective_method_for_format(rule.id, method);
        uint64_t const dictionary_size = parse_size_to_bytes(dictionary_combo_->currentData().toString());
        uint64_t const auto_solid = auto_solid_size_bytes(rule, effective_method, level, dictionary_size);
        ...
        if (rule.id == QStringLiteral("7z")) {
            add_combo_item(solid_combo_, lang_or(4072), QStringLiteral("off"));
        }

        for (int log_size = 20; log_size <= 36; ++log_size) {
            uint64_t const size = static_cast<uint64_t>(1) << log_size;
            add_combo_item(solid_combo_, size_label_for_bytes(size), size_token_for_bytes(size));
        }
        add_combo_item(solid_combo_, lang_or(4073), QStringLiteral("on"));

        set_combo_data_or_default(solid_combo_, keep_current_solid ? previous_solid : QString(), QString());
        set_combo_enabled_if_multiple(solid_combo_);
    }
