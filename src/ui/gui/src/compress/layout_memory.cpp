...
namespace z7::ui::gui::compress_internal {
    ...
    QStringList method_list_for_rule(FormatRule const& rule, int level, bool sfx_enabled) {
        ...
        if (sfx_enabled) {
            methods.erase(std::remove_if(methods.begin(),
                                         methods.end(),
                                         [](QString const& method) { return !supports_sfx_method(method); }),
                          methods.end());
        }
        return methods;
    }
    ...
    uint64_t parse_size_to_bytes(QString const& value) {
        ...
        bool ok = false;
        double const number = trimmed.left(i).toDouble(&ok);
        if (!ok || number <= 0.0) {
            return 0;
        }
    ...
    QString format_bytes(uint64_t bytes) {
        ...
        double const gb = static_cast<double>(bytes) / static_cast<double>(1ull << 30);
        if (gb >= 1.0) {
            return QStringLiteral("%1 GB").arg(QString::number(gb, 'f', 1));
        }

        double const mb = static_cast<double>(bytes) / static_cast<double>(1ull << 20);
        return QStringLiteral("%1 MB").arg(QString::number(mb, 'f', 0));
    }

    int method_level_or_default(QComboBox const* level_combo) {
        bool ok = false;
        int const level = level_combo->currentData().toInt(&ok);
        if (!ok) {
            return 5;
        }
    ...
    uint64_t lzma_auto_dict_for_level(int level) {
        ...
        int const middle_level_limit = static_cast<int>(sizeof(size_t)) / 2 + 4;
        if (level <= middle_level_limit) {
            return static_cast<uint64_t>(1) << (level + 20);
        }
        return static_cast<uint64_t>(1) << (static_cast<int>(sizeof(size_t)) / 2 + 24);
    }
    ...
    uint64_t lzma2_chunk_size(uint64_t dictionary_size) {
        ...
        uint64_t const min_size = static_cast<uint64_t>(1) << 20;
        uint64_t const max_size = static_cast<uint64_t>(1) << 28;
    ...
    uint64_t auto_solid_size_bytes(FormatRule const& rule, QString const& method, int level, uint64_t dictionary_size) {
        ...
        uint64_t const chunk_size = lzma2_chunk_size(dict);
        ...
        if (rule.id == QStringLiteral("7z")) {
            ...
            uint64_t const min_size = static_cast<uint64_t>(1) << 24;
