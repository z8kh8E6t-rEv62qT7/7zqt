...
namespace z7::ui::gui::compress_internal {
    ...
    void set_combo_data_or_text(QComboBox* combo, QString const& value) {
        ...
        int const index = find_combo_data(combo, value);
        if (index >= 0) {
    ...
    void set_combo_data_or_default(QComboBox* combo, QString const& preferred, QString const& default_value) {
        ...
        int const preferred_index = find_combo_data(combo, preferred);
        if (preferred_index >= 0) {
            combo->setCurrentIndex(preferred_index);
            return;
        }

        int const default_index = find_combo_data(combo, default_value);
        if (default_index >= 0) {
    ...
    bool supports_sfx_method(QString const& method) {
        static QStringList const kSfxMethods = {
            QStringLiteral("Copy"), QStringLiteral("LZMA"), QStringLiteral("LZMA2"), QStringLiteral("PPMd")};
        return kSfxMethods.contains(method);
    }

    FormatRule rule_for_format_id(QString const& format_id) {
        QString const id = normalize_format_id(format_id);

        if (id == QStringLiteral("7z")) {
            return {id,
                    QStringLiteral("7z"),
                    kLevelsMaskAll,
                    {QStringLiteral("LZMA2"), QStringLiteral("LZMA"), QStringLiteral("PPMd"), QStringLiteral("BZip2")},
        ...
        if (id == QStringLiteral("wim")) {
            return {
                id, QStringLiteral("wim"), (1u << 0), {}, {}, false, false, false, false, false, false, false, false};
        }
