...
namespace z7::ui::gui {
    ...
    namespace {
        ...
        int extension_dot_pos(QString const& normalized_path) {
            int const dot_pos = normalized_path.lastIndexOf(QLatin1Char('.'));
            int const separator_pos = normalized_path.lastIndexOf(QLatin1Char('/'));
            return dot_pos > separator_pos + 1 ? dot_pos : -1;
        }

        QString basename_validation_error_message(z7::common::BasenameValidationError error) {
            switch (error) {
                case z7::common::BasenameValidationError::kEmpty:
                    return z7::ui::runtime_support::J(QStringLiteral("ui.state.basename_validation.empty"));
                case z7::common::BasenameValidationError::kAbsolutePath:
                    return z7::ui::runtime_support::J(QStringLiteral("ui.state.basename_validation.absolute_path"));
                case z7::common::BasenameValidationError::kDotOrDotDot:
                    return z7::ui::runtime_support::J(QStringLiteral("ui.state.basename_validation.dot_or_dotdot"));
            ...
            return z7::ui::runtime_support::J(QStringLiteral("ui.state.basename_validation.invalid"));
        }

        QString normalized_archive_name_text(QComboBox const* combo) {
            if (combo == nullptr) {
                return QString();
            }
        ...
        z7::common::BasenameValidationResult validate_archive_name_text(QString const& archive_name) {
    ...
    CompressDialog::CompressDialog(CompressCommandOptions const& initial, QWidget* parent) :
        QDialog(parent),
        single_file_input_(initial.single_file_input),
        single_file_name_(z7::ui::archive_support::from_native_string(initial.single_file_name)) {
        build_ui();
    ...
    void CompressDialog::set_archive_fields_from_path(QString const& full_path) {
        QString const normalized_path = QDir::fromNativeSeparators(full_path.trimmed());
        QString file_name;
        QString dir_prefix;
    ...
    QString CompressDialog::current_output_suffix() const {
        ...
        std::string const suffix = z7::common::preferred_archive_output_suffix_copy(current_format_id().toStdString());
    ...
    bool CompressDialog::current_format_keeps_original_name() const {
        ...
        return format_combo_->itemData(format_combo_->currentIndex(), kFormatKeepNameRole).toBool();
    }

    void CompressDialog::replace_archive_name_extension_for_current_format() {
        z7::common::BasenameValidationResult const name_result =
        ...
        if (single_file_input_ && current_format_keeps_original_name() && !single_file_name_.isEmpty()) {
            set_archive_fields_from_path(
                archive_path_with_file_name(archive_dir_prefix_, single_file_name_ + QLatin1Char('.') + suffix));
        ...
        if (dot_pos >= 0) {
            QString const existing_suffix = base_name.mid(dot_pos + 1);
            bool const has_generated_suffix =
                !generated_archive_extension_.isEmpty()
                && existing_suffix.compare(generated_archive_extension_, Qt::CaseInsensitive) == 0;
    ...
    QString CompressDialog::archive_name_validation_error() const {
        z7::common::BasenameValidationResult const result =
            validate_archive_name_text(normalized_archive_name_text(archive_name_combo_));
        if (result.ok) {
            return QString();
        }
    ...
    QString CompressDialog::compose_archive_path() const {
        ...
        if (QDir::isAbsolutePath(archive_name) || archive_name.contains(QLatin1Char('/'))) {
            return QDir::toNativeSeparators(archive_name);
        }

        QString const full_path = archive_dir_prefix_.isEmpty() ? archive_name : archive_dir_prefix_ + archive_name;
    ...
    void CompressDialog::build_ui() {
        ...
        auto add_left_combo_row = [&](uint32_t lang_id,
                                      QComboBox** combo_out,
                                      bool editable = false,
                                      QString const& object_name = QString()) {
        ...
        {
            auto* label = new QLabel(lang_or(4009), left_panel);
        ...
        {
            compress_memory_title_label_ = new QLabel(lang_or(4017), left_panel);
        ...
        {
            decompress_memory_title_label_ = new QLabel(lang_or(4018), left_panel);
        ...
        auto* options_group = new QGroupBox(lang_or(4011), right_panel);
        auto* options_group_layout = new QVBoxLayout(options_group);
        create_sfx_checkbox_ = new QCheckBox(lang_or(4012), options_group);
#ifdef Z7_TESTING
        create_sfx_checkbox_->setObjectName(QStringLiteral("createSfxCheckBox"));
#endif
        compress_shared_checkbox_ = new QCheckBox(lang_or(4013), options_group);
        delete_after_checkbox_ = new QCheckBox(lang_or(4019), options_group);
        ...
        auto* encryption_group = new QGroupBox(lang_or(4014), right_panel);
        ...
        reenter_password_label_ = new QLabel(lang_or(3802), encryption_group);
#ifdef Z7_TESTING
        ...
        show_password_checkbox_ = new QCheckBox(lang_or(3803), encryption_group);
#ifdef Z7_TESTING
...
#ifdef Z7_TESTING
        encryption_method_label_->setObjectName(QStringLiteral("encryptionMethodLabel"));
#endif
        ...
        encrypt_headers_checkbox_ = new QCheckBox(lang_or(4016), encryption_group);
#ifdef Z7_TESTING
        ...
        QPushButton* help_button = buttons->addButton(lang_or(409), QDialogButtonBox::HelpRole);
#ifdef Z7_TESTING
        ...
        update_mode_combo_->addItem(lang_or(4060), QStringLiteral("add"));
        update_mode_combo_->addItem(lang_or(4061), QStringLiteral("update"));
        update_mode_combo_->addItem(lang_or(4062), QStringLiteral("fresh"));
        update_mode_combo_->addItem(lang_or(4063), QStringLiteral("sync"));

        path_mode_combo_->addItem(lang_or(3414), QStringLiteral("relative"));
        ...
        connect(browse_archive_button, &QPushButton::clicked, this, [this]() {
            QString const selected = QFileDialog::getSaveFileName(this, lang_or(4070), compose_archive_path());
            if (!selected.isEmpty()) {
                set_archive_fields_from_path(selected);
            }
        });
        connect(archive_name_combo_, qOverload<int>(&QComboBox::activated), this, [this](int index) {
            QString const selected_path = archive_name_combo_->itemData(index).toString();
        ...
        connect(dictionary_combo_, qOverload<int>(&QComboBox::currentIndexChanged), this, [this](int) {
            if (!is_updating_controls()) {
                recompute_state(true, true, true, false, true);
            }
        });
        connect(threads_combo_, qOverload<int>(&QComboBox::currentIndexChanged), this, [this](int) {
            if (!is_updating_controls()) {
                update_memory_labels();
            }
        });
        ...
        connect(show_password_checkbox_, &QCheckBox::toggled, this, [this](bool) { update_password_echo_mode(); });
    ...
    void CompressDialog::populate_format_combo() {
        QSignalBlocker const blocker(format_combo_);
        format_combo_->clear();

        std::vector<z7::app::CompressFormatCatalogEntry> const catalog = z7::app::list_update_archive_formats();
        ...
        for (z7::app::CompressFormatCatalogEntry const& entry : catalog) {
            QString const id = normalize_format_id(z7::ui::archive_support::from_native_string(entry.type_id));
            if (id.isEmpty()) {
                continue;
            }
        ...
        std::sort(entries.begin(), entries.end(), [](FormatComboEntry const& lhs, FormatComboEntry const& rhs) {
            int const display_compare = QString::compare(lhs.display_name, rhs.display_name, Qt::CaseInsensitive);
        ...
        for (FormatComboEntry const& entry : entries) {
            format_combo_->addItem(entry.display_name, entry.id);
            format_combo_->setItemData(format_combo_->count() - 1, entry.keep_name, kFormatKeepNameRole);
        }
