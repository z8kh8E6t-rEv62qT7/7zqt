...
namespace z7::ui::gui {
    ...
    namespace {
        ...
        QString settings_key(char const* key) {
            return QString::fromLatin1(key);
        }

        QString format_settings_key(QString const& format_id, char const* leaf) {
            QString const normalized = normalize_format_id(format_id);
            if (normalized.isEmpty()) {
                return QString();
            }
            return QString::fromLatin1(kFormatSettingsPrefix)
                 + normalized
                 + QLatin1Char('/')
                 + QString::fromLatin1(leaf);
        }

        QString saved_string(z7::platform::qt::PortableSettings const& settings, QString const& key) {
        ...
        void
        set_string_or_remove(z7::platform::qt::PortableSettings* settings, QString const& key, QString const& value) {
        ...
        QString explicit_or_saved_string(z7::platform::qt::PortableSettings const& settings,
                                         QString const& key,
                                         CompressCommandOptions const* explicit_options,
                                         std::string const CompressCommandOptions::* member) {
            if (explicit_options != nullptr) {
                QString const explicit_value =
                    z7::ui::archive_support::from_native_string((*explicit_options).*member).trimmed();
        ...
        QString joined_extra_parameters(std::vector<std::string> const& extra_parameters) {
            QString joined;
            for (std::string const& token : extra_parameters) {
                QString const value = z7::ui::archive_support::from_native_string(token).trimmed();
                if (value.isEmpty()) {
                    continue;
                }
                if (!joined.isEmpty()) {
        ...
        QString default_encryption_method_for_format(QString const& format_id) {
            QString const normalized = normalize_format_id(format_id);
        ...
        QStringList normalized_archive_history(QStringList history, QString const& new_item = QString()) {
            return z7::ui::common::normalized_path_history(history, new_item, kCompressHistoryMax);
        }

        QString normalize_advanced_options_text(QString const& text) {
        ...
        bool is_bool_switch_token(QString const& token, QString const& short_name) {
            QString const lower = canonical_switch_token(token);
            QString const prefix = QStringLiteral("-%1").arg(short_name.toLower());
            ...
            QChar const next = lower.at(prefix.size());
            return next == QLatin1Char('-') || next == QLatin1Char('=');
        }

        bool bool_switch_enabled(QString const& token, QString const& short_name) {
            QString const lower = canonical_switch_token(token);
            QString const prefix = QStringLiteral("-%1").arg(short_name.toLower());
            if (!lower.startsWith(prefix)) {
                return false;
            ...
            QString const suffix = lower.mid(prefix.size());
            if (suffix == QStringLiteral("-")) {
                return false;
            }
            if (suffix.startsWith(QLatin1Char('='))) {
                QString const value = suffix.mid(1).trimmed();
        ...
        bool has_enabled_switch(QStringList const& tokens, QString const& short_name) {
            bool found = false;
            bool enabled = false;
        ...
        void remove_switch_tokens(QStringList* tokens, QString const& short_name) {
            if (tokens == nullptr) {
                return;
            }
            tokens->erase(
                std::remove_if(tokens->begin(),
                               tokens->end(),
                               [&short_name](QString const& token) { return is_bool_switch_token(token, short_name); }),
                tokens->end());
        }

        void set_bool_switch(QStringList* tokens, QString const& short_name, bool enabled) {
            if (tokens == nullptr) {
                return;
            }
        ...
        SwitchState switch_state_from_tokens(QStringList const& tokens, QString const& short_name) {
            SwitchState state = SwitchState::kAuto;
            for (QString const& token : tokens) {
                if (!is_bool_switch_token(token, short_name)) {
                    continue;
                }
                state = bool_switch_enabled(token, short_name) ? SwitchState::kEnabled : SwitchState::kDisabled;
            }
            return state;
        }

        void set_switch_state(QStringList* tokens, QString const& short_name, SwitchState state) {
            if (tokens == nullptr) {
                return;
            }
            remove_switch_tokens(tokens, short_name);
        ...
        bool is_presence_switch_token(QString const& token, QString const& short_name) {
            return canonical_switch_token(token) == QStringLiteral("-%1").arg(short_name.toLower());
        }

        bool has_presence_switch(QStringList const& tokens, QString const& short_name) {
            return std::any_of(tokens.cbegin(), tokens.cend(), [&short_name](QString const& token) {
                return is_presence_switch_token(token, short_name);
            });
        }

        void remove_presence_switch_tokens(QStringList* tokens, QString const& short_name) {
            if (tokens == nullptr) {
                return;
            }
            tokens->erase(std::remove_if(tokens->begin(),
                                         tokens->end(),
                                         [&short_name](QString const& token) {
                                             return is_presence_switch_token(token, short_name);
                                         }),
                          tokens->end());
        }

        void set_presence_switch(QStringList* tokens, QString const& short_name, bool enabled) {
            if (tokens == nullptr) {
                return;
            }
        ...
        SwitchState checkbox_switch_state(QCheckBox const* checkbox) {
            if (checkbox == nullptr) {
        ...
        QString join_tokens(QStringList const& tokens) {
        ...
        QStringList normalized_advanced_tokens(QString const& text) {
        ...
        void
        save_switch_state_setting(z7::platform::qt::PortableSettings* settings, QString const& key, SwitchState state) {
            if (settings == nullptr || key.isEmpty()) {
                return;
            }
        ...
        void append_switch_token_from_setting(QStringList* tokens,
            ...
            tokens->push_back(QStringLiteral("-%1%2").arg(
                short_name, settings.value(key).toBool() ? QString() : QStringLiteral("-")));
        }

        void append_presence_switch_token_from_setting(QStringList* tokens,
                                                       z7::platform::qt::PortableSettings const& settings,
                                                       QString const& key,
                                                       QString const& short_name) {
        ...
        MethodPropertyToken parse_method_property_token(QString const& raw_token) {
            ...
            token.remove(0, 1);
            if (!token.isEmpty() && (token.front() == QLatin1Char('m') || token.front() == QLatin1Char('M'))) {
                token.remove(0, 1);
            }
            token = token.trimmed();
            if (token.isEmpty()) {
            ...
            qsizetype const separator = token.indexOf(QLatin1Char('='));
            if (separator < 0) {
        ...
        bool method_property_name_equals(MethodPropertyToken const& token, QString const& name) {
            return !token.name.isEmpty() && token.name.compare(name, Qt::CaseInsensitive) == 0;
        }

        bool valid_method_property_value_for_key(MethodPropertyToken const& token, QString const& name) {
        ...
        void append_method_property_token_from_setting(QStringList* tokens,
                                                       z7::platform::qt::PortableSettings const& settings,
                                                       QString const& key,
            ...
            QString const value = settings.value(key).toString().trimmed();
            if (value.isEmpty()) {
                return;
            }
        ...
        void save_method_property_setting(z7::platform::qt::PortableSettings* settings,
                                          QString const& key,
                                          QStringList const& tokens,
                                          QString const& property_name) {
            if (settings == nullptr || key.isEmpty()) {
            ...
            QString value;
            bool found = false;
            for (QString const& raw_token : tokens) {
                MethodPropertyToken const token = parse_method_property_token(raw_token);
        ...
        bool is_structured_method_property_token(QString const& raw_token, QString const& property_name) {
            MethodPropertyToken const token = parse_method_property_token(raw_token);
            return method_property_name_equals(token, property_name)
                && valid_method_property_value_for_key(token, property_name);
        }

        QString advanced_options_text_from_original_keys(z7::platform::qt::PortableSettings const& settings,
                                                         QString const& format_id) {
            QStringList tokens;
            ...
            QString const options = saved_string(settings, format_settings_key(format_id, kFormatOptions));
            tokens.append(normalized_advanced_tokens(options));
            return join_tokens(tokens);
        }

        QString options_text_without_structured_switches(QString const& text) {
            QStringList tokens = normalized_advanced_tokens(text);
            for (QString const& short_name : {QStringLiteral("snl"),
                                              QStringLiteral("snh"),
                                              QStringLiteral("sns"),
                                              QStringLiteral("sni"),
                                              QStringLiteral("mtm"),
        ...
        void save_structured_advanced_options(z7::platform::qt::PortableSettings* settings,
                                              QString const& format_id,
                                              QString const& text) {
            if (settings == nullptr) {
                return;
            }
            QStringList const tokens = normalized_advanced_tokens(text);
            save_bool_switch_setting(settings, settings_key(kSettingsCompressSymLinks), tokens, QStringLiteral("snl"));
            save_bool_switch_setting(settings, settings_key(kSettingsCompressHardLinks), tokens, QStringLiteral("snh"));
            save_bool_switch_setting(
                settings, settings_key(kSettingsCompressAltStreams), tokens, QStringLiteral("sns"));
            save_bool_switch_setting(settings, settings_key(kSettingsCompressSecurity), tokens, QStringLiteral("sni"));
            save_presence_switch_setting(
                settings, settings_key(kSettingsCompressPreserveATime), tokens, QStringLiteral("ssp"));
            save_switch_state_setting(settings,
            ...
            save_method_property_setting(
                settings, format_settings_key(format_id, kFormatTimePrec), tokens, QStringLiteral("tp"));
            save_method_property_setting(
                settings, format_settings_key(format_id, kFormatMemUse), tokens, QStringLiteral("memuse"));
    ...
    QString CompressDialog::selected_encryption_method_spec() const {
        ...
        QString const selected = encryption_method_combo_->currentData().toString().trimmed();
        if (selected.isEmpty()) {
            return QString();
        }

        return selected.compare(default_encryption_method_for_current_format(), Qt::CaseInsensitive) == 0 ? QString()
                                                                                                          : selected;
    }
    ...
    void CompressDialog::load_archive_path_history() {
        ...
        QStringList const history =
            normalized_archive_history(settings.value(settings_key(kSettingsCompressArchiveHistory)).toStringList());
        for (QString const& entry : history) {
            QString const normalized = z7::ui::common::normalize_path_history_entry(entry);
            if (!normalized.isEmpty()) {
                archive_name_combo_->addItem(QDir::toNativeSeparators(normalized), normalized);
            }
        }
    }

    QString CompressDialog::initial_or_saved_archive_type(CompressCommandOptions const& initial) const {
        QString const initial_type =
            normalize_format_id(z7::ui::archive_support::from_native_string(initial.archive_type));
        if (!initial_type.isEmpty()) {
            return initial_type;
        }

        z7::platform::qt::PortableSettings settings;
        return normalize_format_id(settings.value(settings_key(kSettingsCompressArchiver)).toString());
    }

    bool CompressDialog::saved_show_password() const {
        z7::platform::qt::PortableSettings settings;
        return settings.value(settings_key(kSettingsCompressShowPassword), false).toBool();
    }

    bool CompressDialog::initial_or_saved_encrypt_headers(CompressCommandOptions const& initial) const {
        if (!initial.password.empty()) {
            return initial.encrypt_headers;
        }

        z7::platform::qt::PortableSettings settings;
        return settings.value(settings_key(kSettingsCompressEncryptHeaders), false).toBool();
    }

    void CompressDialog::apply_persistent_format_options(QString const& format_id,
                                                         CompressCommandOptions const* explicit_options) {
        QString const normalized_format = normalize_format_id(format_id);
        if (normalized_format.isEmpty()) {
            return;
        }

        z7::platform::qt::PortableSettings settings;
        QString level = explicit_or_saved_string(settings,
        ...
        set_combo_data_or_text(encryption_method_combo_,
                               encryption_method.isEmpty() ? default_encryption_method_for_format(normalized_format)
                                                           : encryption_method);
    ...
    void CompressDialog::save_format_settings(QString const& format_id) const {
        QString const normalized_format = normalize_format_id(format_id);
        if (normalized_format.isEmpty()) {
            return;
        }
        ...
        set_string_or_remove(
            &settings, format_settings_key(normalized_format, kFormatLevel), current_combo_data(level_combo_));
        set_string_or_remove(
            &settings, format_settings_key(normalized_format, kFormatMethod), current_combo_data(method_combo_));
        set_string_or_remove(&settings,
                             format_settings_key(normalized_format, kFormatDictionary),
                             current_combo_data(dictionary_combo_));
        set_string_or_remove(
            &settings, format_settings_key(normalized_format, kFormatOrder), current_combo_data(word_size_combo_));
        set_string_or_remove(
            &settings, format_settings_key(normalized_format, kFormatBlockSize), current_combo_data(solid_combo_));
        set_string_or_remove(
            &settings, format_settings_key(normalized_format, kFormatNumThreads), current_combo_data(threads_combo_));
        set_string_or_remove(&settings,
                             format_settings_key(normalized_format, kFormatEncryptionMethod),
                             selected_encryption_method_spec());
        save_structured_advanced_options(&settings, normalized_format, parameters_edit_->text());
    }
    ...
    void CompressDialog::save_persistent_settings() const {
        ...
        z7::platform::qt::PortableSettings settings;
        settings.setValue(settings_key(kSettingsCompressArchiver), current_format_id());
        set_string_or_remove(&settings, settings_key(kSettingsCompressLevel), current_combo_data(level_combo_));
        settings.setValue(settings_key(kSettingsCompressShowPassword),
                          show_password_checkbox_ != nullptr && show_password_checkbox_->isChecked());
        ...
        QStringList const history = settings.value(settings_key(kSettingsCompressArchiveHistory)).toStringList();
        settings.setValue(settings_key(kSettingsCompressArchiveHistory),
                          normalized_archive_history(history, compose_archive_path()));
        settings.sync();
    }
    ...
    void CompressDialog::on_options_clicked() {
        ...
        z7::ui::runtime_support::PlatformRestrictionUi const ntfs_ui = z7::ui::runtime_support::platform_restriction_ui(
            QStringLiteral("NTFS"), z7::ui::runtime_support::PlatformSupport::kWindowsOnly);
        ...
        auto* mtime_checkbox = new QCheckBox(lang_or(4082), time_group);
#ifdef Z7_TESTING
        mtime_checkbox->setObjectName(QStringLiteral("compressOptionsMTimeCheckBox"));
#endif
        auto* ctime_checkbox = new QCheckBox(lang_or(4083), time_group);
#ifdef Z7_TESTING
        ctime_checkbox->setObjectName(QStringLiteral("compressOptionsCTimeCheckBox"));
#endif
        auto* atime_checkbox = new QCheckBox(lang_or(4084), time_group);
#ifdef Z7_TESTING
        atime_checkbox->setObjectName(QStringLiteral("compressOptionsATimeCheckBox"));
#endif
        auto* archive_time_checkbox = new QCheckBox(lang_or(4085), time_group);
#ifdef Z7_TESTING
        archive_time_checkbox->setObjectName(QStringLiteral("compressOptionsSetArchiveTimeCheckBox"));
#endif
        auto* preserve_atime_checkbox = new QCheckBox(lang_or(4086), time_group);
#ifdef Z7_TESTING
        preserve_atime_checkbox->setObjectName(QStringLiteral("compressOptionsPreserveATimeCheckBox"));
#endif
        ...
        QStringList const initial_tokens = QProcess::splitCommand(parameters_edit_->text().trimmed());
        ...
        set_checkbox_switch_state(mtime_checkbox, switch_state_from_tokens(initial_tokens, QStringLiteral("mtm")));
        set_checkbox_switch_state(ctime_checkbox, switch_state_from_tokens(initial_tokens, QStringLiteral("mtc")));
        set_checkbox_switch_state(atime_checkbox, switch_state_from_tokens(initial_tokens, QStringLiteral("mta")));
        set_checkbox_switch_state(archive_time_checkbox,
                                  switch_state_from_tokens(initial_tokens, QStringLiteral("stl")));
        preserve_atime_checkbox->setChecked(has_presence_switch(initial_tokens, QStringLiteral("ssp")));
        ...
        auto* options_buttons =
            new QDialogButtonBox(QDialogButtonBox::Ok | QDialogButtonBox::Cancel, Qt::Horizontal, &options_dialog);
#ifdef Z7_TESTING
        ...
        QPushButton* options_help = options_buttons->addButton(lang_or(409), QDialogButtonBox::HelpRole);
#ifdef Z7_TESTING
        ...
        QStringList tokens = QProcess::splitCommand(normalize_advanced_options_text(options_edit->toPlainText()));
        if (!ntfs_ui.supported) {
        ...
        set_switch_state(&tokens, QStringLiteral("mtm"), checkbox_switch_state(mtime_checkbox));
        set_switch_state(&tokens, QStringLiteral("mtc"), checkbox_switch_state(ctime_checkbox));
        set_switch_state(&tokens, QStringLiteral("mta"), checkbox_switch_state(atime_checkbox));
        set_switch_state(&tokens, QStringLiteral("stl"), checkbox_switch_state(archive_time_checkbox));
        set_presence_switch(&tokens, QStringLiteral("ssp"), preserve_atime_checkbox->isChecked());
