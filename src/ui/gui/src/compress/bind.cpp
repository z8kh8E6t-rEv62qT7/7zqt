...
namespace z7::ui::gui {
    ...
    void CompressDialog::rebuild_threads_combo(bool keep_current_threads) {
        ...
        QString const previous_threads = threads_combo_->currentData().toString();

        QSignalBlocker const blocker(threads_combo_);
        ...
        int const hardware_threads = std::max(1, QThread::idealThreadCount());
        ...
        } else if (method == QStringLiteral("Copy")
                   || method == QStringLiteral("PPMd")
                   || method == QStringLiteral("PPMdZip")
                   || method == QStringLiteral("Deflate")
                   || method == QStringLiteral("Deflate64")
                   || method == QStringLiteral("GNU")
                   || method == QStringLiteral("POSIX")
                   || method == QStringLiteral("SHA256")
                   || method == QStringLiteral("SHA1")) {
            method_threads_max = 1;
        }

        method_threads_max = std::max(1, method_threads_max);
        int const auto_threads = std::clamp(hardware_threads, 1, method_threads_max);
        add_combo_item(threads_combo_, QStringLiteral("* Auto (%1)").arg(auto_threads), QStringLiteral("auto"));

        if (method_threads_max != auto_threads || auto_threads != 1) {
            int const max_explicit = std::min(method_threads_max, hardware_threads * 2);
        ...
        set_combo_data_or_default(
            threads_combo_, keep_current_threads ? previous_threads : QString(), QStringLiteral("auto"));
        set_combo_enabled_if_multiple(threads_combo_);
    }
    ...
    void CompressDialog::update_encryption_controls() {
        FormatRule const rule = rule_for_format_id(current_format_id());
        QString const previous_method = encryption_method_combo_->currentData().toString();

        bool const encrypt = rule.encrypt;
        ...
        {
            QSignalBlocker const blocker(encryption_method_combo_);
            ...
            if (rule.id == QStringLiteral("7z")) {
                ...
                set_combo_data_or_default(encryption_method_combo_, previous_method, QStringLiteral("AES-256"));
            } else if (rule.id == QStringLiteral("zip")) {
                add_combo_item(encryption_method_combo_, QStringLiteral("ZipCrypto"), QStringLiteral("ZipCrypto"));
                add_combo_item(encryption_method_combo_, QStringLiteral("AES-256"), QStringLiteral("AES-256"));
                set_combo_data_or_default(encryption_method_combo_, previous_method, QStringLiteral("ZipCrypto"));
            }
    ...
    void CompressDialog::update_memory_visibility() {
        FormatRule const rule = rule_for_format_id(current_format_id());
        bool const visible = rule.mem_use;
    ...
    void CompressDialog::update_memory_labels() {
        ...
        request.dictionary_size =
            z7::ui::archive_support::to_native_string(dictionary_combo_->currentData().toString());
        ...
        z7::app::CompressionResourcesEstimate const estimate = z7::app::estimate_compression_resources(request);
        ...
        hardware_threads_label_->setText(QStringLiteral("/ %1").arg(std::max(1, QThread::idealThreadCount())));
    }

    void CompressDialog::update_password_echo_mode() {
        bool const show_password = show_password_checkbox_->isChecked();
        bool const show_reenter = !show_password;

        QLineEdit::EchoMode const mode = show_password ? QLineEdit::Normal : QLineEdit::Password;
    ...
    void CompressDialog::accept() {
        QString const p1 = password_edit_->text();
        QString const p2 = reenter_password_edit_->text();
        bool const enforce_reenter = show_password_checkbox_->isEnabled() && !show_password_checkbox_->isChecked();
        if (enforce_reenter && (!p1.isEmpty() || !p2.isEmpty()) && p1 != p2) {
            error_label_->setText(lang_or(3804));
            return;
        }

        QString const archive_name_error = archive_name_validation_error();
    ...
    CompressCommandOptions CompressDialog::options() const {
        ...
        out.single_file_name = z7::ui::archive_support::to_native_string(single_file_name_);
        ...
        if (rule.encrypt) {
            ...
            out.encrypt_headers =
                !out.password.empty() && rule.encrypt_file_names && encrypt_headers_checkbox_->isChecked();
        }
