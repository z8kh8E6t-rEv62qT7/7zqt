...
namespace z7::ui::gui {
    namespace {
        ...
        bool has_prefix_ci(std::string const& value, std::string const& prefix_lower) {
        ...
        bool is_unsigned_decimal(std::string const& value) {
            if (value.empty()) {
                return false;
            }
        ...
        std::string deduce_archive_format(AddTaskSpec const& spec) {
            ...
            std::filesystem::path const archive_path(spec.archive_path);
        ...
        std::string side_by_side_sfx_module_path() {
            QString const path = QDir(QCoreApplication::applicationDirPath()).filePath(QStringLiteral("7z.sfx"));
            QByteArray const encoded = QFile::encodeName(path);
            return std::string(encoded.constData(), static_cast<size_t>(encoded.size()));
        }

        z7::app::OverwriteMode parse_overwrite_mode(std::string const& overwrite_switch) {
            std::string const lower = to_lower_ascii(overwrite_switch);
        ...
        z7::app::ExtractPathMode parse_extract_path_mode(std::string const& path_mode) {
            std::string const lower = to_lower_ascii(path_mode);
            if (lower == "no") {
        ...
        z7::app::ExtractZoneIdMode parse_extract_zone_id_mode(std::string const& zone_id_mode) {
        ...
        z7::app::ExtractPathRemapMatchKind to_archive_extract_remap_match_kind(ExtractPathRemapMatchKind kind) {
            switch (kind) {
        ...
        uint32_t benchmark_iterations_or_default(GuiTaskSpec const& spec) {
            auto const* benchmark = std::get_if<BenchmarkTaskSpec>(&spec);
            if (benchmark == nullptr) {
                return 10;
            }
            for (std::string const& operand : benchmark->operands) {
                if (!is_unsigned_decimal(operand)) {
                    continue;
                }
                unsigned long long const parsed = std::strtoull(operand.c_str(), nullptr, 10);
                if (parsed == 0 || parsed > static_cast<unsigned long long>(std::numeric_limits<uint32_t>::max())) {
                    continue;
                }
        ...
        std::string join_extract_output_dir(ExtractTaskSpec const& spec) {
            std::string const base_output_dir = spec.output_dir.empty()
                                                  ? std::filesystem::path(QDir::currentPath().toStdString()).string()
                                                  : spec.output_dir;
            if (!spec.split_dest_enabled) {
                return base_output_dir;
            }

            std::string const split_name = spec.split_dest_name;
            if (split_name.empty()) {
                return base_output_dir;
            }

            return (std::filesystem::path(base_output_dir) / std::filesystem::path(split_name)).string();
        }

    } // namespace

    bool build_archive_request(GuiTaskSpec const& spec, GuiTaskRunResult* out, z7::app::ArchiveRequest* request_out) {
        if (out == nullptr || request_out == nullptr) {
            return false;
        }

        return std::visit(
            [&](auto const& typed_spec) -> bool {
                ...
                if constexpr (std::is_same_v<T, AddTaskSpec>) {
                    if (typed_spec.archive_path.empty() || typed_spec.input_paths.empty()) {
                        out->result = z7::app::make_immediate_result(7,
                    ...
                    request.symbolic_links_defined = typed_spec.symbolic_links_defined;
                    request.symbolic_links = typed_spec.symbolic_links;
                    request.hard_links_defined = typed_spec.hard_links_defined;
                    request.hard_links = typed_spec.hard_links;
                    request.alternate_streams_defined = typed_spec.alternate_streams_defined;
                    request.alternate_streams = typed_spec.alternate_streams;
                    request.file_security_defined = typed_spec.file_security_defined;
                    request.file_security = typed_spec.file_security;
                    request.preserve_access_time = typed_spec.preserve_access_time;
                    request.write_mtime_defined = typed_spec.write_mtime_defined;
                    request.write_mtime = typed_spec.write_mtime;
                    request.write_ctime_defined = typed_spec.write_ctime_defined;
                    request.write_ctime = typed_spec.write_ctime;
                    request.write_atime_defined = typed_spec.write_atime_defined;
                    request.write_atime = typed_spec.write_atime;
                    request.set_archive_mtime = typed_spec.set_archive_mtime;
                    ...
                    if (archive_inputs.empty()) {
                        out->result = z7::app::make_immediate_result(
                            7, z7::app::ArchiveErrorDomain::kInvalidArguments, "Extract requires archive path");
                        return false;
                    }
                    ...
                    request.zone_id_mode = parse_extract_zone_id_mode(typed_spec.zone_id_mode);
                    request.path_remaps.reserve(typed_spec.path_remaps.size());
                    for (ExtractPathRemap const& remap : typed_spec.path_remaps) {
                        z7::app::ExtractPathRemap request_remap;
                        request_remap.match_kind = to_archive_extract_remap_match_kind(remap.match_kind);
                    ...
                    request.password = typed_spec.password;
                    request.restore_file_security = typed_spec.restore_file_security
                    ...
                    if (archive_inputs.size() > 1) {
                        ...
                        for (std::string const& operand : archive_inputs) {
                            std::error_code ec;
                    ...
                    if (typed_spec.archive_inputs.empty()) {
                        out->result = z7::app::make_immediate_result(
                            7, z7::app::ArchiveErrorDomain::kInvalidArguments, "Test requires archive path");
                        return false;
                    }
                    ...
                    if (typed_spec.input_paths.empty()) {
                        out->result = z7::app::make_immediate_result(
                            7, z7::app::ArchiveErrorDomain::kInvalidArguments, "Hash requires input files");
                        return false;
                    }
                    z7::app::HashRequest request;
                    request.input_paths = typed_spec.input_paths;
                    request.hash_method = typed_spec.hash_method.empty() ? "CRC32" : typed_spec.hash_method;
