...
namespace z7::ui::runtime_support {
    ...
    public:
        ...
        QString format(uint32_t id, QStringList const& args) const;

        QString current_language() const;
        bool set_language(QString const& lang_id);
        bool set_language_and_persist(QString const& lang_id);
    ...
    private:
        ...
        bool set_language_internal(QString const& lang_id, bool persist);
        bool load_language_file(QString const& lang_id,
        ...
        static QHash<uint32_t, QString> parse_lang2(QByteArray const& bytes,
                                                    QString* english_name_out,
                                                    QString* native_name_out,
                                                    QStringList* comments_out);
        static QHash<uint32_t, QString> parse_rc_string_tables(QByteArray const& bytes);

        static QString substitute_placeholders(QString const& text, QStringList const& args);
        static QString normalize_language_id(QString const& lang_id);

        QString settings_language() const;
        void save_settings_language(QString const& lang_id) const;
