...
namespace z7::ui::runtime_support {
    namespace {
        ...
        constexpr auto kSettingsPasswordPromptShowPassword = "Gui/PasswordPrompt/ShowPassword";
        ...
        bool load_show_password() {
            ...
            return settings.value(QString::fromLatin1(kSettingsPasswordPromptShowPassword), false).toBool();
        }

        void save_show_password(bool show_password) {
            z7::platform::qt::PortableSettings settings;
            settings.setValue(QString::fromLatin1(kSettingsPasswordPromptShowPassword), show_password);
        }
        ...
        void center_dialog_on_parent(QDialog* dialog, QWidget* parent) {
            ...
            QRect const parent_geometry = parent->frameGeometry();
        ...
        ScriptedPasswordPromptReply take_scripted_password_prompt_reply() {
            char const* raw_script = std::getenv("Z7_TEST_PASSWORD_PROMPT_RESPONSES");
            ...
            std::string const script(raw_script);
            if (script != active_script) {
                active_script = script;
                responses = QString::fromUtf8(raw_script).split(QStringLiteral("||"), Qt::KeepEmptyParts);
                index = 0;
            }
            ...
            QString const token = responses.at(index++);
    ...
    std::optional<QString> show_password_prompt_dialog(QWidget* parent, z7::app::PasswordPrompt const& prompt) {
#ifdef Z7_TESTING
        ScriptedPasswordPromptReply const scripted = take_scripted_password_prompt_reply();
        if (scripted.active) {
        ...
        bool const wrong_password = prompt.reason_kind == z7::app::PasswordPromptReason::kWrongPassword;
        dialog.setFixedSize(kDialogWidth, wrong_password ? kWrongPasswordDialogHeight : kDialogHeight);
        QString const archive_path = z7::ui::archive_support::from_native_string(prompt.archive_path);
        ...
        if (wrong_password) {
            auto* warning =
                new QLabel(QStringLiteral("%1 %2").arg(J(QStringLiteral("ui.password_prompt.wrong_password_title")),
        ...
        auto const update_echo_mode = [edit, show_password]() {
            edit->setEchoMode(show_password->isChecked() ? QLineEdit::Normal : QLineEdit::Password);
        };
        update_echo_mode();
