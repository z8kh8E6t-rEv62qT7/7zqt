...
namespace z7::ui::runtime_support::official_lang_catalog_internal {
    ...
    QString normalize_locale_name(QString const& locale_name) {
        ...
        int const at_pos = normalized.indexOf(QLatin1Char('@'));
        if (at_pos >= 0) {
            normalized.truncate(at_pos);
        }
        int const dot_pos = normalized.indexOf(QLatin1Char('.'));
        if (dot_pos >= 0) {
    ...
    QString primary_subtag(QString const& lang_id) {
        QString const normalized = normalize_locale_name(lang_id);
        int const dash = normalized.indexOf(QLatin1Char('-'));
        if (dash < 0) {
            return normalized;
        }
    ...
    void assign_language_marks(QList<LangInfo>* languages) {
        ...
        QStringList const ui_languages = QLocale::system().uiLanguages();
        for (QString const& raw : ui_languages) {
            QString const normalized = normalize_locale_name(raw);
            if (!normalized.isEmpty() && !preferred.contains(normalized)) {
                preferred.append(normalized);
            }
            QString const primary = primary_subtag(normalized);
        ...
        int starred_index = -1;
        for (QString const& wanted : preferred) {
            for (int i = 0; i < languages->size(); ++i) {
                if (i == english_index) {
                    continue;
                }
        ...
        QString const primary = primary_subtag(preferred.front());
