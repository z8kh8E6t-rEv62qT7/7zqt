...
namespace z7::ui::runtime_support {
    ...
    void TaskProgressDialogBase::refresh_metrics() {
        ...
        if (ratio_info_.has_value() && (ratio_info_->input_size_known || ratio_info_->output_size_known)) {
        ...
        if (processed_value_ != nullptr) {
            processed_value_->setText(
                processed_size.has_value() ? z7::ui::archive_support::format_size_short(*processed_size) : QString());
        }
        if (packed_value_ != nullptr) {
            packed_value_->setText(packed_size.has_value() ? z7::ui::archive_support::format_size_short(*packed_size)
                                                           : QString());
        }
        if (ratio_value_ != nullptr) {
            if (!ratio_info_.has_value() || !ratio_info_->input_size_known || !ratio_info_->output_size_known) {
                ...
                quint64 const unpack_size =
                    ratio_info_->compressing_mode ? ratio_info_->input_size : ratio_info_->output_size;
                quint64 const pack_size =
                    ratio_info_->compressing_mode ? ratio_info_->output_size : ratio_info_->input_size;
                if (unpack_size == 0) {
                    ratio_value_->clear();
                } else {
                    quint64 const ratio = pack_size <= (std::numeric_limits<quint64>::max() / 100ULL)
