...
namespace z7::ui::runtime_support {
    namespace {
        ...
        constexpr char const* kSharedSettingsApp = "7z-shared";
        constexpr char const* kBenchmarkMemoryLimitBytesKey = "Benchmark/MemoryLimitBytes";
        constexpr char const* kExtractMemoryLimitGbKey = "Extraction/MemLimit";
    ...
    QString with_extract_memory_limit_platform_suffix_if_unsupported(QString const& text) {
    ...
    int max_extract_memory_limit_gb(quint64 ram_bytes) {
        ...
        quint64 const ram_gb = rounded_ram_gb(ram_bytes);
        if (ram_gb <= 1) {
            return 1;
        }
        ...
        quint64 const max_by_ram = ram_gb - 1;
        if (max_by_ram == 0) {
            return 1;
        }
    ...
    int clamp_extract_memory_limit_gb(int limit_gb, quint64 ram_bytes) {
        int const max_gb = max_extract_memory_limit_gb(ram_bytes);
    ...
    int extract_memory_limit_bytes_to_storage_gb(uint64_t bytes) {
        ...
        uint64_t const gb = (bytes + static_cast<uint64_t>(kBytesPerGiB) - 1) / static_cast<uint64_t>(kBytesPerGiB);
    ...
    ExtractMemoryLimitSettings load_extract_memory_limit_settings() {
        ...
        int const default_limit = clamp_extract_memory_limit_gb(kDefaultLimitGb, ram_bytes);

        ExtractMemoryLimitSettings out;
        out.enabled = settings.contains(QString::fromLatin1(kExtractMemoryLimitGbKey));
        out.limit_gb = clamp_extract_memory_limit_gb(
            settings.value(QString::fromLatin1(kExtractMemoryLimitGbKey), default_limit).toInt(), ram_bytes);
        return out;
    }

    void save_extract_memory_limit_settings(ExtractMemoryLimitSettings const& settings_value) {
        quint64 const ram_bytes = detect_total_ram_bytes();
        z7::platform::qt::PortableSettings settings;
    ...
    uint64_t configured_extract_memory_limit_bytes() {
        ...
        ExtractMemoryLimitSettings const settings = load_extract_memory_limit_settings();
        if (!settings.enabled) {
            return 0;
        }
    ...
    uint64_t load_benchmark_memory_limit_bytes() {
        ...
        return settings.value(QString::fromLatin1(kBenchmarkMemoryLimitBytesKey), QVariant::fromValue<qulonglong>(0))
            .toULongLong();
    }

    void save_benchmark_memory_limit_bytes(uint64_t bytes) {
        z7::platform::qt::PortableSettings settings = shared_settings();
        settings.setValue(QString::fromLatin1(kBenchmarkMemoryLimitBytesKey), QVariant::fromValue<qulonglong>(bytes));
        settings.sync();
    }
