...
void DelayedProgressDialogPresenter::set_dialog(
    TaskProgressDialogBase* dialog) {
  state_->dialog = dialog;
  if (dialog == nullptr) {
    cancel_pending();
...
void DelayedProgressDialogPresenter::set_show_callback(
    ShowCallback callback) {
  state_->show_callback = std::move(callback);
}

void DelayedProgressDialogPresenter::schedule(QObject* context,
                                              int delay_msecs) {
...
void DelayedProgressDialogPresenter::show_state(
