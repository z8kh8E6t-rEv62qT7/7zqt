...
namespace z7::ui::runtime_support {
    ...
    namespace {
        ...
        QString filemanager_rc_file_path(QString const& file_name) {
            return QDir(filemanager_rc_directory_path()).filePath(file_name);
        }

        void merge_string_map(QHash<uint32_t, QString>* target, QHash<uint32_t, QString> const& source) {
            if (target == nullptr) {
                return;
            }
        ...
        QByteArray read_rc_resource_file(QString const& file_name) {
    ...
    QString OfficialLangCatalog::required_english_language_file_path() {
        return QDir(language_directory_path()).filePath(QString::fromLatin1(kRequiredEnglishFileName));
    }
    ...
    bool OfficialLangCatalog::validate_required_language_resources(QString* error_out) {
        ...
        QString const en_path = required_english_language_file_path();
        ...
        if (!en_file.open(QIODevice::ReadOnly)) {
            if (error_out != nullptr) {
                *error_out =
                    QStringLiteral("Cannot open required language file \"%1\": %2").arg(en_path, en_file.errorString());
            }
            return false;
        }
        ...
        QHash<uint32_t, QString> const parsed = parse_lang2(en_file.readAll(), &english_name, &native_name, &comments);
        if (parsed.isEmpty()) {
            if (error_out != nullptr) {
    ...
    OfficialLangCatalog::OfficialLangCatalog() {
        {
            QByteArray const property_name_rc = read_rc_resource_file(QStringLiteral("PropertyName.rc"));
            QByteArray const property_name_res_h = read_rc_resource_file(QStringLiteral("PropertyNameRes.h"));
        ...
        {
            QByteArray const resource_rc = read_rc_resource_file(QStringLiteral("resource.rc"));
            QByteArray const resource_h = read_rc_resource_file(QStringLiteral("resource.h"));
            if (!resource_rc.isEmpty()) {
                merge_string_map(&resource_fallbacks_, parse_rc_string_tables(resource_h + "\n" + resource_rc));
            }
        }
        {
            QByteArray const resource_gui_rc = read_rc_resource_file(QStringLiteral("resourceGui.rc"));
            QByteArray const resource_gui_h = read_rc_resource_file(QStringLiteral("resourceGui.h"));
            if (!resource_gui_rc.isEmpty()) {
                merge_string_map(&resource_fallbacks_, parse_rc_string_tables(resource_gui_h + "\n" + resource_gui_rc));
            }
        }
        ...
        if (en_file.open(QIODevice::ReadOnly)) {
            english_ = parse_lang2(en_file.readAll(), &english_name, &native_name, &english_comments_);
    ...
    QString OfficialLangCatalog::text(uint32_t id) const {
        auto const active_it = active_.constFind(id);
    ...
    QString OfficialLangCatalog::format(uint32_t id, QStringList const& args) const {
    ...
    bool OfficialLangCatalog::set_language(QString const& lang_id) {
        return set_language_internal(lang_id, false);
    }

    bool OfficialLangCatalog::set_language_and_persist(QString const& lang_id) {
    ...
    void OfficialLangCatalog::reload_from_settings() {
        QString const stored = settings_language();
    ...
    void OfficialLangCatalog::rebuild_language_index() {
        ...
        english_info.english_name = english_.value(kLangEnglishNameId, QStringLiteral("English"));
        english_info.native_name = english_.value(kLangNativeNameId, QStringLiteral("English"));
        ...
        QStringList files = lang_dir.entryList(QStringList{QStringLiteral("*.txt")}, QDir::Files, QDir::Name);
        files.sort(Qt::CaseInsensitive);

        for (QString const& file_name : files) {
            QString const lang_id = file_name.left(file_name.size() - 4).toLower();
            if (lang_id.isEmpty()) {
                continue;
            }
    ...
    bool OfficialLangCatalog::set_language_internal(QString const& lang_id, bool persist) {
        QString const normalized = normalize_language_id(lang_id);
    ...
    bool OfficialLangCatalog::load_language_file(QString const& lang_id,
        ...
        QString const path =
            QDir(language_directory_path()).filePath(QStringLiteral("%1.txt").arg(normalize_language_id(lang_id)));
        QFile file(path);
        if (!file.open(QIODevice::ReadOnly)) {
            return false;
        }

        *map_out = parse_lang2(file.readAll(), english_name_out, native_name_out, comments_out);
        apply_resource_fallbacks(map_out);
        return !map_out->isEmpty();
    }

    QHash<uint32_t, QString> OfficialLangCatalog::parse_lang2(QByteArray const& bytes,
                                                              QString* english_name_out,
                                                              QString* native_name_out,
        ...
        for (int line_index = 1; line_index < lines.size(); ++line_index) {
            ...
            if (is_decimal_number(trimmed)) {
                bool ok = false;
                uint32_t const explicit_id = trimmed.toUInt(&ok);
                if (ok) {
    ...
    QHash<uint32_t, QString> OfficialLangCatalog::parse_rc_string_tables(QByteArray const& bytes) {
        QHash<uint32_t, QString> map;
        ...
        QRegularExpression const begin_re(QStringLiteral("^\\s*STRINGTABLE\\s*$"));
        QRegularExpression const block_begin_re(QStringLiteral("^\\s*BEGIN\\s*$"));
        QRegularExpression const block_end_re(QStringLiteral("^\\s*END\\s*$"));
        QRegularExpression const entry_re(QStringLiteral("^\\s*([A-Z0-9_]+)\\s+\"((?:[^\"\\\\]|\\\\.)*)\"\\s*$"));
        QRegularExpression const define_re(QStringLiteral("^\\s*#define\\s+([A-Z0-9_]+)\\s+(\\d+)\\s*$"));
        ...
        for (QString line : lines) {
            ...
            auto const define_match = define_re.match(line);
            if (define_match.hasMatch()) {
                bool ok = false;
                uint32_t const id = define_match.captured(2).toUInt(&ok);
                if (ok) {
            ...
            auto const entry_match = entry_re.match(line);
            if (!entry_match.hasMatch()) {
                continue;
            }

            QString const symbol = entry_match.captured(1);
            if (!ids_by_name.contains(symbol)) {
                continue;
            }

            QString const decoded = decode_lang2_text_line(entry_match.captured(2));
    ...
    QString OfficialLangCatalog::substitute_placeholders(QString const& text, QStringList const& args) {
        QString out = text;
    ...
    QString OfficialLangCatalog::normalize_language_id(QString const& lang_id) {
        QString const trimmed = lang_id.trimmed();
    ...
    QString OfficialLangCatalog::settings_language() const {
        QString const key = QString::fromLatin1(kSettingsKeyLang);
    ...
    void OfficialLangCatalog::save_settings_language(QString const& lang_id) const {
        QString const normalized = normalize_language_id(lang_id);
        QString const key = QString::fromLatin1(kSettingsKeyLang);
    ...
    QString LF(uint32_t id, QStringList const& args) {
        return OfficialLangCatalog::instance().format(id, args);
    }

    QString strip_mnemonic(QString const& text) {
        QString out;
        out.reserve(text.size());

        for (int i = 0; i < text.size(); ++i) {
            QChar const ch = text[i];
            if (ch != QLatin1Char('&')) {
                out.append(ch);
                continue;
            }
