...
namespace z7::common {
    ...
    inline bool is_legacy_archive_type_alias(std::string_view raw_token) {
        std::string const token = normalize_archive_type_token_copy(raw_token);
    ...
    inline bool is_gzip_family_archive_suffix(std::string_view raw_suffix) {
        std::string const suffix = normalize_archive_type_token_copy(raw_suffix);
        return suffix == "gz" || suffix == "gzip" || suffix == "tgz" || suffix == "tpz" || suffix == "tar.gz";
    }

    inline bool is_bzip2_family_archive_suffix(std::string_view raw_suffix) {
        std::string const suffix = normalize_archive_type_token_copy(raw_suffix);
        return suffix == "bz2" || suffix == "bzip2" || suffix == "tbz2" || suffix == "tbz" || suffix == "tar.bz2";
    }

    inline std::string canonical_archive_type_from_filename_suffix_copy(std::string_view raw_suffix) {
    ...
    inline std::string preferred_archive_output_suffix_copy(std::string_view archive_type_token) {
        std::string const canonical = canonical_archive_type_token_copy(archive_type_token);
        if (canonical == "gzip") {
            return "gz";
        }
        if (canonical == "bzip2") {
