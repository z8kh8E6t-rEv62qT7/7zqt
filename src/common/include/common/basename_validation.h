...
namespace z7::common {
    ...
    inline BasenameValidationResult validate_basename_only_name(std::string_view raw_name) {
        BasenameValidationResult result;
        std::string normalized = normalize_native_separators_copy(trim_ascii_space_copy(raw_name));
        if (normalized.empty()) {
            result.error = BasenameValidationError::kEmpty;
            return result;
        }
        if (normalized.front() == '/' || basename_validation_is_drive_absolute_path(normalized)) {
