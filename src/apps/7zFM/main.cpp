...
namespace {
    ...
    QString startup_request_key(z7::apps::filemanager::OpenRequest const& request) {
    ...
    QVector<z7::apps::filemanager::OpenRequest>
    take_undispatched_startup_requests(z7::apps::filemanager::FileOpenApplication* app,
        ...
        QVector<z7::apps::filemanager::OpenRequest> const pending_requests = app->take_pending_open_requests();
        undispatched_requests.reserve(pending_requests.size());
        for (z7::apps::filemanager::OpenRequest const& request : pending_requests) {
            QString const key = startup_request_key(request);
    ...
    void open_startup_request_in_window(z7::ui::filemanager::MainWindow* window,
                                        z7::apps::filemanager::OpenRequest const& request) {
        if (window == nullptr) {
            return;
        }
    ...
    void open_request_in_new_window(z7::apps::filemanager::OpenRequest const& request) {
        auto* const window = new z7::ui::filemanager::MainWindow();
        open_startup_request_in_window(window, request);
    }

    void launch_open_request_in_new_process(z7::apps::filemanager::OpenRequest const& request) {
...
int main(int argc, char* argv[]) {
    ...
    z7::apps::filemanager::StartupOpenArgumentParseResult const startup_open_args =
    ...
    auto open_in_primary_window = [&](z7::apps::filemanager::OpenRequest const& request) {
    ...
    auto dispatch_undispatched_startup_requests = [&]() -> bool {
        QVector<z7::apps::filemanager::OpenRequest> const startup_requests =
            take_undispatched_startup_requests(&app, &dispatched_startup_request_keys);
        if (startup_requests.isEmpty()) {
            return false;
        }
        z7::apps::filemanager::dispatch_startup_open_requests(
            startup_requests, open_in_primary_window, open_request_in_new_window);
    ...
    auto install_runtime_open_request_handler = [&app]() {
    ...
    if (primary_window != nullptr) {
        ...
        auto finish_startup_file_open_handoff = [&primary_window,
        ...
        app.set_open_request_handler(
            [&app,
             ...
             &startup_file_open_handoff_finished](z7::apps::filemanager::OpenRequest const& request) {
                if (startup_file_open_handoff_finished) {
                    if (startup_request_key(request) == startup_file_open_handoff_key) {
                        return;
                    }
                ...
                app.remember_dispatched_startup_open_requests(QVector<z7::apps::filemanager::OpenRequest>{request});
                open_in_primary_window(request);
                QTimer::singleShot(0, &app, install_runtime_open_request_handler);
            });

        QTimer::singleShot(kStartupFileOpenGraceMs, &app, finish_startup_file_open_handoff);
    }
#else
    if (primary_window == nullptr) {
        primary_window = new z7::ui::filemanager::MainWindow();
