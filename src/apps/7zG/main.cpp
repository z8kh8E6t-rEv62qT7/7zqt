...
namespace {
    ...
    bool parse_task_ipc_identity(QStringList const& args,
        ...
        for (QString const& arg : args) {
            if (arg.startsWith(QStringLiteral("--task-ipc-session="))) {
                bool ok = false;
                quint64 const parsed = arg.mid(QStringLiteral("--task-ipc-session=").size()).toULongLong(&ok);
                if (!ok) {
                    return false;
                }
                if (out_session_id != nullptr) {
            ...
            if (arg.startsWith(QStringLiteral("--task-ipc-generation="))) {
                bool ok = false;
                quint32 const parsed = arg.mid(QStringLiteral("--task-ipc-generation=").size()).toUInt(&ok);
                if (!ok) {
                    return false;
                }
                if (out_generation != nullptr) {
            ...
            if (arg.startsWith(QStringLiteral("--task-ipc-shm="))) {
                if (out_shm_name != nullptr) {
                    *out_shm_name = arg.mid(QStringLiteral("--task-ipc-shm=").size()).trimmed();
                }
                continue;
            }
            if (arg.startsWith(QStringLiteral("--task-ipc-sem="))) {
                if (out_sem_name != nullptr) {
                    *out_sem_name = arg.mid(QStringLiteral("--task-ipc-sem=").size()).trimmed();
                }
            }
        }
        ...
        return has_identity
            ...
            && out_sem_name != nullptr
            && !out_sem_name->isEmpty();
#else
    ...
    bool contains_task_ipc_identity_arg(QStringList const& args) {
        for (QString const& arg : args) {
    ...
    bool publish_task_ipc_completion_with_fallback(z7::task_ipc_runtime::TaskIpcClaimedTask const& task,
                                                   int result_code,
                                                   QString const& summary) {
        QString task_ipc_error;
        if (z7::task_ipc_runtime::publish_task_ipc_completion(task, result_code, summary, &task_ipc_error)) {
            return true;
        }
        return z7::task_ipc_runtime::publish_task_ipc_completion_minimal(task, result_code, nullptr);
    }
...
int main(int argc, char* argv[]) {
    ...
    if (!contains_task_ipc_identity_arg(raw_user_args)) {
        z7::platform::qt::AppStartupConfig const startup = z7::platform::qt::startup_config_with_persisted_overrides(
            z7::platform::qt::StartupAppKind::kGui, argc > 0 ? QString::fromLocal8Bit(argv[0]) : QString());
        z7::platform::qt::apply_pre_app_startup(startup);
        QApplication app(argc, argv);
        z7::platform::qt::apply_post_app_startup(app, startup);

        int const runtime_status = validate_runtime_environment();
        if (runtime_status != 0) {
    ...
    QString shm_name;
    QString sem_name;
    QStringList const user_args = app.arguments().mid(1);
    ...
    z7::ui::gui::SharedTaskCancellation const task_cancellation = z7::ui::gui::TaskCancellation::create();
    z7::ui::gui::GuiAppController controller;
    int exit_code = 255;
    auto const finalize_exit = [&app, &exit_code](int result) {
        exit_code = result;
        app.exit(result);
    };
    ...
    QTimer::singleShot(0, &app, [&controller, &finalize_exit, session_id, generation, task_cancellation]() {
        z7::task_ipc_runtime::TaskIpcClaimedTask task;
        QString claim_error;
        if (!z7::task_ipc_runtime::claim_task_ipc_task_for_worker(session_id, generation, &task, &claim_error)) {
            QString const detail = claim_error.trimmed().isEmpty()
        ...
        bool const complete_on_claim = task.payload.complete_on_claim;
        bool const completion_published_on_claim =
            complete_on_claim && publish_task_ipc_completion_with_fallback(task, 0, QString());
        ...
        if (task_cancellation->is_canceled()) {
            publish_task_ipc_completion_with_fallback(task, 5, QStringLiteral("Operation canceled."));
            finalize_exit(5);
            return;
        }

        if (task.payload.command == z7::task_ipc_runtime::TaskIpcCommandKind::kCli) {
            ...
            if (!task.payload.cli.has_value()) {
                ...
                worker_result = z7::apps::gui::run_cli_worker_payload(*task.payload.cli);
                summary = worker_result.summary;
            }
            if (worker_result.exit_code != 0 && summary.isEmpty() && !worker_result.error_dialog_shown) {
                summary = QStringLiteral("7zG CLI task failed (exit code %1).").arg(worker_result.exit_code);
            }
            if (!(complete_on_claim && completion_published_on_claim)) {
                publish_task_ipc_completion_with_fallback(task, worker_result.exit_code, summary);
            }
        ...
        controller.run_task_ipc_payload_async(
            task.payload,
            task_cancellation,
            [&finalize_exit, task, complete_on_claim, completion_published_on_claim](
                z7::ui::gui::GuiTaskCompletion const& completion) {
                QString summary = completion.summary.trimmed();
                if (summary.isEmpty() && completion.exit_code != 0) {
                    summary = QStringLiteral("7zG task failed (exit code %1).").arg(completion.exit_code);
                }
                if (!(complete_on_claim && completion_published_on_claim)) {
                    publish_task_ipc_completion_with_fallback(task, completion.exit_code, summary);
                }
