...
namespace z7::apps::gui {
    namespace {
        ...
        QString from_ustring(UString const& value) {
            return QString::fromWCharArray(value.Ptr(), static_cast<int>(value.Len()));
        }

        QString from_astring(AString const& value) {
            return QString::fromLocal8Bit(value.Ptr(), static_cast<int>(value.Len()));
        }

        UString to_ustring(QString const& value) {
            std::wstring const wide = value.toStdWString();
            return UString(wide.c_str());
        }

        UStringVector to_command_strings(QStringList const& argv) {
            UStringVector out;
        ...
        std::string to_native_string(QString const& value) {
            QByteArray const encoded = QFile::encodeName(value);
            return std::string(encoded.constData(), static_cast<size_t>(encoded.size()));
        }

        std::string to_native_string(UString const& value) {
            return to_native_string(from_ustring(value));
        }

        QString from_fstring(FString const& value) {
        ...
        QString unsupported_email_message() {
            return QStringLiteral("Unsupported command-line mode: 7zG direct CLI does not support email "
                                  "archive sending. Use file-based archive commands.");
        }

        QString unsupported_show_dialog_message() {
            return QStringLiteral("Unsupported command-line mode: -ad show-dialog direct CLI currently "
        ...
        QString validation_exception_message(QString const& header, QString const& detail);

#ifdef Z7_TESTING
        constexpr char const* kSuppressGuiErrorDialogsForTestsEnv = "Z7_SUPPRESS_GUI_RESULT_DIALOGS_FOR_TESTS";
#endif

        bool parse_cli_options(QStringList const& argv, CArcCmdLineOptions* options, QString* error_message) {
            ...
            try {
                ...
                return true;
            } catch (CNewException const&) {
                if (error_message != nullptr) {
                    *error_message = QStringLiteral("ERROR: Can't allocate required memory!");
                }
            } catch (CMessagePathException const& error) {
                if (error_message != nullptr) {
                    *error_message = from_ustring(error).trimmed();
                }
            } catch (CSystemException const& error) {
                if (error_message != nullptr) {
                    *error_message =
                        validation_exception_message(QStringLiteral("System ERROR:"),
                                                     from_ustring(NWindows::NError::MyFormatMessage(error.ErrorCode)));
                }
            } catch (UString const& error) {
                if (error_message != nullptr) {
                    *error_message = validation_exception_message(QStringLiteral("ERROR:"), from_ustring(error));
                }
            } catch (AString const& error) {
                if (error_message != nullptr) {
                    *error_message = validation_exception_message(QStringLiteral("ERROR:"), from_astring(error));
                }
            } catch (char const* error) {
                if (error_message != nullptr) {
                    *error_message = validation_exception_message(
                        QStringLiteral("ERROR:"), QString::fromLocal8Bit(error != nullptr ? error : "Unknown Error"));
                }
            } catch (...) {
                if (error_message != nullptr) {
        ...
        QString validation_exception_message(QString const& header, QString const& detail = QString()) {
...
#ifdef Z7_TESTING
        void write_stderr_line(QString const& text) {
            QByteArray const encoded = text.toLocal8Bit();
            if (!encoded.isEmpty()) {
        ...
        int map_exception_to_exit_code(CSystemException const& error) {
        ...
        public:
            ...
            ScopedStderrCapture(ScopedStderrCapture const&) = delete;
            ScopedStderrCapture& operator=(ScopedStderrCapture const&) = delete;
            ...
            bool start(QString* error_message) {
                ...
                reader_ = std::thread([this]() {
                    char buffer[4096];
                    for (;;) {
                        int const n = Z7_READ(read_fd_, buffer, sizeof(buffer));
                        if (n <= 0) {
                            break;
                        }
                        qsizetype const remaining = kMaxCapturedBytes - captured_.size();
                        if (remaining > 0) {
        ...
        int run_original_console_main_caught(QStringList const& argv) {
            ...
            try {
                ...
                std::fputs("\n\nCommand Line Error:\n", stderr);
                QByteArray const text = from_ustring(error).toLocal8Bit();
                ...
                if (error.ErrorCode == E_ABORT) {
                    ...
                    std::fputs("\n\nSystem ERROR:\n", stderr);
                    UString const message = NWindows::NError::MyFormatMessage(error.ErrorCode);
                    QByteArray const text = from_ustring(message).toLocal8Bit();
                    std::fwrite(text.constData(), 1, static_cast<size_t>(text.size()), stderr);
                    std::fputc('\n', stderr);
                }
        ...
        bool has_unsupported_email_mode(CArcCmdLineOptions const& options) {
            return options.UpdateOptions.EMailMode;
        }

        bool is_original_7zg_gui_command_supported(CArcCmdLineOptions const& options) {
        ...
        bool show_cli_error_dialog(QString const& message, bool suppress_by_yes_to_all) {
            if (message.trimmed().isEmpty() || suppress_by_yes_to_all) {
                return false;
            }
#ifdef Z7_TESTING
            if (qEnvironmentVariable(kSuppressGuiErrorDialogsForTestsEnv).trimmed() == QStringLiteral("1")) {
                write_stderr_line(QStringLiteral("[Z7_TESTING] GUI error dialog suppressed: title=%1\n%2")
        ...
        CliWorkerResult make_cli_worker_error(int exit_code, QString const& message, bool suppress_gui_error) {
            CliWorkerResult result;
            result.exit_code = exit_code;
            result.summary = message.trimmed();
            result.error_dialog_shown = show_cli_error_dialog(result.summary, suppress_gui_error);
            return result;
        }

        QString console_error_dialog_message(QString const& stderr_text, QString const& stdout_text) {
            QString const stderr_trimmed = stderr_text.trimmed();
            if (!stderr_trimmed.isEmpty()) {
                return stderr_trimmed;
            }
            QString const stdout_trimmed = stdout_text.trimmed();
        ...
        std::optional<z7::ui::gui::GuiTaskSpec> build_show_dialog_spec(CArcCmdLineOptions const& options,
            ...
            if (options.Command.CommandType == NCommandType::kExtract
                ...
                spec.overwrite_switch = to_overwrite_switch(options.ExtractOptions.OverwriteMode);
            ...
            if (options.Command.CommandType == NCommandType::kAdd
                ...
                spec.update_mode = options.Command.CommandType == NCommandType::kUpdate ? "update" : "add";
                ...
                for (unsigned i = 0; i < options.Censor.CensorPaths.Size(); ++i) {
                    NWildcard::CCensorPath const& path = options.Censor.CensorPaths[i];
                    if (!path.Include) {
                        continue;
                    }
        ...
        CliWorkerResult run_show_dialog_spec_blocking(z7::ui::gui::GuiTaskSpec const& spec) {
            ...
            controller.run_task_spec_async(
                ...
                [&loop, &exit_code, &summary](z7::ui::gui::GuiTaskCompletion const& completion) {
    ...
    CliWorkerResult run_cli_worker_payload(z7::task_ipc_runtime::TaskIpcCliPayload const& payload) {
        QString const working_dir = payload.working_dir.trimmed();
        if (!working_dir.isEmpty()) {
            QDir::setCurrent(working_dir);
        }

        CliValidationResult const validation = validate_cli_arguments(payload.argv);
        if (!validation.ok) {
            return make_cli_worker_error(NExitCode::kUserError,
        ...
        if (!is_original_7zg_gui_command_supported(options)) {
            return make_cli_worker_error(NExitCode::kFatalError, unsupported_gui_command_message(), options.YesToAll);
        }
        if (options.ShowDialog) {
            QString spec_error;
            std::optional<z7::ui::gui::GuiTaskSpec> const spec = build_show_dialog_spec(options, &spec_error);
            if (!spec.has_value()) {
                return make_cli_worker_error(NExitCode::kUserError,
        ...
        if (!stderr_capture.start(&capture_error)) {
            return make_cli_worker_error(NExitCode::kFatalError, capture_error, false);
        }
        int const exit_code = run_original_console_main_caught(payload.argv);
        std::fflush(stdout);
        std::fflush(stderr);
        ...
        result.exit_code = exit_code;
        if (exit_code != 0) {
            result.summary = console_error_dialog_message(stderr_capture.captured_text(), QString());
            result.error_dialog_shown = show_cli_error_dialog(result.summary, options.YesToAll);
        }
        return result;
    }

    int run_cli_launcher(QStringList const& argv) {
        CliValidationResult const validation = validate_cli_arguments(argv);
        if (validation.no_command) {
            show_cli_error_dialog(validation.error_message, false);
            return 0;
        }
        if (!validation.ok) {
            show_cli_error_dialog(validation.error_message, validation.suppress_gui_error);
            return NExitCode::kUserError;
        }
        ...
        if (!z7::task_ipc_runtime::ensure_task_ipc_bootstrap_ready(&ipc_error)) {
            show_cli_error_dialog(ipc_error.trimmed().isEmpty() ? QStringLiteral("Failed to initialize task IPC.")
                                                                : ipc_error.trimmed(),
                                  false);
            return NExitCode::kFatalError;
        }

        QString const owner_instance_id = QUuid::createUuid().toString(QUuid::WithoutBraces);
        ...
        if (!z7::task_ipc_runtime::dispatch_task_ipc_task_managed_process(QCoreApplication::applicationFilePath(),
                                                                          ...
                                                                          &started_process,
                                                                          &ipc_error)) {
            show_cli_error_dialog(ipc_error.trimmed().isEmpty() ? QStringLiteral("Failed to start 7zG task IPC worker.")
                                                                : ipc_error.trimmed(),
                                  false);
            return NExitCode::kFatalError;
        }
        ...
        while (!completed) {
            ...
            if (!z7::task_ipc_runtime::collect_task_ipc_events(owner_instance_id, &events, &collect_error)) {
                show_cli_error_dialog(collect_error.trimmed().isEmpty()
                                          ? QStringLiteral("Failed to collect 7zG task IPC events.")
                                          : collect_error.trimmed(),
                                      false);
                return NExitCode::kFatalError;
            }
            for (z7::task_ipc_runtime::TaskIpcEvent const& event : events) {
                if (event.session_id == dispatch.session_id
                    && event.generation == dispatch.generation
                    && event.event_kind == z7::task_ipc_runtime::TaskIpcEventKind::kCompleted) {
                    completion = event;
                    completed = true;
                }
            ...
            if (!completed) {
                if (worker_process && worker_process->state() == QProcess::NotRunning) {
                    show_cli_error_dialog(QStringLiteral("7zG task worker exited before publishing completion."),
                                          false);
                    return worker_process->exitCode() != 0 ? worker_process->exitCode() : NExitCode::kFatalError;
                }
                QThread::msleep(20);
            }
        }
        ...
        if (worker_process && completion.result_code == 0 && !launch_options.StdOutMode) {
            QByteArray const child_stdout = worker_process->readAllStandardOutput();
            if (!child_stdout.isEmpty()) {
                std::fwrite(child_stdout.constData(), 1, static_cast<size_t>(child_stdout.size()), stdout);
                std::fflush(stdout);
            }
        }
#ifdef Z7_TESTING
        if (qEnvironmentVariable(kSuppressGuiErrorDialogsForTestsEnv).trimmed() == QStringLiteral("1")) {
            QByteArray const child_stderr = worker_process ? worker_process->readAllStandardError() : QByteArray();
            if (!child_stderr.isEmpty()) {
                std::fwrite(child_stderr.constData(), 1, static_cast<size_t>(child_stderr.size()), stderr);
                std::fflush(stderr);
            }
        }
#endif
