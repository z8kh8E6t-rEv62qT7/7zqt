...
namespace z7::shell_integration::menu_internal {
    namespace {
        ...
        QString shell_key(QString const& suffix) {
            return QStringLiteral("shell.actions.%1").arg(suffix);
        }

        QString i18n_action_key(QString const& action_id) {
    ...
    QSet<QString> effective_visible_actions(ShellIntegrationConfig const& config) {
        if (config.visible_actions_configured) {
            return config.visible_actions;
        }
        QStringList const defaults = default_shell_integration_visible_actions();
    ...
    bool action_visible_in_config(ShellIntegrationConfig const& config, QString const& action_or_group) {
        ...
        QSet<QString> const visible_actions = effective_visible_actions(config);
    ...
    menu_title_for_action(QString const& action_id, ShellIntegrationMenuPlan const& plan, QString const& locale_key) {
        ...
        if (action_id == QString::fromLatin1(kActionExtractTo)) {
            return z7::i18n::format(shell_key(i18n_action_key(action_id)), {plan.extract_subdir}, locale_key);
        }
        ...
        if (action_id == QString::fromLatin1(kActionAddTo7z)) {
            return z7::i18n::format(shell_key(i18n_action_key(action_id)), {plan.archive_name_7z}, locale_key);
        }
        if (action_id == QString::fromLatin1(kActionAddToZip)) {
            return z7::i18n::format(shell_key(i18n_action_key(action_id)), {plan.archive_name_zip}, locale_key);
        }
        ...
        if (action_id == QString::fromLatin1(kActionGenerateSha256)) {
            QString const dynamic = shell_integration_create_archive_name_from_paths(plan.selected_paths, true, nullptr)
                                  + QStringLiteral(".sha256");
            return z7::i18n::format(shell_key(i18n_action_key(action_id)), {dynamic}, locale_key);
        }
        ...
        for (ShellIntegrationHashMethodDef const& item : shell_integration_hash_method_defs()) {
    ...
    void append_action_if_visible(ShellIntegrationMenuPlan* plan,
        ...
        plan->actions.push_back({action_id, menu_title_for_action(action_id, *plan, locale_key)});
    }
