...
namespace z7::shell_integration {
    namespace {
        ...
        std::array<ContextMenuFlagMapping, 10> const& context_menu_flag_mappings() {
            static constexpr std::array<ContextMenuFlagMapping, 10> kMappings = {{
                ...
                {kActionCrcShaMenu, kContextMenuFlagCrcTopLevel | kContextMenuFlagCrcCascaded},
            }};
            return kMappings;
        }
    ...
    std::array<ShellIntegrationHashMethodDef, 11> const& shell_integration_hash_method_defs() {
        static std::array<ShellIntegrationHashMethodDef, 11> const kDefs = {{
            {"CRC-32", "CRC32"},
            {"CRC-64", "CRC64"},
            {"XXH64", "XXH64"},
            {"MD5", "MD5"},
    ...
    QString shell_integration_normalize_fs_name(QString name) {
        static QRegularExpression const kInvalidChars(QStringLiteral(R"([\\/:*?\"<>|])"));
    ...
    bool shell_integration_do_need_extract_name(QString const& name) {
        static QSet<QString> const kExtractExcluded = [] {
            QSet<QString> out;
            QString const joined = QStringLiteral("3gp aac ans ape asc asm asp aspx avi awk "
                                                  "bas bat bmp "
                                                  ...
                                                  "mkv wav webm wma wv "
                                                  "xml xsd xsl xslt");
            for (QString const& part : joined.split(QLatin1Char(' '), Qt::SkipEmptyParts)) {
                out.insert(part.toLower());
            }
            return out;
        }();

        QString const ext = QFileInfo(name).suffix().toLower();
        if (ext.isEmpty()) {
            return true;
        }
    ...
    QString shell_integration_extract_subfolder_name(QString const& archive_name) {
        auto is_extract_arc_ext = [](QString const& ext) {
            static QSet<QString> const kArcExts = {QStringLiteral("7z"), QStringLiteral("rar"), QStringLiteral("zip")};
            QString const lowered = ext.toLower();
        ...
        int const dot_pos = archive_name.lastIndexOf(QLatin1Char('.'));
        if (dot_pos < 0) {
            return shell_integration_normalize_fs_name(archive_name) + QStringLiteral("~");
        }

        QString const ext = archive_name.mid(dot_pos + 1);
        QString base = archive_name.left(dot_pos).trimmed();
        int const dot2 = base.lastIndexOf(QLatin1Char('.'));
        if (dot2 > 0) {
            QString const ext2 = base.mid(dot2 + 1);
            bool const remove_prev =
                (ext.compare(QStringLiteral("001"), Qt::CaseInsensitive) == 0 && is_extract_arc_ext(ext2))
    ...
    shell_integration_create_archive_name_from_paths(QStringList const& paths, bool is_hash, QString* base_name_out) {
        ...
        if (paths.size() == 1) {
            ...
            QFileInfo const first(paths.front());
            QDir const parent = first.absoluteDir();
            QString parent_name = parent.dirName();
#ifdef Q_OS_WIN
            if (parent_name.isEmpty()) {
                QString const root = parent.absolutePath();
                if (!root.isEmpty()) {
        ...
        if (has_single_info) {
            ...
            if (!single_info.isDir() && !keep_name) {
                int const first_dot = name.indexOf(QLatin1Char('.'));
        ...
        QSet<QString> const valid_exts =
            is_hash ? QSet<QString>{QStringLiteral("sha256")}
                    : QSet<QString>{
                          QStringLiteral("7z"), QStringLiteral("zip"), QStringLiteral("tar"), QStringLiteral("wim")};

        bool simple_allowed = true;
        QSet<int> used_ids;
        for (QString const& path : paths) {
            QString const file_name = QFileInfo(path).fileName();
            if (!file_name.startsWith(name, Qt::CaseInsensitive)) {
                continue;
            }

            QString const suffix = file_name.mid(name.size());
            int const dot_pos = suffix.lastIndexOf(QLatin1Char('.'));
            if (dot_pos < 0) {
                continue;
            }

            QString const ext = suffix.mid(dot_pos + 1).toLower();
            if (!valid_exts.contains(ext)) {
                continue;
            }

            QString const marker = suffix.left(dot_pos);
            if (marker.isEmpty()) {
            ...
            bool ok = false;
            int const value = marker.mid(1).toInt(&ok);
            if (ok && value > 0) {
    ...
    QStringList shell_integration_context_menu_action_ids() {
        return {QString::fromLatin1(kActionOpen),
    ...
    QStringList normalize_shell_integration_visible_actions(QStringList const& action_ids) {
        ...
        for (QString const& action_id : action_ids) {
            QString const normalized = action_id.trimmed();
        ...
        QStringList out;
        for (QString const& action_id : shell_integration_context_menu_action_ids()) {
    ...
    std::uint32_t shell_integration_context_menu_flags_from_visible_actions(QStringList const& action_ids) {
        QStringList const normalized = normalize_shell_integration_visible_actions(action_ids);
        QSet<QString> const visible(normalized.cbegin(), normalized.cend());

        std::uint32_t flags = 0;
        for (ContextMenuFlagMapping const& mapping : context_menu_flag_mappings()) {
