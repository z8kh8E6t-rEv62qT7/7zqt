...
namespace {
    ...
    std::string to_native_path_string(QString const& path) {
        QByteArray const encoded = QFile::encodeName(path);
        return std::string(encoded.constData(), static_cast<size_t>(encoded.size()));
    }

    QString from_summary(z7::app::OperationOutcome const& outcome) {
    ...
    std::string self_7z_type_hint(QString const& archive_path) {
        qint64 const archive_size = QFileInfo(archive_path).size();
        if (archive_size <= 0) {
            return "7z";
        }
    ...
    bool is_switch_prefix(QString const& arg, QLatin1Char key) {
        if (arg.size() < 2) {
            return false;
        }
        QChar const prefix = arg.at(0);
    ...
    bool parse_options(QStringList const& args, SfxOptions* options, QString* error) {
        if (options == nullptr) {
            return false;
        }
        if (error != nullptr) {
        ...
        for (int i = 1; i < args.size(); ++i) {
            ...
            if (arg.startsWith(QStringLiteral("--z7-sfx-test-output="))) {
                options->output_dir = arg.mid(QStringLiteral("--z7-sfx-test-output=").size());
                continue;
            }
            if (arg.startsWith(QStringLiteral("--z7-sfx-test-password="))) {
                options->password = arg.mid(QStringLiteral("--z7-sfx-test-password=").size());
                options->password_defined = true;
                continue;
            }
    ...
    public:
        explicit SfxExtractDialog(QString output_dir, QWidget* parent = nullptr) : QDialog(parent) {
            ...
            extract_button_ = buttons->addButton(QStringLiteral("Extract"), QDialogButtonBox::AcceptRole);
            ...
            connect(browse, &QPushButton::clicked, this, [this]() {
                QString const selected = QFileDialog::getExistingDirectory(
                    this, QStringLiteral("Select Folder"), output_edit_->text().trimmed());
                if (!selected.isEmpty()) {
        ...
        QString output_dir() const { return output_edit_ != nullptr ? output_edit_->text().trimmed() : QString(); }
    ...
    public:
        SfxArchiveDelegate(QString password, bool password_defined, bool yes_to_all, bool test_auto_extract) :
            password_(std::move(password)),
            password_defined_(password_defined),
            yes_to_all_(yes_to_all),
            test_auto_extract_(test_auto_extract) {}

        void set_progress_dialog(QProgressDialog* progress_dialog) { progress_dialog_ = progress_dialog; }

        void set_finished_callback(std::function<void()> callback) { finished_callback_ = std::move(callback); }

        std::optional<z7::app::OperationOutcome> const& outcome() const { return outcome_; }
        ...
        std::optional<z7::app::OverwriteDecision> request_overwrite(z7::app::OverwritePrompt const& prompt) override {
            ...
            invoke_on_ui_thread_blocking([&]() {
                QMessageBox box(QMessageBox::Question,
                                ...
                                nullptr);
                QAbstractButton* yes = box.addButton(QStringLiteral("Yes"), QMessageBox::YesRole);
                QAbstractButton* yes_to_all = box.addButton(QStringLiteral("Yes to All"), QMessageBox::YesRole);
                QAbstractButton* no = box.addButton(QStringLiteral("No"), QMessageBox::NoRole);
                QAbstractButton* no_to_all = box.addButton(QStringLiteral("No to All"), QMessageBox::NoRole);
                QAbstractButton* rename = box.addButton(QStringLiteral("Auto Rename"), QMessageBox::ActionRole);
                QAbstractButton* cancel = box.addButton(QStringLiteral("Cancel"), QMessageBox::RejectRole);
        ...
        std::optional<z7::app::PasswordReply> request_password(z7::app::PasswordPrompt const& prompt) override {
            ...
            invoke_on_ui_thread_blocking([&]() {
                ...
                dialog.setLabelText(prompt.reason_kind == z7::app::PasswordPromptReason::kWrongPassword
        ...
        std::optional<z7::app::ChoiceReply> request_choice(z7::app::ChoicePrompt const& prompt) override {
            ...
            invoke_on_ui_thread_blocking([&]() {
                ...
                for (std::string const& choice : prompt.choices) {
                    buttons.push_back(box.addButton(QString::fromStdString(choice), QMessageBox::ActionRole));
                }
                if (prompt.default_index >= 0 && prompt.default_index < buttons.size()) {
                    box.setDefaultButton(qobject_cast<QPushButton*>(buttons.at(prompt.default_index)));
                }
                box.exec();
                int const index = buttons.indexOf(box.clickedButton());
                if (index >= 0) {
                ...
                [this, progress]() {
                    ...
                    if (!progress.current_path.empty()) {
                        progress_dialog_->setLabelText(
                            QStringLiteral("Extracting %1").arg(QString::fromStdString(progress.current_path)));
                    }
                },
                Qt::QueuedConnection);
        }
    ...
    private:
        ...
        void invoke_on_ui_thread_blocking(Callback&& callback) {
            ...
            QMetaObject::invokeMethod(this, std::forward<Callback>(callback), Qt::BlockingQueuedConnection);
        }
    ...
    int run_extract(QString const& archive_path, SfxOptions const& options, QWidget* parent) {
        QString const output_dir =
            options.output_dir.isEmpty() ? QFileInfo(archive_path).absolutePath() : options.output_dir;
        if (!QDir().mkpath(output_dir)) {
            QString const message = QObject::tr("Cannot create output directory:\n%1").arg(output_dir);
#ifdef Z7_TESTING
...
#endif
            QMessageBox::critical(parent, QStringLiteral("7-Zip self-extracting archive"), message);
            return 2;
        }
        ...
        extract.overwrite_mode = options.yes_to_all ? z7::app::OverwriteMode::kOverwrite : z7::app::OverwriteMode::kAsk;
        ...
        auto delegate = std::make_shared<SfxArchiveDelegate>(options.password,
        ...
        QProgressDialog progress(QStringLiteral("Extracting..."), QStringLiteral("Cancel"), 0, 100, parent);
        ...
        QObject::connect(&progress, &QProgressDialog::canceled, [&session]() { session.cancel(); });
        ...
        std::optional<z7::app::OperationOutcome> const& outcome = delegate->outcome();
        ...
        QString const message = from_summary(*outcome);
#ifdef Z7_TESTING
        if (options.test_auto_extract) {
            std::fprintf(stderr, "%s\n", message.toLocal8Bit().constData());
...
int main(int argc, char* argv[]) {
    ...
    QString const self_path = QCoreApplication::applicationFilePath();
...
#endif
        QMessageBox::critical(nullptr, QStringLiteral("7-Zip self-extracting archive"), parse_error);
        return 7;
    }
